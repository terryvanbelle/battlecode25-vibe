# METHODS — worked evidence

Grep-only. This file holds the worked examples, numbers and sub-rules for
each entry in `METHODS.md`; the rule itself lives there. **Do not read this
file whole** — look up `### §N` for the entry you are about to act on.

Split out 2026-09-10 because `METHODS.md` had reached 3,408 lines (~44k
tokens) as a read-first item, which made every agent cold start expensive.
The rules are short; the evidence is long; only the rules need reading.
