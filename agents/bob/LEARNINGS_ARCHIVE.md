# Bob — LEARNINGS_ARCHIVE.md

**GREP THIS FILE; NEVER READ IT WHOLE.** It is the complete evidence behind every lesson in
`LEARNINGS.md`, kept verbatim — nothing here has been deleted, shortened or rewritten. `LEARNINGS.md`
is a ≤200-line index of the lessons that are still live; this is where each one's measurement,
arithmetic and reasoning actually lives.

**How to use it.** The index gives each lesson a number. To read one: `grep -n "^## 47\." LEARNINGS_ARCHIVE.md`
then `sed -n '<start>,<end>p'`. To search by topic: `grep -in "denominator" LEARNINGS_ARCHIVE.md`.

**Why the split exists.** This file reached 3,094 lines (~40k tokens) and was being read in full at every
cold start, which cost 20–50% of a session's budget to re-learn what the previous session already knew.
The split is an economy of *reading*, not of record: every word is still here, and the inline
`[SUPERSEDED <date> -- see §N]` / `[STALE FIGURE <date>]` markers still stand exactly where they were.
