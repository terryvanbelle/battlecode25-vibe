...
| 4 | candidates that reshape bob's own **opening spend** / the `reserve = 1200` chip floor | CLOSED, **re-open condition TESTED AND NOT MET** (iter53) | the deficit is opponent-inflicted; the same code reaches 4–7 towers against a different opponent. **iter53, zero games**: the reserve does block spawns on 56% of r31–200 rounds (0.8% in r1–30, so it is the constant not the economy) — but on tournament games bob is blocked **LESS in games it loses (44.8%) than it wins (68.8%)** and holds **52% MORE idle chips in losses**. Relaxing it would hand bob more of a resource it already fails to spend in exactly those games. And **alice runs on 41% less money** (83.2% below bob's own threshold) while beating bob 60–40 | evidence that bob's own spend binds **in a game it loses** — the mirror corpus cannot supply this, it must be measured against an opponent bob did not produce |
...
| 26 | the **territory** half of the per-turn paint penalty (end turns on own paint) | CLOSED | iter51, zero games: bob's units are on their OWN paint **89.9%** of the time; the bill is ~7,386 paint/game and **83% of it is ENEMY** territory, i.e. the price of attacking. The free (NEUTRAL) part is **0.035 paint/unit-round** = **~+0.9 wins** at the exchange rate iteration 49 measured. And **alice pays 0.360 to bob's 0.192 — 88% MORE — while beating bob 60–40** | bob's own-paint share falls below ~80%, or a design is found that removes the enemy-territory cost without ceasing to attack |
| 27 | **paint-saving MOVEMENT mechanisms as a family** | CLOSED at the bar | measured exchange rate: 1.0 paint/unit-round saved ≈ 25 wins/50 (iter49: 0.240 → +6), so +10 needs ≥0.40 saved — **and the rate saturates**: iter50's e9w saved 0.285 and also scored +6. The largest saving in the whole budget is crowding's avoidable 0.600, and half of it already saturates | a mechanism spends saved paint without moving units off productive ground (same re-open as #25) |
| 28 | **paint-tower UPGRADE policy** as a paint-income lever (incl. lowering `UPGRADE_RESERVE`) | CLOSED | iter52, zero games: foregone income 16,231/game is a **ceiling priced at chips bob does not have** — median chip balance reaches the L2 trigger (6,500) only ~r800 and the L3 trigger (9,000) never before r1100, past most games' end. And **alice upgrades WORSE than bob** (26.2% of paint-tower-rounds at L3 vs bob's 31.1%; 26,362 foregone vs 23,304) while beating bob 60–40 | bob's median chip balance clears 9,000 before ~r400, **or** an expansion-saturation signal lets the reserve be spent only when there is nothing left to expand into |
| 29 | **pattern-completion throughput** (how fast bob converts a ruin into a tower) | CLOSED | iter55, zero games: median mark→completion latency **bob 31 rounds vs alice 37 (0.84x — bob FASTER)**, and faster at p75 too. Registered bands were ≥1.5x build / ≤1.1x close. Bob builds *fewer* towers, each no slower | bob's latency rises above ~1.5x a live opponent's in the r60–400 window |
