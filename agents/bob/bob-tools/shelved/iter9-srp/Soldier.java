...
public class Soldier {
    ...
    /**
     * One ruin in MONEY_SHARE becomes a money tower; the rest become paint towers.
     * Dose parameter: 1 = every other ruin (the iteration 0-3 behaviour, i.e. the
     * zero arm), 3 = one in four, larger = fewer money towers still.
     */
    static final int MONEY_SHARE = 3;

    /**
     * Tower type for a ruin (iteration 4): mostly paint, a minority money.
     *
     * Engine-verified: a MONEY tower has paintPerTurn == 0, and InternalRobot's
     * constructor gives a newly completed tower paintAmount == 0 -- so every money
     * tower we build is a permanently STERILE spawn point. It can never accumulate
     * the 200 paint a soldier costs, and it mines the one resource we already pile
     * up unspent (551k chips at r2000 even after iteration 3's upgrades), while
     * paint income is what actually gates unit production and hence coverage. The
     * iteration 0 rule made half of every ruin we captured one of these.
     *
     * The choice must stay a pure function of the ruin and never of time: a soldier
     * marks the 5x5 with one type's pattern, and the "already marked?" probe in
     * workOnRuin then prevents any re-mark, so a ruin whose type changed mid-build
     * could never be completed by a later soldier.
     */
    static UnitType towerTypeFor(MapLocation ruin) {
        return ((ruin.x + ruin.y) & MONEY_SHARE) == 0
