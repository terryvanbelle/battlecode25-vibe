...
## Iteration 37 — PRE-REGISTERED before launch: idle soldiers wander RANDOMLY past visible unpainted ground

**Area check first.** Today's session closed the production axis by prior measurement, so I checked
this candidate against every closed area before building it. It is **not** spawn rate (iterations 24
and 27 bracket that from both sides), **not** spawn mix (iteration 20, interior peak at 2:2:1),
**not** ruin selection (35, mean choice set 0.83), **not** tower-type rules (22/26/29/32), and **not**
iteration 21's migration beacon — that was a *comms* beacon giving a *long-range* objective to
*denial units*; this is a soldier using its own local vision, no comms, no shared state.

### The mechanism, and the pre-check that picked it out of three candidates

`Soldier.run` step 3: when there is no ruin to work, the soldier calls `Nav.wander()` — a persistent
**random** direction held for 6-15 steps, with no awareness of where unpainted ground is. Step 4 then
tries to paint. If the random walk moved it away from empty ground, the turn is wasted.

I could see three explanations for a wasted turn and they have three different fixes, so I ran the
discriminating case rather than naming one. `src/bob_eprobe` classifies every early soldier turn by
whether the action was consumed, implemented as a **wrapper** around an unmodified `Soldier.run()`
(`isActionReady()` before/after) rather than as counters at each call site — a call-site counter that
missed one action branch would mislabel an ACT as an IDLE, which is the exact error the probe exists
to detect. DefaultSmall, 8 soldiers, 389 early turns:

```
  acted 235 (60.4%)   cooldown 53 (13.6%)   paint-floor 18 (4.6%)   IDLE-with-action-and-paint 83 (21.3%)

  of the 83 idle turns:
     mark-blocked (paintSomething's getMark()==EMPTY filter)      1   ( 1%)
     EMPTY tile in VISION but outside action range               50   (68%)
     no empty tile visible at all                                23   (31%)
```

**One of the three candidates carries 68% of the idle turns and another is dead at 1%.** The mark
filter — the explanation I would have picked from reading the source, since it visibly refuses
marked tiles — accounts for a single turn in 389. The fault is *movement*, not target selection.

**A control worth recording**: the same probe run against `bob_iter20` and against `bob_denier`
produced **identical** counters (389/235/53/18/83 both times), because on DefaultSmall there is no
contact before round 60. So this 21.3% is an **intrinsic property of my soldier logic, not something
an opponent inflicts** — which is what makes it a legitimate target after today's census showed the
*coverage* deficit vs carol is opponent-inflicted.

### The change

`nearestFrontier()` returns the nearest visible EMPTY passable tile **outside** action range (one
inside would be painted by `paintSomething` this same turn, so stepping toward it buys nothing), and
is scanned only on turns that would otherwise wander, to bound the bytecode.

**Arms** (one shared 25-map sample, 150 games, generated by `bob-tools/make-frontier-arms.sh` so they
differ in exactly one constant — iteration 33 was voided by a hand-edit that also perturbed RNG
ordering):

```
  bob_t0   FRONTIER_MODE 0   unchanged                                    <- NULL
  bob_t1   FRONTIER_MODE 1   always seek the nearest visible empty tile
  bob_t2   FRONTIER_MODE 2   seek only when paint > REFILL_BELOW (50)
```

**`bob_t0` is an EXACT zero arm by construction**: at mode 0 the condition short-circuits before
`nearestFrontier()` is called, `f` stays null, and `wander()` runs with identical control flow and
identical RNG consumption. So it must come out **25/50 with all 25 maps split**, as iteration 27b's
zero arm did.

### Gates, pre-registered

- **VOID** if `bob_t0` is not 25/50 with all 25 maps split. That is a stronger null than a symmetry
  argument: it checks the arm generator too.
...
**Prediction, committing to a shape.** I expect **`bob_t2` >= `bob_t1`**: mode 1 drags paint-poor
soldiers away from the towers they must refill at, and ~95% of soldier deaths are starvation, so the
unconditional form should pay for its extra coverage in dead units. If `bob_t1 > bob_t2` the
starvation story is wrong and I will say so.

**Secondary, registered now and read only after the primary** — the new early-coverage instrument
(`bob-tools/early-census.sh` over this run's replay dir): **round-30 coverage differential should rise
for the seeking arms.** This is the mechanism check. If the win rate moves but early coverage does
not, the win came from something other than the mechanism I am claiming, and I will not attribute it.

**What would falsify the framing**: if both seeking arms sit at the null, then converting idle turns
