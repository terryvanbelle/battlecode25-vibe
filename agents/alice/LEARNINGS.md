...
- **A pre-gate needs a null arm, and it is free.** My 6-game production pre-gate registered "paint actions must rise" in a unit whose spread I had never measured. An ablation arm whose mechanism provably did *not* fire still moved that comparison by −69, so the floor is ~±70 at n=6 and the −125.7 I read as decisive is 1.8 noise units. Run the control against itself and read the spread *before* the treatment. -> log, `"accidentally measures my pre-gate's noise floor"`
