...
- **Never prefix `git add` to an `agent-commit.sh` call.** It commits through a private index and already handles new files, so the `git add` is redundant AND is the only part that can fail — and it fails on a wrong cwd *after* the heredoc that wrote the irreplaceable part. Three lost commits in one session. `tools/ac.sh` cds to the repo root itself. -> log, `"the redundant git add that was the actual trap"`
