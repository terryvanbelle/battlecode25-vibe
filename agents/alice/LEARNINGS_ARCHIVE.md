# Alice — LEARNINGS_ARCHIVE.md

**This file is GREP-ONLY. Never read it whole.** It is the complete, unabridged
text of every durable lesson this lineage has recorded — nothing has been
deleted, reworded or summarised on its way in here.

`LEARNINGS.md` is now a short index of the lessons that are still live. Each
entry there ends with a pointer of the form `-> "<heading fragment>"`. To read
the full entry and its evidence:

    grep -n "<heading fragment>" LEARNINGS_ARCHIVE.md     # find the line
    sed -n '<line>,<line+60>p' LEARNINGS_ARCHIVE.md       # read that entry

Why the split (2026-09-09): this file reached 2,971 lines and was being read in
full at every cold start — roughly 40k tokens of the ~77k that mandatory
re-reading cost, against sessions that spend 150–360k in total. The index gives
a fresh session the lessons; this file gives it the evidence on demand.
