...
public class RobotPlayer {
    ...
    /** Iteration 53 arm switch. Compile-time constant: at false javac
     *  eliminates the whole condition, so the control is byte-identical. */
    static final boolean UPG_PRIORITY = false;
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        // ITERATION 53: once expansion is finished, chips have NO COMPOUNDING SINK --
        // there is no ruin left to convert them into a tower. Iteration 26's closure says
        // chips compound and paint does not, and that is exactly why this is gated on the
        // END of compounding rather than fighting it: after that point a MONEY upgrade
        // buys the currency alice already hoards ($2,930 idle at r300 on small maps) and
        // a PAINT upgrade buys the one it starves for (145 paint/tower against a 200
        // soldier cost). The tower MIX is untouched; only WHICH tower spends the chips
        // first, which today is decided by TURN ORDER and nothing else.
        // expansionFinished is iteration 39's accepted signal and is NOT a latch, so if
        // expansion resumes, money upgrades resume with it by construction.
        //
        // WHY if/else RATHER THAN A GUARDED CONDITION: `UPG_PRIORITY && rc.getType()...`
        // still leaves a live rc.getType() call in the control. That is precisely the
        // iteration 44 mistake -- guarding the body but not the call -- which left a
        // control that was NOT byte-identical while the log claimed it was. javac
        // eliminates the dead branch of `if (compile-time-false)`, so the control below
        // compiles to exactly the pre-iteration-53 statement.
        if (UPG_PRIORITY) {
            UnitType i53t = rc.getType();
            boolean i53block = expansionFinished
                    && (i53t == UnitType.LEVEL_ONE_MONEY_TOWER
                     || i53t == UnitType.LEVEL_TWO_MONEY_TOWER);
            if (!i53block && nextLevel != null
                    && rc.getMoney() >= nextLevel.moneyCost + CHIP_RESERVE
