...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("K7 " + k7new + " " + k7old + " "
    ...
    /** ITERATION K7 -- two constants at one site, priced off the K6 CROSS-TAB.
     *  56.1% of the towers a hungry, ready unit can reach hold 26-50 paint, so the
     *  reserve of 50 sits ON TOP OF THE MODAL BUCKET; moving it to 25 takes gate-4
     *  pass from 23.5% to 79.6% WITHIN that population. And 1,026 further events
     *  are 'ready but not hungry' -- 2.5x the served population -- which gate 1
     *  discards. Together: ~1,032 permitted against 97 today, about 10.6x.
     *  WHY IT SHOULD PAY, on measured quantities and not on the parity story I
     *  retired: paint is conserved, towers are at cap 0.0% of the time, so a refill
     *  DIVERTS paint from spawning. 200 paint spawns a unit delivering 9.54 paint
     *  actions; 200 paint refilled is up to 40. Refilling avoids paying upkeep
     *  twice. */
    static final boolean K7_WIDE = false;
    /** Same-game counterfactual, the ONLY trustworthy manipulation instrument here:
     *  a cross-game refill rate has a floor of sd = 125% of its mean, so at n=3 the
     *  1.15x I once reported was 0.15 sd. These two counters are evaluated on the
     *  SAME turns in the SAME game, so their ratio has no game-to-game term. */
    static int k7new, k7old;
    ...
    static void tryRefill(RobotController rc) throws GameActionException {
        ...
        int p = rc.getPaint();
        boolean k7r = rc.isActionReady();
        RobotInfo[] k7n = rc.senseNearbyRobots(2, rc.getTeam());
        int k7b = Integer.MIN_VALUE;
        for (int k = 0; k < k7n.length; k++) {
            if (!k7n[k].getType().isTowerType()) continue;
            int pa = k7n[k].getPaintAmount();
            if (pa > k7b) k7b = pa;
        }
        if (k7b > Integer.MIN_VALUE && k7r) {
            if (p * 2 < cap && k7b - 50 > 0) k7old++;
            if (p < cap && k7b - 25 > 0) k7new++;
        }
        if (K7_WIDE) { if (p >= cap) return; }
        else { if (p * 2 >= cap) return; }        // not hungry
        ...
        for (int i = 0; i < near.length; i++) {
            ...
            int surplus = t.getPaintAmount() - (K7_WIDE ? 25 : 50);
