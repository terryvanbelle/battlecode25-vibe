...
public class RobotPlayer {
    ...
    static int maxBc = 0;
    // --- ITERATION 22 PAINT-BUDGET CENSUS (per-robot; statics are per-robot) ---
    static int pbMark = 0, pbPat = 0, pbSelf = 0, pbArea = 0, pbBorn = -1;
    // --- PRICING THE tile-under-self BRANCH (both sides, per coordinator) ---
    // After each self-paint we spent 5 to make the tile ally, which zeroes an
    // upkeep of 1 (neutral) or 2 (enemy) for as long as we STAY on it. Record
    // the stay length and the penalty rate we avoided, so the branch can be
    // priced instead of assumed.
    static MapLocation psTile = null;   // tile we last self-painted
    static int psStay = 0;              // turns held on it since
    static int psRate = 0;              // upkeep/turn we are avoiding there
    static int psSaved = 0;             // total upkeep actually avoided
    static int psSpent = 0;             // total paint spent on self-paints
    static int psRepaid = 0;            // self-paints that returned >= 5
    static int psN = 0;                 // self-paints closed out          // worst bytecode usage seen
        ...
        while (true) {
            ...
            try {
                ...
                if (rc.getType() == UnitType.SOLDIER) {
                    if (pbBorn < 0) pbBorn = rc.getRoundNum();
                    int spent = 5 * (pbPat + pbSelf + pbArea) + 25 * pbMark;
                    // A soldier starts with 200 and alice never withdraws from a
                    // tower, so anything unaccounted for is movement upkeep.
                    int upkeep = 200 - spent - rc.getPaint();
                    rc.setIndicatorString("PS n=" + psN + " saved=" + psSaved
                            + " spent=" + psSpent + " repaid=" + psRepaid
                            + " PB mark=" + pbMark + " pat=" + pbPat
                            + " self=" + pbSelf + " area=" + pbArea
                            + " spent=" + spent + " upkeep=" + upkeep
                            + " mp=" + rc.getPaint()
                            + " age=" + (rc.getRoundNum() - pbBorn));
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if (ruin != null) {
            ...
            if (rc.canSenseLocation(markCheck)
                ...
                rc.markTowerPattern(wantTower, ruin); pbMark++;
            }
            ...
            for (MapInfo t : rc.senseNearbyMapInfos(ruin, 8)) {
                ...
                if (mark != PaintType.EMPTY && mark != t.getPaint()) {
                    if (rc.canAttack(t.getMapLocation())) {
                        rc.attack(t.getMapLocation(), mark == PaintType.ALLY_SECONDARY); pbPat++;
                        break;
                    }
                }
            }
        ...
        // close out the previous self-paint if we have left its tile
        if (psTile != null && !psTile.equals(cur)) {
            psSaved += psStay * psRate;
            if (psStay * psRate >= 5) psRepaid++;
            psN++; psTile = null; psStay = 0;
        } else if (psTile != null) {
            psStay++;                       // still standing on it: one turn saved
        }
        if (!here.getPaint().isAlly() && rc.canAttack(cur)) {
            // the rate we are about to zero out, read BEFORE painting
            psRate = here.getPaint().isEnemy() ? 2 : 1;
            rc.attack(cur); pbSelf++; psSpent += 5;
            psTile = cur; psStay = 0;
        }
        ...
        if (rc.isActionReady() && rc.getPaint() >= 15) {
            ...
            if (paintTarget != null) { rc.attack(paintTarget); pbArea++; }
        }
    }
