...
public class RobotPlayer {
    ...
    // --- ITERATION 24 PRE-CHECK: count the DECISION, behaviour unchanged ---
    // i24a would fire only when a soldier is action-ready, holds 5-14 paint, and
    // stands on an EMPTY tile -- the one case iteration 22's deletion gave up.
    // If that state is rare the candidate is priced out before it costs a run.
    static int lpTurns = 0;   // soldier turns reaching this point with the action ready
    static int lpBand  = 0;   //   ... of those, holding 5-14 paint (the affordable-but-gated band)
    static int lpFire  = 0;   //   ... of those, ALSO standing on an EMPTY tile -> i24a would attack
    static int lpDry   = 0;   //   ... in the band but underfoot is not empty -> i24a does nothing
        ...
        while (true) {
            ...
            try {
                ...
                if (rc.getType() == UnitType.SOLDIER) {
                    rc.setIndicatorString("LP turns=" + lpTurns + " band=" + lpBand
                            + " fire=" + lpFire + " dry=" + lpDry
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if (rc.isActionReady()) {
            ...
            if (mp >= 5 && mp < 15) {
                lpBand++;
                if (rc.senseMapInfo(cur).getPaint() == PaintType.EMPTY && rc.canAttack(cur)) lpFire++;
                else lpDry++;
            }
        }
