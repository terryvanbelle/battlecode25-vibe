...
public class RobotPlayer {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        // ITERATION 22 ABLATION: the tile-under-self branch is GATED OFF.
        // Priced at saved/spent = 0.039 (box) and 0.052 (UnderTheSea): across
        // 7,506 self-paints on two maps exactly ONE returned its 5 paint.
        if (false && !here.getPaint().isAlly() && rc.canAttack(cur)) {
            rc.attack(cur);
        }
        // ITERATION 22 ABLATION: the opportunistic area branch is GATED OFF
        // (this is iteration 21's arm A, re-measured on THIS map sample so the
        // 2x2 is exact rather than compared across runs).
