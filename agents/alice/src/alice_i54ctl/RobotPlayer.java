...
public class RobotPlayer {
    ...
    /** Iteration 54 arm switches. Compile-time constants: at false javac
     *  eliminates the guarded branch entirely. */
    static final boolean HEAD_PICK = false;   // lever A: informed wander heading
    static final boolean MOP_GIVE  = false;   // lever B: mopper -> soldier paint

    /** ITERATION 54, LEVER A: choose the wander heading by what it walks into,
     *  instead of uniformly at random.
     *
     *  Scores each of eight 45-degree sectors of vision by the END-OF-TURN TERRAIN
     *  UPKEEP a unit standing there would pay -- ally 0, neutral 1, enemy 2 -- which
     *  is the engine's own charge (processEndOfTurn, javap-verified), not a proxy.
     *  Ballistic run LENGTH is untouched (WANDER_RUN stays 25), so this does not
     *  disturb iterations 12/14; only the direction becomes informed.
     *
     *  The scan START is randomised. A fixed compass order would be exactly the
     *  fixed-absolute-order tie-break Phase 0 item 7 warns compounds into a
     *  per-side tempo edge on a symmetric map.
     *
     *  Measured basis: excess ally-share over the winner's-curse noise floor is
     *  +67.7 per mille on small maps (t=+2.38, 4/5 positive), 8-map pre-check.
     *  Ceiling ~17% of the production gap -- which is why it is screened as half
     *  of a BUNDLE and not alone. */
    static Direction pickHeading(RobotController rc) {
        try {
            MapInfo[] vis = rc.senseNearbyMapInfos();
            int[] cost = new int[8], tot = new int[8];
            MapLocation me = rc.getLocation();
            for (int i = 0; i < vis.length; i++) {
                if (!vis[i].isPassable()) continue;      // a wall costs no upkeep and blocks travel
                ...
                cost[k] += p.isAlly() ? 0 : p.isEnemy() ? 2 : 1;
            }
            int start = rnd(8), bestK = -1, bestScore = 1 << 30;
            for (int j = 0; j < 8; j++) {
                int k = (start + j) & 7;
                if (tot[k] == 0) continue;
                int score = cost[k] * 1000 / tot[k];
                if (score < bestScore) { bestScore = score; bestK = k; }
            }
            return bestK < 0 ? directions[rnd(8)] : directions[bestK];
        } catch (Exception e) {
            return directions[rnd(8)];
        }
    }

    /** ITERATION 54, LEVER B: a mopper hands surplus paint to a starving ally soldier.
     *
     *  Engine-verified (assertCanTransferPaint): "Only moppers can give paint to
     *  allies!" -- giving is a mopper-only verb and alice has never used it;
     *  tryRefill filters to isTowerType() so alice only ever WITHDREW from towers.
     *
     *  NON-DIVERTING BY CONSTRUCTION, which is iteration 44's re-open standard: it
     *  runs only when the mopper's action is STILL ready after the mop attempt, i.e.
     *  on turns with no mop available -- iteration 49 measured that at 37% of mopper
     *  turns. Those turns produce nothing today.
     *
     *  MOP_KEEP is ENGINE-DERIVED, not searched: below 50% of its tank a robot's
     *  cooldowns scale by (100-2X)% (addActionCooldownTurns, javap-verified), so
     *  keeping half keeps the mopper out of the penalty band. */
    static final int MOP_KEEP = 50;

    static void mopperGive(RobotController rc) throws GameActionException {
        int spare = rc.getPaint() - MOP_KEEP;
        if (spare <= 0) return;
        RobotInfo[] allies = rc.senseNearbyRobots(2, rc.getTeam());
        RobotInfo needy = null;
        int worst = 1 << 30;
        for (int i = 0; i < allies.length; i++) {
            RobotInfo a = allies[i];
            if (a.getType() != UnitType.SOLDIER) continue;
            int p = a.getPaintAmount();
            if (p * 2 >= UnitType.SOLDIER.paintCapacity) continue;   // same hunger test as tryRefill
            if (p < worst) { worst = p; needy = a; }
        }
        if (needy == null) return;
        int room = UnitType.SOLDIER.paintCapacity - worst;
        int amt = spare < room ? spare : room;
        if (amt > 0 && rc.canTransferPaint(needy.getLocation(), amt))
            rc.transferPaint(needy.getLocation(), amt);
    }
    ...
    static void runMopper(RobotController rc) throws GameActionException {
        ...
        // LEVER B: the action is still ready only if NO mop was available this turn.
        if (MOP_GIVE && rc.isActionReady()) mopperGive(rc);
    ...
    static void wander(RobotController rc) throws GameActionException {
        ...
        if (wanderDir == null || wanderSteps <= 0) {
            if (HEAD_PICK) { wanderDir = pickHeading(rc); }
            else { wanderDir = directions[rnd(8)]; }
