...
public class RobotPlayer {
    ...
    // --- mopper-position census (alice_mopstand) ---
    static char msUnder = '?';   // paint under me: A ally, E empty, X enemy
    static int  msAdjEnemy = 0;  // enemy-painted tiles within r^2<=2 (moppable now)
    static int  msMopped = 0;    // 1 if a mop actually fired this turn
    static int  msFront = 0;     // ALLY tiles in vision that are adjacent to >=1 enemy tile
    static int  msHold = 0;      // 1 if this turn took the "already in range, hold position" branch
    static int  msAlt = 0;       // DECISION-SCALE supply: of the 8 tiles I can actually move to,
                                 // how many are ALLY paint AND still adjacent to >=1 enemy tile
                                 // (i.e. a zero-terrain-drain spot that keeps me mopping)
    static final byte[] msGrid = new byte[81]; // 9x9 vision window, 0 off/wall 1 empty 2 ally 3 enemy
        ...
        while (true) {
            ...
            try {
                ...
                if (rc.getType() == UnitType.MOPPER) {
                    rc.setIndicatorString("MS u=" + msUnder + " ae=" + msAdjEnemy
                            + " mop=" + msMopped + " fs=" + msFront
                            + " hold=" + msHold + " alt=" + msAlt
                            + " p=" + rc.getPaint() + " bc=" + bc
    ...
    static void runMopper(RobotController rc) throws GameActionException {
        ...
        if (rc.isActionReady()) {
            ...
            if (best != null) { rc.attack(best); msMopped = 1; }
        }
    ...
    /** Mopper-position census. Costs nothing to the bot's decisions -- it only reads.
     *  Builds a 9x9 paint window (vision is r^2<=20, so dx,dy in -4..4) and reports:
     *    u  -- paint under me (A/E/X): standing on ENEMY paint drains -4/turn, ALLY 0
     *    ae -- enemy tiles within r^2<=2, i.e. what I could mop from where I stand
     *    fs -- ALLY tiles in vision adjacent to >=1 enemy tile: the SUPPLY of
     *          zero-drain frontier standing spots. This is the count whose absence
     *          killed the ruin-collision hypothesis; it is pre-registered here. */
    static void msCensus(RobotController rc) throws GameActionException {
        msUnder = '?'; msAdjEnemy = 0; msMopped = 0; msFront = 0;
        msHold = 0; msAlt = 0;
        for (int i = 0; i < 81; i++) msGrid[i] = 0;
        MapLocation me = rc.getLocation();
        int mx = me.x, my = me.y;
        for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
            MapLocation l = t.getMapLocation();
            int dx = l.x - mx, dy = l.y - my;
            if (dx < -4 || dx > 4 || dy < -4 || dy > 4) continue;
            byte c;
            PaintType pt = t.getPaint();
            if (!t.isPassable()) c = 0;
            else if (pt == PaintType.EMPTY) c = 1;
            else if (pt.isAlly()) c = 2;
            else c = 3;
            msGrid[(dx + 4) * 9 + (dy + 4)] = c;
        }
        byte under = msGrid[4 * 9 + 4];
        msUnder = under == 2 ? 'A' : under == 1 ? 'E' : under == 3 ? 'X' : '?';
        // adjacent enemy tiles (r^2<=2 == the 8 neighbours)
        for (int dx = -1; dx <= 1; dx++)
            for (int dy = -1; dy <= 1; dy++)
                if ((dx != 0 || dy != 0) && msGrid[(dx + 4) * 9 + (dy + 4)] == 3) msAdjEnemy++;
        // frontier standing spots: ally tiles with an enemy neighbour. Scan the
        // interior 7x7 only, so every neighbour lookup stays inside the window and
        // no spot is credited on incomplete information.
        for (int x = 1; x <= 7; x++) {
            for (int y = 1; y <= 7; y++) {
                if (msGrid[x * 9 + y] != 2) continue;
                if (msGrid[(x - 1) * 9 + y] == 3 || msGrid[(x + 1) * 9 + y] == 3
                        || msGrid[x * 9 + y - 1] == 3 || msGrid[x * 9 + y + 1] == 3
                        || msGrid[(x - 1) * 9 + y - 1] == 3 || msGrid[(x - 1) * 9 + y + 1] == 3
                        || msGrid[(x + 1) * 9 + y - 1] == 3 || msGrid[(x + 1) * 9 + y + 1] == 3) {
                    msFront++;
                }
            }
        }
        // DECISION-SCALE supply. msFront above is a VISION-scale count, and the
        // decision I would actually change picks among the 8 tiles I can step to
        // this turn -- so msFront is the wrong referent for it. A candidate spot
        // must be (a) actually movable-to right now (canMove covers walls AND
        // occupancy, which the paint grid does not), (b) ALLY paint, so the
        // terrain upkeep is 0 rather than the mopper's -4 on enemy paint, and
        // (c) still adjacent to >=1 enemy tile, so I keep mopping from it.
        // Every neighbour-of-a-neighbour lookup stays within grid coords 2..6,
        // so no spot is credited on incomplete information.
        for (int di = 0; di < 8; di++) {
            Direction d = directions[di];
            if (!rc.canMove(d)) continue;
            int gx = d.dx + 4, gy = d.dy + 4;
            if (msGrid[gx * 9 + gy] != 2) continue;
            if (msGrid[(gx - 1) * 9 + gy] == 3 || msGrid[(gx + 1) * 9 + gy] == 3
                    || msGrid[gx * 9 + gy - 1] == 3 || msGrid[gx * 9 + gy + 1] == 3
                    || msGrid[(gx - 1) * 9 + gy - 1] == 3 || msGrid[(gx - 1) * 9 + gy + 1] == 3
                    || msGrid[(gx + 1) * 9 + gy - 1] == 3 || msGrid[(gx + 1) * 9 + gy + 1] == 3) {
                msAlt++;
            }
        }
