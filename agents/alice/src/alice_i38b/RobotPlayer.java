...
public class RobotPlayer {
    ...
    static int spawnSeq = 0;       // ITERATION 38: per-tower spawn counter (see runTower)
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            // ITERATION 38 (arm b): make the mopper rate EXPLICIT, deleting a hidden parameter.
            //
            // MEASURED: `alice_phase` is this bot with ONE change -- the PRNG seed offset
            // rc.getID() * 31 + 17 -> + 18, which alters no policy at all -- and it scored
            // +12 net swept over the full 75-map census (87/150, 58.0%). A change with zero
            // policy content outscored iteration 37a's real mechanism change (+7).
            //
            // WHY a seed can do that: rngState is seeded from rc.getID(), and STARTING-TOWER
            // IDs are FIXED for a given map (Gears id1-id4, DefaultMedium id4-id7). The engine
            // is deterministic, so those towers draw the SAME sequence in every game on that
            // map. `rnd(4) == 0` therefore is NOT 25% for the towers that matter most -- it is
            // a fixed draw per map, chosen by one constant. Worked out for one starting tower:
            //   seed offset 17 (shipping): 0,0,3,1,2,1,0,0,2,2,0,1  -> 5 moppers of 12
            //   seed offset 18          : 1,1,2,2,3,1,2,2,3,2,3,2  -> 0 moppers of 12
            // Iteration 5 measured early moppers crowding out soldiers as an ABSORBING STATE.
            //
            // HONEST LIMIT ON THE INFERENCE: the seed change moves ALL EIGHT rnd() call sites
            // -- mopper choice, spawn direction, and five in wander/tie-breaking -- so the +12
            // is an AGGREGATE and I cannot attribute it to the mopper rate. This iteration
            // treats "the opening mix is a fixed draw nobody chose" as the defect worth fixing,
            // which stands on the mechanism regardless of where the +12 came from.
            //
            // The fix is a per-tower counter, so the rate is exactly 1 in 8 by construction
            // and no longer depends on the tower's ID or on the seed constant. This DELETES the
            // hidden parameter rather than tuning it -- tuning the offset would be overfitting
            // to a fixed opening and would break under any change to spawn order.
            UnitType want = (spawnSeq++ % 8 == 0) ? UnitType.MOPPER : UnitType.SOLDIER;
