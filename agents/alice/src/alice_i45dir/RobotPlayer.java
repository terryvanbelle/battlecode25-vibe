...
public class RobotPlayer {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if (ruin != null) {
            ...
            // Guarded so that at EXPLORE=false this collapses to a bare
            // wander(rc) with no residual load or branch -- see EXPLORE above.
            if (EXPLORE) {
                if (!goExplore(rc)) wander(rc);
            } else {
                wander(rc);
            }
        }
    ...
    // ------------------------------------------------------------------ moves
    // ---- ITERATION 45: send a SATURATED soldier to the frontier -------------
    // MEASURED (alice_i45probe, roads r200/r300 and BatSignal r380, counters off
    // the indicator string; identity-checked -- probe and src/alice both end
    // BatSignal at round 1209): a soldier has a free action, enough paint and
    // NOTHING it may paint within action range on 70% of its turns early and 96%
    // late, while global coverage plateaus at 50-58% against a 700-per-mille win
    // line. On 72-92% of those idle turns there is no unpainted tile anywhere in
    // VISION either, so the frontier is beyond r^2=20 and the soldier cannot see
    // it. wander is a persistent random heading: a random walk in a bounded arena
    // has no outward pressure, so soldiers mill in the painted core they spawned
    // in. This gives a soldier that is provably in saturated ground a long-range
    // destination instead, using getMapWidth/getMapHeight -- two calls the
    // iteration 45 API sweep found this bot has NEVER made.
    //
    // Pre-check 4 (added after iteration 44): what were these turns doing before?
    // Measured, not assumed -- nothing. They paint no tile (a disjoint counter
    // bucket) and have no tile to paint in vision. The soldier already moves on
    // them; only the heading changes. Iteration 44 failed because it captured
    // turns that WERE producing; these are the turns that are not.
    //
    // At false, javac's conditional compilation removes the guarded block whole
    // and the call site collapses to a bare wander(rc), so the control arm is
    // byte-identical by construction -- and behaviourally identity-checked below.
    static final boolean EXPLORE = true;
    static final int EXPLORE_TTL = 60;      // give up on an unreachable target
    // ABLATION ARM (alice_i45dir). Identical to alice_i45 EXCEPT that the
    // saturated-ground branch commits to a random DIRECTION for the same TTL
    // instead of travelling to a map-wide random LOCATION. No getMapWidth /
    // getMapHeight call anywhere. It isolates the two things goExplore changed
    // at once: destination choice (the claimed mechanism) against path shape
    // (a longer, straighter walk than wander's heading-plus-slide).
    static Direction exploreDir = null;
    static int exploreSince = 0;

    /** @return true if this turn's move was spent travelling to the frontier. */
    static boolean goExplore(RobotController rc) throws GameActionException {
        if (!rc.isMovementReady()) return false;
        // Act ONLY in saturated ground -- byte-for-byte the same test as the arm.
        for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
            if (t.getPaint() == PaintType.EMPTY && t.isPassable()) {
                exploreDir = null;
                return false;
        ...
        if (exploreDir == null || r - exploreSince > EXPLORE_TTL) {
            exploreDir = directions[rnd(8)];
            exploreSince = r;
        }
        tryMove(rc, exploreDir);
        return true;
    }
