...
public class RobotPlayer {
    ...
    // --- iteration 57 RELAY FUNNEL (purely additive; no behaviour change) ---
    // Messaging is robot<->TOWER only (engine: "Only (robot <-> tower) communication
    // is allowed!"), so a rendezvous needs a RELAY: soldier A tells a tower, the
    // tower tells soldier B. That requires TWO soldiers reachable from the SAME
    // tower. This is the adjacency question that has now decided three directions,
    // so it gets the same funnel and is measured BEFORE any protocol is written.
    //
    // Measured from the TOWER's side deliberately: a tower sees its own r^2=20
    // neighbourhood in full, whereas a soldier asking "is another soldier near that
    // tower" can only see the part inside its OWN vision and would undercount.
    // canSendMessage is the engine's own predicate, so the ally-paint-path
    // requirement is enforced by the engine and not approximated by me.
    static long m_turns = 0;   // tower turns
    static long m_one   = 0;   //  ... >=1 soldier it can message
    static long m_two   = 0;   //  ... >=2  = A RELAY IS POSSIBLE
            ...
            if (rc.getType().isTowerType()) {
                ...
                for (int i = 0; i < a.length; i++)
                    if (a[i].getType() == UnitType.SOLDIER
                            && rc.canSendMessage(a[i].getLocation())) n++;
                m_sum += n;
                if (n >= 1) m_one++;
                if (n >= 2) m_two++;
                ...
                RobotInfo[] a = rc.senseNearbyRobots(-1, rc.getTeam());
                for (int i = 0; i < a.length; i++)
                    if (a[i].getType().isTowerType()
                            && rc.canSendMessage(a[i].getLocation())) { q_reach++; break; }
            }
