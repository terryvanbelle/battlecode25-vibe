...
public class RobotPlayer {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if (ruin != null) {
            ...
            // ITERATION 26: shift the mix from ~50/50 to ~25/75 money/paint.
            // Measured mismatch: at r1200-1600 of the tournament replay vs bob I hold
            // $36,830 unspent chips while total tower paint is 680 across 10 towers --
            // 68 each. A money tower produces 20/30/40 chips by level, a paint tower
            // 5/10/15 paint (RULES.md). The tower census puts the chip gate open on
            // 80-94% of tower-turns against paint-affordability of 6-11%, so paint is
            // the binding resource by every measurement I have and half my tower slots
            // were producing the other one.
            //
            // ONLY THE RATIO CHANGES. The trigger stays a pure function of the ruin's
            // coordinates -- identical for both teams, evaluated at MARK time but not
            // DEPENDENT on mark time -- which preserves exactly the property the parity
            // rule was introduced for: immunity to the money-at-mark-time timing
            // artifact that once made every early mark a money tower. A rule that reads
            // rc.getMoney() here would re-introduce that bug, which is why this does not.
            //   & 1 -> sums divisible by 2 -> ~50% money   (previous)
            //   & 3 -> sums divisible by 4 -> ~25% money   (this)
