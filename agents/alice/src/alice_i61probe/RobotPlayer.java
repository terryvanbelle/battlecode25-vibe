...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("V t=" + v_turns + " f=" + v_free + " e=" + v_empty + " n=" + v_n + " | " + "i25=" + i25Refills + "/" + i25Paint + " "
    ...
    // --- iteration 61: is R2 BLOCKED BY AVAILABILITY? the decisive test ---
    // R2's arithmetic says spending a 200 tank in 40 turns instead of 88 is worth
    // +10.5 paint actions per soldier. That is a CEILING on a counterfactual; the
    // question is whether any mechanism can reach it. Routes, and where each dies:
    //   (a) act more often      -> blocked IF no legal paint target exists on idle
    //                              turns. THAT IS WHAT THIS PROBE MEASURES.
    //   (b) spawn fewer         -> tower paint's only sink is spawning, so unspent
    //                              paint idles (atcap measured 1-3%). Arithmetically
    //                              worse: 2 soldiers at 50% beat 1 at 80%.
    //   (c) cull idle units     -> disintegrate() just throws RobotDeathException,
    //                              recovering nothing (javap, pinned jar). Paint is
    //                              already spent. No gain.
    //   (d) cut the per-turn rate -> the rate family, priced 7-17%, closed.
    //
    // So (a) is the only live route and this is its reachability test. A soldier
    // gains coverage ONLY on an EMPTY tile: it cannot paint enemy paint (engine-
    // verified, 0 of 12,409 at corpus scale) and repainting its own gains nothing.
    static long v_turns = 0;   // soldier turns
    static long v_free  = 0;   //  ... action still ready at end of turn
    static long v_empty = 0;   //  ... AND an EMPTY tile in action range (r^2<=9)
    static long v_n     = 0;   // empty tiles in range, summed over v_free turns

    static void availFunnel(RobotController rc) {
        try {
            ...
            v_free++;
            int n = 0;
            MapInfo[] near = rc.senseNearbyMapInfos(UnitType.SOLDIER.actionRadiusSquared);
            for (int i = 0; i < near.length; i++) {
                if (!near[i].isPassable()) continue;
                if (near[i].getPaint() != PaintType.EMPTY) continue;   // only EMPTY gains coverage
                if (!rc.canAttack(near[i].getMapLocation())) continue; // engine's own predicate
                n++;
            }
            v_n += n;
            if (n > 0) v_empty++;
