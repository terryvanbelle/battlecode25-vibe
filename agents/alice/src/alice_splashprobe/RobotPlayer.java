...
public class RobotPlayer {
    ...
    // ---- SPLASH PROBE (instrumentation only; never in src/alice) -------------
    // Executes the exact next step the iteration-24 tower census PARKED: field
    // splashers with the spend gate relaxed IN THIS BUILD ONLY, and record
    // splashScore at the centre the splasher actually chooses -- because the
    // 1.37-of-13 figure from alice_splashcensus was measured at the positions a
    // SOLDIER occupies, and a splasher goes somewhere else. That gap was the
    // whole reason the direction was parked.
    static int spBuilt, spTurns, spReady, spAttacks, spScoreSum, spBest;
    static final int[] spHist = new int[26];   // splashScore at the CHOSEN centre
    static int spNoTarget;                     // ready, but nothing scored above threshold
    static String spStr = "";
        ...
        while (true) {
            ...
            try {
                switch (rc.getType()) {
                    ...
                    case SPLASHER:
                        spTurns++; if (rc.isActionReady()) spReady++;
                        runSplasher(rc); break;
                ...
                if (rc.getType() == UnitType.SPLASHER || !rc.getType().isRobotType()) {
                    StringBuilder h = new StringBuilder();
                    for (int i = 0; i <= 25; i++) if (spHist[i] > 0) h.append(i).append(':').append(spHist[i]).append(',');
                    spStr = " SP built=" + spBuilt + " turns=" + spTurns + " rdy=" + spReady
                            + " atk=" + spAttacks + " sum=" + spScoreSum + " best=" + spBest
                            + " noTgt=" + spNoTarget + " h=" + h;
                } else spStr = "";
                rc.setIndicatorString(spStr + "i25=" + i25Refills + "/" + i25Paint + " "
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            // PROBE ONLY: in the stall regime -- chips far beyond anything the
            // reserve protects -- prefer a SPLASHER. The tower census measured the
            // policy-permitted splasher rate at 0.15-0.25% of tower-turns, so
            // without relaxing the gate this build would field almost none and
            // measure nothing. This is the relaxation the parked plan named.
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (rc.canBuildRobot(want, loc)) {
                    ...
                    if (want == UnitType.SPLASHER) spBuilt++;
                    break;
                }
            }
        }
    ...
    static void runSplasher(RobotController rc) throws GameActionException {
        ...
        if (rc.isActionReady() && rc.getPaint() >= 60) {
            ...
            if (best != null) {
                rc.attack(best);
                spAttacks++; spScoreSum += bestScore;
                if (bestScore > spBest) spBest = bestScore;
                spHist[bestScore < 25 ? bestScore : 25]++;
            } else {
                spNoTarget++;
            }
        }
