...
public class RobotPlayer {
    ...
    // --- ITERATION 23 mechanism verification (same encoding as alice_i23diag,
    // so the two are directly comparable on the same map) ---
    static int pgOk = 0;      // pattern attacks that LANDED (empty-or-ally target)
    static int pgSkip = 0;    // enemy pattern tiles skipped -- the paint NOT burned
    static int pgTurns = 0;   // soldier turns that reached the pattern loop
    static int pgFall = 0;    // turns where the whole pattern was enemy-held -> fell through to area paint
        ...
        while (true) {
            ...
            try {
                ...
                if (rc.getType() == UnitType.SOLDIER) {
                    rc.setIndicatorString("PG ok=" + pgOk + " en=" + pgSkip
        ...
        if (ruin != null) {
            ...
            for (MapInfo t : rc.senseNearbyMapInfos(ruin, 8)) {
                ...
                if (mark != PaintType.EMPTY && mark != t.getPaint()) {
                    if (t.getPaint().isEnemy()) { pgSkip++; continue; }
