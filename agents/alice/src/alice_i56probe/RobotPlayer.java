...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                towerFunnel(rc);   // PROBE: additive, runs after every decision this turn
    ...
    // --- iteration 56 REACHABILITY FUNNEL (purely additive; no behaviour change) ---
    // Same shape as lever B, which fired on 1.2% of turns because it was priced on
    // SUPPLY and never on DELIVERY: a unit with a capability, that must be NEXT TO
    // something, on a turn it has nothing else to do. So the funnel runs BEFORE the
    // build this time. Registered kill condition: under 2% would-fire and the
    // direction closes.
    static long w_turns = 0;   // soldier turns
    ...
    static void towerFunnel(RobotController rc) {
        try {
            ...
            w_turns++;
            if (!rc.isActionReady()) return;      // it painted; not a free turn
            w_free++;
            RobotInfo[] en = rc.senseNearbyRobots(
                    UnitType.SOLDIER.actionRadiusSquared, rc.getTeam().opponent());
            RobotInfo best = null;
            for (int i = 0; i < en.length; i++) {
                if (!en[i].getType().isTowerType()) continue;
                if (best == null || en[i].getHealth() < best.getHealth()) best = en[i];
            }
            if (best == null) return;
            w_tower++;
