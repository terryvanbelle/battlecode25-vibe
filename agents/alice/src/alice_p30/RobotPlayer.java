...
public class RobotPlayer {
    ...
    // ---- REFILL-COLLAPSE PROBE (instrumentation only; NEVER in src/alice) ----
    // Tests the iteration 31 rejection hypothesis. STATIC fact: iteration 30 returns
    // from runSoldier before the role can spend the action, so tryRefill's
    // "if (!isActionReady()) return" passes on arrival; iteration 31 paints en route
    // and that guard rejects it. UNMEASURED: how often that actually bites. These two
    // builds differ ONLY in the pre-emption, so the did= gap is the answer.
    static int rcHungry;      // at tryRefill's call site, below half paint
    static int rcBlocked;     // ... and the ACTION was already spent
    static int rcAdj;         // ... and a tower with surplus was adjacent
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("RC h=" + rcHungry + " blk=" + rcBlocked
                        + " adj=" + rcAdj + " did=" + i25Refills + " | i25=" + i25Refills + "/" + i25Paint + " "
    ...
    static void tryRefill(RobotController rc) throws GameActionException {
        ...
        int p = rc.getPaint();
        if (p * 2 < cap) {                      // PROBE ONLY: decides nothing
            rcHungry++;
            if (!rc.isActionReady()) rcBlocked++;
            RobotInfo[] pn = rc.senseNearbyRobots(2, rc.getTeam());
            for (int i = 0; i < pn.length; i++)
                if (pn[i].getType().isTowerType()
                        && pn[i].getPaintAmount() - UnitType.SOLDIER.paintCost > 0) { rcAdj++; break; }
        }
