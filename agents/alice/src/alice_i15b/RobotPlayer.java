...
public class RobotPlayer {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        // Iteration 15: attack an ENEMY TOWER in reach BEFORE any tile painting.
        // 15a placed this after the underfoot paint and it was DEAD CODE: that paint
        // consumes the action nearly every turn, so isActionReady() was false here. Engine-verified (RULES.md): a soldier's
        // attack on a tile holding an enemy tower does 50 damage, so ~20 actions
        // destroy an L1 tower and take with it a spawn point, 5-15 paint/turn and
        // the 500 paint it started with.
        //
        // This lineage had NEVER pointed a soldier at a robot: all three rc.attack
        // calls in runSoldier targeted tiles. The asymmetry was measured in the
        // tournament, Paintball r200-r300: bob's soldiers attacked alice's towers
        // 32 times while alice's soldiers attacked bob's towers ZERO times -- and
        // alice's soldiers absorbed 38 tower attacks in the same window, so they
        // were standing in range the whole time and never hit back.
        //
        // Placed in the IDLE branch on purpose. Ruin building is this lineage's
        // proven master variable and iterations 10 and 11 were both rejected for
        // displacing it, so this may only spend turns that were already wasted.
        // Weakest tower first: 50 damage is worth far more finishing a tower than
        // spread across two.
        MapLocation cur = rc.getLocation();
        if (rc.isActionReady() && rc.getPaint() >= 15) {
            MapLocation towerTarget = null;
            int bestHp = 1 << 30;
            for (RobotInfo f : rc.senseNearbyRobots(rc.getType().actionRadiusSquared,
                                                    rc.getTeam().opponent())) {
                if (!f.getType().isTowerType()) continue;
                if (f.getHealth() < bestHp && rc.canAttack(f.getLocation())) {
                    bestHp = f.getHealth();
                    towerTarget = f.getLocation();
                }
            }
            if (towerTarget != null) rc.attack(towerTarget);
        }
