...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("K5 " + k5Adj + " " + k5HR + " " + k5RnotH
                        + " " + k5HnotR + " " + k5b0 + " " + k5b25 + " "
                        + k5b50 + " " + k5b100 + " " + k5b200 + " " + k5bhi + " "
    ...
    /** K5 probe (ADDITIVE). K4 recorded two THRESHOLDS of adjacent-tower paint
     *  and so could price only the change already made. This records the
     *  DISTRIBUTION, plus which gate blocks when adjacency succeeds -- the two
     *  things needed to price the next dose instead of guessing it.
     *  Adjacency is evaluated on EVERY call, not only past gates 1-2, so the
     *  denominators are right; that costs bytecode (peak was 9% of budget) and
     *  the identity check afterwards is what proves behaviour is unchanged. */
    static int k5Adj, k5HR, k5RnotH, k5HnotR;
    static int k5b0, k5b25, k5b50, k5b100, k5b200, k5bhi;

    static void tryRefill(RobotController rc) throws GameActionException {
        int cap = rc.getType().paintCapacity;
        int p = rc.getPaint();
        boolean k5h = p * 2 < cap;
        boolean k5r = rc.isActionReady();
        RobotInfo[] k5n = rc.senseNearbyRobots(2, rc.getTeam());
        int k5best = Integer.MIN_VALUE;
        for (int k = 0; k < k5n.length; k++) {
            if (!k5n[k].getType().isTowerType()) continue;
            int pa = k5n[k].getPaintAmount();
            if (pa > k5best) k5best = pa;
        }
        if (k5best > Integer.MIN_VALUE) {
            k5Adj++;
            if (k5h && k5r) k5HR++;
            else if (k5r) k5RnotH++;
            else if (k5h) k5HnotR++;
            if (k5best <= 0) k5b0++;
            else if (k5best <= 25) k5b25++;
            else if (k5best <= 50) k5b50++;
            else if (k5best <= 100) k5b100++;
            else if (k5best <= 200) k5b200++;
            else k5bhi++;
        }
