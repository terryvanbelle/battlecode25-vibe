...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("K8 " + k8new + " " + k8old + " " + k8new3
                        + " " + k8old3 + " "
    ...
    /** ITERATION K8 -- the splasher chip gate, one constant.
     *  The gate needs CHIP_RESERVE(1450) + 2500 = 3,950 chips. Measured: in r1-300,
     *  the window that predicts 79-81% of winners, alice holds that on 2.8% of
     *  frames -- against 58.9% for reserve + a splasher's own 400. A 21x
     *  reachability difference, inside the deciding window. That is prosperity
     *  gating: the lever opens only once the surplus gating it has accumulated.
     *  WHY THE CLOSURE'S STANDARD IS MET, and it rests on a fact the closure could
     *  not have known: the +2500 exists to protect completeTowerPattern's 1000
     *  chips (its own comment says so), but CHIP_RESERVE=1450 ALREADY guarantees
     *  that, and E1 measured tower construction to be RUIN-limited, not
     *  chip-limited -- 97.4% of tower-frames hold >=1000 chips while only 1.6 bare
     *  ruins are visible. The +2500 guards a starvation that cannot occur.
     *  No substitution problem: SPLASHER displaces a cheaper unit and costs MORE
     *  paint (300 v 200/100), so it makes fewer, better-converting units -- the
     *  right direction at 1.35x carol's spawns and 0.52x its per-unit output. */
    static final boolean K8_LOW_GATE = false;
    /** Same-game counterfactual: BOTH gates evaluated on the identical turn, so the
     *  ratio has no game-to-game term. A cross-game spawn rate would sit on a floor
     *  of sd = 125% of its mean, which is what invalidated K3's check. */
    static int k8new, k8old, k8new3, k8old3;
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            int k8m = rc.getMoney();
            if (k8m >= CHIP_RESERVE + UnitType.SPLASHER.moneyCost) {
                k8new++; if (rc.getRoundNum() <= 300) k8new3++;
            }
            if (k8m >= CHIP_RESERVE + 2500) {
                k8old++; if (rc.getRoundNum() <= 300) k8old3++;
            }
            if (k8m >= CHIP_RESERVE + (K8_LOW_GATE ? UnitType.SPLASHER.moneyCost : 2500)
                    || (expansionFinished
