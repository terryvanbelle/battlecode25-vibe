...
public class RobotPlayer {
    ...
    // ITERATION 41b dose: roughly 1 in SPLASH_EVERY soldier-class spawns is a splasher.
    // 0 disables the mechanism entirely; SPLASH_EVERY is a compile-time constant, so at
    // 0 javac eliminates every statement below and the arm is byte-identical bytecode.
    static final int SPLASH_EVERY = 0;
    static int sinceSplash = 0;
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            // --- ITERATION 41b: an EARLY splasher share, counted on SPAWNS not TURNS ---
            // 41a set want=SPLASHER on every k-th eligible tower TURN and left the spawn
            // loop alone. Two defects, both found by reading the loop after the screen:
            //   (1) the loop builds nothing at all when `want` is unaffordable, so an
            //       unaffordable splasher COST A SOLDIER -- the arm silently suppressed
            //       total unit production instead of only reallocating it;
            //   (2) the counter advanced per turn, so the realised splasher share was not
            //       1-in-k of spawns and was not the quantity I pre-registered.
            // Both are fixed here: choose a splasher only when one is actually due, fall
            // back to a soldier rather than idling, and advance the counter on a BUILD.
            boolean splashDue = SPLASH_EVERY > 0 && want == UnitType.SOLDIER
                    && sinceSplash >= SPLASH_EVERY - 1;
            if (splashDue) want = UnitType.SPLASHER;
            int off = rnd(8);
            boolean built = false;
            for (int i = 0; i < 8; i++) {
                MapLocation loc = rc.getLocation().add(directions[(i + off) % 8]);
                if (rc.canBuildRobot(want, loc)) { rc.buildRobot(want, loc); built = true; break; }
            }
            if (SPLASH_EVERY > 0) {
                if (!built && splashDue) {
                    // Unaffordable splasher: build the soldier we would have built anyway.
                    // The splash slot is NOT consumed, so it is retried next spawn.
                    for (int i = 0; i < 8; i++) {
                        MapLocation loc = rc.getLocation().add(directions[(i + off) % 8]);
                        if (rc.canBuildRobot(UnitType.SOLDIER, loc)) {
                            rc.buildRobot(UnitType.SOLDIER, loc); built = true;
                            want = UnitType.SOLDIER; break;
                        }
                    }
                }
                if (built) {
                    if (want == UnitType.SPLASHER) sinceSplash = 0;
                    else if (want == UnitType.SOLDIER) sinceSplash++;
                }
            }
        }
    }
