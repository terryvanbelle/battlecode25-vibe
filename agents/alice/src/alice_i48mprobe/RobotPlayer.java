...
public class RobotPlayer {
            ...
            try {
                ...
                rc.setIndicatorString("i25=" + i25Refills + "/" + i25Paint + " "
                        ...
                        ? "i24=" + i24Moves + " p=" + rc.getPaint()
                          + " M t=" + m_turn + " a=" + m_act + " b0=" + m_b0 + " b1=" + m_b1
                          + " b2=" + m_b2 + " bn=" + m_none + " v0=" + m_tv0 + " v1=" + m_tv1
                          + " vn=" + m_tvNone + " os=" + m_openSum + " bs=" + m_blockSeen
                          + " es=" + m_enemySeen + " " : "")
                        + "bc=" + bc + " max=" + maxBc
    ...
    // ----------------------------------------------------------------- mopper
    // ---- ITERATION 48 PROBE: mopper decision counters (no behaviour change) ----
    // The other half of the pincer. Soldiers sit action-ready at enemy-blocked
    // patterns on 42.2% of ruin-target turns (iteration 47 probe), and iteration
    // 19's mopper priority built to clear exactly those tiles measured b0 = 0 over
    // 140 mops on two maps. This counts the mopper's DECISION, not its outcome:
    // does it ever SEE pattern-blocking enemy paint (m_tv0), as against never
    // ranking it first once seen? Those are different faults with the same zero.
    static int m_turn, m_act, m_b0, m_b1, m_b2, m_none,
               m_tv0, m_tv1, m_tvNone, m_openSum, m_blockSeen, m_enemySeen;
    ...
    static void runMopper(RobotController rc) throws GameActionException {
        ...
        if (rc.isActionReady()) {
            ...
            if (best != null) {
                if (bestPri == 0) m_b0++; else if (bestPri == 1) m_b1++; else m_b2++;
            } else m_none++;
            if (best != null) rc.attack(best);
        }
        ...
        for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
            ...
            // PROBE: the SUPPLY of work in vision, independent of what was chosen.
            m_enemySeen++;
            if (pri == 0) m_blockSeen++;
        }
        if (target == null) m_tvNone++; else if (bestPri == 0) m_tv0++; else m_tv1++;
