...
public class RobotPlayer {
    ...
    /** ITERATION 32: use the mopper's OTHERWISE-DISCARDED action to mop-swing.
     *
     *  runMopper's attack block scans senseNearbyMapInfos(2) for enemy PAINT and, finding
     *  none, spends no action at all -- even with enemy robots standing next to it. mopSwing
     *  needs no ground paint; it drains enemy ROBOTS.
     *
     *  GEOMETRY, verified from InternalRobot.mopSwing by reading how its offset tables are
     *  CONSUMED (two int[4][6], dx and dy, indexed [dir][k], loop bound 6) -- NOT from how
     *  they are built, which misled me twice today:
     *
     *        . X X X .     <- 2 ahead   (dist^2 = 4..5, BEYOND actionRadiusSquared = 2)
     *        . X X X .     <- 1 ahead
     *        . . M . .
     *
     *  Six tiles, 3 wide by 2 deep, cardinal only. Each enemy robot in the block loses
     *  MOPPER_SWING_PAINT_DEPLETION = 5. The far row out-ranges the mopper's own attack --
     *  the Phase 0 item 3 radius asymmetry.
     *
     *  RISK, named before the run: mopSwing hard-codes +20 action cooldown, so the action is
     *  spare THIS turn but the cooldown is borrowed from the next two. Iteration 31 was
     *  exactly a case where spending a free-looking resource lost to discarding it.
     */
    static void mopSwingSpare(RobotController rc) throws GameActionException {
        MapLocation me = rc.getLocation();
        Direction bestDir = null;
        int bestN = 0;
        for (int i = 0; i < 8; i += 2) {              // directions[] is N,NE,E,SE,S,SW,W,NW
            Direction d = directions[i];
            if (!rc.canMopSwing(d)) continue;
            Direction perp = d.rotateRight().rotateRight();   // 2 x 45deg = 90deg
            int n = 0;
            for (int depth = 1; depth <= 2; depth++) {
                for (int off = -1; off <= 1; off++) {
                    MapLocation l = me.translate(d.dx * depth + perp.dx * off,
                                                 d.dy * depth + perp.dy * off);
                    if (!rc.canSenseLocation(l)) continue;
                    RobotInfo r = rc.senseRobotAtLocation(l);
                    if (r != null && r.getTeam() != rc.getTeam() && r.getType().isRobotType()) n++;
                }
            }
            if (n > bestN) { bestN = n; bestDir = d; }
        }
        if (bestDir != null) rc.mopSwing(bestDir);    // bestN >= 1 by construction
    }
    ...
    static void runMopper(RobotController rc) throws GameActionException {
        ...
        if (rc.isActionReady()) {
            ...
            if (best != null) rc.attack(best);
            else mopSwingSpare(rc);        // ITERATION 32: the action was going to be discarded
        }
