...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("P1 " + p1cNew + " " + p1cOld + " " + p1jNew
                        + " " + p1jOld + " "
    ...
    /** PAIR P1 -- the two halves that each failed for want of the other.
     *  i53 (UPG_PRIORITY): block MONEY-tower upgrades once expansion is finished so
     *  upgrade chips reach PAINT towers. Measured then: +639 tower paint, +50% above
     *  control -- and REJECTED at -2, because the extra paint was spent by a spawn
     *  decision that reads chips, never paint, so it became more of the 65%-soldier
     *  mix alice already over-produces 1.35x.
     *  K8 (LOW_GATE): lower the splasher chip gate. Measured: 2.22x more openings --
     *  and CAPPED, because a splasher needs 300 PAINT and towers hold 154.8, clearing
     *  300 on only 29.0% of decisive-window frames.
     *  EACH HALF'S FAILURE IS THE OTHER HALF'S ABSENCE. i53 makes paint the spend
     *  cannot direct; K8 directs a spend there is no paint for.
     *  This probe does NOT assume they compose -- it measures the JOINT counterfactual
     *  on identical turns, because my static reachability estimate was 10x wrong today
     *  and both halves consume the resource their own gate reads. */
    static final boolean P1_PAIR = false;
    static int p1cNew, p1cOld, p1jNew, p1jOld;
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            int p1m = rc.getMoney(); boolean p1p = rc.getPaint() >= UnitType.SPLASHER.paintCost;
            if (p1m >= CHIP_RESERVE + UnitType.SPLASHER.moneyCost) { p1cNew++; if (p1p) p1jNew++; }
            if (p1m >= CHIP_RESERVE + 2500) { p1cOld++; if (p1p) p1jOld++; }
            if (p1m >= CHIP_RESERVE + (P1_PAIR ? UnitType.SPLASHER.moneyCost : 2500)
                    || (expansionFinished
