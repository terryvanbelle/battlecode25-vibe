...
public class RobotPlayer {
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            // SYNTHETIC ARCHETYPE (alice_painter): the PURE PAINTER pole -- never
            // builds a mopper, so it never erases enemy paint. It exists to break a
            // specific correlation in my opponent pool: every opponent I own
            // descends from a lineage that is ~90% moppers and therefore erases my
            // paint heavily, so "fewer moppers" has only ever been measured against
            // opponents for whom mopping is essential. That is the self-referential
            // blind spot in its exact form. This pole answers "how does a low-mopper
            // build fare against someone who does not contest my paint at all".
            // NOT an imitation of bob: I briefly believed bob fielded no moppers and
            // retracted it -- over a full game bob is 16-18% moppers, he merely
            // DEFERS them. This pole is justified as a span of the strategy space,
            // which is a reason that does not depend on what any rival does.
            // One line changed from src/alice; resync it whenever src/alice moves,
            // because a stale archetype silently inflates win rates.
