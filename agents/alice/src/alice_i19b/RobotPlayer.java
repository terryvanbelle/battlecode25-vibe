...
public class RobotPlayer {
    ...
    static void runMopper(RobotController rc) throws GameActionException {
        ...
        int bd = 1 << 30;
        // Arm B (exclusive dose): if ANY pattern-blocking enemy paint is in vision,
        // the mopper ignores every other enemy tile entirely.
        boolean anyBlocking = false;
        for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
            if (t.getPaint().isEnemy() && blocksPattern(openRuins, nOpen, t.getMapLocation())) {
                anyBlocking = true; break;
            }
        }
        for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
            if (!t.getPaint().isEnemy()) continue;
            MapLocation l = t.getMapLocation();
            if (anyBlocking && !blocksPattern(openRuins, nOpen, l)) continue;
