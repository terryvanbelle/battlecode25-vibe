...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rendezvousFunnel(rc);   // PROBE: additive
                ...
                rc.setIndicatorString("U t=" + u_turns + " r=" + u_ruin + " tw=" + u_tower + " rl=" + u_relay + " | " + "i25=" + i25Refills + "/" + i25Paint + " "
    ...
    // --- iteration 60 SOLDIER-ORIGINATED RENDEZVOUS FUNNEL (additive) ---
    // The tower-originated payload is CLOSED by geometry: no two ruins are within
    // vision (min d^2 = 25 vs vision 20, verified over all 75 maps), and a tower
    // stands exactly ON a ruin, so it can never see a callable target. A soldier
    // sits NEAR a ruin (d^2<=2), so a second ruin reaches d^2 >= 12.9 and IS
    // visible -- which is why the protocol has to originate at the soldier.
    //
    // Measured JOINTLY at the soldier, which is where the code would live. Per
    // iteration 59, a chain must not be estimated as a product of separately
    // measured marginals: 34.0% and 9.2% are not independent and multiplying them
    // would be a guess dressed as a measurement.
    //
    // KNOWN CONSERVATIVE BIAS, stated rather than hidden: step 4 asks whether the
    // tower can reach a SECOND soldier, but a soldier only sees its own r^2=20, so
    // allies near the tower but outside my vision are missed. This UNDERCOUNTS the
    // relay, i.e. it biases toward closing the direction -- safe for a kill, unsafe
    // for a pass, and I will say so if it passes narrowly.
    static long u_turns = 0, u_ruin = 0, u_tower = 0, u_relay = 0;

    static void rendezvousFunnel(RobotController rc) {
        try {
            if (rc.getType() != UnitType.SOLDIER) return;
            u_turns++;
            // 1. an OPEN, UNDER-MANNED ruin in sight
            MapLocation[] rr = rc.senseNearbyRuins(-1);
            RobotInfo[] allies = rc.senseNearbyRobots(-1, rc.getTeam());
            MapLocation target = null;
            for (int i = 0; i < rr.length; i++) {
                if (rc.canSenseRobotAtLocation(rr[i])) continue;      // tower already built
                int w = 0;
                for (int j = 0; j < allies.length; j++)
                    if (allies[j].getType() == UnitType.SOLDIER
                            && allies[j].getLocation().distanceSquaredTo(rr[i]) <= 8) w++;
                if (w < 2) { target = rr[i]; break; }                 // under-manned
            }
            if (target == null) return;
            u_ruin++;
            // 2. can I message a tower?
            RobotInfo tower = null;
            for (int j = 0; j < allies.length; j++)
                if (allies[j].getType().isTowerType()
                        && rc.canSendMessage(allies[j].getLocation())) { tower = allies[j]; break; }
            if (tower == null) return;
            u_tower++;
            // 3. can that tower reach a SECOND soldier? (lower bound: my vision only)
            for (int j = 0; j < allies.length; j++)
                if (allies[j].getType() == UnitType.SOLDIER
                        && allies[j].getID() != rc.getID()
                        && allies[j].getLocation().distanceSquaredTo(tower.getLocation()) <= 20) {
                    u_relay++; break;
                }
