...
public class RobotPlayer {
            ...
            try {
                ...
                rc.setIndicatorString("i25=" + i25Refills + "/" + i25Paint + " "
                        ...
                        + (rc.getType() == UnitType.SOLDIER
                           ? " P45 t=" + P_turns + " busy=" + P_actBusy
                             + " low=" + P_lowPaint + " gated=" + P_couldPaintButGated
                             + " painted=" + P_paintedIt + " idle=" + P_idleNoTarget
                             + " vis=" + P_emptyInVision + " novis=" + P_noEmptyInVision
                             + " dsum=" + P_emptyDistSum
                           : ""));
                Clock.yield();
            }
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        // ---- ITERATION 45 PROBE -------------------------------------------
        // What are a soldier's turns actually DOING? The i44 post-mortem's rule:
        // before diverting a turn, measure what it produced before. Counters are
        // pure observation -- no behaviour below is changed.
        P_turns++;
        int p45 = rc.getPaint();
        boolean ready45 = rc.isActionReady();
        if (!ready45) P_actBusy++;
        else if (p45 < 15) P_lowPaint++;              // blocked by the >=15 gate
        if (p45 >= 5 && p45 < 15) P_couldPaintButGated++;   // 5 buys a paint; gate says no
        if (ready45 && p45 >= 15) {
            boolean inAction = false;
            for (MapInfo t : rc.senseNearbyMapInfos(9)) {
                if (t.getPaint() == PaintType.EMPTY && t.isPassable()
                        && rc.canAttack(t.getMapLocation())) { inAction = true; break; }
            }
            if (inAction) P_paintedIt++;
            else {
                P_idleNoTarget++;                      // action free, paint fine, nothing to paint
                // Is there EMPTY ground in VISION that the soldier is not walking to?
                int bestD45 = 1 << 30;
                for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
                    if (t.getPaint() == PaintType.EMPTY && t.isPassable()) {
                        int d = cur.distanceSquaredTo(t.getMapLocation());
                        if (d < bestD45) bestD45 = d;
                    }
                }
                if (bestD45 < (1 << 30)) { P_emptyInVision++; P_emptyDistSum += bestD45; }
                else P_noEmptyInVision++;
            }
        }
        // -------------------------------------------------------------------
    ...
    static int P_turns, P_actBusy, P_lowPaint, P_couldPaintButGated,
               P_paintedIt, P_idleNoTarget, P_emptyInVision, P_noEmptyInVision;
    static long P_emptyDistSum;
