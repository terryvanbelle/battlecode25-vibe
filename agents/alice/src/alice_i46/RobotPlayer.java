...
public class RobotPlayer {
    ...
    static void runMopper(RobotController rc) throws GameActionException {
        ...
        //
        // ITERATION 46: break TIES by RETENTION. A mopped tile is only worth an
        // action if MY side paints it next; if the enemy repaints it, the mop
        // bought nothing but the ~5 paint the enemy spent. Replay census over
        // three games of src/alice self-play (Portal / Bunny / box, every
        // UnpaintAction matched to the next PaintAction on the same tile):
        //
        //   map     my mops of enemy paint   repainted by ME   handed back
        //   Portal        331                  206 (62%)        125 (38%)
        //   Bunny        2976                 2203 (74%)        773 (26%)
        //   box            39                    6 (15%)         33 (85%)
        //
        // and the hand-backs are FAST: median 1 round, against 3-6 rounds for my
        // own reclaims. The enemy is already standing there. The discriminator
        // replicates in 6/6 team-map cells: a handed-back tile has more enemy
        // paint activity within r^2<=8 in the previous 5 rounds than a kept one
        // (Portal 1.50 vs 0.79, Bunny 1.84 vs 1.59, box 0.79 vs 0.33).
        //
        // The in-game analogue of that proxy is enemy ROBOTS near the tile, which
        // a mopper can sense directly. So among candidates the bot ALREADY treats
        // as equal, prefer the one with fewest enemy robots within r^2<=2.
        //
        // Deliberately a TIE-BREAK and not a gate. The same census says a gate
        // LOSES tiles: gating Portal at "ally painted nearby >= 1" raises kept
        // from 62% to 73% but fires on only 148 of 331 mops, so kept tiles fall
        // 206 -> 108. A suppressed mop does not become a better mop. Re-ranking
        // preserves the action count exactly, which is also pre-check 4: these
        // turns were mopping before and are mopping after.
        //
        // Priorities 0 (pattern-blocking) and 1 (robot on the tile: steal paint)
        // are UNCHANGED and still outrank this. Note the tension, recorded rather
        // than silently resolved: pri 1 deliberately prefers a tile with an enemy
        // robot ON it, which is the class this census says is handed back fastest.
        // Whether the paint drain is worth the tile is a SEPARATE question and a
        // separate iteration; touching it here would confound two mechanisms.
        if (rc.isActionReady()) {
            MapLocation best = null;
            int bestPri = 3;
            int bestFoes = 1 << 30;
            for (MapInfo t : rc.senseNearbyMapInfos(2)) {
                ...
                int foes = (pri == 2) ? foesNear(rc, l) : 0;   // only rank the plain class
                if (pri < bestPri || foes < bestFoes) {
                    bestPri = pri; bestFoes = foes; best = l;
                    if (pri == 0) break;
                }
            }
            if (best != null) rc.attack(best);
    ...
    /** Iteration 46: enemy robots within r^2<=2 of `l` that this mopper can sense.
     *  A proxy for "will the enemy repaint this tile next round" -- the census says
     *  the hand-back happens at a MEDIAN OF 1 ROUND, which only an already-adjacent
     *  enemy unit can do. Vision-limited by construction: a robot outside vision is
     *  counted as absent, so the score is an underestimate, never a hallucination. */
    static int foesNear(RobotController rc, MapLocation l) throws GameActionException {
        int n = 0;
        RobotInfo[] foes = rc.senseNearbyRobots(l, 2, rc.getTeam().opponent());
        for (int i = 0; i < foes.length; i++) if (foes[i].getType().isRobotType()) n++;
        return n;
    }
