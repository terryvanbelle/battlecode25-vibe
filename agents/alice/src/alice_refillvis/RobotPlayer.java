...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("RV h=" + rvHungry + " vis=" + rvVis + " adj=" + rvAdj + " blind=" + rvBlind + " dry=" + rvDry + " i25=" + i25Refills + "/" + i25Paint + " "
    ...
    // ---- REFILL-VISION PROBE (instrumentation only) ------------------------
    // Sizes the gap goRefill leaves: it walks only to a tower it can SEE.
    // hungry            = soldier turns below half paint (goRefill's own test)
    // visSurplus        = of those, a surplus tower within VISION (what goRefill uses)
    // adjacent          = of those, one already within r^2<=2 (tryRefill's case)
    // blind             = of those, NO surplus tower in vision -> soldier wanders dry
    // dry               = hungry turns with paint < 5 (cannot paint at all)
    static int rvHungry, rvVis, rvAdj, rvBlind, rvDry;
    ...
    static boolean goRefill(RobotController rc) throws GameActionException {
        ...
        rvHungry++;
        if (rc.getPaint() < 5) rvDry++;
        {   // probe: classify this hungry turn BEFORE the existing logic runs
            MapLocation q = rc.getLocation();
            boolean vis = false, adj = false;
            RobotInfo[] all = rc.senseNearbyRobots(-1, rc.getTeam());
            for (int i = 0; i < all.length; i++) {
                RobotInfo t = all[i];
                if (!t.getType().isTowerType()) continue;
                if (t.getPaintAmount() - UnitType.SOLDIER.paintCost <= 0) continue;
                vis = true;
                if (q.distanceSquaredTo(t.getLocation()) <= 2) adj = true;
            }
            if (vis) { rvVis++; if (adj) rvAdj++; } else rvBlind++;
        }
