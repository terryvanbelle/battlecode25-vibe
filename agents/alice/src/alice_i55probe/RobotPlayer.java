...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("K t=" + k_turns + " f=" + k_free + " sp=" + k_spare
                        + " so=" + k_sold + " hu=" + k_hungry + " sum=" + k_spareSum + " | "
    ...
    // --- iteration 55 FUNNEL PROBE (purely additive; no behaviour change) ---
    // Lever B fired on ~1% of mopper turns and I do not know WHICH of its four
    // conditions collapsed. Counting them IN ORDER, each as a share of the
    // previous, names the binding one -- which is the difference between a dose
    // change and a dead direction. Priced supply is not delivery.
    static long k_turns = 0;   // mopper turns
    static long k_free  = 0;   //  ... action still ready after the mop attempt
    static long k_spare = 0;   //  ... AND paint - MOP_KEEP > 0
    static long k_sold  = 0;   //  ... AND a SOLDIER within r^2<=2
    static long k_hungry= 0;   //  ... AND that soldier below half tank  (= would fire)
    static long k_spareSum = 0;  // total spare paint available on would-fire turns
    static final int PROBE_KEEP = 50;

    static void mopFunnel(RobotController rc) {
        try {
            k_turns++;
            if (!rc.isActionReady()) return;
            k_free++;
            if (rc.getPaint() - PROBE_KEEP <= 0) return;
            k_spare++;
            RobotInfo[] allies = rc.senseNearbyRobots(2, rc.getTeam());
            boolean anySoldier = false, anyHungry = false;
            for (int i = 0; i < allies.length; i++) {
                if (allies[i].getType() != UnitType.SOLDIER) continue;
                anySoldier = true;
                if (allies[i].getPaintAmount() * 2 < UnitType.SOLDIER.paintCapacity) anyHungry = true;
            }
            if (!anySoldier) return;
            k_sold++;
            if (!anyHungry) return;
            k_hungry++;
            k_spareSum += rc.getPaint() - PROBE_KEEP;
    ...
    static void runMopper(RobotController rc) throws GameActionException {
        ...
        mopFunnel(rc);   // PROBE: additive, measures WHY lever B never fired
