...
public class RobotPlayer {
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            // ITERATION 36 ARM B -- the splasher replaces the MOPPER, never the soldier.
            //
            // Same defect as arm A, different repair. The line below (iteration 28/29) gates
            // the splasher on a runaway CHIP surplus and then overrides `want`
            // UNCONDITIONALLY, including over iteration 5's paint guard three lines above.
            // Because the chip gate never closes once it opens, EVERY tower builds ONLY
            // splashers for the rest of the game and soldier production stops dead.
            //
            ...
            //                        tower paint 86 per tower, SOLDIERS ALIVE = 1,
            //                        splashers 12, cumulative +soldiers 0 / +splashers 20.
            //   Barcode       r1100: $16,890 chips, 15 towers (frozen since r500), 18 splashers.
            // Only soldiers paint tower patterns here, so "no soldiers" means "no new towers",
            // and both maps show tower count frozen from the moment the gate opens.
            //
            // Arm A adds a paint-surplus conjunct, which risks ABLATING iterations 28/29
            // outright: tower paint late-game is ~86 per tower against arm A's 500 threshold,
            // so arm A may build almost no splashers at all. This arm takes the narrower
            // reading of the defect -- the bug is that the override is TOTAL, not that
            // splashers are wrong -- and spends only the quarter of the unit mix that was
            // already going to a mopper. Soldier production can then never stop, while the
            // chip gate iteration 29 tuned is left exactly as it is.
            if (want == UnitType.MOPPER && rc.getMoney() >= CHIP_RESERVE + 2500) {
