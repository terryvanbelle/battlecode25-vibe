...
public class RobotPlayer {
    ...
    // ---- MIN_SPLASH_TILES PROBE (instrumentation only; NEVER in src/alice) ---
    // This build is BEHAVIOURALLY IDENTICAL to alice_iter28. It changes no decision:
    // the attack still fires only when a centre beats MIN_SPLASH_TILES, and the centre
    // chosen is the same one. All it adds is a record of the score that was AVAILABLE.
    //
    // WHY. alice_splashprobe histograms splashScore at the centre the splasher CHOSE,
    // which is by construction already filtered to > MIN_SPLASH_TILES. That distribution
    // cannot say anything about the threshold, because every sample in it passed the
    // threshold. The quantity that reads the dose is the best score AVAILABLE on a
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                // GameConstants.INDICATOR_STRING_MAX_LENGTH is 256 (javap'd on the
                // engine jar, not assumed). A 19-bucket histogram with 4-digit counts
                // plus the standard i25/bc tail overflows that, so a splasher emits the
                // probe string ALONE. Losing the bytecode tail on splashers is fine:
                // overruns are still counted and every other unit still reports.
                if (rc.getType() == UnitType.SPLASHER) {
                    StringBuilder h = new StringBuilder(200);
                    for (int i = 0; i <= 18; i++)
                        if (spAvail[i] > 0) h.append(i).append(':').append(spAvail[i]).append(',');
    ...
    static void runSplasher(RobotController rc) throws GameActionException {
        ...
        if (rc.isActionReady() && rc.getPaint() >= 60) {
            ...
            int bestAny = 0;                    // PROBE ONLY: unfiltered, decides nothing
            for (MapInfo t : rc.senseNearbyMapInfos(rc.getType().actionRadiusSquared)) {
                ...
                if (score > bestAny) bestAny = score;          // PROBE ONLY
                if (score > bestScore) { bestScore = score; best = c; }
            }
            spReadyTurns++;                                    // PROBE ONLY
            spAvail[bestAny < 25 ? bestAny : 25]++;            // PROBE ONLY
            if (best != null) { rc.attack(best); spFired++; }
        }
        wander(rc);
    }
