...
public class RobotPlayer {
    ...
    /** ITERATION M1 (rebuilt, ADDITIVE) -- remove the unit worth zero. Nothing else.
     *  Production efficiency, one pass over full action logs (tiles painted / paint
     *  spent spawning): SPLASHER 0.1291, SOLDIER 0.0603, MOPPER 0.0000 -- the same
     *  ordering replicating independently on another design's units.
     *  The mopper is SUPERSEDED, not inefficient: splasherAttack overwrites enemy
     *  paint directly within r^2<=2 (engine-verified, the iconst_2 branch), so the
     *  ground a mop only ENABLES a splasher takes outright. Mopper marginal value is
     *  0 under absolute coverage, <=0.0976 units/paint under the reading most
     *  generous to it, against a splasher's >=0.1291.
     *  ATTEMPT 1 OF THIS ARM DISABLED THE CHIP-GATED SPLASHER OVERRIDE and the
     *  splasher share FELL 38.2% -> 5.4%: I had priced that override at 2.8% of
     *  decisive-window frames AGAINST CAROL and removed it in a population where it
     *  produced 38.2% of spawns. tools/baseline-yield.py now measures what a
     *  mechanism is already producing in the TEST population before it is touched.
     *  So this build is ADDITIVE: the override stays, volume is untouched, and the
     *  ONLY change is that moppers become soldiers. */
    static final boolean M1_NO_MOPPERS = false;
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            UnitType want = (!M1_NO_MOPPERS && rnd(4) == 0)
                    ? UnitType.MOPPER : UnitType.SOLDIER;
