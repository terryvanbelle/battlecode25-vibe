...
public class RobotPlayer {
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            // ITERATION 36 ARM A -- require a PAINT surplus as well as the chip surplus.
            //
            // The line below (iteration 28/29) gates the splasher on a runaway CHIP surplus
            // and then overrides `want` UNCONDITIONALLY -- including over iteration 5's paint
            // guard three lines above, which exists precisely to stop a tower spending its
            // scarce paint on a unit it can afford more often than a soldier.
            //
            // The splasher is the MOST paint-expensive unit I field: SPLASHER.paintCost 300
            // against SOLDIER 200 and MOPPER 100. So iteration 28 gated the cheapest resource
            // (chips, which it had a runaway surplus of) and left the expensive one ungated.
            // Once money passes CHIP_RESERVE + 2500 the gate never closes again, so EVERY
            // tower builds ONLY splashers for the rest of the game and soldier production
            // stops dead.
            //
            // MEASURED in the SHIPPING bot (alice_iter30) from this run's own replays:
            //   DefaultMedium r1000: $72,980 chips, 12 towers (frozen since r500),
            //                        tower paint 1,036 across 12 towers = 86 each,
            //                        SOLDIERS ALIVE = 1, splashers 12, and in that window
            //                        cumulative +soldiers 0, +splashers 20.
            //   Barcode       r1100: $16,890 chips, 15 towers (frozen since r500), 18 splashers.
            // Only soldiers paint tower patterns in this bot, so "no soldiers" means "no new
            // towers", and both maps show tower count frozen from the moment the gate opens.
            // That is iteration 5's absorbing state, re-entered through a door iteration 28
            // opened after iteration 5 had shut it.
            //
            // The fix is iteration 5's own rule applied to the unit added after it: take the
            // splasher only when the tower could still have afforded a soldier as well.
            // Self-calibrating from UnitType constants, no searched threshold, and the CHIP
            // gate is left exactly as iteration 29 tuned it -- the diff is one conjunct.
            if (rc.getMoney() >= CHIP_RESERVE + 2500
                    && rc.getPaint() >= UnitType.SPLASHER.paintCost + UnitType.SOLDIER.paintCost) {
