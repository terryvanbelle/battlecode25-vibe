...
public class RobotPlayer {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if (ruin != null) {
            ...
            // Iteration 17: choose the tower type for the SCARCER resource instead
            // of by map geometry. Signal = mean PAINT FULLNESS of ally towers in
            // vision, a fraction of each tower's own paintCapacity.
            //
            // Why this signal and not the obvious one: the parity rule it replaces
            // exists because comparing money to a constant AT MARK TIME made every
            // early mark a money tower -- money is always low just as a mark
            // happens. A stock FRACTION is scale-free and slow: tower reserves move
            // over hundreds of rounds, so the decision reflects sustained scarcity
            // rather than the instant of marking. Measured motivation: a mirror game
            // showed 4,885 tower paint idle while chips stayed pinned, i.e. paint
            // income exceeding chip income, which parity cannot respond to.
            //
            // Towers full of paint  -> paint is abundant -> build a MONEY tower.
            // Towers drained        -> paint is scarce   -> build a PAINT tower.
            // No ally tower in vision -> fall back to parity (unchanged behaviour).
            int tp = 0, tcap = 0;
            for (RobotInfo a : rc.senseNearbyRobots(-1, rc.getTeam())) {
                if (!a.getType().isTowerType()) continue;
                tp += a.getPaintAmount();
                tcap += a.getType().paintCapacity;
            }
            UnitType wantTower;
            if (tcap > 0) {
                wantTower = (100 * tp / tcap) >= 75
                        ? UnitType.LEVEL_ONE_MONEY_TOWER : UnitType.LEVEL_ONE_PAINT_TOWER;
            } else {
                wantTower = ((ruin.x + ruin.y) & 1) == 0
                        ? UnitType.LEVEL_ONE_MONEY_TOWER : UnitType.LEVEL_ONE_PAINT_TOWER;
            }
