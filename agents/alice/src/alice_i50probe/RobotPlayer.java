...
public class RobotPlayer {
    ...
    // --- iteration 49 UPKEEP PROBE (purely additive; no behaviour change) ---
    // Discharges the standing re-open condition on LEARNINGS section 3, which was
    // retracted for a broken instrument and says: "Do not plan against clumping
    // without first measuring adjacency on a corrected trace."
    static long q_ut = 0;                       // robot-turns sampled
    static long q_adjU = 0;                     // sum of adjacent ally NON-tower robots
    static long q_adjT = 0;                     // sum of adjacent ally TOWERS (engine counts these)
    static long q_tA = 0, q_tN = 0, q_tE = 0;   // turns ending on ally / neutral / enemy paint
    static long q_up = 0;                       // total end-of-turn paint charged
    static long q_upAdj = 0;                    //   ... of which the adjacency part
    static long q_upT = 0;                      //   ... of which the TOWER-adjacency part

    /** Replicates InternalRobot.processEndOfTurn EXACTLY (javap-verified against the
     *  pinned 3.1.0 jar, 2026-09-09):
     *      mult = (MOPPER) ? 2 : 1
     *      adj  = ally robots within r^2<=2, excluding SELF BY ID, TOWERS INCLUDED
     *      NEUTRAL -> -1*mult - adj ; ENEMY -> -2*mult - 2*adj ; ALLY -> -adj
     *  Towers are included because the engine's loop excludes only self by ID and has
     *  no isRobotType() filter, and getAllRobotsWithinRadiusSquared filters only on
     *  team. My RULES.md said towers were free; that was wrong, and the whole point of
     *  splitting q_adjT out is to SIZE the term I had been told to ignore. */
    static void probeTick(RobotController rc) {
        try {
            if (!rc.getType().isRobotType()) return;
            int myId = rc.getID();
            int adjU = 0, adjT = 0;
            RobotInfo[] near = rc.senseNearbyRobots(2, rc.getTeam());
            for (int i = 0; i < near.length; i++) {
                if (near[i].getID() == myId) continue;   // engine excludes self by ID
                if (near[i].getType().isRobotType()) adjU++; else adjT++;
            }
            int adj = adjU + adjT;
            int mult = rc.getType() == UnitType.MOPPER ? 2 : 1;
            PaintType p = rc.senseMapInfo(rc.getLocation()).getPaint();
            int terrain, adjMult;
            if (p.isAlly())       { terrain = 0;        adjMult = 1; q_tA++; }
            else if (p.isEnemy()) { terrain = 2 * mult; adjMult = 2; q_tE++; }
            else                  { terrain = 1 * mult; adjMult = 1; q_tN++; }
            q_ut++; q_adjU += adjU; q_adjT += adjT;
            if (rerolled) { rerolled = false; headingScan(rc); }
            q_up    += terrain + adj * adjMult;
            q_upAdj += adj * adjMult;
            q_upT   += adjT * adjMult;
        } catch (Exception e) { /* a probe must never change play */ }
    }

    // --- iteration 50 pre-check: is the HEADING choice set degenerate? ---
    // Iteration 47 died because its choice set was a singleton on 100% of turns.
    // Before writing any heading rule, measure whether the 8 candidate directions
    // actually differ in what they walk into. Scaled by 1000 to stay integer.
    static boolean rerolled = false;   // set by wander() when it re-rolls the heading
    static long r_n = 0;               // re-rolls sampled
    static long r_chosen = 0;          // ally-share (per mille) of the direction actually taken
    static long r_best = 0;            // ... of the best of the 8
    static long r_worst = 0;           // ... of the worst of the 8
    static long r_chosenE = 0;         // ENEMY-share (per mille) of the direction taken
    static long r_bestE = 0;           // lowest enemy-share available
    static int r_dirIdx = 0;           // index of the heading wander() actually chose
    static long r_sect = 0;            // live (non-empty) sectors summed over samples

    /** Bucket everything in vision by which of the 8 compass directions it lies in,
     *  and return per-mille ally / enemy paint shares per bucket. Runs in `finally`,
     *  never inside wander(): an overrun mid-wander could suppress the move and
     *  silently change play, which would make the whole probe worthless. */
    static void headingScan(RobotController rc) {
        try {
            MapInfo[] vis = rc.senseNearbyMapInfos();
            int[] ally = new int[8], enemy = new int[8], tot = new int[8];
            MapLocation me = rc.getLocation();
            for (int i = 0; i < vis.length; i++) {
                MapLocation l = vis[i].getMapLocation();
                int dx = l.x - me.x, dy = l.y - me.y;
                if (dx == 0 && dy == 0) continue;
                // True 45-degree sectors, boundaries at 22.5 and 67.5 degrees
                // (tan = 0.414 and 2.414). Bucketing by SIGN instead would give the
                // cardinal buckets only the tiles exactly on an axis -- ~8 tiles of
                // 69 -- and the resulting shares would be noise. Index order matches
                // the `directions` array: N NE E SE S SW W NW.
                int ax = dx < 0 ? -dx : dx, ay = dy < 0 ? -dy : dy;
                int k;
                if (1000 * ay < 414 * ax) k = dx > 0 ? 2 : 6;
                else if (1000 * ay > 2414 * ax) k = dy > 0 ? 0 : 4;
                else if (dx > 0) k = dy > 0 ? 1 : 3;
                else k = dy > 0 ? 7 : 5;
                tot[k]++;
                PaintType p = vis[i].getPaint();
                if (p.isAlly()) ally[k]++; else if (p.isEnemy()) enemy[k]++;
            }
            int best = -1, worst = 1001, bestE = 1001, chosen = 0, chosenE = 0;
            boolean chosenValid = false;
            int secCount = 0;
            for (int k = 0; k < 8; k++) {
                if (tot[k] == 0) continue;
                secCount++;
                int a = ally[k] * 1000 / tot[k], e = enemy[k] * 1000 / tot[k];
                if (a > best) best = a;
                if (a < worst) worst = a;
                if (e < bestE) bestE = e;
                if (k == r_dirIdx) { chosen = a; chosenE = e; chosenValid = true; }
            }
            // BUG FIXED 2026-09-09: without chosenValid, a heading pointing off the
            // map (its 45-degree sector holds no visible tiles) recorded chosen=0 and
            // still counted, dragging the chosen mean BELOW the worst sector and
            // inflating the headroom. Caught by the impossibility `worst > chosen` in
            // the first run -- a sample must never score outside [worst, best].
            if (best < 0 || !chosenValid) return;
            // Also track the total number of live sectors, so a spread computed over
            // few sectors is not read as if it came from eight.
            r_sect += secCount;
            r_n++; r_chosen += chosen; r_best += best; r_worst += worst;
            r_chosenE += chosenE; r_bestE += bestE;
        } catch (Exception e) { /* a probe must never change play */ }
    }
    ...
    // --- ITERATION 39: the expansion-phase signal (rc.getNumberTowers) ---
    // Iterations 28/29/37 all answer "has expansion finished?" with the same PROXY --
    // chips have piled up -- which is true only because there is nothing left to buy.
    // That proxy is map-dependent: chips accumulate at a rate set by how many money
    // towers a map's ruins happened to yield. The engine knows the real quantity and
    // this bot had never called it (found by the scheduled API sweep at iteration 37;
    // engine-verified: getNumberTowers -> TeamInfo.getTotalNumberOfTowers(myTeam)).
    //
    // MEASURED (alice_i39diag, AlarmClock): last tower actually gained r208; this
    // signal fires durably at r274; the chip proxy does not open until r418 -- and on
    // that game the proxy's firing was transient (3 tower-turns all game), so the bot
    // built ZERO splashers while its coverage collapsed 502 -> 294 and it lost.
    //
    // Self-calibrating: no searched constant. Expansion is declared finished when the
    // current dry spell exceeds every gap between tower gains seen so far. TWO gains
    // are required first, because one inter-gain gap does not exist until then --
    // without that, longestGap starts at 0, every dry spell beats it, and the signal
    // would be true from round 2 on every map (verified degenerate before building).
    static int lastTowers = -1;      // running MAX of getNumberTowers()
    static int lastGrowthRound = 0;
    static int longestGap = 0;
    static int towerGrowths = 0;
    static boolean expansionFinished = false;
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                probeTick(rc);   // additive counter; runs after every decision this turn
                ...
                rc.setIndicatorString("R n=" + r_n + " ch=" + r_chosen + " bs=" + r_best
                        + " wo=" + r_worst + " chE=" + r_chosenE + " bsE=" + r_bestE + " se=" + r_sect + " "
                        + "Q ut=" + q_ut + " adjU=" + q_adjU + " adjT=" + q_adjT
                        + " tA=" + q_tA + " tN=" + q_tN + " tE=" + q_tE
                        + " up=" + q_up + " upAdj=" + q_upAdj + " upT=" + q_upT + " | "
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        // --- iteration 39: update the expansion-phase signal (towers only) ---
        int i39round = rc.getRoundNum();
        int i39nt = rc.getNumberTowers();
        if (lastTowers < 0) {                  // first sighting: establish, score no gap
            lastTowers = i39nt; lastGrowthRound = i39round;
        } else if (i39nt > lastTowers) {
            int gap = i39round - lastGrowthRound;
            if (gap > longestGap) longestGap = gap;
            lastGrowthRound = i39round; lastTowers = i39nt; towerGrowths++;
        }
        // NOT a latch: a resumed expansion turns this back off, by construction.
        expansionFinished = towerGrowths >= 2 && (i39round - lastGrowthRound) > longestGap;
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            // ARM C -- AUGMENT rather than replace: fire on whichever signal comes
            // first. This can only ADD splasher production relative to the baseline,
            // never remove it, so it cannot revert iterations 28/29. On maps where the
            // chip proxy already opens early it is byte-identical to the baseline until
            // the phase signal fires, which makes the map-level prediction checkable.
            if (rc.getMoney() >= CHIP_RESERVE + 2500
                    || (expansionFinished
                        && rc.getMoney() >= CHIP_RESERVE + UnitType.SPLASHER.moneyCost)) {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if (ruin != null) {
            ...
            // ITERATION 43. The parity key degenerates: on the four corpus maps whose
            // ruins all share one (x+y) parity (CastleDefense 0/6, Filter 5/0,
            // Snowman 6/0, gridworld 21/0) this builds ONE tower type for the whole
            // game, forfeiting all chip income or all paint income. Censused on
            // CastleDefense: towers sit at the 1000 paint cap for 27% of the game
            // destroying their income, with no money to spend it. Iterations 34/35
            // measured a re-key at 0 on the 75-map corpus -- but 4 degenerate maps
            // cap a corpus effect at +/-8 net swept against a floor of sd 5.29, so
            // that census never had the power to see it. Same hash and constants as
            // alice_i35a, whose ratio (49.1% money) was chosen on map COVERAGE and
            // matches parity's 53.3%; mod 105 rather than 100 because 100 aliases
            // with the ruin lattices and drives one map to 100% money.
            final int MONEY_CUT = 52;
            final int TOWER_KEY_MOD = 105;
            int towerKey = (ruin.x * 13 + ruin.y * 29) % TOWER_KEY_MOD;
            UnitType wantTower = towerKey < MONEY_CUT
    ...
    static void wander(RobotController rc) throws GameActionException {
        ...
        if (wanderDir == null || wanderSteps <= 0) {
            int q = rnd(8);                     // PROBE: capture the index, same draw
            wanderDir = directions[q];
            wanderSteps = WANDER_RUN;
            r_dirIdx = q; rerolled = true;      // PROBE: additive, no behaviour change
        }
        if (rc.canMove(wanderDir)) {
            ...
            if (rc.canMove(l)) { rc.move(l); wanderSteps--; }
                ...
                int q2 = rnd(8);                // PROBE: capture the index, same draw
                wanderDir = directions[q2];
                wanderSteps = WANDER_RUN;
                r_dirIdx = q2; rerolled = true;  // PROBE: additive, no behaviour change
