package alice_paintthief;

/*  SYNTHETIC ARCHETYPE -- a FIXED rung for progress/roster_extra.txt. Never edited
 *  after it is committed; that is the entire point, exactly as for a frozen snapshot.
 *
 *  WHY THIS ONE. Doctrine's self-referential blind spot: every rung of my roster
 *  descends from my own code, so none can detect a weakness the whole lineage shares.
 *  I have a documented one. For 25 accepted iterations this bot could not take
 *  enemy-painted ground AT ALL -- a soldier cannot overwrite enemy paint and a mopper
 *  only clears to EMPTY (RULES.md) -- and no opponent in my pool ever attacked me that
 *  way either, because they are all me. Iteration 28 finally fields splashers, but only
 *  above a runaway chip surplus: it fired on 44 of 75 maps and never fired on the other
 *  31. So "what happens when the opponent takes MY paint, early and constantly" is
 *  still a question my instruments cannot ask.
 *
 *  This archetype asks it. It is a PAINT-DENIAL AGGRESSOR: it builds splashers
 *  whenever it can afford them rather than only out of surplus, and its splashers
 *  attack on a much lower threshold, so it contests ground from the early game.
 *
 *  It also fixes the roster-saturation problem the coordinator raised. After iteration
 *  28 every existing rung sits at 80-100% and three are at exactly 50-0, which is zero
 *  information: a saturated rung reads the same whether I am improving, flat, or
 *  sliding. The remedy is a HARDER fixed reference, never retiring old rungs.
 */
...
public class RobotPlayer {
    ...
    /** ARCHETYPE: attack on almost any conversion, not only a profitable one. */
    static final int MIN_SPLASH_TILES = 2;
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            // ARCHETYPE: splashers ALWAYS when affordable, not only out of surplus.
            if (rc.canBuildRobot(UnitType.SPLASHER, rc.getLocation().add(directions[rnd(8)])))
