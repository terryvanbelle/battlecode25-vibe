...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("C t=" + c_turns + " lg=" + c_legal + " ch=" + c_choice
                        + " sp=" + c_spread + " hd=" + c_head + " mn=" + c_min
                        + " cu=" + c_cur + " | " + "i25=" + i25Refills + "/" + i25Paint + " "
    ...
    // --- iteration 58 CROWD CHOICE-SET PROBE (additive; no behaviour change) ---
    // A movement tie-break can only act if the candidate moves DIFFER in crowd.
    // Iteration 47 died on a singleton choice set; lever B died on 1.2% delivery.
    // So this measures whether a choice EXISTS before anything is built, using an
    // UNBIASED statistic -- a raw spread (max-min), not a best-of-N, which would
    // carry the winner's-curse bias the heading probe had to simulate away.
    // Registered kill: a real choice on under 25% of movement turns.
    static long c_turns = 0;   // movement-ready unit turns
    static long c_legal = 0;   // legal destinations, summed
    static long c_choice = 0;  //  ... turns where max-min adjacent allies >= 1
    static long c_spread = 0;  // (max-min) summed, for the mean spread
    static long c_head = 0;    // (crowd at the tile actually occupied) - (min available)
    static long c_min = 0, c_cur = 0;

    static void crowdChoice(RobotController rc) {
        try {
            if (!rc.getType().isRobotType()) return;
            if (!rc.isMovementReady()) return;
            c_turns++;
            MapLocation me = rc.getLocation();
            int mn = 1 << 30, mx = -1, n = 0;
            for (int d = 0; d < 8; d++) {
                if (!rc.canMove(directions[d])) continue;
                MapLocation t = me.add(directions[d]);
                RobotInfo[] near = rc.senseNearbyRobots(t, 2, rc.getTeam());
                int crowd = 0;
                for (int i = 0; i < near.length; i++)
                    if (near[i].getID() != rc.getID()) crowd++;
                n++;
                if (crowd < mn) mn = crowd;
                if (crowd > mx) mx = crowd;
            }
            if (n == 0) return;
            c_legal += n;
            if (mx - mn >= 1) c_choice++;
            c_spread += (mx - mn);
            // crowd where the unit actually IS, for the headroom statistic
            RobotInfo[] hereN = rc.senseNearbyRobots(me, 2, rc.getTeam());
            int hc = 0;
            for (int i = 0; i < hereN.length; i++)
                if (hereN[i].getID() != rc.getID()) hc++;
            c_cur += hc; c_min += mn; c_head += (hc - mn);
