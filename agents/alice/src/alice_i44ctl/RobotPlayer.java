...
public class RobotPlayer {
    ...
    // ---- ITERATION 44: remember where my own towers are ---------------------
    // MEASURED (alice_refillvis, roads): of a soldier's 158 hungry turns, a
    // surplus tower was in VISION on 6 (3.8%) and adjacent on 2 (1.3%) --
    // goRefill is inert on 96.2% of the turns it exists to serve, because it
    // walks only to a tower it can SEE (senseNearbyRobots(-1), r^2=20) and the
    // bot keeps no memory of towers it has already stood next to. Soldiers then
    // die at 0 paint (5-19 per 100-round window) while towers hold 300-600.
    // At false, javac eliminates every added statement: the control arm is
    // byte-identical by construction.
    static final boolean REMEMBER_TOWERS = false;
    static int nTw = 0;
    static final MapLocation[] TW = new MapLocation[12];

    /** Record every ally tower currently in vision. Pure bookkeeping. */
    static void noteTowers(RobotController rc) throws GameActionException {
        RobotInfo[] near = rc.senseNearbyRobots(-1, rc.getTeam());
        for (int i = 0; i < near.length; i++) {
            if (!near[i].getType().isTowerType()) continue;
            MapLocation l = near[i].getLocation();
            boolean have = false;
            for (int j = 0; j < nTw; j++) { if (TW[j].equals(l)) { have = true; break; } }
            if (!have && nTw < TW.length) TW[nTw++] = l;
        }
    }
    ...
    static boolean goRefill(RobotController rc) throws GameActionException {
        if (REMEMBER_TOWERS) noteTowers(rc);
        ...
        if (tgt == null && REMEMBER_TOWERS) {
            // Nothing in vision -- the 96.2% case. Walk to the nearest tower this
            // soldier has previously stood in vision of. On arrival the normal
            // sense path takes over, so a destroyed or empty tower costs travel
            // and self-corrects rather than latching.
            int bd = 1 << 30;
            for (int i = 0; i < nTw; i++) {
                int d = me.distanceSquaredTo(TW[i]);
                if (d > 2 && d < bd) { bd = d; tgt = TW[i]; }
            }
        }
        if (tgt == null) return false;           // no tower known: nothing to walk to
