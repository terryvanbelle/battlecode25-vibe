        ...
        if (ruin != null) {
            ...
            for (MapInfo t : rc.senseNearbyMapInfos(ruin, 8)) {
                ...
                if (mark != PaintType.EMPTY && mark != t.getPaint()) {
                    // SEPARATING ARM for iteration 23's two co-occurring effects.
                    // alice_i23 does two things in one clause: (1) stops paying 5
                    // paint for an engine-refused attack, and (2) `continue`s, so
                    // the soldier scans ONWARD and lands on a paintable tile
                    // further along the same pattern. This arm keeps (1) and
                    // removes (2): it declines to pay, then STOPS, exactly as the
                    // baseline's control flow did. Difference between this arm and
                    // alice_i23 prices effect (2); difference between this arm and
                    // alice_iter22 prices effect (1).
                    // Two code paths in one line -- section 5b warns the price does
                    // not have to divide the way the story does.
                    if (t.getPaint().isEnemy()) break;
