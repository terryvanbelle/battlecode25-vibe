...
public class RobotPlayer {
    ...
    // ITERATION 41 dose: every SPLASH_EVERY-th soldier spawn becomes a splasher.
    // 0 disables the mechanism entirely (byte-identical control).
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            // --- ITERATION 41: an EARLY, ungated splasher share ---
            // Both existing splasher triggers (the chip proxy, and iteration 39's phase
            // signal) are late-game conditions BY CONSTRUCTION. Measured on four sanctioned
            // tournament replays: on ruin-sparse maps alice builds ZERO splashers while both
            // siblings spawn them from round 2, alice's coverage PEAKS early then declines,
            // and the games end at r400-r654 on MAJORITY_PAINTED -- before any late gate
            // could fire. Alice is at 44.0% on the 25 sparsest maps vs 62.7% elsewhere
            // (2.39 sd, deduplicated on the commit pair, clustered at map level).
            //
            // This REALLOCATES the binding resource (tower paint: 300 to a splasher rather
            // than 200 to a soldier) instead of freeing a non-binding one, which is what
            // iterations 20, 26 and 39a each did and why all three were null-or-negative.
            //
            // A COUNTER, not rnd(): an extra PRNG draw would desynchronise the stream and
            // the k=0 arm would not be a true null -- exactly the defect that made
            // `alice_phase` (a pure seed-offset change, zero policy content) move a 75-map
            // census by +12. With SPLASH_EVERY = 0 the guard short-circuits before the
            // counter is touched, so k=0 is byte-identical to the baseline.
            if (SPLASH_EVERY > 0 && want == UnitType.SOLDIER
                    && ++spawnTick % SPLASH_EVERY == 0) {
