...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("Z t=" + z_turns + " a=" + z_adj + " | " + "i25=" + i25Refills + "/" + i25Paint + " "
    ...
    // Manipulation counter, present IDENTICALLY in the arm, the control and the
    // null arm, so the crowd comparison is like-for-like and comes free in the
    // same batch. Counters are additive -- verified on four probes today, each
    // reproducing its baseline's exact round counts.
    static long z_turns = 0, z_adj = 0;
    static void crowdCount(RobotController rc) {
        try {
            if (!rc.getType().isRobotType()) return;
            z_turns++;
            RobotInfo[] n = rc.senseNearbyRobots(2, rc.getTeam());
            int me = rc.getID();
            for (int i = 0; i < n.length; i++) if (n[i].getID() != me) z_adj++;
        } catch (Exception e) { }
    }


    /** Iteration 58 arm switch. Compile-time constant: at false javac
     *  eliminates the guarded branch entirely. */
    static final boolean CROWD_MOVE = false;

    /** ITERATION 58: choose among the moves already being considered by CROWD.
     *
     *  Re-keys the existing 3-candidate set (heading, then its two rotations) to
     *  prefer the destination with the fewest adjacent ally robots, breaking ties
     *  by directness in the order the caller supplies -- which preserves the
     *  randomised left/right ordering that Phase 0 item 7 requires, and leaves the
     *  HEADING itself untouched so iterations 12/14's ballistic run is unaffected.
     *
     *  Narrow by design: lever A's argmax over all 8 sectors made a WRONG objective
     *  (cheap terrain, i.e. alice's own paint, where a soldier cannot score) bite
     *  harder. The objective here is inverted -- away from own units, toward ground
     *  no ally has worked -- and a narrow dose is the right first test of it.
     *
     *  Crowd is counted with the same predicate the engine charges on: ally robots
     *  within r^2<=2, self excluded by ID (processEndOfTurn, javap-verified). */
    static int crowdAt(RobotController rc, MapLocation t) {
        try {
            RobotInfo[] n = rc.senseNearbyRobots(t, 2, rc.getTeam());
            int c = 0, me = rc.getID();
            for (int i = 0; i < n.length; i++) if (n[i].getID() != me) c++;
            return c;
        } catch (Exception e) { return 0; }
    }

    /** Move along the first legal candidate, or -- with CROWD_MOVE -- the legal
     *  candidate with the least crowd, ties going to the earlier candidate. */
    static boolean moveBest(RobotController rc, Direction a, Direction b, Direction c)
        ...
        if (CROWD_MOVE) {
            ...
            int bc = 1 << 30;
            for (int i = 0; i < 3; i++) {
                Direction d = i == 0 ? a : i == 1 ? b : c;
                if (d == null || !rc.canMove(d)) continue;
                int cr = crowdAt(rc, me.add(d));
                if (cr < bc) { bc = cr; best = d; }
            }
            if (best != null) { rc.move(best); return true; }
            return false;
        } else {
            if (a != null && rc.canMove(a)) { rc.move(a); return true; }
            if (b != null && rc.canMove(b)) { rc.move(b); return true; }
            if (c != null && rc.canMove(c)) { rc.move(c); return true; }
            return false;
        }
    }
    ...
    static void tryMove(RobotController rc, Direction dir) throws GameActionException {
        ...
        // ITERATION 58. Applied HERE and deliberately NOT in wander(): tryMove is
        // DIRECTED movement toward a target, which is exactly where units converge
        // and therefore where crowding is generated. Leaving wander() alone means
        // the ballistic run length iterations 12/14 bought is untouched by
        // construction rather than by argument.
        //
        // if/else with the original verbatim in the else branch, so javac's
        // dead-branch elimination leaves the control byte-identical to `alice` --
        // including its PRNG call pattern, since rnd(2) is consumed only when the
        // direct move is blocked and moving that call would desynchronise the draw.
        if (CROWD_MOVE) {
