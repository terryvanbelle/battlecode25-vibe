...
public class RobotPlayer {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if (ruin != null) {
            ...
            // Iteration 8: DON'T MOVE while there is still paintable ground in reach.
            // Measured: a soldier converts only ~8 of its 200 paint into painted tiles
            // (~1.6 paint actions per lifetime); the other ~192 goes to upkeep. And
            // upkeep is self-inflicted -- the engine charges 0 on an ALLY tile, -1 on
            // neutral, -2 on enemy. A soldier standing on its own paint with no ally
            // adjacent pays NOTHING. Wandering is what drains it: every step lands on
            // unpainted ground and costs -1 for the privilege.
            // So while an empty tile remains within action radius (r^2<=9, ~28 tiles),
            // stand still and paint it from here. Only wander once the neighbourhood
            // is finished.
            boolean workInReach = false;
            for (MapInfo t : rc.senseNearbyMapInfos(9)) {
                if (t.getPaint() == PaintType.EMPTY && t.isPassable()) { workInReach = true; break; }
            }
            if (!workInReach) wander(rc);
        }
