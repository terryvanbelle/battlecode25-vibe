...
public class RobotPlayer {
    ...
    // ------------------------------------------------- iteration 25: refill
    /** ITERATION E2 (the count leg). Measured, not guessed:
     *   - towers are paint-starved on 82.9% of tower-frames (90.9% large), and
     *     at-cap on 0.0% of 2,542 frames -- BROKE, not declining to spawn;
     *   - spawning therefore runs at 15-22% of the engine's CD-10 ceiling;
     *   - meanwhile chips sit at a mean 42,712 on large maps, 42.7 towers'
     *     worth, while a tower cannot afford one 200-paint soldier.
     *  One currency is in 42x surplus and the other binds, so the chips this
     *  forgoes are chips converting into nothing (this is also how the
     *  iteration-34 category error is avoided -- by surplus, not by a rate).
     *  ONE CONSTANT MOVES: the money share of new towers, 49.5% -> 33.3%.
     *  The iteration-43 de-degeneration is untouched -- same hash, same mod 105,
     *  only the cut point -- so no map reverts to building a single type.
     */
    static final boolean E2_MORE_PAINT_TOWERS = false;
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if (ruin != null) {
            ...
            final int MONEY_CUT = E2_MORE_PAINT_TOWERS ? 35 : 52;
