...
public class RobotPlayer {
    ...
    /** ITERATION K3 -- the refill reserve. Measured, not guessed:
     *   - the deficit is PER-UNIT OUTPUT: alice delivers 0.52x carol's paint
     *     actions per unit-round while holding 1.10x towers, 1.35x spawns and
     *     1.29x units alive. Count was never the problem;
     *   - carol withdraws paint from its towers 5.3x more often per unit;
     *   - the guard below reserves a FULL SPAWN COST (200) before a unit may
     *     draw, and towers hold a mean of 154.8 -- below 200 on 82.9% of
     *     tower-frames. The mechanism is disabled in exactly the regime it
     *     exists for: towers are poor BECAUSE they spend everything spawning,
     *     and the refill is gated on them not being poor.
     *  Reserve 200 fires on 17.1% of tower-frames; 50 fires on 98.2% -- a 5.7x
     *  opportunity gain against carol's measured 5.3x transfer advantage.
     *  ONE CONSTANT MOVES. It deliberately trades spawns for endurance, which is
     *  the correct direction given 1.35x spawns at 0.52x per-unit output.
     */
    static final boolean K3_LOW_RESERVE = false;
    ...
    static void tryRefill(RobotController rc) throws GameActionException {
        ...
        for (int i = 0; i < near.length; i++) {
            ...
            int surplus = t.getPaintAmount() - (K3_LOW_RESERVE ? 50 : UnitType.SOLDIER.paintCost);
