...
public class RobotPlayer {
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            // ITERATION 37 (arm a): open the splasher gate EARLIER.
            //
            // Iteration 28 set this gate at CHIP_RESERVE + 5000 (an L3 upgrade, "more chips
            // than anything else the game lets me buy"); iteration 29 lowered it to + 2500
            // (an L2) and was accepted. This arm continues that measured direction to
            // + 1500.
            //
            // WHY, from my own replays rather than from the threshold's own logic:
            // on DefaultMedium at r1000 alice_iter30 has ZERO soldiers and 12 splashers and
            // its coverage climbs 495 -> 623 -> 662, while an arm that kept building soldiers
            // saw coverage FALL 534 -> 312. After expansion ends the splasher is the only unit
            // I field that takes enemy paint, so it is the only unit whose output still
            // converts ground. Iteration 36 tried to REDUCE splashers and one game refuted it;
            // this is that refutation followed in the direction it actually points.
            // My measured deficit is r500-1200 and this gate currently opens around r600 on
            // that map -- late, and only because chips happened to accumulate.
            //
            // The override stays TOTAL, deliberately. Near the threshold the team's money
            // OSCILLATES across it (DefaultMedium: $3,140 at r200, $1,270 at r400, $4,010 at
            // r600), so a lower gate does not mean "only splashers" -- it means a MIXED
            // composition that shifts toward splashers as the surplus grows. That is the
            // behaviour I want, and it needs no new mixing constant.
            //
            // RISK, named before the run: this walks toward iteration 26's cliff, which lost
            // 21 net swept by starving tower construction. A splasher costs 400 CHIPS and
            // completeTowerPattern gates on getMoney() >= 1000, so the gate is only safe while
            // it still sits clear of expansion funding. CHIP_RESERVE and every other chip gate
            // are untouched; the diff is this one constant.
            if (rc.getMoney() >= CHIP_RESERVE + 1500) want = UnitType.SPLASHER;
            int off = rnd(8);
            for (int i = 0; i < 8; i++) {
