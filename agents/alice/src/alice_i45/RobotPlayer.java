...
public class RobotPlayer {
    ...
    static MapLocation exploreTgt = null;
    ...
    static boolean goExplore(RobotController rc) throws GameActionException {
        ...
        // Act ONLY in saturated ground. Any unpainted tile in vision and the
        // normal wander/paint path is left completely alone.
        for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
            if (t.getPaint() == PaintType.EMPTY && t.isPassable()) {
                exploreTgt = null;
                return false;
        ...
        if (exploreTgt == null
                || me.distanceSquaredTo(exploreTgt) <= 8
                || r - exploreSince > EXPLORE_TTL) {
            exploreTgt = new MapLocation(rnd(rc.getMapWidth()), rnd(rc.getMapHeight()));
            exploreSince = r;
        }
        tryMove(rc, me.directionTo(exploreTgt));
        return true;
    }
