...
public class RobotPlayer {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if (ruin != null) {
            ...
            tryMove(rc, toRuin);
            me = rc.getLocation();
            // ITERATION 35 (arm a): DOSE-FIND the money/paint tower share on ONE key.
            //
            // Iteration 34 set this share to 24% money on the reasoning that a soldier needs
            // 250 chips and 200 tower paint, so balanced soldier throughput wants 3.2 paint
            // towers per money tower. It lost catastrophically, and the replay says why:
            // on Barcode the candidate's TOWER COUNT FROZE AT 7 from r200 to r1121 while the
            // baseline compounded to 15, and the extra tower paint (2,201 vs 1,129 at r300)
            // simply STRANDED -- it drained away unspent while the army stayed at ~10 soldiers
            // and coverage decayed 437 -> 306.
            //
            // The arithmetic was right and the UNITS were wrong. Chips buy TOWERS
            // (completeTowerPattern gates on getMoney() >= 1000; upgrades cost 2,500/5,000),
            // and a tower produces chips AND paint forever. Paint buys nothing that produces.
            // So 20 chips/turn is not "0.08 soldiers/turn", it is ~0.02 TOWERS/turn, and
            // dividing both currencies by SOLDIER.*Cost erases exactly the asymmetry that
            // matters. A per-turn ratio between an investment and a consumable is a category
            // error, not a dose. (LEARNINGS: "chips COMPOUND, paint does not" -- third firing.)
            //
            // So this is an HONEST DOSE SEARCH along a gradient I have now measured, not a
            // derivation, and I am labelling it as one. Three points on ONE key:
            //     24.3% money  = iteration 34, full census, catastrophic
            //     49.1% money  = THIS ARM
            //   parity (the shipping rule) sits at 53.3% but is DEGENERATE on four maps:
            //   gridworld/Filter/Snowman have even-only ruins (100% money, zero paint towers)
            //   and CastleDefense odd-only (zero money towers).
            //
            // The key (13, 29) mod 105 is unchanged from iteration 34 and was validated
            // against all 1,374 ruin coordinates in the 75 official maps: at this threshold
            // NO map is single-branch. Modulus 105 rather than 100 for exactly that reason --
            // mod 100 aliases with the ruin lattices and drives one map to 100% money.
            // Selection is on map COVERAGE, never on win rate.
            final int MONEY_CUT = 52;      // 49.1% of ruins -> money tower
