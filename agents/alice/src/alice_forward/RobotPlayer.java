...
public class RobotPlayer {
    ...
    // STRESS ARCHETYPE. Alice cannot concentrate units on a target (1.01 soldiers
    // arrive at an enemy tower) and its SOLDIERS NEVER TARGET TOWERS AT ALL (0% of
    // its tower damage). So a tower planted inside alice's half cannot be removed by
    // the units standing around it -- tower damage is permanent [E]. This archetype
    // expands FORWARD to exploit exactly that, and in doing so manufactures the
    // soldier-tower contact iteration 15b named as its re-open instrument.
    static MapLocation home, enemyRef;
    static void initRefs(RobotController rc) {
        if (home != null) return;
        home = rc.getLocation();
        enemyRef = new MapLocation(rc.getMapWidth() - 1 - home.x,
                                   rc.getMapHeight() - 1 - home.y);
    }
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        for (MapLocation r : ruins) {
            ...
            int d = enemyRef.distanceSquaredTo(r);   // ARCHETYPE: forward, not nearest
    ...
    static void wander(RobotController rc) throws GameActionException {
        ...
        if (wanderDir == null || wanderSteps <= 0) {
            // ARCHETYPE: head for the enemy half 3 rolls in 4; keep a random roll so
            // the army still spreads rather than filing along one line.
            Direction toEnemy = rc.getLocation().directionTo(enemyRef);
            wanderDir = (rnd(4) != 0 && toEnemy != Direction.CENTER)
                    ? toEnemy : directions[rnd(8)];
