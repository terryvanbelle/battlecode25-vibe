...
public class RobotPlayer {
    ...
    // --- ITERATION 23 PRE-CHECK: count the DECISION, not the outcome ---
    // soldierAttack debits 5 paint at bytecode offset 58, unconditionally, and
    // only THEN checks the target; an enemy-painted tile returns at offset 207
    // with the paint already gone. This counts how many of a soldier's
    // ruin-pattern attacks are aimed at a tile the engine will refuse.
    static int pgOk = 0;      // pattern attacks aimed at an empty-or-ally tile (real work)
    static int pgEnemy = 0;   // pattern attacks aimed at ENEMY paint (5 paint, no-op)
    static int pgTurns = 0;   // soldier turns that reached the pattern loop at all
    static int pgBreak = 0;   // turns where the wasted attack also consumed the `break`
        ...
        while (true) {
            ...
            try {
                ...
                if (rc.getType() == UnitType.SOLDIER) {
                    rc.setIndicatorString("PG ok=" + pgOk + " en=" + pgEnemy
                            + " turns=" + pgTurns + " mp=" + rc.getPaint()
            ...
            for (MapInfo t : rc.senseNearbyMapInfos(ruin, 8)) {
                ...
                if (mark != PaintType.EMPTY && mark != t.getPaint()) {
                    if (rc.canAttack(t.getMapLocation())) {
                        if (t.getPaint().isEnemy()) { pgEnemy++; pgBreak++; } else pgOk++;
