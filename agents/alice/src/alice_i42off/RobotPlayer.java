...
public class RobotPlayer {
    ...
    /** ITERATION 42. The splasher preference below is an UNCONDITIONAL assignment: once
     *  team money passes CHIP_RESERVE+2500 it is true on essentially every tower turn
     *  forever, so past that point the tower builds splashers and NOTHING else. Censused
     *  on BatSignal: the productive tower spent 427 of 1410 turns (30%) holding 200-299
     *  paint -- enough for a soldier, not enough for a splasher's 300 -- and built nothing.
     *  Since markTowerPattern/completeTowerPattern live only in runSoldier, zero soldiers
     *  means expansion stops permanently, which is the mechanism behind the coverage that
     *  peaks and then declines. This falls back to the unit the tower can afford rather
     *  than idling. It never cancels a splasher build; it only fills a dead turn.
     *  A compile-time constant, so at false javac removes every added statement and the
     *  control arm is byte-identical -- no extra rnd() draw, which is the alice_phase trap. */
    static final boolean SOLDIER_FALLBACK = false;
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            if (SOLDIER_FALLBACK && want == UnitType.SPLASHER
