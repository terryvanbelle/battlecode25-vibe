...
public class RobotPlayer {
    ...
    /** Is there ground THIS TYPE could work on, anywhere in vision (not just action range)?
     *  soldier: EMPTY passable. splasher: EMPTY passable or enemy paint. mopper: enemy paint. */
    static int _visWork(RobotController rc) throws GameActionException {
        UnitType ty = rc.getType();
        for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
            if (!t.isPassable()) continue;
            PaintType pt = t.getPaint();
            if (ty == UnitType.MOPPER) { if (pt.isEnemy()) return 1; }
            else if (ty == UnitType.SPLASHER) { if (pt == PaintType.EMPTY || pt.isEnemy()) return 1; }
            else { if (pt == PaintType.EMPTY) return 1; }
        }
        return 0;
    }
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if (rc.isActionReady() && rc.getPaint() >= 15) {
            ...
            if (paintTarget != null) { rc.attack(paintTarget); _ab = 1; }
            else _ab = (_visWork(rc) == 1) ? 6 : 4;
        }
    }
    ...
    static void runSplasher(RobotController rc) throws GameActionException {
        ...
        if (rc.isActionReady() && rc.getPaint() >= 60) {
            ...
            if (best != null) { rc.attack(best); _ab = 1; }
            else _ab = (_visWork(rc) == 1) ? 6 : 4;
        }
        wander(rc);
    }
