...
public class RobotPlayer {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if (ruin != null) {
            ...
            tryMove(rc, toRuin);
            me = rc.getLocation();
            // ITERATION 34: choose the tower TYPE by which resource actually binds.
            //
            // The old rule was ruin parity -- a ~50/50 money/paint mix decided by an
            // arbitrary geometric coin-flip. It has two measured defects.
            //
            // 1. THE RATIO IS WRONG, and the right one is an engine constant.
            //    A soldier costs 250 CHIPS and 200 TOWER PAINT. Per turn:
            //      money tower  moneyPerTurn 20 / 250 = 0.080 soldiers
            //      paint tower  paintPerTurn  5 / 200 = 0.025 soldiers
            //    A money tower funds soldiers 3.2x faster than a paint tower does, so
            //    balancing soldier throughput needs 3.2 paint towers per money tower --
            //    a 23.8% money share, not 50%. All four constants verified by javap on
            //    battlecode25-java-3.1.0 (UnitType field order is paintCost, moneyCost,
            //    attackCost, health, level, paintCapacity, actionCooldown,
            //    actionRadiusSquared, attackStrength, aoeAttackStrength, paintPerTurn,
            //    moneyPerTurn, attackMoneyBonus).
            //    Worse than the ratio: a money tower has paintPerTurn 0. It spawns two
            //    soldiers from the 500 paint it is born with and is then dead weight for
            //    unit production forever, absent a mopper transfer.
            //
            // 2. PARITY DEGENERATES ON FOUR OF THE 75 MAPS (tools/mapdata/README.md):
            //    gridworld, Filter and Snowman have ruins on EVEN tiles only, so this
            //    line builds 100% MONEY towers and ZERO paint towers there; CastleDefense
            //    is all-odd and builds zero money towers.
            //
            // Both defects push the same way, and the evidence says paint is what binds:
            // iteration 5 measured that a tower "never accumulates the 200 a soldier
            // needs"; replay forensics has my chips IDLE at $1,000-3,700 while my soldier
            // count stalls at 10-23; and iterations 26 (-21), 32 (-1) and 33 (0) all
            // spent CHIPS and all netted <= 0.
            //
            // The dose is engine-derived. The KEY (13, 29) is not -- it was picked from a
            // short list by testing every one of the 1,374 ruin coordinates in the 75
            // official maps and keeping the only candidate with NO single-branch map.
            // That is selection on map COVERAGE, not on win rate, which is the same
            // criterion tools/mapdata already demands. Realized: 24.3% money corpus-wide,
            // every map between 6% and 50%, minimum 1 money and 3 paint towers per map.
            // Paint-tower share over the corpus goes 46.7% -> 75.7%.
            //
            // Still purely positional, so still decided by the map and still immune to
            // the money-at-mark-time timing artifact the parity rule was written to dodge.
            final int SOLDIERS_PER_MONEY_TOWER =
                    1000 * UnitType.LEVEL_ONE_MONEY_TOWER.moneyPerTurn / UnitType.SOLDIER.moneyCost;   // 80
            final int SOLDIERS_PER_PAINT_TOWER =
                    1000 * UnitType.LEVEL_ONE_PAINT_TOWER.paintPerTurn / UnitType.SOLDIER.paintCost;   // 25
            int towerKey = (ruin.x * 13 + ruin.y * 29) % (SOLDIERS_PER_MONEY_TOWER + SOLDIERS_PER_PAINT_TOWER);
            UnitType wantTower = towerKey < SOLDIERS_PER_PAINT_TOWER
