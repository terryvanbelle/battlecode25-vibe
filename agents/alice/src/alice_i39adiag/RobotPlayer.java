...
public class RobotPlayer {
    ...
    // --- iteration 39 diag: the expansion-phase signal vs the chip proxy ---
    static int lastTowers = -1;    // running MAX of rc.getNumberTowers()
    static int lastGrowthRound = 0;
    static int longestGap = 0;     // longest observed gap between tower gains
    static int towerGrowths = 0;   // gains observed by THIS robot
    static int expFinRound = -1;   // first round the state test said "finished"
    static int chipGateRound = -1; // first round the shipping chip proxy opened
    static boolean expansionFinished = false;
    // decision-point counters for the SPLASHER override (instrument the decision,
    // not the outcome): how often the gate fires, and what happens when it does.
    static int gateFired = 0;      // turns the chip gate made want=SPLASHER
    static int gateNoPaint = 0;    //   ... of those, tower paint < 300 (unbuildable)
    static int gateBuilt = 0;      //   ... of those, a splasher was actually built
    static int buildAttempts = 0;  // turns we reached the build block at all
    static int builtNothing = 0;   // turns the build block built nothing
    // THE discriminating counter: the gate fired, a SOLDIER was affordable in
    // paint, and the splasher was not -- so the override displaced a real unit
    // and produced nothing. This is the reallocation's price, not its benefit.
    static int gateAteSoldier = 0;
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString(!rc.getType().isRobotType()
                        ? ("I39 E=" + expFinRound + " C=" + chipGateRound
                           + " nt=" + lastTowers + " g=" + towerGrowths
                           + " G=" + longestGap + " lgr=" + lastGrowthRound
                           + " $=" + rc.getMoney() + " fin=" + (expansionFinished ? 1 : 0)
                           + " ATT=" + buildAttempts + " NIL=" + builtNothing
                           + " GF=" + gateFired + " GNP=" + gateNoPaint + " GB=" + gateBuilt
                           + " ATE=" + gateAteSoldier)
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        // --- iteration 39 diag: measure both phase signals every tower turn ---
        int rnum = rc.getRoundNum();
        int nt = rc.getNumberTowers();
        if (lastTowers < 0) {                 // first sighting: establish, do not score a gap
            lastTowers = nt; lastGrowthRound = rnum;
        } else if (nt > lastTowers) {
            int gap = rnum - lastGrowthRound;
            if (gap > longestGap) longestGap = gap;
            lastGrowthRound = rnum; lastTowers = nt; towerGrowths++;
        }
        // Needs TWO gains before one inter-gain gap exists; without this the
        // floorless form of the draft fires at round 2 on every map.
        expansionFinished = towerGrowths >= 2 && (rnum - lastGrowthRound) > longestGap;
        if (expansionFinished && expFinRound < 0) expFinRound = rnum;
        if (rc.getMoney() >= CHIP_RESERVE + 2500 && chipGateRound < 0) chipGateRound = rnum;
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            boolean gate = rc.getMoney() >= CHIP_RESERVE + 2500
                    && rc.getPaint() >= UnitType.SPLASHER.paintCost;   // iteration 39 guard
            if (gate) want = UnitType.SPLASHER;
            buildAttempts++;
            if (gate) {
                gateFired++;
                int tp = rc.getPaint();
                if (tp < UnitType.SPLASHER.paintCost) {
                    gateNoPaint++;
                    if (tp >= UnitType.SOLDIER.paintCost) gateAteSoldier++;
                }
            }
            int off = rnd(8);
            boolean did = false;
            for (int i = 0; i < 8; i++) {
                ...
                if (rc.canBuildRobot(want, loc)) { rc.buildRobot(want, loc); did = true; break; }
            }
            if (!did) builtNothing++;
            if (gate && did) gateBuilt++;
        }
    }
