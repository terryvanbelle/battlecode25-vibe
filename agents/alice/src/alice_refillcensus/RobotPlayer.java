...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                if (rc.getType().isRobotType()) refillCensus(rc);
                switch (rc.getType()) {
                ...
                rc.setIndicatorString((rc.getType() == UnitType.MOPPER
                        ...
                        + (nearMisses > 0 ? " near=" + nearMisses : "") + qStr);
                Clock.yield();
            }
        }
    }
    ...
    // ---- REFILL REACHABILITY CENSUS (instrumentation only; never src/alice) --
    // Question: a robot can WITHDRAW paint from an ally tower for ZERO chips
    // (transferPaint(towerLoc, -N), r^2 <= 2, CD 10). The bot has never called it.
    // Towers demonstrably sit on idle paint (tower census: paint >= 300 on 6.8% of
    // tower-turns, unspendable because the CHIP gate is shut). Starvation is the
    // largest single death cause here (31% after iteration 24).
    // But BEFORE building anything: are low-paint robots ever actually in range of
    // an ally tower? That is the reachability question the splasher work just
    // taught me to ask FIRST. All queries below are pure; none consumes the action.
    static int qT, qP0, qLow25, qLow50, qAdj, qCan, qLow25Can, qLow50Can;
    static int qGain, qTowPaint;
    static String qStr = "";

    static void refillCensus(RobotController rc) throws GameActionException {
        qT++;
        int cap = rc.getType().paintCapacity;
        int p = rc.getPaint();
        if (p == 0) qP0++;
        boolean low25 = p * 4 < cap;
        boolean low50 = p * 2 < cap;
        if (low25) qLow25++;
        if (low50) qLow50++;
        // ally towers within the transfer radius (r^2 <= 2)
        RobotInfo[] near = rc.senseNearbyRobots(2, rc.getTeam());
        boolean adj = false, can = false;
        int bestGain = 0;
        for (int i = 0; i < near.length; i++) {
            RobotInfo t = near[i];
            if (!t.getType().isTowerType()) continue;
            adj = true;
            if (rc.canTransferPaint(t.getLocation(), -1)) {
                can = true;
                int g = cap - p;
                int tp = t.getPaintAmount();
                if (tp < g) g = tp;
                if (g > bestGain) bestGain = g;
                if (tp > qTowPaint) qTowPaint = tp;
            }
        }
        if (adj) qAdj++;
        if (can) qCan++;
        if (low25 && can) qLow25Can++;
        if (low50 && can) { qLow50Can++; qGain += bestGain; }
        qStr = " RC t=" + qT + " p0=" + qP0 + " l25=" + qLow25 + " l50=" + qLow50
                + " adj=" + qAdj + " can=" + qCan + " l25c=" + qLow25Can
                + " l50c=" + qLow50Can + " gain=" + qGain + " tp=" + qTowPaint;
    }
