...
public class RobotPlayer {
    ...
    /** ITERATION 47: the ruin this soldier has given up on, and the round it may
     *  be reconsidered. See TRAINING_LOG: on 42.2% of all ruin-target turns
     *  (6,424 of 15,229, 3 maps) the soldier stands action-ready at a MARKED
     *  pattern whose every mismatched tile is ENEMY paint -- a state no soldier
     *  action can resolve, because a soldier can never overwrite enemy paint.
     *  Probe v2: on 100.0% of those turns the soldier saw exactly ONE open ruin,
     *  so there is no better ruin to switch to. The only alternative to standing
     *  there is to LEAVE, which is why this is a search mechanism and not a
     *  ranking one. Dose I47_AVOID; 0 is byte-identical to iteration 46. */
    static MapLocation i47avoidRuin;
    static int i47avoidUntil;
    static final int I47_AVOID = 250;
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        for (MapLocation r : ruins) {
            ...
            // ITERATION 47: a ruin this soldier gave up on as unresolvable.
            if (I47_AVOID > 0 && r.equals(i47avoidRuin)
                    && rc.getRoundNum() < i47avoidUntil) continue;
        ...
        if (ruin != null) {
            ...
            int qMarked = 0, qMism = 0, qEnemy = 0;
            ...
            for (MapInfo t : rc.senseNearbyMapInfos(ruin, 8)) {
                ...
                if (mark != PaintType.EMPTY && mark != t.getPaint()) {
                    qMism++;
                    if (t.getPaint().isEnemy()) { qEnemy++; continue; }  // refused by the engine; 5 paint for nothing
            ...
            // ITERATION 47. The unresolvable state, exactly as the probe classified
            // it: a marked pattern, the action still in hand, and every mismatched
            // tile enemy paint. Nothing this unit can do changes it. Give the ruin
            // up and let the iteration-12 ballistic wander look elsewhere.
            // NB no ruin-centre special case is needed: GameWorld.setMarker drops
            // the write on any unpaintable tile, so the centre is never marked
            // (RULES.md, corrected 2026-09-09; probe measured cm=0 over 15,229 turns).
            if (I47_AVOID > 0 && !qAttacked && qMarked > 0 && qMism > 0
                    && qMism == qEnemy && rc.isActionReady()) {
                i47avoidRuin = ruin;
                i47avoidUntil = rc.getRoundNum() + I47_AVOID;
            }
