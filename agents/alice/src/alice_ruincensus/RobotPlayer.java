...
public class RobotPlayer {
    ...
    // --- RUIN-COLLISION PRE-CHECK (must precede any comms schema) ---
    // Soldiers pick the NEAREST unclaimed ruin in vision, independently and with
    // no coordination. The comms hypothesis is that several converge on one ruin
    // while others go unworked. That is a countable quantity and I have not
    // counted it -- so this reports each soldier's CURRENT TARGET each turn and
    // the counting happens offline, across soldiers, from the replay. Nothing is
    // decided in-bot, because a soldier cannot see another soldier's target and
    // pretending otherwise would be the instrument inventing the answer.
    static MapLocation rcTarget = null;   // this soldier's chosen ruin this turn, or null
        ...
        while (true) {
            ...
            try {
                ...
                if (rc.getType() == UnitType.SOLDIER) {
                    rc.setIndicatorString("RT "
                            + (rcTarget == null ? "none" : rcTarget.x + "," + rcTarget.y)
