...
public class RobotPlayer {
    ...
    /** Iteration 9: unbuilt ruins this robot has seen. senseNearbyRuins only reaches
     *  vision (r^2=20), so without memory the bot discovers ruins by stumbling into
     *  them and forgets every one it does not act on immediately. Towers are the
     *  master variable (traces: 16v2, 8v6, 8v6 towers decided those games). */
    static final MapLocation[] knownRuins = new MapLocation[16];
    static int nRuins = 0;
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        for (MapLocation r : ruins) {
            boolean built = rc.canSenseRobotAtLocation(r);
            noteRuin(r, built);                          // iteration 9: remember it
            if (built) continue;                         // tower already there
        ...
        if (ruin != null) {
            ...
            // Iteration 9: no unbuilt ruin in sight -- head for the nearest one we
            // REMEMBER rather than wandering at random. Wandering is currently the
            // bot's only ruin-discovery mechanism, which is why it plateaus at a few
            // towers while maps carry 12-32 ruins.
            MapLocation goal = null;
            int gd = 1 << 30;
            for (int i = 0; i < nRuins; i++) {
                int d = me.distanceSquaredTo(knownRuins[i]);
                if (d < gd) { gd = d; goal = knownRuins[i]; }
            }
            if (goal != null) tryMove(rc, me.directionTo(goal));
            else wander(rc);
        }
    ...
    /** Record an unbuilt ruin; drop one that has since been built. */
    static void noteRuin(MapLocation r, boolean built) {
        for (int i = 0; i < nRuins; i++) {
            if (knownRuins[i].equals(r)) {
                if (built) { knownRuins[i] = knownRuins[--nRuins]; }
                return;
            }
        }
        if (!built && nRuins < knownRuins.length) knownRuins[nRuins++] = r;
    }
