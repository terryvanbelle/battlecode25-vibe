...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("K6 " + k6hr[0] + " " + k6hr[1] + " " + k6hr[2]
                        + " " + k6hr[3] + " " + k6hr[4] + " " + k6hr[5]
                        + " " + k6rn[0] + " " + k6rn[1] + " " + k6rn[2]
                        + " " + k6rn[3] + " " + k6rn[4] + " " + k6rn[5]
                        + " " + k6acc + " "
    ...
    /** K6 probe (ADDITIVE): the CROSS-TABULATION, not the stages.
     *  K4 counted stages and K5 counted a distribution; both are marginals of
     *  the thing needed, and the same threshold read 23.5% one way and 42.2%
     *  the other. A funnel's natural instrumentation is per-stage and per-stage
     *  counters are marginals BY CONSTRUCTION, so this records the joint cell
     *  directly: reachable-tower paint bucketed, SPLIT by (hungry, ready).
     *  k6hr = hungry AND ready, the population the shipped mechanism serves.
     *  k6rn = ready but NOT hungry, the population a gate-1 relaxation adds --
     *  and for that cell the paint a unit could ACCEPT is capped at cap-p, so
     *  k6acc accumulates it: rate and volume must be priced together. */
    static final int[] k6hr = new int[6];
    static final int[] k6rn = new int[6];
    static int k6acc;
    ...
    static void tryRefill(RobotController rc) throws GameActionException {
        ...
        int p = rc.getPaint();
        boolean k6h = p * 2 < cap;
        boolean k6r = rc.isActionReady();
        if (k6r) {
            RobotInfo[] k6n = rc.senseNearbyRobots(2, rc.getTeam());
            int k6best = Integer.MIN_VALUE;
            for (int k = 0; k < k6n.length; k++) {
                if (!k6n[k].getType().isTowerType()) continue;
                int pa = k6n[k].getPaintAmount();
                if (pa > k6best) k6best = pa;
            }
            if (k6best > Integer.MIN_VALUE) {
                int b = k6best <= 0 ? 0 : k6best <= 25 ? 1 : k6best <= 50 ? 2
                        : k6best <= 100 ? 3 : k6best <= 200 ? 4 : 5;
                if (k6h) k6hr[b]++;
                else { k6rn[b]++; k6acc += cap - p; }
            }
        }
