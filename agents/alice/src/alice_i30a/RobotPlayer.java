...
public class RobotPlayer {
    ...
    // ---------------------------------------------------------------- soldier
    /** ARM A -- the zero-opportunity-cost dose. Divert only once the soldier
     *  cannot paint a single tile, so the painting it gives up is provably none.
     *  attackCost is an engine constant, not a searched one. */
    static boolean REFILL_WALK(RobotController rc) {
        return rc.getPaint() < rc.getType().attackCost;
    }
