...
public class RobotPlayer {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        if (ruin != null) {
            ...
            // ITERATION 27: the MIRROR dose of iteration 26, on the same knob.
            //
            // Iteration 26 moved this ratio to 25% money / 75% paint and lost the census
            // 54-150 (-21 net swept), with the candidate holding 3-160x the baseline's
            // tower paint while trailing it on map COVERAGE, which is the win condition.
            // Reading: at 50/50 the binding resource for actually FIELDING units is
            // chips, not paint. A soldier costs 250 chips AND 200 paint, and chips also
            // pay for tower construction and upgrades, so paint accumulates unspendably
            // in towers when the chip side of the pair runs out first.
            //
            // If that reading is right the slope keeps rising past 50%, so this tests
            // the OTHER side of the knob -- the direction no iteration has ever tried.
            // If it is wrong, 50/50 is an interior optimum and the knob is settled, which
            // closes the question either way. That is why the mirror dose is worth a
            // census even though it is a guess: both outcomes are conclusive.
            //
            // Same predicate, same immunity. Still a pure function of the ruin's
            // coordinates -- identical for both teams, evaluated at mark time but not
            // dependent on it, so the money-at-mark-time artifact stays fixed.
            //   & 1  == 0 -> ~50% money   (iteration 25, the baseline)
            //   & 3  == 0 -> ~25% money   (iteration 26, REJECTED at 36.0%)
            //   & 3  != 0 -> ~75% money   (this)
            UnitType wantTower = ((ruin.x + ruin.y) & 3) != 0
