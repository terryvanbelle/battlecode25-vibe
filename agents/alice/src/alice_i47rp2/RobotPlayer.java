...
public class RobotPlayer {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("i25=" + i25Refills + "/" + i25Paint + " "
                        ...
                        + (rc.getType() == UnitType.SOLDIER
                        ? "Q at=" + q_at + " atk=" + q_atk + " ac=" + q_atkCentre
                          + " cm=" + q_centreMism + " um=" + q_unmarked + " fu=" + q_full
                          + " na=" + q_noact + " se=" + q_stallEnemy + " oo=" + q_stallOOR
                          + " mk=" + q_mark + " dn=" + q_done
                          + " nr=" + q_near + " fr=" + q_far + " sn=" + q_seNear
                          + " c1=" + q_cs1 + " c2=" + q_cs2 + " c3=" + q_cs3
                          + " os=" + q_openSum + " " : "")
                        + "bc=" + bc + " max=" + maxBc
    ...
    // ---- ITERATION 47 PROBE: ruin-capture counters (no behaviour change) ----
    static int q_at, q_atk, q_atkCentre, q_centreMism, q_unmarked, q_full,
               q_noact, q_stallEnemy, q_stallOOR, q_mark, q_done;
    // PROBE v2: the CHOICE SET at the moment the enemy-blocked stall is classified,
    // and the travel/stand split that v1 could not make.
    // q_cs1/2/3 = open ruins in vision (1, 2, >=3) on a `se` turn.
    // q_near/q_far = ruin-target turns standing at the ruin (d^2<=2) vs travelling.
    static int q_cs1, q_cs2, q_cs3, q_near, q_far, q_seNear, q_openSum;
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        MapLocation me = rc.getLocation();
        int bestD = 1 << 30;
        int qOpen = 0;                       // PROBE v2: size of the choice set
        ...
        if (ruin != null) {
            ...
            // ---- ITERATION 47 PROBE. Counters only; every branch that decides an
            // action is byte-for-byte the original. qMism/qEnemy EXCLUDE the ruin
            // centre, which RULES.md says always carries a mark it can never satisfy.
            int qMarked = 0, qMism = 0, qEnemy = 0, qCentre = 0;
            boolean qAttacked = false;
            q_at++;
            q_openSum += qOpen;
            boolean qNear = me.distanceSquaredTo(ruin) <= 2;   // `me` is post-move here
            if (qNear) q_near++; else q_far++;
            for (MapInfo t : rc.senseNearbyMapInfos(ruin, 8)) {
                PaintType mark = t.getMark();
                if (mark != PaintType.EMPTY) qMarked++;
                if (mark != PaintType.EMPTY && mark != t.getPaint()) {
                    boolean isCentre = t.getMapLocation().equals(ruin);
                    if (isCentre) qCentre++; else qMism++;
                    if (t.getPaint().isEnemy()) { if (!isCentre) qEnemy++; continue; }
                    if (rc.canAttack(t.getMapLocation())) {
                        rc.attack(t.getMapLocation(), mark == PaintType.ALLY_SECONDARY);
                        qAttacked = true; q_atk++;
                        if (isCentre) q_atkCentre++;
                        break;
                    }
                }
            }
            if (qCentre > 0) q_centreMism++;
            if (!qAttacked) {
                if (qMarked == 0) q_unmarked++;
                else if (qMism == 0) q_full++;
                else if (!rc.isActionReady()) q_noact++;
                else if (qMism == qEnemy) {
                    q_stallEnemy++;
                    if (qNear) q_seNear++;
                    if (qOpen <= 1) q_cs1++; else if (qOpen == 2) q_cs2++; else q_cs3++;
                }
                else q_stallOOR++;
            }
            ...
            if (rc.canCompleteTowerPattern(UnitType.LEVEL_ONE_PAINT_TOWER, ruin)) {
                rc.completeTowerPattern(UnitType.LEVEL_ONE_PAINT_TOWER, ruin); q_done++;
            } else if (rc.canCompleteTowerPattern(UnitType.LEVEL_ONE_MONEY_TOWER, ruin)) {
                rc.completeTowerPattern(UnitType.LEVEL_ONE_MONEY_TOWER, ruin); q_done++;
            }
        } else {
            wander(rc);
        }
