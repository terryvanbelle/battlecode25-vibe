...
public class RobotPlayer {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        ...
        // ITERATION 24: recover the one case iteration 22's deletion lost.
        // Exhaustive case analysis of the deleted branch (TRAINING_LOG): of the
        // four states of the tile underfoot, ALLY and EMPTY-with-paint>=15 are
        // byte-identical between the two builds (the area branch below scans
        // r^2<=9, which includes distance 0, and a d=0 empty tile always wins its
        // strict `d < paintD` selection); ENEMY was the engine trap that deletion
        // correctly removed; and EMPTY with 5 <= paint < 15 is a legitimate paint
        // that deletion silently gave up, because the area branch is gated at 15.
        // A soldier with 5-14 paint standing on empty ground can afford exactly
        // one attack and the cheapest target on the board is the tile it already
        // occupies -- no movement, no travel. The >=15 gate exists to stop it
        // spending its last paint on DISTANT ground; distance 0 is not that.
        // Guarded on the engine's own predicate (== EMPTY), never on a proxy.
        if (rc.isActionReady() && rc.getPaint() < 15
                && rc.senseMapInfo(cur).getPaint() == PaintType.EMPTY
                && rc.canAttack(cur)) {
            rc.attack(cur);
        }
