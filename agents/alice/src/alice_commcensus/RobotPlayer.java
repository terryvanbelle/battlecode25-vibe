...
public class RobotPlayer {
    ...
    // --- COMMS PRE-CHECK: is the communication channel even reachable? ---
    // sendMessage/readMessages/broadcastMessage have never been called in 23
    // iterations. Before designing a schema, count the DECISION using the
    // ENGINE'S OWN predicate -- rc.canSendMessage(loc) -- rather than my
    // reconstruction of "within r^2<=20 and paint-connected". Today's whole
    // lesson is that a proxy for an engine predicate diverges exactly when the
    // board state shifts, and connectivity is a board-state property.
    static int ccTurns  = 0;   // robot turns counted
    static int ccTower  = 0;   //   ... with at least one ALLY TOWER in vision
    static int ccCanSend= 0;   //   ... where canSendMessage() to such a tower is TRUE
    static int ccBest   = 0;   // max ally towers simultaneously sendable-to
    ...
    /** Counts only; sends nothing, so behaviour is unchanged. */
    static void commCensus(RobotController rc) throws GameActionException {
        ccTurns++;
        RobotInfo[] allies = rc.senseNearbyRobots(-1, rc.getTeam());
        boolean sawTower = false;
        int sendable = 0;
        for (RobotInfo a : allies) {
            if (!a.getType().isTowerType()) continue;
            sawTower = true;
            if (rc.canSendMessage(a.getLocation())) sendable++;
        }
        if (sawTower) ccTower++;
        if (sendable > 0) ccCanSend++;
        if (sendable > ccBest) ccBest = sendable;
    }
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                if (rc.getType().isRobotType()) commCensus(rc);
                switch (rc.getType()) {
                ...
                if (rc.getType().isRobotType()) {
                    rc.setIndicatorString("CC turns=" + ccTurns + " tower=" + ccTower
                            + " cansend=" + ccCanSend + " best=" + ccBest
                            + " bc=" + bc + (overruns > 0 ? " OVR=" + overruns : ""));
