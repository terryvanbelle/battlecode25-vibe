...
public class RobotPlayer {
    ...
    // --- SPAWN CENSUS (probe only; adds no action and consumes no PRNG draw) ---
    // Every tower turn falls in EXACTLY ONE bucket, so the buckets must sum to
    // csTurns. Anything else means the classification is wrong, not the bot.
    static int csTurns = 0;    // tower turns seen
    static int csSkipMoney = 0;// team money < CHIP_RESERVE, spawn block never entered
    static int csNotReady = 0; // action cooldown not ready
    static int csPaint = 0;    // tower paint < want.paintCost
    static int csUnitMoney=0;  // team money < want.moneyCost
    static int csBuilt = 0;    // a robot was built
    static int csRecov = 0;    // all 8 adjacent blocked, but a d2 tile WOULD have worked
    static int csBlock12 = 0;  // all 12 legal tiles blocked
    static int csAtCap = 0;    // tower turns spent at full paint capacity (income wasted)
    // The 4 tiles at d^2 == 4 that the shipping build never tries. The engine's
    // assertCanBuildRobot calls assertCanActLocation(loc, 4) -- a hardcoded r^2 of
    // 4, NOT the tower's actionRadiusSquared -- so the legal set is 12 tiles.
    static final int[] D2X = { 0, 0, 2, -2 };
    static final int[] D2Y = { 2, -2, 0,  0 };
    // Money at the decision point on turns the CHIP_RESERVE gate skipped the block.
    // The question this answers is REACHABILITY: a lower gate is dead code unless
    // the treasury actually spends time in the band between the old and new value.
    static int csM0 = 0;    // money <  250   (below even a soldier's chip cost)
    static int csM250 = 0;  // 250 <= money < 1000
    static int csM1000 = 0; // 1000 <= money < 1250  (a completion still funded)
    static int csM1250 = 0; // 1250 <= money < 1450  (a soldier AND a completion fit)
    // The splasher/refill deadlock: want == SPLASHER (300) while the tower sits in
    // [200,300), which is exactly the band tryRefill's SOLDIER.paintCost reserve holds.
    static int csDead = 0;
    static int csPaintTower = 0; // this tower's paintPerTurn > 0
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                if (csTurns > 0 && !rc.getType().isRobotType()) {
                    rc.setIndicatorString("CS n=" + csTurns
                            + " built=" + csBuilt + " skipM=" + csSkipMoney
                            + " nrdy=" + csNotReady + " paint=" + csPaint
                            + " unitM=" + csUnitMoney + " RECOV=" + csRecov
                            + " blk12=" + csBlock12 + " atcap=" + csAtCap
                            + " DEAD=" + csDead + " pt=" + csPaintTower
                            + " m<250=" + csM0 + " m<1k=" + csM250
                            + " m<1250=" + csM1000 + " m<1450=" + csM1250
                            + " p=" + rc.getPaint() + " bc=" + bc
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        csTurns++;
        if (rc.getPaint() >= rc.getType().paintCapacity) csAtCap++;
        boolean csReady = rc.isActionReady();
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            int off = rnd(8);
            boolean csDone = false;
            for (int i = 0; i < 8; i++) {
                MapLocation loc = rc.getLocation().add(directions[(i + off) % 8]);
                if (rc.canBuildRobot(want, loc)) { rc.buildRobot(want, loc); csDone = true; break; }
            }
            // --- census classification, read at the DECISION point ---
            // getPaint()/getMoney() below are read on turns where nothing was
            // built, so no build has spent them; the attack above is a tower
            // attack, which the engine charges no cooldown for (attack() adds
            // cooldown only if type.isRobotType(), and a tower is not one).
            if (csDone) csBuilt++;
            else if (!csReady) csNotReady++;
            else if (rc.getPaint() < want.paintCost) {
                csPaint++;
                if (want == UnitType.SPLASHER && rc.getPaint() >= UnitType.SOLDIER.paintCost) csDead++;
            }
            else if (rc.getMoney() < want.moneyCost) csUnitMoney++;
            else {
                boolean rec = false;
                for (int d = 0; d < 4; d++) {
                    MapLocation l2 = rc.getLocation().translate(D2X[d], D2Y[d]);
                    if (rc.canBuildRobot(want, l2)) { rec = true; break; }
                }
                if (rec) csRecov++; else csBlock12++;
            }
        } else {
            csSkipMoney++;
            int m = rc.getMoney();
            if (m < 250) csM0++;
            else if (m < 1000) csM250++;
            else if (m < 1250) csM1000++;
            else csM1250++;
        }
        if (rc.getType().paintPerTurn > 0) csPaintTower = 1;
    }
