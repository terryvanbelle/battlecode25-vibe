...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("K4 c=" + k4Calls + " hun=" + k4Hungry
                        + " rdy=" + k4Ready + " adj=" + k4Adj
                        + " s50=" + k4S50 + " s200=" + k4S200 + " "
    ...
    /** K4 probe (ADDITIVE, no behaviour change): the refill's FOUR gates,
     *  counted SEQUENTIALLY so the numbers are a joint funnel and not four
     *  marginals. Sizing a dose from one gate of a conjunction is exactly how
     *  K3's dose missed by 5x. k4S200 is the counterfactual: how often the OLD
     *  reserve of 200 would have permitted the draw, measured at the same site
     *  on the same turns. */
    static int k4Calls, k4Hungry, k4Ready, k4Adj, k4S50, k4S200;
    ...
    static void tryRefill(RobotController rc) throws GameActionException {
        ...
        boolean k4a = false; int k4best = Integer.MIN_VALUE;
        for (int k = 0; k < near.length; k++) {
            if (!near[k].getType().isTowerType()) continue;
            k4a = true;
            int pa = near[k].getPaintAmount();
            if (pa > k4best) k4best = pa;
        }
        if (k4a) {
            k4Adj++;
            if (k4best - 50 > 0) k4S50++;
            if (k4best - UnitType.SOLDIER.paintCost > 0) k4S200++;
        }
        int bestAmt = 0;
