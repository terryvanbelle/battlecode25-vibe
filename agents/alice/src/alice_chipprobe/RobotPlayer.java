...
public class RobotPlayer {
    ...
    // ---- CHIP RESERVE PROBE (instrumentation only; NEVER in src/alice) -------
    // Hypothesis under test, from the tournament dumps: unit building needs
    // money >= CHIP_RESERVE (1450) while completeTowerPattern needs only 1000, so
    // in the band [1000, 1450) a tower can be built and a unit cannot -- and my
    // mean money sits inside that band on five of seven maps. If true, tower
    // construction permanently outbids unit production and my army is throttled
    // by a constant introduced in iteration 2 for an unrelated reason.
    //
    // This is a HYPOTHESIS. The same table has two maps where the gate is shut
    // only 50-77% and the army is still small, so the story does not explain
    // everything, and I have twice today derived a confident fix from a coherent
    // reading of source that measurement then refuted. Measure first.
    //
    // Decides nothing: every counter below is written and never read by logic.
    static int cpTurns;        // tower turns
    static int cpBand;         // money in [1000, CHIP_RESERVE)  -- tower yes, unit NO
    static int cpAbove;        // money >= CHIP_RESERVE           -- unit allowed
    static int cpBelow1000;    // money < 1000                    -- neither allowed
    static int cpBuilt;        // buildRobot actually issued
        ...
        while (true) {
            ...
            try {
                ...
                if (!rc.getType().isRobotType()) {
                    rc.setIndicatorString("CP t=" + cpTurns + " lo=" + cpBelow1000
                            + " BAND=" + cpBand + " ok=" + cpAbove + " built=" + cpBuilt
                            + " $=" + rc.getMoney() + (overruns > 0 ? " OVR=" + overruns : ""));
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        cpTurns++;                                     // PROBE ONLY
        int cpM = rc.getMoney();
        if (cpM < 1000) cpBelow1000++;
        else if (cpM < CHIP_RESERVE) cpBand++;
        else cpAbove++;
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (rc.canBuildRobot(want, loc)) { rc.buildRobot(want, loc); cpBuilt++; break; }
            }
        }
    }
