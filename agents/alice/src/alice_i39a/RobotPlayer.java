...
public class RobotPlayer {
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            // ITERATION 39: the splasher override must respect PAINT, not just chips.
            //
            // Iteration 28 added this override AFTER iteration 5's paint guard, and
            // it overwrites `want` unconditionally -- so it bypasses the guard that
            // exists precisely because tower paint, not chips, is the binding
            // constraint. A splasher costs 300 tower paint (1.5x a soldier), and a
            // tower's paint income is 5-15/turn.
            //
            // MEASURED (alice_i39diag, decision-point counters, 5 maps): when this
            // gate fires the tower cannot afford the paint 96-100% of the time, so
            // the build slot produces NOTHING -- not a splasher, not a soldier.
            // DefaultHuge: gate fired on 1374 of 1801 tower build-turns, succeeded 59
            // times, and on 465 of them a SOLDIER was affordable and was displaced by
            // a splasher that could not be built. Mirage 108, DefaultLarge 67,
            // Barcode 46. That is pure waste, and removing it is the winner's profile
            // the algorithm names: capability preserved at zero marginal cost.
            //
            // The threshold is the engine's own SPLASHER.paintCost, not a searched
            // constant -- the same self-calibrating discipline as iteration 5's "only
            // build a mopper if a soldier was affordable too" and iteration 25's
            // reserve.
            //
            // NAMED RISK, pre-registered: paint spent on soldiers is paint that never
            // reaches 300, so this can suppress splasher production entirely rather
            // than merely un-wasting slots. That is a real reallocation (soldiers now
            // vs splashers later), not a free win, and the manipulation check below
            // measures splashers-built directly rather than assuming.
            if (rc.getMoney() >= CHIP_RESERVE + 2500
                    && rc.getPaint() >= UnitType.SPLASHER.paintCost) want = UnitType.SPLASHER;
            int off = rnd(8);
            for (int i = 0; i < 8; i++) {
