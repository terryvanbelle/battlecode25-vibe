...
public class RobotPlayer {
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("R1 " + r1spawn + " " + r1skip + " " + r1spl + " "
    ...
    /** R1 -- the coordinated unit-economy change. Four premises adopted together
     *  because none is independently choosable (survey, commit ff7424d):
     *    A fewer units   B no moppers   C ~30%% splashers   D resupply first-class
     *  The composition: spawning fewer units stops towers draining themselves, so
     *  tower paint stays high -- which is precisely the constraint that capped
     *  splashers (>=300 paint on 29%% of frames early, 0%% after r1200) and made
     *  refills unreachable (adjacency 2.61%%). A pays for C and D.
     *  NOT gated on chips: a chip threshold is a member of the class closed this
     *  morning -- it opens exactly when the paint is gone (rank corr -0.496).
     *  PRESERVED, because they are measured and a rewrite that loses them is worse
     *  than none: the engine-trap guards, ballistic wander, symmetric tie-breaks,
     *  and expansion-first (r300 tower leader wins 81%%). */
    static final boolean R1_ECONOMY = false;
    static final int R1_SKIP_TENTHS = 4;        // skip 40% of spawns: fewer units, paint retained
    static int r1spawn, r1skip, r1spl;
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            UnitType want;
            if (R1_ECONOMY) {
                // ATTEMPT 2. Attempt 1 used a THRESHOLD (paint >= cost + 100) and it
                // sat above the distribution it tested -- towers hold ~154, so a
                // soldier needed 300 and 97.7% of spawn decisions were skipped. That
                // is the K8 defect committed inside the rewrite. A RATE cannot be
                // mis-set against a distribution: it delivers its intended effect
                // whatever the resource is doing.
                want = UnitType.SOLDIER;
                if (rc.getPaint() >= UnitType.SPLASHER.paintCost && rnd(10) < 3) {
                    want = UnitType.SPLASHER; r1spl++;
                }
                if (rnd(10) < R1_SKIP_TENTHS) { r1skip++; return; }  // fewer units, by RATE
                r1spawn++;
            } else {
                want = (rnd(4) == 0) ? UnitType.MOPPER : UnitType.SOLDIER;
            }
            ...
            if (!R1_ECONOMY && want == UnitType.MOPPER && rc.getPaint() < UnitType.SOLDIER.paintCost) {
                want = UnitType.SOLDIER;
            }
            ...
            if (!R1_ECONOMY && (rc.getMoney() >= CHIP_RESERVE + 2500
                    || (expansionFinished
