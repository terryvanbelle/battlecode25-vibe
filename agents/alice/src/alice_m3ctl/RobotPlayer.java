...
public class RobotPlayer {
    ...
    static int _ab;   // PROBE: this turn's action-slot disposition
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                rc.setIndicatorString("ZZ"
                        + (rc.getType()==UnitType.SOLDIER?"S":rc.getType()==UnitType.MOPPER?"M":rc.getType()==UnitType.SPLASHER?"P":"T")
                        + _ab + " " + "i25=" + i25Refills + "/" + i25Paint + " "
    ...
    /** ITERATION M2 -- shift composition toward the TOP-ranked type.
     *  Production efficiency, one pass over full action logs (tiles painted / paint
     *  spent spawning): SPLASHER 0.1291, SOLDIER 0.0603, MOPPER 0.0000, and the same
     *  ordering replicates on another design's units. M1 converted the zero into the
     *  MIDDLE and realised 4.1x soldiers with splashers up 12% -- a valid experiment
     *  aimed at a weaker claim than the measurement supports, retired unrun.
     *  This builds a SPLASHER whenever the tower can afford one, gated on the
     *  tower's OWN PAINT and never on chips (a chip threshold opens exactly when the
     *  paint is gone, rank corr -0.496). ADDITIVE: the existing chip-gated override
     *  stays -- baseline-yield.py measured it producing 38.2% of spawns here, which
     *  is the control I built after deleting it by mistake. Moppers untouched, so
     *  exactly one thing changes. No volume mechanism, so volume movement is a
     *  consequence and does not void. */
    static final boolean M2_TOP_MIX = false;
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        if (rc.getMoney() >= CHIP_RESERVE) {
            ...
            if (M2_TOP_MIX && rc.getPaint() >= UnitType.SPLASHER.paintCost) {
    ...
    static void runSoldier(RobotController rc) throws GameActionException {
        if (goRefill(rc)) { if (rc.isActionReady()) _ab = 5; return; }
        ...
        if (rc.isActionReady() && rc.getPaint() < 5) _ab = 2;
        else if (rc.isActionReady() && rc.getPaint() < 15) _ab = 3;
        if (rc.isActionReady() && rc.getPaint() >= 15) {
            ...
            if (paintTarget != null) { rc.attack(paintTarget); _ab = 1; } else _ab = 4;
        }
    }
    ...
    static void runSplasher(RobotController rc) throws GameActionException {
        ...
        if (rc.isActionReady() && rc.getPaint() < 50) _ab = 2;
        else if (rc.isActionReady() && rc.getPaint() < 60) _ab = 3;
        if (rc.isActionReady() && rc.getPaint() >= 60) {
            ...
            if (best != null) { rc.attack(best); _ab = 1; } else _ab = 4;
        }
        wander(rc);
    }
