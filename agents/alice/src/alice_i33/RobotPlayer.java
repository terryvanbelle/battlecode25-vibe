...
public class RobotPlayer {
    ...
    static void runTower(RobotController rc) throws GameActionException {
        ...
        // ITERATION 33: reserve EXACTLY one tower completion, not the flat CHIP_RESERVE.
        //
        // MEASURED: across seven tournament maps my money is below CHIP_RESERVE (1450) on
        // 60 of 69 sampled rounds -- 87%. Money is a TEAM-WIDE pool, so below it EVERY
        // tower stops building at once. That is the largest single throttle I have measured
        // on this bot, and 1450 was hand-set in iteration 2 to guarantee a tower completion
        // could always fund.
        //
        // VERIFIED from the engine, not from my own comment (which I checked and which was
        // right for once): LEVEL_ONE_MONEY_TOWER.moneyCost and LEVEL_ONE_PAINT_TOWER.moneyCost
        // are both 1000, and RobotControllerImpl.completeTowerPattern charges exactly
        // TeamInfo.addMoney(team, -type.moneyCost). So the thing 1450 was protecting costs
        // 1000, and the honest reserve is "leave a tower completion funded" -- which is
        // 1000 + the unit's own moneyCost, not a flat number 450 chips above it.
        //
        // SOLDIER.moneyCost = 250 -> 1250; MOPPER 300 -> 1300; SPLASHER 400 -> 1400.
        // All below 1450, so the gate genuinely opens; none dips under the 1000 reserve.
        // Self-calibrating from engine constants, not a searched value.
        //
        // RISK, named before the run: this spends chips that iteration 2 measured as needed,
        // and iteration 26 lost -21 net swept by starving tower construction. The invariant
        // is preserved exactly (1000 always remains), so the failure mode would be indirect
        // -- more units competing for the same tower PAINT. Diagnostic named below.
        final int TOWER_COMPLETION = UnitType.LEVEL_ONE_MONEY_TOWER.moneyCost;   // 1000
        if (rc.getMoney() >= TOWER_COMPLETION + UnitType.SOLDIER.moneyCost) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (rc.getMoney() < TOWER_COMPLETION + want.moneyCost) break;  // ITERATION 33
