...
public class RobotPlayer {
            ...
            try {
                ...
                rc.setIndicatorString("i25=" + i25Refills + "/" + i25Paint + " "
                        ...
                        ? "i24=" + i24Moves + " p=" + rc.getPaint()
                          + " P47 n=" + p47mops + " b0=" + p47pri0 + " b1=" + p47pri1
                          + " b2=" + p47pri2 + " tie=" + p47tie + " tieC=" + p47tieClean + " "
                        : "")
                        + "bc=" + bc + " max=" + maxBc
    ...
    // ----------------------------------------------------------------- mopper
    static int p47mops, p47pri0, p47pri1, p47pri2, p47tie, p47tieClean;
    ...
    static void runMopper(RobotController rc) throws GameActionException {
        ...
        if (rc.isActionReady()) {
            ...
            for (MapInfo t : rc.senseNearbyMapInfos(2)) {
                ...
                if (pri == 0) cand0++; else if (pri == 1) cand1++; else cand2++;
                if (pri < bestPri) { bestPri = pri; best = l; if (pri == 0) break; }
            }
            // ITERATION 47 PROBE (counters only; the `break` above means cand0/cand1/cand2
            // count candidates SEEN before the scan short-circuits, which is exactly the
            // set the chooser actually ranked -- not the full neighbourhood).
            if (best != null) {
                p47mops++;
                if (bestPri == 0) p47pri0++; else if (bestPri == 1) p47pri1++; else p47pri2++;
                // was the iteration 46 tie-break even reachable? it ranks the PLAIN class,
                // so it can only matter when the winner is plain AND there were >=2 plain
                // candidates to choose between.
                if (bestPri == 2 && cand2 >= 2) p47tie++;
                if (bestPri == 2 && cand2 >= 2 && cand1 == 0 && cand0 == 0) p47tieClean++;
            }
            if (best != null) rc.attack(best);
        }
