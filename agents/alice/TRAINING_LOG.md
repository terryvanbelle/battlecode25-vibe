...
## Replication on disjoint maps: the ORDERING holds, the "3x" does NOT

Run `20260908-120914` — the same two arms against the same baseline (`alice_iter29`) on the
**35 maps the dose-finding run did not use**, disjoint by construction, 140 games.

| arm | original, 40 maps | per map | replication, 35 maps | per map |
|---|---|---|---|---|
| `alice_i30a` | **+3** | 0.075 | **0** | 0.000 |
| `alice_i30b` | **+10** | 0.250 | **+2** | 0.057 |

| | original | replication |
|---|---|---|
| B − A gap, per map | +0.175 | +0.057 |
| **on a common 75-map scale** | **+13.1** | **+4.3** |

**The ordering replicates: `i30b` >= `i30a` in both samples, and A never beat B.** So choosing
the larger dose was not wrong, and iteration 30's accept is untouched — it was confirmed
independently by a *full 75-map census* at +12, which samples nothing.

**The magnitude does not replicate.** The gap fell by a factor of three, from +13 to +4 on a
common scale. My LEARNINGS headline — *"B beat A by more than three to one"* — was reading a
**sample-specific ratio as a property of the mechanism.** Corrected there.

### The empirical resolution, which is the number the coordinator actually asked for

Two independent paired samples of the same quantity returned per-map gaps of **+0.175** and
**+0.057** — a spread of 0.118 per map, or **~9 net swept on a 75-map scale**. Since these are
paired (both arms share the maps within a run, so map difficulty largely cancels), this is a
direct empirical estimate rather than an assumed one:

> **A 35-40 map dose comparison in this lineage cannot resolve a difference below roughly
> 5 net swept, and two such runs of the same pair can disagree by ~9 on a 75-map scale.**

That is not `se = 0`, and it is also not the ~3-wins-per-arm figure applied naively — the
paired design does help, but nowhere near enough to make a 7-point gap trustworthy. **I would
have shipped that 7-point gap as a general methodological lesson**, and did, for about three
hours.

### What survives, stated precisely

- **Iteration 30's accept**: untouched. Full census, no sampling, +12.
- **Choosing `i30b` over `i30a`**: correct, and now supported by two disjoint samples.
- **"The conservative dose is not automatically the safe one"**: survives *as an ordering
  claim*. A never beat B in either sample. The reasoning behind it — that arm A's
  zero-opportunity-cost argument priced the painting given up and ignored the unit lost — is
  unaffected by effect size.
- **"By more than three to one"**: retracted. That ratio was noise.

**This is the seventh correction today and the first one I generated by running a check I had
