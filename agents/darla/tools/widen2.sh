...
echo "$(date -uIs) WIDEN2 START $ARM -- 75 maps x 3 mid-lineage snapshots x 2 sides"
MAPS="$(tr '\n' ' ' < ../../tools/bc25-maps.txt)" BOT="$ARM" \
  OPPONENTS="alice_iter25 bob_iter11 carol_iter30" MAXJOBS=2 \
  ../../tools/gauntlet.sh 201>&- | tee /tmp/darla-run-$$.out || { echo "$(date -uIs) WIDEN2 ERROR $ARM"; exit 1; }
run=$(sed -n 's/^gauntlet \([0-9-]*\) .*/\1/p' /tmp/darla-run-$$.out | head -1)
rm -f /tmp/darla-run-$$.out
echo "$(date -uIs) WIDEN2 COMPLETE $ARM $run -- $(grep -E '^overall' gauntlet/$run/summary.txt)"
