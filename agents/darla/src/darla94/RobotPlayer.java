...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla94";
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " rt=" + refillTrips + " ht=" + homeTurns + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak + " mv=" + mvStuck + "/" + mvTry + " sb=" + sbChips + "/" + sbPaint + "/" + sbOk
            + " | " + state);
        Clock.yield();
    }
    ...
    // darla94: INSTRUMENTATION ONLY. Every map where i3 is swept 0-2 by two or more of
    // the standard roster is LARGE -- seven of seven, against a pool that is 49% large,
    // p ~ 0.007. On TheBest (60x60, 44 ruins) our tower count goes 4 -> 6 -> 10 across
    // 1400 rounds while chips sit at 3,620 falling to 1,370, so expansion there is not
    // chip-limited. This counts what the spawn decision is ACTUALLY blocked on:
    // sbChips = turns the chip test failed, sbPaint = turns it passed but the tower had
    // too little paint, sbOk = turns a robot was actually spawned. The large-map weakness
    // is the oldest open item in this lineage and every previous attempt on it guessed at
    // the binding constraint rather than reading it.
    static int sbChips = 0, sbPaint = 0, sbOk = 0;
    ...
    static String runTower() throws GameActionException {
        ...
        boolean afford = chips >= reserve + want.moneyCost; if (!afford) sbChips++; else if (rc.getPaint() - want.paintCost < 200) sbPaint++; else sbOk++;
