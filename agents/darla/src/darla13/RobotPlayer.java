...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla13";
    ...
    static String runTower() throws GameActionException {
        ...
        // ---- darla13: SPLASHER PAINT FLOOR. carol's PAINT_FLOOR above protects the
        // SOLDIER from the MOPPER -- the cheap unit crossing its paint line first and
        // resetting the stash, so the expensive one is never afforded. The same
        // dynamic runs one unit further up and nothing guards it: a SOLDIER costs 200
        // paint and a SPLASHER 300, so the soldier crosses first and the tower never
        // reaches 300.
        //
        // Measured on darla10 (intended splasher share raised to 40%) on DefaultHuge:
        //   round  600   +sold 32   +spl 67    coverage 552
        //   round 1200   +sold 227  +spl 52    coverage 423
        //   round 2000   +sold 113  +spl  0    coverage 284, chips $28,850
        // Splasher production falls to ZERO while chips are abundant, because tower
        // paint sits at ~54 each against a 300 cost. Raising the ROLL cannot help when
        // the tower cannot pay -- which is why darla10 lost 11 games.
        final int SPLASH_PAINT_FLOOR = 150;
        if (afford && want.paintCost < UnitType.SPLASHER.paintCost
                && rc.getPaint() - want.paintCost < SPLASH_PAINT_FLOOR) {
            afford = false;
        }
        if (afford) {
