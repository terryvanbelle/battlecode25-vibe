...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla87";
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " rt=" + refillTrips + " ht=" + homeTurns + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak + " mv=" + mvStuck + "/" + mvTry + " ls=" + lowN + "/" + lowSum + "/" + lowMax
            + " | " + state);
        Clock.yield();
    }
    ...
    // darla87: INSTRUMENTATION ONLY. The i3 census puts splashers at 21% lowScore and
    // only 4% SPLASH -- holding paint and an action with nothing scoring above
    // SPLASH_MIN_SCORE. bestScore is ALREADY computed on those turns, so recording its
    // distribution costs two adds and no scan: if the best is 12-13 the splasher is
    // sitting next to work it just misses, and if it is 0-3 there is genuinely nothing
    // there and the splasher should be moving rather than waiting. Those two readings
    // call for opposite arms, which is exactly why this must be measured before either.
    static int lowN = 0, lowSum = 0, lowMax = 0;
    ...
    static String runSplasher() throws GameActionException {
        ...
        if (ready && fueled) {
            ...
            if (best != null && bestScore >= SPLASH_MIN_SCORE) {
                ...
                tag = (best == null) ? " noTgt" : " lowScore"; if (best != null) { lowN++; lowSum += bestScore; if (bestScore > lowMax) lowMax = bestScore; }
            }
        } else {
