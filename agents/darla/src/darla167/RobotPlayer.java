...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla167";
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " srp=" + srpMark + "/" + srpDone + "/" + srpQuit + " mp=" + moneyPick + " os=" + openSold + " ma=" + mixAsk + " mf=" + mixFlip
    ...
    static String runSoldier() throws GameActionException {
        ...
        if (rc.canCompleteResourcePattern(me0)) { rc.completeResourcePattern(me0); srpDone++; srp = null; state += " SRPdone"; }
            ...
            boolean safe = false; for (RobotInfo a : rc.senseNearbyRobots(-1, rc.getTeam())) if (a.type.isTowerType()) { safe = true; break; } // darla165: mark only in sight of a friendly tower
        ...
        if (rc.isActionReady() && rc.getPaint() >= UnitType.SOLDIER.attackCost) {
            ...
            if (myTile.getPaint() == PaintType.EMPTY && rc.canAttack(me)) {
                rc.attack(me, myTile.getMark() == PaintType.ALLY_SECONDARY);   // darla167: honour a mark
                state += " slf";
            } else {
                MapLocation best = null;
                int bestD = Integer.MAX_VALUE;
                ...
                if (best != null) { rc.attack(best, rc.senseMapInfo(best).getMark() == PaintType.ALLY_SECONDARY); state += " pnt"; }   // darla167: honour a mark
                else {
        ...
        if (ready && fueled) {
            ...
            for (MapLocation c : rc.getAllLocationsWithinRadiusSquared(me, 4)) {
                ...
                for (MapInfo t : rc.senseNearbyMapInfos(c, 4)) {
                    // darla167: never splash over a marked resource pattern -- a primary disk
                    // overwrites its secondary corner blocks and the team then re-buys it.
                    if (t.getMark() == PaintType.ALLY_SECONDARY) { score = -1; break; }
                    PaintType p = t.getPaint();
                    if (p.isEnemy()) {
