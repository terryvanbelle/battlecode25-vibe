...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla91";
        ...
        if (rc.isActionReady() && rc.getPaint() >= UnitType.SOLDIER.attackCost) {
            ...
            if (myTile.getPaint() == PaintType.EMPTY && rc.canAttack(me)) {
                ...
                if (best != null) { rc.attack(best); state += " pnt"; }
                    ...
                    if (ally >= foe) { // darla91: darla86 ungated this branch entirely and came out +1.70 combined -- positive but under the bar, and with a legible split: it LOST to both alice builds and WON against both carol builds. The gate is right when the enemy holds the ground and wrong when we do. ally and foe are already counted two lines above, so this costs nothing and adds no constant: seek the frontier when we hold at least as much nearby paint as they do, stay and contest when they hold more.
