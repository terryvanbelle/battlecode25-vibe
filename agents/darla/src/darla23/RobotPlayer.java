...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla23";
    /** Paint a mule keeps for itself so it can still mop (attack cost is small). */
    static final int MULE_KEEP = 20;
    static int muleGiven = 0, muleTrips = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " mu=" + muleTrips + "/" + muleGiven + " rt=" + refillTrips + " ht=" + homeTurns + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak
            + " | " + state);
        Clock.yield();
    }
    ...
    static String runMopper() throws GameActionException {
        ...
        // darla23: MOPPER AS PAINT MULE. Two measurements meet here.
        //  1. Splashers run dry and walk home: HOME turns are a standing share of the
        //     splasher budget, and every one is a turn not painting. Paint, not chips, is
        //     the binding resource late (tower paint ~54 each against a 300 splasher cost).
        //  2. Moppers are nearly worthless in their current role -- removing them entirely
        //     cost only 5 games (darla2), and giving them alice's accepted navigation was a
        //     dead tie (darla3). They are 10% of production doing almost nothing.
        // And the engine allows exactly one fix: [E, carol RULES.md] "only moppers transfer
        // robot->robot". A soldier cannot mule; a mopper can. So the near-useless unit is
        // the ONLY unit that can carry paint to the unit that runs out of it.
        // Transfer range is a hardcoded r2<=2 for every type, so the mopper must touch.
        if (rc.isActionReady() && rc.getPaint() > MULE_KEEP) {
            for (RobotInfo ally : rc.senseNearbyRobots(2, rc.getTeam())) {
                if (ally.type != UnitType.SPLASHER) continue;
                int room = UnitType.SPLASHER.paintCapacity - ally.paintAmount;
                if (room < 50) continue;                       // not worth a 10-cooldown act
                int give = Math.min(rc.getPaint() - MULE_KEEP, room);
                if (give > 0 && rc.canTransferPaint(ally.location, give)) {
                    rc.transferPaint(ally.location, give);
                    muleGiven += give; muleTrips++;
                    state += " mule" + give;
                    break;
                }
            }
