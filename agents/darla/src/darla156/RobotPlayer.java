...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla156";
    ...
    static int ruinSold = 0, msgSent = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " ms=" + msgSent + " rs=" + ruinSold + " ma=" + mixAsk + " mf=" + mixFlip
    ...
    static String runTower() throws GameActionException {
        ...
        boolean ruinNear = rc.readMessages(rc.getRoundNum() - 1).length > 0;
        ...
        if (((rc.getRoundNum() < 100 && rc.getNumberTowers() <= 2) || ruinNear) && want != UnitType.SOLDIER) { want = UnitType.SOLDIER; mixFlip++; }
        ...
        if (afford && want != UnitType.SPLASHER && !(want == UnitType.SOLDIER && ruinNear) && chips - want.moneyCost < SPLASH_FLOOR) {
            afford = false;
        }
        ...
        if (afford) {
            ...
            if (rc.canBuildRobot(want, loc)) { rc.buildRobot(want, loc); if (want == UnitType.SOLDIER && ruinNear) ruinSold++; }
        }
    ...
    static String runSoldier() throws GameActionException {
        ...
        if (ruin != null) for (RobotInfo tw : rc.senseNearbyRobots(GameConstants.MESSAGE_RADIUS_SQUARED, rc.getTeam())) if (tw.type.isTowerType() && rc.canSendMessage(tw.location)) { rc.sendMessage(tw.location, 1); msgSent++; break; }
        if (ruin != null) {
            state += " ruin=" + ruin;
            workOnRuin(ruin);
        }
