...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla112";
    ...
    static int rollS = 0, dReserve = 0, dFloor = 0, dCbr = 0, dBuilt = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " sr=" + rollS + "/" + dReserve + "/" + dFloor + "/" + dCbr + "/" + dBuilt + " ma=" + mixAsk + " mf=" + mixFlip
    ...
    static String runTower() throws GameActionException {
        ...
        if (want == UnitType.SOLDIER) { rollS++; if (!afford) dReserve++; }
        if (afford && want != UnitType.SPLASHER && chips - want.moneyCost < SPLASH_FLOOR) { if (want == UnitType.SOLDIER) dFloor++;
            afford = false;
        }
        ...
        if (afford) {
            ...
            if (rc.canBuildRobot(want, loc)) { rc.buildRobot(want, loc); if (want == UnitType.SOLDIER) dBuilt++; } else if (want == UnitType.SOLDIER) dCbr++;
        }
