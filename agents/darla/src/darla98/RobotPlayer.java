...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla98";
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " rt=" + refillTrips + " ht=" + homeTurns + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak + " mv=" + mvStuck + "/" + mvTry + " pp=" + pullTotal + "/" + spawnPaint
            + " | " + state);
        Clock.yield();
    }
    ...
    // darla98: INSTRUMENTATION ONLY. After iteration 4, tower paint is STILL the
    // dominant spawn blocker across the whole size range -- Justice 21x20 is blocked
    // 449 to 60, Flower 408 to 97, Restart 306 to 191 -- so more paint is still the
    // lever. But every arm so far has treated that as a PRODUCTION problem (more paint
    // towers) without ever measuring CONSUMPTION. A tower accrues 5 paint/turn against a
    // 1000 cap and spends it two ways: building robots, and handing it to robots that
    // walk up and refill. Iteration 2 made those robots stop stalling at dry towers --
    // it did not make them take less. pullTotal is paint drained by refills (counted
    // robot-side, where the transfer happens); spawnPaint is paint spent on robots
    // (counted tower-side). If refills dwarf spawns, the spawn block is self-inflicted
    // and every "build more paint towers" follow-up is pouring water into a leaking
    // bucket.
    static int pullTotal = 0, spawnPaint = 0;
    ...
    static String runTower() throws GameActionException {
        ...
        if (afford) {
            ...
            if (rc.canBuildRobot(want, loc)) { rc.buildRobot(want, loc); spawnPaint += want.paintCost; }
        }
        ...
        for (RobotInfo ally : rc.senseNearbyRobots(2, rc.getTeam())) {
            if (ally.type.isTowerType()) {
                ...
                if (want > 0 && rc.canTransferPaint(ally.location, -want)) {
                    rc.transferPaint(ally.location, -want); pullTotal += want;
                    return;
                }
            }
        }
