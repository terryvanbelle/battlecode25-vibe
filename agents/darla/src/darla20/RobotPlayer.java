...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla20";
    ...
    static String runSplasher() throws GameActionException {
        ...
        if (siege != null && rc.isMovementReady()) {
            ...
            // darla20: GENERALISE THE ATTRACTOR. With no enemy tower in vision, `siege`
            // is null and the splasher wandered at random -- no heading at all. The
            // measured lever in this design is POSITIONING: removing the tower bonus
            // (darla17) cost 17 games and collapsed time in the firing ring from 59
            // turns to 11, because the bonus is what walks splashers onto enemy ground.
            // That attractor only reaches as far as a visible tower. Enemy PAINT is the
            // same signal without the range limit, and unlike a soldier a splasher can
            // actually overwrite it -- which is why darla9's identical heading was
            // worthless for soldiers (-10) and should not be for splashers.
            MapLocation me2 = rc.getLocation();
            MapLocation lure = null; int lureD = 1 << 30;
            for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
                if (!t.getPaint().isEnemy() || !t.isPassable()) continue;
                int d2 = me2.distanceSquaredTo(t.getMapLocation());
                if (d2 < lureD) { lureD = d2; lure = t.getMapLocation(); }
            }
            if (lure != null) { moveExploring(lure); tag += " lure"; }
