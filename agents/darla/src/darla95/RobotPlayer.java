...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla95";
    ...
    static String runTower() throws GameActionException {
        ...
        pinnedTurns = pinned ? pinnedTurns + 1 : (pinnedTurns > 0 ? pinnedTurns - 1 : 0); // darla95: DECAY instead of RESET. darla94 measured the spawn decision blocked on chips 397-544 times per tower on large maps (against 74-78 on small ones), while the indicator shows pin=1 and pin=2 -- the release needs TEN CONSECUTIVE rounds inside a 250-chip window and income crosses that window constantly, so the counter is reset before it can ever reach the threshold. Decaying by one instead of resetting to zero lets intermittent pinning accumulate, which is what pinning actually looks like in a treasury that is still earning. No constant changed.
