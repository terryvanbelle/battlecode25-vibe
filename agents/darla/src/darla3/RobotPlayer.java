...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla3";
    static int mopSeek = 0, mopSeekHit = 0;   // darla3 mechanism counters
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " ms=" + mopSeek + "/" + mopSeekHit + " rt=" + refillTrips + " ht=" + homeTurns + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak
            + " | " + state);
        Clock.yield();
    }
    ...
    static String runMopper() throws GameActionException {
        ...
        // darla3, ported from alice iteration 7 (ACCEPTED there at 20/23, 87%, with
        // unpaints per mopper alive up ~3x): walk toward the nearest enemy paint
        // ANYWHERE in vision instead of wandering. This mopper picks targets within
        // r2<=2 -- its 8 adjacent tiles -- while its vision is r2=20, roughly 60
        // tiles, so it has been blind to ~90% of what it can already see. Population
        // is unchanged; the same few units simply stop being blind, which is the
        // "capability preserved at zero marginal cost" shape.
        MapLocation me = rc.getLocation();
        ...
        mopSeek++;
        if (target == null) { moveExploring(null); return state + " roam"; }
        if (bd > 2) { moveExploring(target); mopSeekHit++; return state + " seek d2=" + bd; }
        return state + " hold";   // already in mopping range: stay and keep mopping
    }
