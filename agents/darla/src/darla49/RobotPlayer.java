...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla49";
    /** darla49: the last enemy tower THIS robot saw. A tower is the only stable
     *  landmark on the map -- that is why the attractor is worth 99-51 while every
     *  paint-targeted heading loses (-7.84 sd, -10.78 sd). The attractor only reaches
     *  as far as vision (r2=20); remembering the location extends the ONE thing that
     *  works past its range, without inventing a new signal. */
    static MapLocation lastSeenEnemyTower = null;
    ...
    static String runSplasher() throws GameActionException {
        ...
        if (siege != null) lastSeenEnemyTower = siege;
        // Out of vision, keep walking to where the tower WAS. It cannot have moved.
        if (siege == null && lastSeenEnemyTower != null
                && rc.getLocation().distanceSquaredTo(lastSeenEnemyTower) > 20) {
            siege = lastSeenEnemyTower;
            tag += " recall";
        }
