...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla107";
    ...
    static int fxStall = 0, cpDone = 0, cpC8 = 0, cpC6 = 0, cpC4 = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " fx=" + fxStall + " cp=" + cpDone + "/" + cpC8 + "/" + cpC6 + "/" + cpC4 + " ma=" + mixAsk + " mf=" + mixFlip
    ...
    static void workOnRuin(MapLocation ruin) throws GameActionException {
        ...
        if (marked > 0 && !rc.canCompleteTowerPattern(kind, ruin) && rc.canCompleteTowerPattern(alt, ruin)) fxStall++;
        if (rc.canCompleteTowerPattern(kind, ruin)) {
            ...
            int szx = rc.getMapWidth() + rc.getMapHeight();
            if (dcx * 4 <= szx) cpC8++;
            if (dcx * 3 <= szx) cpC6++;
            if (dcx * 2 <= szx) cpC4++;
            return;
