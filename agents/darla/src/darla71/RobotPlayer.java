...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla71";
    ...
    /** Eliminate any candidate whose mirrored start is visible and holds no ruin. */
    static void refineSymmetry() throws GameActionException {
        if (myStart == null) return;
        for (int k = 0; k < 3; k++) {
            boolean live = (k == 0) ? symRot : (k == 1) ? symHorz : symVert;
            if (!live) continue;
            MapLocation m = mirrorOf(myStart, k);
            if (!rc.canSenseLocation(m)) continue;
            boolean ruinThere = false;
            for (MapLocation r : rc.senseNearbyRuins(-1)) if (r.equals(m)) { ruinThere = true; break; }
            if (!ruinThere) {
