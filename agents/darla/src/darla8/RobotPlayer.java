...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla8";
        ...
        if (rc.isActionReady() && rc.getPaint() >= UnitType.SOLDIER.attackCost) {
            ...
            if (myTile.getPaint() == PaintType.EMPTY && rc.canAttack(me)) {
                ...
                if (best != null) { rc.attack(best); state += " pnt"; }
                    ...
                    // darla8: the `foe == 0` guard is GONE. Censused on DefaultHuge,
                    // 777 soldier-turns: 53.2% are IDLE-ENEMY -- enemy paint within r2=9
                    // that a soldier cannot overwrite -- and every one of those turns
                    // skipped this branch entirely, so the soldier kept whatever stale
                    // heading it had while standing on ground it can never use. Those are
                    // exactly the turns that most need a new target. Vision is r2=20
                    // against an action radius of r2=9, so there is usually reachable
                    // empty ground in sight past the enemy paint.
                    //
                    // This spends NOTHING: no build, no attack, no extra paint. It
                    // redirects a move that already happens, which is the "capability
                    // preserved at zero marginal cost" shape -- the only profile that has
                    // ever won in this project.
