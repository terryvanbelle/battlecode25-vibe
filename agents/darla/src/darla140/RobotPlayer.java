...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla140";
    ...
    static boolean walkHomeIfDry(int cap) throws GameActionException {
        ...
        if (!refilling && paint > REFILL_LOW) { MapLocation h0 = nearestRememberedTower(); if (h0 == null) return false; MapLocation me0 = rc.getLocation(); PaintType pt = rc.senseMapInfo(me0).getPaint(); int pen = pt.isEnemy() ? GameConstants.PENALTY_ENEMY_TERRITORY : (pt == PaintType.EMPTY ? GameConstants.PENALTY_NEUTRAL_TERRITORY : 0); int steps = Math.max(Math.abs(me0.x - h0.x), Math.abs(me0.y - h0.y)); if (paint > steps * pen) return false; earlyTrips++; }
