...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla130";
    ...
    static int xrNew = 0, xrArrive = 0, xrStuck = 0, xrAge = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " xr=" + xrNew + "/" + xrArrive + "/" + xrStuck + "/" + xrAge + " mb=" + mvBlocked + " ma=" + mixAsk + " mf=" + mixFlip
    ...
    static void moveExploring(MapLocation target) throws GameActionException {
        ...
        if (explore == null || me.distanceSquaredTo(explore) <= 8
            ...
            if (explore == null) xrNew++; else if (me.distanceSquaredTo(explore) <= 8) xrArrive++; else if (stuckTurns >= 6) xrStuck++; else xrAge++;
            newExploreTarget();
            stuckTurns = 0;
        }
    ...
    static boolean stepToward(MapLocation to) throws GameActionException {
        ...
        MapLocation nx = rc.getLocation().add(d);
        boolean terrain = !rc.onTheMap(nx) || !rc.senseMapInfo(nx).isPassable();
        if (following || (stuckLast && terrain)) { boolean lf = (rc.getID() & 1) == 0; if (!following) { bugDir = d; bugStartD = d0; } bugTo = to; for (int i = 0; i < 8; i++) { bugDir = lf ? bugDir.rotateLeft() : bugDir.rotateRight(); if (rc.canMove(bugDir)) { rc.move(bugDir); bugDir = lf ? bugDir.rotateRight() : bugDir.rotateLeft(); return true; } } }
