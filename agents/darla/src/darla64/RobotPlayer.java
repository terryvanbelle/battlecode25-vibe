...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla64";
    ...
    static String runTower() throws GameActionException {
        ...
        // darla64: LOCAL expansion signal. darla62/63 tried to detect the expansion
        // death spiral from the team tower count and both failed, because "stalled" and
        // "finished expanding" are the same number. The signal that separates them is
        // "is there unclaimed ruin work left?", which I closed as unavailable -- wrongly.
        // A TOWER can sense ruins itself, and an unclaimed ruin in its own vision is
        // exactly "expansion work exists HERE", with no team-wide state, no
        // communication and no latching: it goes away by itself the moment the ruin is
        // taken, which is precisely what the tower-count triggers could never do.
        boolean ruinWork = false;
        for (MapLocation r : rc.senseNearbyRuins(-1)) {
            if (!rc.canSenseRobotAtLocation(r)) { ruinWork = true; break; }
        }
        int roll = rng.nextInt(20);
        if (ruinWork) roll = 19;                // 19 is always a soldier
