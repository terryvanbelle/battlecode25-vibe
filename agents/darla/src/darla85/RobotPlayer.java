...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla85";
    ...
    // darla85: between darla82 (hug always, -14) and darla84 (hug for one turn after the
    // stuck condition, +13). Engage on the same measured trigger, but PERSIST while
    // engaged -- a one-shot hug may not be enough to leave a deep concave pocket, since
    // one sidestep can look like progress and drop the state. bugDir is cleared the moment
    // a direct move succeeds, so the hug still ends as soon as the path is genuinely open.
    ...
    static boolean stepToward(MapLocation to) throws GameActionException {
        ...
        if (stuckLast || (bugDir != null && to.equals(bugTo))) { boolean lf = (rc.getID() & 1) == 0; if (bugDir == null || !to.equals(bugTo)) bugDir = d; bugTo = to; for (int i = 0; i < 8; i++) { bugDir = lf ? bugDir.rotateLeft() : bugDir.rotateRight(); if (rc.canMove(bugDir)) { rc.move(bugDir); bugDir = lf ? bugDir.rotateRight() : bugDir.rotateLeft(); return true; } } }
