...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla65";
    static int msgSent = 0, msgHeard = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " ms=" + msgSent + "/" + msgHeard + " rt=" + refillTrips + " ht=" + homeTurns + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak
            + " | " + state);
        Clock.yield();
    }
    ...
    static String runTower() throws GameActionException {
        ...
        boolean ruinReported = false;
        for (Message m : rc.readMessages(-1)) if (m.getBytes() == 1) { ruinReported = true; break; }
        if (ruinReported) msgHeard++;
        int roll = rng.nextInt(20);
        if (ruinReported) roll = 19;            // 19 is always a soldier
    ...
    static String runSoldier() throws GameActionException {
        ...
        // darla65: REPORT expansion work. The expansion spiral (6 frozen towers against
        // alice's 25) needs towers to know unclaimed ruins exist, and a tower cannot see
        // one -- its vision is 4.5 tiles against a 9.4-tile median ruin spacing (darla64).
        // A SOLDIER standing at a ruin can see it, and [E, carol RULES.md] a robot may
        // message a tower within r2=20 along an ally-paint path, 1 message per turn.
        // This is the only route to the signal, and communication is a capability no
        // lineage in this project has ever used.
        MapLocation ruin = nearestEmptyRuin();
        if (ruin != null && rc.isActionReady()) {
            for (RobotInfo a : rc.senseNearbyRobots(20, rc.getTeam())) {
                if (!a.type.isTowerType()) continue;
                if (rc.canSendMessage(a.location)) { rc.sendMessage(a.location, 1); msgSent++; break; }
            }
        }
        if (ruin != null) {
            state += " ruin=" + ruin;
