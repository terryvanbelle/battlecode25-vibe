...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla97";
    ...
    // darla97: INSTRUMENTATION ONLY, and a re-measurement rather than a new idea.
    // darla94 measured the spawn decision on the i3 build: blocked on chips 397-544
    // times per tower on large maps, on TOWER PAINT 423-435 on small ones, with a tower
    // spawning once or twice a game on a large map against 32 on a small one. Iteration
    // 4 then changed the tower mix -- paint share 67%% -> 86%% -- which is the input that
    // block reads. So the honest question is not "what next" but "did the constraint
    // move, or did it only shift": same counters, new build.
