...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla141";
    ...
    static int[] remRuin = new int[32]; static int remN = 0, rmIdle = 0, rmIdleAny = 0, rmGo = 0, raTower = 0, raFoe = 0, raAlly = 0, raClear = 0; static MapLocation raTo = null;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " rm=" + rmIdle + "/" + rmIdleAny + "/" + rmGo + " ra=" + raTower + "/" + raFoe + "/" + raAlly + "/" + raClear + " ma=" + mixAsk + " mf=" + mixFlip
    ...
    static String runSoldier() throws GameActionException {
        ...
        for (MapLocation rr : rc.senseNearbyRuins(-1)) { if (rc.canSenseRobotAtLocation(rr) || ruinBanned(rr)) continue; boolean foeT = false; for (MapInfo t : rc.senseNearbyMapInfos(rr, 8)) if (t.getPaint().isEnemy()) { foeT = true; break; } if (foeT) continue; int key = (rr.x << 6) | rr.y; boolean known = false; for (int i = remN; --i >= 0; ) if (remRuin[i] == key) { known = true; break; } if (!known && remN < 32) remRuin[remN++] = key; }
        for (int i = remN; --i >= 0; ) { MapLocation rl = new MapLocation(remRuin[i] >> 6, remRuin[i] & 63); boolean drop = false; if (rc.canSenseLocation(rl)) { if (rc.canSenseRobotAtLocation(rl)) drop = true; else for (MapInfo t : rc.senseNearbyMapInfos(rl, 8)) if (t.getPaint().isEnemy()) { drop = true; break; } } if (drop) remRuin[i] = remRuin[--remN]; }
        if (raTo != null && rc.getLocation().distanceSquaredTo(raTo) <= 8) { if (rc.canSenseRobotAtLocation(raTo)) raTower++; else { boolean foeP = false, allyS = false; for (MapInfo t : rc.senseNearbyMapInfos(raTo, 8)) if (t.getPaint().isEnemy()) { foeP = true; break; } for (RobotInfo a : rc.senseNearbyRobots(raTo, 8, rc.getTeam())) if (a.type == UnitType.SOLDIER) { allyS = true; break; } if (foeP) raFoe++; else if (allyS) raAlly++; else raClear++; } raTo = null; }
        if (ruin != null) {
            state += " ruin=" + ruin;
            workOnRuin(ruin);
        }
            ...
            if (myTile.getPaint() == PaintType.EMPTY && rc.canAttack(me)) {
                ...
                if (best != null) { rc.attack(best); state += " pnt"; }
                    ...
                    if (foe == 0) {
                        ...
                        else { MapLocation rr0 = null; int rd0 = Integer.MAX_VALUE; for (int i = remN; --i >= 0; ) { MapLocation rl = new MapLocation(remRuin[i] >> 6, remRuin[i] & 63); int dd = me.distanceSquaredTo(rl); if (dd < rd0) { rd0 = dd; rr0 = rl; } } if (rr0 != null) { explore = rr0; exploreAge = 0; rmGo++; raTo = rr0; state += " remRuin"; } else state += " frontNone"; }
                    }
                    state += (foe > 0 ? " IDLE-ENEMY" : " IDLE-ALLY") + ally + "/" + foe;
                    rmIdleAny++; if (remN > 0) rmIdle++;
                }
            }
        }
