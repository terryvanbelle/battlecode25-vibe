...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla114";
    ...
    static int idleT = 0, nearNone = 0, dNear = 0, dMid = 0, dFar = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " id=" + idleT + "/" + nearNone + "/" + dNear + "/" + dMid + "/" + dFar + " ma=" + mixAsk + " mf=" + mixFlip
    ...
    static String runSplasher() throws GameActionException {
        ...
        if (ready && fueled) {
            ...
            if (best != null && bestScore >= SPLASH_MIN_SCORE) {
                ...
                idleT++;
                int bd = 999;
                for (MapInfo t2 : rc.senseNearbyMapInfos(-1)) { if (t2.getPaint() == PaintType.EMPTY && t2.isPassable()) { int dd = me.distanceSquaredTo(t2.getMapLocation()); if (dd < bd) bd = dd; } }
                if (bd == 999) nearNone++; else if (bd <= 2) dNear++; else if (bd <= 8) dMid++; else dFar++;
            }
        } else {
