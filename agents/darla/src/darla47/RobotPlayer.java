...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla47";
    ...
    static String runSplasher() throws GameActionException {
        ...
        if (siege != null && rc.isMovementReady()) {
            ...
            // darla47: a DISPERSED landmark, which is the distinction the session's two
            // best results turn on. The tower attractor is worth 99-51 and every shared
            // heading has lost (-22 at 15% splashers, -7.84 sd at 70%), and the difference
            // is not the signal but the SPREAD: each splasher locks onto whichever tower
            // IT can see, so the fleet disperses while still aiming at enemy ground. A
            // centroid pulls everyone to one point and the overlap penalty scales with the
            // number of units overlapping.
            //
            // So: when no tower is visible, index the visible enemy tiles by this robot's
            // OWN id. Same signal as darla22, deliberately de-correlated across units.
            MapLocation pick = null; int k = 0, want = -1;
            for (MapInfo t : rc.senseNearbyMapInfos(-1)) if (t.getPaint().isEnemy()) k++;
            if (k > 0) {
                want = Math.floorMod(rc.getID(), k);
                int i = 0;
                for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
                    if (!t.getPaint().isEnemy()) continue;
                    if (i++ == want) { pick = t.getMapLocation(); break; }
                }
            }
            if (pick != null) { moveExploring(pick); tag += " spread" + k; }
            else { moveExploring(null); tag += " roam"; }
        }
