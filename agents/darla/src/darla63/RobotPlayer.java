...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla63";
    /** darla63: expansion-STALL detection. darla62 keyed on "fewer than 8 towers",
     *  which is a proxy for EARLY rather than STALLED -- every game starts at 2, so it
     *  fired through the whole opening and cost 40 games. The spiral is defined by
     *  expansion having STOPPED (DonkeyKong sat at 6 towers from round 300 to 590),
     *  so trigger on that instead. Inert in a healthy opening, which is adding towers
     *  continuously. */
    static int lastTowerCount = -1, lastGrowthRound = 0;
    static final int STALL_ROUNDS = 150;
    ...
    static String runTower() throws GameActionException {
        ...
        if (towers != lastTowerCount) { lastTowerCount = towers; lastGrowthRound = rc.getRoundNum(); }
        boolean stalled = rc.getRoundNum() - lastGrowthRound >= STALL_ROUNDS;
        int roll = rng.nextInt(20);
        if (stalled) roll = 19;                 // 19 is always a soldier
