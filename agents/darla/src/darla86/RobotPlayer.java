...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla86";
        ...
        if (rc.isActionReady() && rc.getPaint() >= UnitType.SOLDIER.attackCost) {
            ...
            if (myTile.getPaint() == PaintType.EMPTY && rc.canAttack(me)) {
                ...
                if (best != null) { rc.attack(best); state += " pnt"; }
                    ...
                    if (true) { // darla86: UNGATE frontier-seeking. Iteration 14 built it for the IDLE-ALLY case and gated it on foe==0, which switches it off on every turn enemy paint is in sight -- and the i3 census makes those 45% of soldier turns, the largest block in the bot. A soldier that cannot paint where it stands should head for paintable ground whether or not enemy paint is also visible; the enemy paint is not what stops it moving.
