...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla103";
    ...
    static String runTower() throws GameActionException {
        ...
        UnitType want = (roll < SPLASHER_IN_20) ? UnitType.SPLASHER
                      ...
                      : UnitType.SOLDIER;
        // darla103: SOLDIERS UNTIL THE FIRST NEW TOWER, not for a fixed 100 rounds.
        // darla102 forced soldiers for rounds 1-100 and did fix the opening completely --
        // first tower r266 to r30 on shell, ahead of v3 -- but lost 7 games to v3, because
        // 100 rounds without splashers also costs 100 rounds of area paint, and coverage is
        // this bot every game. Two changes bundled, one of them unwanted.
        // Both sides start with 4 towers, so getNumberTowers() > 4 means expansion has
        // actually begun. The override then switches itself off, which bounds the coverage
        // cost by the thing it is buying instead of by a round number picked off a chart.
        if (rc.getNumberTowers() <= 4 && want != UnitType.SOLDIER) { want = UnitType.SOLDIER; mixFlip++; }
