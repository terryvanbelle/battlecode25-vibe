...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla137";
    ...
    static int xrNew = 0, xrArrive = 0, xrStuck = 0, xrAge = 0, xdSum = 0, xdN = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " xr=" + xrNew + "/" + xrArrive + "/" + xrStuck + "/" + xrAge + " xd=" + xdSum + "/" + xdN + " ma=" + mixAsk + " mf=" + mixFlip
    ...
    static void newExploreTarget() {
        ...
        MapLocation best = null;
        int bestD = -1;
        boolean far = true; try { for (MapInfo ti : rc.senseNearbyMapInfos(-1)) if (ti.getPaint().isEnemy()) { far = false; break; } } catch (GameActionException e) {}
        for (int i = 0; i < 4; i++) {   // sample a few, keep the farthest
            ...
            if (far ? d > bestD : (bestD < 0 || d < bestD)) { bestD = d; best = c; }
        }
        explore = best; xdSum += (int) Math.sqrt(bestD); xdN++;
        exploreAge = 0;
    }
