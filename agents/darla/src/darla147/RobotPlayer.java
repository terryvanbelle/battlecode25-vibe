...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla147";
    ...
    static boolean mopperDue = true; static int mopBuilt = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " mb=" + mopBuilt + " ma=" + mixAsk + " mf=" + mixFlip
    ...
    static String runTower() throws GameActionException {
        ...
        boolean foeSeen = false; for (MapInfo tf : rc.senseNearbyMapInfos(-1)) if (tf.getPaint().isEnemy()) { foeSeen = true; break; }
        ...
        if (afford && want.paintCost < UnitType.SOLDIER.paintCost && !(want == UnitType.MOPPER && mopperDue && foeSeen && rc.getPaint() >= want.paintCost)
        ...
        if (afford) {
            ...
            if (rc.canBuildRobot(want, loc)) { rc.buildRobot(want, loc); if (want == UnitType.SOLDIER) mopperDue = true; else if (want == UnitType.MOPPER) { mopperDue = false; mopBuilt++; } }
        }
