...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla22";
    ...
    static String runSplasher() throws GameActionException {
        ...
        if (siege != null && rc.isMovementReady()) {
            ...
            // darla22: head for the CENTROID of visible enemy paint, not the nearest tile.
            //
            // darla20 used the nearest enemy tile and lost 38 games (-6.33 sd). The census
            // says exactly why: 240 of 328 splasher-turns came back `lowScore lure` --
            // holding paint (median 99 against a 50 cost), action ready, target acquired,
            // and REFUSING TO FIRE because the best centre scored under SPLASH_MIN_SCORE.
            // The nearest enemy tile is a lone frontier tile; a splash there covers one or
            // two enemy tiles at +3 each and never reaches 8.
            //
            // So the tower attractor was never really about towers. A tower sits inside a
            // MASS of enemy paint, and mass is what makes a splash worth firing. Nearest
            // is a proxy for proximity, which is worthless; the centroid is a proxy for
            // density, which is the thing. It is also O(n) over vision and needs no memory.
            MapLocation me2 = rc.getLocation();
            int sx = 0, sy = 0, k = 0;
            for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
                if (!t.getPaint().isEnemy()) continue;
                sx += t.getMapLocation().x; sy += t.getMapLocation().y; k++;
            }
            if (k > 0) { moveExploring(new MapLocation(sx / k, sy / k)); tag += " mass" + k; }
