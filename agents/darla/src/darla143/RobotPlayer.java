...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla143";
    ...
    static int earlyTrips = 0;
    static int penaltyOf(PaintType pt) { return pt.isEnemy() ? GameConstants.PENALTY_ENEMY_TERRITORY : (pt == PaintType.EMPTY ? GameConstants.PENALTY_NEUTRAL_TERRITORY : 0); }
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " et=" + earlyTrips + " ma=" + mixAsk + " mf=" + mixFlip
    ...
    static boolean walkHomeIfDry(int cap) throws GameActionException {
        ...
        if (!refilling) { int need = REFILL_LOW; MapLocation h0 = nearestRememberedTower(); if (h0 != null) { MapLocation cur = rc.getLocation(); int steps = Math.max(Math.abs(cur.x - h0.x), Math.abs(cur.y - h0.y)); int lastPen = penaltyOf(rc.senseMapInfo(cur).getPaint()); int cost = 0; for (int s = 0; s < steps; s++) { cur = cur.add(cur.directionTo(h0)); if (rc.canSenseLocation(cur)) lastPen = penaltyOf(rc.senseMapInfo(cur).getPaint()); cost += lastPen; } need += cost; } if (paint > need) return false; if (paint > REFILL_LOW) earlyTrips++; }
