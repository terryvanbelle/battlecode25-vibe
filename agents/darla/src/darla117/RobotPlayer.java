...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla117";
    ...
    static int rollM = 0, mReserve = 0, mFloor = 0, mPaint = 0, mCbr = 0, mBuilt = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " mr=" + rollM + "/" + mReserve + "/" + mFloor + "/" + mPaint + "/" + mCbr + "/" + mBuilt + " ma=" + mixAsk + " mf=" + mixFlip
    ...
    static String runTower() throws GameActionException {
        ...
        if (want == UnitType.MOPPER) { rollM++; if (!afford) mReserve++; }
        if (afford && want != UnitType.SPLASHER && chips - want.moneyCost < SPLASH_FLOOR) { if (want == UnitType.MOPPER) mFloor++;
            afford = false;
        }
        ...
        if (afford && want.paintCost < UnitType.SOLDIER.paintCost
                && rc.getPaint() - want.paintCost < PAINT_FLOOR) { if (want == UnitType.MOPPER) mPaint++;
            afford = false;
        }
        if (afford) {
            Direction dir = DIRS[rng.nextInt(8)];
            MapLocation loc = rc.getLocation().add(dir);
            if (rc.canBuildRobot(want, loc)) { rc.buildRobot(want, loc); if (want == UnitType.MOPPER) mBuilt++; } else if (want == UnitType.MOPPER) mCbr++;
        }
