...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla92";
        ...
        if (rc.isActionReady() && rc.getPaint() >= UnitType.SOLDIER.attackCost) {
            ...
            if (myTile.getPaint() == PaintType.EMPTY && rc.canAttack(me)) {
                ...
                if (best != null) { rc.attack(best); state += " pnt"; }
                    ...
                    if (foe == 0 || foe > ally) { // darla92: darla86 (no gate) was +11 on this roster and darla91 (ally >= foe) was -5, with carol swinging +9 to -9 -- so the earning turns are exactly the ones darla91 gated OFF, where the enemy holds more nearby paint than we do. Keep the original foe==0 branch untouched and add back only that case: leave when the ground is already theirs, stay and contest when it is even or ours.
