...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla89";
    ...
    static String runSplasher() throws GameActionException {
        ...
        if (siege != null && rc.isMovementReady()) {
            ...
            { MapLocation ep = null; if (rc.getPaint() >= UnitType.SPLASHER.attackCost) { int bd = 9999; MapLocation sm = rc.getLocation(); for (MapInfo t : rc.senseNearbyMapInfos(-1)) { if (t.getPaint().isEnemy()) { int dd = sm.distanceSquaredTo(t.getMapLocation()); if (dd < bd) { bd = dd; ep = t.getMapLocation(); } } } if (ep != null) { Direction back = ep.directionTo(sm); ep = ep.add(back).add(back); } } moveExploring(ep); } // darla89: darla88 sent splashers to the nearest enemy paint TILE and lost 143 of 150 -- standing on enemy paint drains paint every turn, so the 300-paint unit walked into the one place that kills it. The splash centre sits r2<=4 from the splasher, so the useful position is TWO TILES SHORT of the cluster, on our own side of it: in range, out of the drain.
        }
