...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla9";
                ...
                if (best != null) { rc.attack(best); state += " pnt"; }
                    ...
                    if (foe == 0) {
                        ...
                        if (f != null) { explore = f; exploreAge = 0; state += " frontFound"; }
                            ...
                            // darla9: frontNone fired on 185 of 187 IDLE-ALLY turns --
                            // the soldier is deep inside our own paint with no empty tile
                            // anywhere in r2=20, so it falls back to newExploreTarget(),
                            // which picks a RANDOM map coordinate and therefore points it
                            // at more of our own territory as often as not.
                            //
                            // Enemy paint IS the frontier: it is the nearest ground that is
                            // neither ours nor already taken by us, and a splasher following
                            // the same heading can overwrite it. So head for the nearest
                            // enemy paint in vision instead of a random coordinate. carol's
                            // rule -- a persistent heading beats a nearest-target rule --
                            // is preserved, because this sets `explore` (a heading held
                            // across turns) rather than steering per-turn.
                            MapLocation e = nearestVisibleEnemyPaint();
                            if (e != null) { explore = e; exploreAge = 0; state += " frontFoe"; }
    ...
    /** Nearest enemy-painted passable tile anywhere in vision (r2=20), or null. */
