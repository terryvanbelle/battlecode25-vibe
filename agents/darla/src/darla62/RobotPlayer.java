...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla62";
    /** Below this many team towers, every build is a soldier (darla62). */
    static final int EXPAND_FLOOR = 8;
    ...
    static String runTower() throws GameActionException {
        ...
        // darla62: EXPANSION INSURANCE. Contrast census of two large maps vs alice --
        // DefaultHuge (win): darla 7 -> 23 towers. DonkeyKong (loss): darla frozen at 6
        // while alice reached 25. The losing shape is a death spiral: soldiers collapse
        // to 1, only soldiers claim ruins, so no towers, so no paint income, so no
        // soldiers. Once expansion stalls this build never recovers.
        //
        // So force a soldier whenever the team is still small, regardless of the roll.
        // getNumberTowers() is team-wide and identical for every tower, so they agree
        // without communicating. This fires ONLY in the failure state -- a healthy game
        // passes the threshold in the opening and never sees it again -- which is the
        // "helps uniformly or not at all" shape that the darla56 trade argued for.
        int roll = rng.nextInt(20);
        if (rc.getNumberTowers() < EXPAND_FLOOR) roll = 19;   // 19 is always a soldier
