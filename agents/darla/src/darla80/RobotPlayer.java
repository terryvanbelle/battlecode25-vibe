...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla80";
    ...
    // darla80: INSTRUMENTATION ONLY. RESEARCH.md §5 is the most consistent story in
    // the whole digest -- every year converges on bug-navigation plus an escape for
    // concave obstacles -- and stepToward here is memoryless greedy with no stuck
    // detection at all. Before building that, measure whether the failure it fixes
    // actually happens: count consecutive calls toward the SAME target where the
    // distance did not fall. Iteration 2 removed the supply stall, so if movement is
    // now what wastes soldier turns, this counter is where it shows up first.
    ...
    static boolean stepToward(MapLocation to) throws GameActionException {
        { int d0 = rc.getLocation().distanceSquaredTo(to); if (to.equals(mvLastTo)) { mvTry++; if (d0 >= mvLastD) mvStuck++; } mvLastTo = to; mvLastD = d0; }
