...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla73";
    /** darla70: SYMMETRY INFERENCE -- reference/RESEARCH.md section 6, and item 2 on its
     *  own "when stuck" list: standard practice everywhere else, never built by any
     *  lineage here, needs no communication, and is a DERIVATION rather than a
     *  heuristic. Every BC map is rotational, horizontal or vertical symmetric for
     *  fairness, so our starting tower's mirror must hold a ruin. Keep all three
     *  candidates live and eliminate any whose mirror we can see and which has no ruin.
     *
     *  Why this is the heading to try: tonight established that positioning needs a
     *  STABLE target (the tower attractor is worth 99-51) and that nearest-target
     *  headings lose badly (darla9 -10, darla20 -38, darla41 -10.78 sd). The enemy base
     *  is the most stable landmark on the map and is known from round 1 -- the one
     *  heading type never tried. It fills the IDLE-ALLY gap: 24% of soldier turns, with
     *  the frontier search finding a target 2 times in 187. */
    static MapLocation myStart = null;
    static boolean symRot = true, symHorz = true, symVert = true;
    static int symElim = 0, symUsed = 0;

    /** Mirror of `l` under a candidate: 0 rotational, 1 horizontal, 2 vertical. */
    static MapLocation mirrorOf(MapLocation l, int kind) {
        int w = rc.getMapWidth() - 1, h = rc.getMapHeight() - 1;
        if (kind == 0) return new MapLocation(w - l.x, h - l.y);
        if (kind == 1) return new MapLocation(l.x, h - l.y);
        return new MapLocation(w - l.x, l.y);
    }

    /** darla72: eliminate from terrain ALREADY IN VISION, which is what
     *  RESEARCH.md means by "eliminate them as terrain is observed". darla71 checked
     *  whether the mirrored START held a ruin, which needs a journey across the map --
     *  so eliminations were 0 in every game and the arm ran an un-eliminated guess.
     *
     *  Here every visible tile constrains every candidate immediately: if tile A is a
     *  wall and its mirror B is also in vision and is NOT a wall, that symmetry is
     *  impossible. No travel, no memory, exact. Costs one vision sweep per turn, and a
     *  robot only needs a few asymmetric pairs to pin the map. */
    static void refineSymmetry() throws GameActionException {
        // darla73: BOUND THE COST. darla72 did a full vision sweep with three candidates
        // and a senseMapInfo per mirror, EVERY turn, and produced bytecode overruns
        // (ov=3) -- robots missing whole turns, which by the rule registered at darla8
        // voids an arm regardless of its score. The inference itself worked (sy=1/0,
        // sy=2/0: candidates were being eliminated).
        //
        ...
        for (MapInfo t : seen) {
            MapLocation a = t.getMapLocation();
            for (int k = 0; k < 3; k++) {
                boolean live = (k == 0) ? symRot : (k == 1) ? symHorz : symVert;
                if (!live) continue;
                MapLocation b = mirrorOf(a, k);
                if (!rc.canSenseLocation(b)) continue;
                if (rc.senseMapInfo(b).isPassable() != t.isPassable()) {
                    if (k == 0) symRot = false; else if (k == 1) symHorz = false; else symVert = false;
                    symElim++;
                }
            }
        }
    ...
    /** Best surviving guess at the enemy base, or null. */
    static MapLocation enemyBase() {
        if (myStart == null) return null;
        if (symRot)  return mirrorOf(myStart, 0);
        if (symHorz) return mirrorOf(myStart, 1);
        if (symVert) return mirrorOf(myStart, 2);
        return null;
    }
     ...
     */
    static final int SPLASHER_IN_20 = 14;   // ACCEPTED iteration 1: 97-53 head-to-head vs darla_iter0 (+3.59 sd), 32 swept to 10.
    ...
    static final int SPLASH_MIN_SCORE = 14;   // ACCEPTED iteration 1, jointly with SPLASHER_IN_20 above. Neither is established alone.
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            String state = "";
            try {
                // darla71: statics are PER ROBOT in this engine. darla70 set myStart only
                // on towers, so every soldier kept myStart == null, enemyBase() returned
                // null, and the whole mechanism was a no-op (sy=0/0, all 75 maps
                // identical). Any ALLIED TOWER works as the reference -- it stands on a
                // ruin, so its mirror holds a ruin too, and the derivation is unchanged.
                if (myStart == null) {
                    if (rc.getType().isTowerType()) myStart = rc.getLocation();
                    else for (RobotInfo a : rc.senseNearbyRobots(-1, rc.getTeam()))
                        if (a.type.isTowerType()) { myStart = a.location; break; }
                }
                if (myStart != null) refineSymmetry();
                switch (rc.getType()) {
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " sy=" + symElim + "/" + symUsed + " rt=" + refillTrips + " ht=" + homeTurns + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak
            + " | " + state);
        Clock.yield();
    }
                ...
                if (best != null) { rc.attack(best); state += " pnt"; }
                    ...
                    if (foe == 0) {
                        ...
                        if (f != null) { explore = f; exploreAge = 0; state += " frontFound"; }
                            ...
                            MapLocation eb = enemyBase();
                            if (eb != null) { explore = eb; exploreAge = 0; symUsed++; state += " symTgt"; }
    ...
    static String runSplasher() throws GameActionException {
        ...
        for (RobotInfo f : foes) {
            ...
            // Money towers first: killing the last one freezes the enemy treasury outright.
            int rank = (f.type.moneyPerTurn > 0) ? 2 : 1;
            if (rank > siegeRank) { siegeRank = rank; siege = f.location; }
        }
        ...
        if (ready && fueled) {
            ...
            for (MapLocation c : rc.getAllLocationsWithinRadiusSquared(me, 4)) {
                ...
                // AoE reaches r2<=4 from the centre -- so a splasher can strike from distance 4
                // while a paint/money tower answers only to r2=9 (distance 3).
                for (RobotInfo f : foes) {
                    if (f.type.isTowerType() && c.distanceSquaredTo(f.location) <= 4)
                        score += (f.type.moneyPerTurn > 0) ? 100 : 60;
                }
        ...
        if (siege != null && rc.isMovementReady()) {
            int d = rc.getLocation().distanceSquaredTo(siege);
            if (d > 16) {
                stepToward(siege);                 // close to the firing ring
                tag += " approach";
            } else if (d <= 9) {
                Direction away = siege.directionTo(rc.getLocation());
                ...
                tag += " backoff";                 // inside the tower's r2=9 -- retreat
            } else {
