...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla99";
    ...
    static String runTower() throws GameActionException {
        ...
        // darla99: a DEMAND-DRIVEN mopper, the one unit that can clear enemy paint.
        // The expansion stall is now traced end to end: a soldier skips ~40 ruins per game
        // as banned (bs=37-41) and the bans are DENY bans (dn=3-7) -- enemy paint sitting on
        // the tower pattern. Soldiers cannot overwrite enemy paint; only moppers can. We
        // build no moppers, because iteration 30's PAINT_FLOOR gates exactly the mopper and
        // darla81 measured removing that floor at -22 games. But darla81 removed it
        // ALTOGETHER and got 25 moppers in 42 robots -- a flood. This is the opposite: a
        // mopper only when THIS tower can see enemy paint, which is the only situation the
        // unit is for. Towers run at ~450 of 20,000 bytecode, so a vision sweep here is free.
        if (afford && want != UnitType.MOPPER && rc.getPaint() >= UnitType.MOPPER.paintCost) {
            for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
                if (t.getPaint().isEnemy()) { want = UnitType.MOPPER; mixFlip++; break; }
            }
        }
