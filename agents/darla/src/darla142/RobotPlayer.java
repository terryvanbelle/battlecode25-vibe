...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla142";
    ...
    static boolean walkHomeIfDry(int cap) throws GameActionException {
        ...
        if (!refilling) { int need = REFILL_LOW; MapLocation h0 = nearestRememberedTower(); if (h0 != null) { MapLocation me0 = rc.getLocation(); PaintType pt = rc.senseMapInfo(me0).getPaint(); int pen = pt.isEnemy() ? GameConstants.PENALTY_ENEMY_TERRITORY : (pt == PaintType.EMPTY ? GameConstants.PENALTY_NEUTRAL_TERRITORY : 0); need += Math.max(Math.abs(me0.x - h0.x), Math.abs(me0.y - h0.y)) * pen; } if (paint > need) return false; if (paint > REFILL_LOW) earlyTrips++; }
