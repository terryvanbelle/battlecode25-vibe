...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla88";
    ...
    static String runSplasher() throws GameActionException {
        ...
        if (siege != null && rc.isMovementReady()) {
            ...
            { MapLocation ep = null; if (rc.getPaint() >= UnitType.SPLASHER.attackCost) { int bd = 9999; MapLocation sm = rc.getLocation(); for (MapInfo t : rc.senseNearbyMapInfos(-1)) { if (t.getPaint().isEnemy()) { int dd = sm.distanceSquaredTo(t.getMapLocation()); if (dd < bd) { bd = dd; ep = t.getMapLocation(); } } } } moveExploring(ep); } // darla88: darla87 measured the best available splash score on lowScore turns at a mean of 7.1 against a threshold of 14 -- the splasher is not missing work, it is standing where there is none. So a fueled splasher with nothing to hit walks toward the nearest enemy paint in VISION instead of exploring at random. Same class of change as darla86 (trigger reads the opponent) so the roster run decides, not the head-to-head.
        }
