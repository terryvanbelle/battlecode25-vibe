...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla82";
    ...
    // darla82: bug-navigation state. darla80 measured 21.6% (Bread) and 27.8% (Portal)
    // of same-target moves failing to reduce distance, against 0.0% on open Fossil.
    ...
    static boolean stepToward(MapLocation to) throws GameActionException {
        ...
        if (rc.canMove(d)) { rc.move(d); bugDir = null; bugTo = to; return true; }  // direct path clear: drop the hug
{ boolean left = (rc.getID() & 1) == 0; if (bugDir == null || !to.equals(bugTo)) bugDir = d; bugTo = to; for (int i = 0; i < 8; i++) { bugDir = left ? bugDir.rotateLeft() : bugDir.rotateRight(); if (rc.canMove(bugDir)) { rc.move(bugDir); bugDir = left ? bugDir.rotateRight() : bugDir.rotateLeft(); return true; } } return false; } // darla82: follow the obstacle from the LAST DIRECTION MOVED instead of re-scanning from the target every turn, which is what let a robot in a concave pocket oscillate forever. Turn preference stays keyed on robot ID, as the old tie-break was, so play symmetry is unchanged.
    }
}
