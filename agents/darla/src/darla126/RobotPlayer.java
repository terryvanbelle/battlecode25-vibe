...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla126";
    ...
    static int bugStartD = -1, mvBlocked = 0;
    static MapLocation arrTo = null; static boolean arrDone = false; static int mvTargets = 0, mvArrive = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " ar=" + mvArrive + "/" + mvTargets + " mb=" + mvBlocked + " ma=" + mixAsk + " mf=" + mixFlip
    ...
    static boolean stepToward(MapLocation to) throws GameActionException {
        int d0 = rc.getLocation().distanceSquaredTo(to); stuckLast = false; if (!to.equals(arrTo)) { arrTo = to; mvTargets++; arrDone = false; } if (!arrDone && d0 <= 2) { mvArrive++; arrDone = true; } if (to.equals(mvLastTo)) { mvTry++; if (d0 >= mvLastD) { mvStuck++; stuckLast = true; } } mvLastTo = to; mvLastD = d0;
        Direction d = rc.getLocation().directionTo(to);
        if (d == Direction.CENTER) return false;
        boolean following = (bugDir != null && to.equals(bugTo));
        if (rc.canMove(d) && (!following || d0 < bugStartD)) { rc.move(d); bugDir = null; bugTo = to; return true; }
        if (following || stuckLast) { boolean lf = (rc.getID() & 1) == 0; if (!following) { bugDir = d; bugStartD = d0; } bugTo = to; for (int i = 0; i < 8; i++) { bugDir = lf ? bugDir.rotateLeft() : bugDir.rotateRight(); if (rc.canMove(bugDir)) { rc.move(bugDir); bugDir = lf ? bugDir.rotateRight() : bugDir.rotateLeft(); return true; } } }
