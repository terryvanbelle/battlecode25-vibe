...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla166";
    ...
    static MapLocation srp = null;
    static int srpSplash = 0, srpSeen = 0;          // darla166: splasher-laid patterns
    static MapLocation srpGo = null; static boolean srpSec = false;
    static int srpTurns = 0, srpMark = 0, srpDone = 0, srpQuit = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " srp=" + srpMark + "/" + srpDone + "/" + srpQuit + " ss=" + srpSplash + "/" + srpSeen + " mp=" + moneyPick + " os=" + openSold + " ma=" + mixAsk + " mf=" + mixFlip
    ...
    static String runTower() throws GameActionException {
        ...
        if (afford && want != UnitType.SPLASHER && !(want == UnitType.SOLDIER && opening) && chips - want.moneyCost < SPLASH_FLOOR) {
            afford = false;
        }
        ...
        if (afford) {
            ...
            if (rc.canBuildRobot(want, loc)) { rc.buildRobot(want, loc); if (want == UnitType.SOLDIER && opening) openSold++; }
        }
    ...
    static String runSoldier() throws GameActionException {
        ...
        if (rc.canCompleteResourcePattern(me0)) { rc.completeResourcePattern(me0); srpDone++; srp = null; state += " SRPdone"; }
        else if (srp == null && ruin == null && rc.getNumberTowers() >= 3 && rc.senseNearbyRuins(8).length == 0 && rc.canMarkResourcePattern(me0)) {
            boolean safe = true;
            for (MapInfo t : rc.senseNearbyMapInfos(-1)) if (t.getPaint().isEnemy()) { safe = false; break; }
            if (safe) { rc.markResourcePattern(me0); srp = me0; srpTurns = 0; srpMark++; state += " SRPmark"; }
        }
        if (srp != null && ruin != null) { srp = null; srpQuit++; state += " SRPyield"; }
        if (srp != null) {
            if (rc.canCompleteResourcePattern(srp)) { rc.completeResourcePattern(srp); srpDone++; srp = null; state += " SRPdone"; }
            else if (++srpTurns > RUIN_PATIENCE) { srp = null; srpQuit++; state += " SRPquit"; }
            else for (MapInfo t : rc.senseNearbyMapInfos(srp, 8)) {
                PaintType mk = t.getMark();
                if (mk == PaintType.EMPTY) continue;
                if (t.getPaint().isEnemy()) { srp = null; srpQuit++; state += " SRPfoe"; break; }
                if (mk != t.getPaint() && rc.canAttack(t.getMapLocation())) { rc.attack(t.getMapLocation(), mk == PaintType.ALLY_SECONDARY); state += " SRPpnt"; break; }
            }
        }
        ...
        if (srp != null && rc.getLocation().equals(srp)) state += " SRPhold";
        else moveExploring(ruin != null ? ruin : srp);
    ...
    /**
     * darla166: find a marked resource pattern in vision and pick the splash that advances it.
     * Jar-verified (InternalRobot.splasherAttack): a splash sets every EMPTY or ALLY tile within
     * r2<=4 of the centre to the chosen colour, and enemy tiles only within r2<=2. The resource
     * pattern (GameConstants.RESOURCE_PATTERN) is secondary on the centre and the twelve cells
     * at d2 in {5, 8}, primary on the twelve cells at d2 in {1, 2, 4} -- i.e. the r2<=4 diamond
     * minus the centre. So: a secondary splash centred on each corner cell (d2=8) lays that
     * corner block, and a primary splash on the centre then lays the whole diamond in one turn.
     * Corners first, centre last, because the corner splashes spill onto the diamond. The centre
     * cell itself is left to a soldier's single-tile attack (or is already secondary).
     */
    static void srpSplashTarget() throws GameActionException {
        srpGo = null;
        for (MapInfo t : rc.senseNearbyMapInfos()) {
            if (t.getMark() != PaintType.ALLY_SECONDARY) continue;
            ...
            for (Direction d : new Direction[] { Direction.NORTH, Direction.EAST, Direction.SOUTH, Direction.WEST }) {
                MapLocation n = c.add(d);
                if (!rc.canSenseLocation(n) || rc.senseMapInfo(n).getMark() != PaintType.ALLY_PRIMARY) { centre = false; break; }
            }
            if (!centre) continue;
            srpSeen++;
            if (rc.canCompleteResourcePattern(c)) { rc.completeResourcePattern(c); srpDone++; return; }
            boolean cornerBad = false, ringBad = false; MapLocation corner = null;
            for (MapInfo u : rc.senseNearbyMapInfos(c, 8)) {
                MapLocation l = u.getMapLocation(); int d2 = c.distanceSquaredTo(l); PaintType p = u.getPaint();
                if (p.isEnemy()) { cornerBad = false; ringBad = false; break; }   // a soldier quits these too (SRPfoe)
                if (d2 == 5 || d2 == 8) {
                    if (p != PaintType.ALLY_SECONDARY && corner == null) { cornerBad = true; corner = new MapLocation(c.x + (l.x > c.x ? 2 : -2), c.y + (l.y > c.y ? 2 : -2)); }
                } else if (d2 > 0 && d2 <= 4 && p != PaintType.ALLY_PRIMARY) ringBad = true;
            }
            if (cornerBad) { srpGo = corner; srpSec = true; return; }
            if (ringBad) { srpGo = c; srpSec = false; return; }
        }
    }
    ...
    static String runSplasher() throws GameActionException {
        ...
        // darla166: a marked resource pattern in vision is laid with area paint. See DESIGN.md.
        if (fueled && rc.getPaint() >= 150) srpSplashTarget(); else srpGo = null;
        if (ready && fueled && srpGo != null && rc.canAttack(srpGo)) { rc.attack(srpGo, srpSec); srpSplash++; tag = " SRPsplash"; }
        else if (ready && fueled) {
            MapLocation me = rc.getLocation();
            MapLocation best = null;
        ...
        } else if (srpGo != null && rc.isMovementReady()) {
            stepToward(srpGo); tag += " SRPgo";
        } else {
            moveExploring(null);
        }
