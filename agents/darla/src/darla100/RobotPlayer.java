...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla100";
    ...
    static String runTower() throws GameActionException {
        ...
        // darla100: keep the ROLL, relax only the FLOOR. darla99 overrode `want` whenever
        // a tower could see enemy paint and produced 23 moppers in 29 robots -- 22/150, a
        // worse flood than darla81's. The error was not the idea but the trigger: I assumed
        // "tower sees enemy paint" was a rare, targeted condition and never measured it. In a
        // contested game it is nearly always true, so it carries no information at all.
        // Here the 10%% mopper roll still decides HOW OFTEN, and the enemy-paint test only
        // decides whether such a roll may bypass PAINT_FLOOR. Mopper share is capped at
        // MOPPER_IN_20 by construction, so a flood is arithmetically impossible.
        boolean moppableWork = false;
        if (want == UnitType.MOPPER && rc.getPaint() >= UnitType.MOPPER.paintCost) {
            for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
                if (t.getPaint().isEnemy()) { moppableWork = true; mixFlip++; break; }
            }
        }
