...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla35";
            ...
            for (MapLocation c : rc.getAllLocationsWithinRadiusSquared(me, 4)) {
                ...
                for (RobotInfo f : foes) {
                    if (f.type.isTowerType() && c.distanceSquaredTo(f.location) <= 4)
                        score += 0;   // darla35: positioning control re-run on the ACCEPTED build. Worth -17 at 15% splashers; the army is now 70% splashers, so it should matter MORE, and that is a prediction rather than a re-run.
                }
