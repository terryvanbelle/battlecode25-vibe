...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla138";
    ...
    static String runSoldier() throws GameActionException {
        ...
        RobotInfo[] enemies = rc.senseNearbyRobots(-1, rc.getTeam().opponent());
        for (RobotInfo e : enemies) if (e.type.isTowerType() && rc.getLocation().distanceSquaredTo(e.location) <= 9) { state += " TR"; break; }
        for (RobotInfo e : enemies) {
    ...
    static String runSplasher() throws GameActionException {
        ...
        MapLocation siege = null; int siegeRank = -1; boolean inTR = false;
        for (RobotInfo f : foes) {
            ...
            if (rc.getLocation().distanceSquaredTo(f.location) <= 9) inTR = true;
        }
