...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla21";
            ...
            for (MapLocation c : rc.getAllLocationsWithinRadiusSquared(me, 4)) {
                ...
                for (RobotInfo f : foes) {
                    if (f.type.isTowerType() && c.distanceSquaredTo(f.location) <= 4)
                        score += (f.type.moneyPerTurn > 0) ? 20 : 12;   // darla21: BELOW the 35-point paint ceiling, so paint can actually outvote a tower. The first dose on this axis that is not a guaranteed no-op.
                }
