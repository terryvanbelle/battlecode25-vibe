...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla79";
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " rt=" + refillTrips + " ht=" + homeTurns + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak + " gv=" + gvOpp + "/" + gvBlocked
            + " | " + state);
        Clock.yield();
    }
    ...
    // darla79: INSTRUMENTATION ONLY, no action taken, so play must be byte-identical.
    // darla78 put a paint hand-off in runMopper and it fired ZERO times in 150 games.
    // Before guessing at placement again, count the two things that decide whether the
    // mechanism has any surface at all: how often a mopper stands within transfer range
    // of an ally that is at or below REFILL_LOW while holding a surplus itself (gvOpp),
    // and how many of those turns the mopper was ALSO latched for its own refill and so
    // returned before ever reaching a hand-off (gvBlocked). On Flower, 23 of 54 mopper
    // turns were "M HOME", which is the suspicion this counter exists to confirm or kill.
    static int gvOpp = 0, gvBlocked = 0;
    ...
    static String runMopper() throws GameActionException {
        String state = "M";
        if (rc.getPaint() > REFILL_LOW) for (RobotInfo a : rc.senseNearbyRobots(2, rc.getTeam())) { if (!a.type.isTowerType() && a.paintAmount <= REFILL_LOW) { gvOpp++; if (rc.getPaint() < UnitType.MOPPER.paintCapacity / 2) gvBlocked++; break; } }
        refillIfPossible();
