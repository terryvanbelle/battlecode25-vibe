...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla139";
    ...
    static String runSoldier() throws GameActionException {
        ...
        for (MapLocation rr : rc.senseNearbyRuins(-1)) { if (rc.canSenseRobotAtLocation(rr) || ruinBanned(rr)) continue; int key = (rr.x << 6) | rr.y; boolean known = false; for (int i = remN; --i >= 0; ) if (remRuin[i] == key) { known = true; break; } if (!known && remN < 32) remRuin[remN++] = key; }
        for (int i = remN; --i >= 0; ) { MapLocation rl = new MapLocation(remRuin[i] >> 6, remRuin[i] & 63); if (rc.canSenseLocation(rl) && rc.canSenseRobotAtLocation(rl)) remRuin[i] = remRuin[--remN]; }
