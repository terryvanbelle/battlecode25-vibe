...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla78";
    ...
    static String runMopper() throws GameActionException {
        ...
        // darla78: moppers are the ONLY unit that can transfer paint robot->robot, and this bot has never used it. Iteration 2 established that paint SUPPLY is what binds: a soldier at or below REFILL_LOW abandons its work and commutes. So hand paint to such a soldier if we are above REFILL_LOW ourselves. Placed after mop and swing so it only ever spends an action the mopper was not going to use.
        for (RobotInfo a : rc.senseNearbyRobots(2, rc.getTeam())) {
            if (a.type.isTowerType() || a.paintAmount > REFILL_LOW) continue;
            int give = Math.min(rc.getPaint() - REFILL_LOW, a.type.paintCapacity - a.paintAmount);
            if (give > 0 && rc.canTransferPaint(a.location, give)) { rc.transferPaint(a.location, give); state += " give"; break; }
        }
        moveExploring(null);
        return state;
    }
