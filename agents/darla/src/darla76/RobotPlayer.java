...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla76";
    ...
    static void rememberTowers() throws GameActionException {
        ...
        for (RobotInfo t : rc.senseNearbyRobots(-1, rc.getTeam())) {
            ...
            int key = (l.x << 6) | l.y | (t.type.getBaseType() == UnitType.LEVEL_ONE_PAINT_TOWER ? 4096 : 0); // darla76: remember WHICH towers can give paint
            boolean known = false;
    ...
    static MapLocation nearestRememberedTower() {
        ...
        for (int i = towerN; --i >= 0; ) {
            MapLocation l = new MapLocation((towerMem[i] >> 6) & 63, towerMem[i] & 63);
            int d = me.distanceSquaredTo(l); if ((towerMem[i] & 4096) != 0) d -= 100000; // darla76: a paint tower beats any money tower; distance still orders within a class
