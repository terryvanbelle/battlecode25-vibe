...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "darla93";
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " rt=" + refillTrips + " ht=" + homeTurns + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak + " mv=" + mvStuck + "/" + mvTry + " rw=" + rwTurns + "/" + rwCrowd
            + " | " + state);
        Clock.yield();
    }
    ...
    // darla93: INSTRUMENTATION ONLY. The i3 census leaves `S ruin` at ~20% of soldier
    // turns as the last unexamined block, and this bot has NO coordination at all --
    // statics are per-robot and the messaging API is unused -- so nothing stops several
    // soldiers converging on the same ruin. Only soldiers can call workOnRuin, so every
    // redundant one is a ruin elsewhere not being claimed, which is the coverage race the
    // win condition was established on. rwTurns counts ruin-work turns; rwCrowd counts
    // those where another ally ROBOT is already within r2<=8 of the same ruin. If crowding
    // is rare this closes without an arm; if it is common it is the largest unexploited
    // coordination gap left, and it is opponent-independent.
    static int rwTurns = 0, rwCrowd = 0;
    ...
    static String runSoldier() throws GameActionException {
        ...
        if (ruin != null) {
            state += " ruin=" + ruin;
            workOnRuin(ruin); { rwTurns++; for (RobotInfo a : rc.senseNearbyRobots(-1, rc.getTeam())) { if (!a.type.isTowerType() && a.ID != rc.getID() && a.location.distanceSquaredTo(ruin) <= 8) { rwCrowd++; break; } } }
        }
