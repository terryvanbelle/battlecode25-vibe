...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i33";

    // ---- Iteration 33: un-gate frontier-seeking on IDLE-ENEMY turns ------------------------
    // Iteration 14 added frontier-seeking and gated it `if (foe == 0)`. Iteration 32's traces
    // measured, ON THIS BUILD, what that gate costs: IDLE-ENEMY is 1,424 of 1,444 idle turns on
    // DefaultLarge, 7,099 of 10,579 on gridworld, 1,980 of 8,577 on Parking_lot. So iteration
    // 14's mechanism is switched OFF on the single largest block of idle turns there is.
    static int ideTurns = 0;   // IDLE-ENEMY turns reaching the branch (the whole budget)
    static int ideFound = 0;   // ... where nearestVisibleEmpty() != null -> the redirect fires
    static int ideNone  = 0;   // ... where nothing empty is visible either
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " IT=" + ideTurns + " IF=" + ideFound + " IN=" + ideNone
            + " | " + state);
        Clock.yield();
    }
        ...
        if (rc.isActionReady() && rc.getPaint() >= UnitType.SOLDIER.attackCost) {
            ...
            if (myTile.getPaint() == PaintType.EMPTY && rc.canAttack(me)) {
                ...
                if (best != null) { rc.attack(best); state += " pnt"; }
                    ...
                    // Iteration 33: the `foe == 0` gate is GONE. Reaching this line means
                    // there is nothing paintable in the ACTION radius (r2=9) -- and when foe>0
                    // that is a CAPABILITY wall, not a navigation one: a soldier physically
                    // cannot overwrite enemy paint (RULES.md: soldier attack "paints the tile
                    // ONLY if empty or already own-team paint"). So the soldier stands beside
                    // ground it can never convert while `newExploreTarget()` sends it to a
                    // RANDOM map coordinate. Vision is r2=20, more than twice the action area,
                    // so there is usually genuinely EMPTY ground in sight instead.
                    //
                    // Zero marginal cost, which is the shape iteration 14 was accepted on: this
                    // spends no paint and no action, it only redirects a move that was already
                    // going to happen. The gate was never measured -- it came in with iteration
                    // 14 as part of the ally/foe census that DIAGNOSED the two idle kinds, and
                    // the diagnosis was then reused as a policy without being tested as one.
                    MapLocation f = nearestVisibleEmpty();
                    if (foe > 0) {
                        ideTurns++;
                        if (f != null) ideFound++; else ideNone++;
                    }
