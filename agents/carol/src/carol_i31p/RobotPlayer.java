...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i31p";
    ...
    static String runTower() throws GameActionException {
        ...
        // PROBE (iteration 31): tower paint read at the DECISION point, BEFORE the build.
        // The shipping indicator reads rc.getPaint() in the RETURN statement, i.e. AFTER
        // buildRobot has already spent the stash -- post-spend state, doctrine 15. Pure read.
        int tpIn = rc.getPaint();
        ...
        boolean cbr = false, built = false;
        if (afford) {
            Direction dir = DIRS[rng.nextInt(8)];
            MapLocation loc = rc.getLocation().add(dir);
            cbr = rc.canBuildRobot(want, loc);
            if (cbr) { rc.buildRobot(want, loc); built = true; }
        }
        // Team-level econ trace (towers see chips + tower count; paint is per-tower).
        return "TD tpIn=" + tpIn + " want=" + want + " cost=" + want.paintCost
             + " afford=" + (afford ? 1 : 0) + " cbr=" + (cbr ? 1 : 0) + " built=" + (built ? 1 : 0)
             + " | T r=" + rc.getRoundNum() + " chips=" + chips + " tw=" + rc.getNumberTowers()
