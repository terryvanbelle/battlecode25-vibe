...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i51a";
    ...
    // ---- Iteration 50: SRP by MARK-AND-WALK-AWAY ---------------------------------------------
    // Iteration 49 built SRPs by PARKING a soldier on the pattern until it was done. That lost
    // 7/50 (-36, -4.2 sd), with the damage monotone in how many legal SRP centres a map has and
    // EXACTLY ZERO on the two maps with none -- so the mechanism was the cause, and the cost was
    // the parking, not the pattern. Coverage collapsed 701 -> 110 while tower paint piled up
    // unspent at 9701: soldiers standing still stop expanding, and coverage is the win condition.
    //
    // So: mark a centre and KEEP WALKING. Never commit a soldier to a pattern.
    //
    // The 25 marks persist as ordinary map state. Any soldier that later passes within action
    // range sees marked tiles whose paint mismatches and fixes them -- the same mark-driven idiom
    // workOnRuin already uses for tower patterns -- and whoever is standing there when the team
    // can pay completes it. The mark IS the shared to-do list, written into the map and read by
    // whoever arrives, which is coordination with no communication (reference/RESEARCH.md 11).
    //
    // Price against what it displaces: 25 paint for the mark, and NO forgone movement -- which was
    // the entire cost that killed iteration 49. Painting a pattern tile is not a new cost either:
    // ALLY_SECONDARY is ally paint (PaintType.isAlly(), disassembled), so a pattern tile scores as
    // area exactly like any other tile a soldier would have painted.
    //
    // SRP_MIN_CHIPS is the dose; Integer.MAX_VALUE disables the mechanism, making the zero arm
    // byte-identical to carol_iter44 (the iteration-49 zero arm verified byte-identical twice).
    static final int SRP_MIN_CHIPS = 1500;
    // 9 candidate centres at spacing 2, all inside the engine's asserted r2=8 mark radius.
    static final int[] SRP_DX = { 0,  2, -2,  0,  0,  2,  2, -2, -2 };
    static final int[] SRP_DY = { 0,  0,  0,  2, -2,  2, -2,  2, -2 };
    static int srpAsk = 0, srpMark = 0, srpDone = 0, srpPaint = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " sA=" + srpAsk + " sM=" + srpMark + " sD=" + srpDone + " sP=" + srpPaint
            + " | " + state);
        Clock.yield();
    }
    ...
    static String runSoldier() throws GameActionException {
        ...
        // Iteration 50: SRP, mark-and-walk-away. Ruins keep absolute priority; unlike iteration
        // 49 there is NO soldier state and NO movement suppression, so nothing here can detain a
        // unit -- the failure mode that cost 36 games is structurally absent rather than tuned.
        boolean srpActed = (ruin == null) && srpWork();
        ...
        if (!srpActed && rc.isActionReady() && rc.getPaint() >= UnitType.SOLDIER.attackCost) {
    ...
    /**
     * SRP work, stateless and non-committal. Three independent things, in priority order, at most
     * one action: complete any finished pattern in reach, fix one mismatched pattern tile in
     * reach, or mark a new centre. The soldier holds NO state about a pattern and is never
     * detained by one -- that is the whole difference from iteration 49.
     *
    ...
    static boolean srpWork() throws GameActionException {
        ...
        //    emergent half: a soldier that walks past someone else's marks finishes them.
        //
        //    ITERATION 51 -- the chips gate now sits BELOW this loop, not above it. Painting a
        //    marked pattern tile costs 5 PAINT and ZERO CHIPS: it delivers work already paid for,
        //    exactly like the completion loop above. SRP_MIN_CHIPS exists to decide whether it is
        //    worth STARTING a pattern, and starting is step 3 alone.
        //
        //    This is the THIRD instance of the same bug shape in this file (49: the release
        //    timeout under the held-state guard; 50: the completion loop under the entry gate),
        //    and the first two were already written up in LEARNINGS before I found this one --
        //    which is the finding. Fixing the completion branch did not make me re-examine the
        //    OTHER branches under the same gate, so the doctrine was applied to the instance that
        //    prompted it and no further.
        //
        //    Measured motivation, not aesthetics: at the gate, an entire game passed only ~50
        //    soldier-turns into this function (sA=10 marks-attempts + sP=40 paints), while the 4
        //    marks laid demanded 4 x 25 = 100 correct tiles. Supply 40 vs demand 100 is why sD
        //    was 0. Below the gate this loop can run on EVERY soldier-turn, and it costs nothing
        //    where no marks exist -- so the downside is bounded by how many patterns step 3
        //    actually started.
        for (MapInfo t : rc.senseNearbyMapInfos(9)) {
            PaintType mk = t.getMark();
            if (mk == PaintType.EMPTY || mk == t.getPaint()) continue;
            if (t.getPaint().isEnemy()) continue;          // iteration 29: unfixable by a soldier
        ...
        // 3. Lay a new pattern, then walk on. Marking is not an action (it charges 25 paint), so
        //    this does not consume the turn's attack; the soldier paints and moves as usual.
        //    STARTING is the only thing SRP_MIN_CHIPS may gate, so the gate lives here now.
