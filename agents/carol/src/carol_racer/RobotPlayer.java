...
/* SYNTHETIC ARCHETYPE -- "the racer". FROZEN: never edit this file.
 *
 * Provenance: this is iteration 47's candidate (carol_iter44 with SPLASH_FLOOR
 * lowered 2000 -> 1400), which was REJECTED as a bot -- it lost its sampled
 * screen at -8 and drew the dense census at 0. It is kept as an OPPONENT, which
 * is a different job and one it is uniquely qualified for.
 *
 * Why it exists. Every opponent this lineage owns descends from a build that
 * grinds: carol wins the round-2000 coverage tiebreak and loses the race to the
 * >70% paint condition. The tournament says that is exactly what beats us --
 * on maps with >=24 ruins carol goes 8-45 in games decided by the paint
 * condition and 7-8 in tiebreaks, and alice and bob win 60-68% there against
 * carol's 22%. Nothing in the pool posed that threat, so no gate could measure
 * a defence against it (doctrine 7 and 17).
 *
 * It qualifies on the criterion registered BEFORE it was promoted: it closes
 * dense maps by the 70% condition -- 32 of 34 against a fixed opponent, and 13
 * of the 18 dense games anyone finished against carol_iter44, which managed 5.
 * It scores 42% against carol_iter44 overall, inside the 30-90% peer band and
 * far from saturation, so it is a live rung and not a dead one.
 *
 * It makes the pool HARDER, not more INDEPENDENT: it shares every blind spot
 * this lineage has. It is not a substitute for the tournament.
 */
