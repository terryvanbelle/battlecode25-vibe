...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i45r";
    ...
    /** PROBE ONLY (iteration 45r): drain rank of a destination tile. 0 = ally (drain 0),
     *  1 = neutral (-1), 2 = enemy (-2). Read-only. */
    ...
    /** PROBE ONLY: record what stepToward chose and whether a strictly cheaper legal
     *  alternative existed among the directions it already considers. Appends to stepDbg;
     *  changes nothing about which move is made. */
    static String stepDbg = "";
    static void probeStep(MapLocation from, Direction chosen, Direction[] cands)
        ...
        stepDbg += " st" + ck + (best < ck ? "alt" + best : "");
    }
    ...
    static boolean stepToward(MapLocation to) throws GameActionException {
        ...
        if (rc.canMove(d)) { probeStep(from0, d, cands0); rc.move(d); return true; }
        ...
        if ((rc.getID() & 1) == 0) {
            if (rc.canMove(l)) { probeStep(from0, l, cands0); rc.move(l); return true; }
            if (rc.canMove(r)) { probeStep(from0, r, cands0); rc.move(r); return true; }
        } else {
            if (rc.canMove(r)) { probeStep(from0, r, cands0); rc.move(r); return true; }
            if (rc.canMove(l)) { probeStep(from0, l, cands0); rc.move(l); return true; }
        }
        Direction l2 = l.rotateLeft(), r2 = r.rotateRight();
        if (rc.canMove(l2)) { probeStep(from0, l2, cands0); rc.move(l2); return true; }
        if (rc.canMove(r2)) { probeStep(from0, r2, cands0); rc.move(r2); return true; }
        return false;
    }
}
