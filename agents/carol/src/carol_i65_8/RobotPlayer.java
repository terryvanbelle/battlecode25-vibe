...
public class RobotPlayer {
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " cw=" + crowdSum + "/" + crowdTurns + " rt=" + refillTrips + " ht=" + homeTurns + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak
            + " | " + state);
        Clock.yield();
    }
    ...
    // ---- ITERATION 65: de-clumping movement ------------------------------------------
    // Traced deficit (free, from a replay on disk): tripling the splasher population with D3
    // cost 19.5% of per-splash value -- mean splash score 20.5 (carol_iter44, 4 splashers) ->
    // 16.5 (carol_iter45, 17-22), measured on BOTH teams in the SAME game and round window.
    // Map saturation is refuted by sign: carol_iter44 painted MORE of that map and scored
    // HIGHER, where saturation predicts the reverse.
    //
    // A splasher's attack is an r^2<=4 AoE, so two splashers standing together OVERLAP and the
    // overlap is pure waste -- a payer bob's soldier-primary architecture cannot have, since a
    // soldier paints one tile and cannot overlap another soldier. (Direction taken from
    // agents/bob/CLOSED.md #25/#27, which he closed AT THE BAR, not refuted: +6/+5/+6
    // wins_above_half out of 50 across three arms, never negative, crowd/unit-round -26..-48%.
    // In my units those are 31/50. A verdict does not transfer -- this needs my own gate.)
    //
    // Movement ranks legal directions by distance to target (ranks 0..n) and adds
    // CROWD_W * (allies within r^2<=2 of the destination). 0 disables it entirely: the
    // greedy path below is then byte-identical to carol_iter45.
    static final int CROWD_W = 8;
    static long crowdSum = 0;      // allies adjacent to a splasher, summed over splasher turns
    static int crowdTurns = 0;
    ...
    static String runSplasher() throws GameActionException {
        {   // instrumentation only -- counts allies adjacent to this splasher this turn
            int c = 0;
            for (RobotInfo a : rc.senseNearbyRobots(2, rc.getTeam())) {
                if (!a.type.isTowerType()) c++;
            }
            crowdSum += c; crowdTurns++;
        }
        refillIfPossible();
    ...
    /** Rank legal moves by closeness to the target, then penalise crowding at the destination. */
    static boolean stepTowardDeclumped(MapLocation to) throws GameActionException {
        MapLocation me = rc.getLocation();
        if (me.equals(to)) return false;
        RobotInfo[] allies = rc.senseNearbyRobots(-1, rc.getTeam());
        int n = 0;
        Direction[] legal = new Direction[8];
        int[] dist = new int[8];
        int[] crowd = new int[8];
        for (int i = 0; i < 8; i++) {
            Direction d = DIRS[i];
            if (!rc.canMove(d)) continue;
            MapLocation dest = me.add(d);
            int c = 0;
            for (int j = allies.length; --j >= 0; ) {
                RobotInfo a = allies[j];
                if (a.type.isTowerType()) continue;
                if (a.getLocation().distanceSquaredTo(dest) <= 2) c++;
            }
            legal[n] = d; dist[n] = dest.distanceSquaredTo(to); crowd[n] = c; n++;
        }
        if (n == 0) return false;
        Direction best = null;
        int bestScore = Integer.MAX_VALUE;
        for (int i = 0; i < n; i++) {
            int rank = 0;                          // how many legal moves are strictly closer
            for (int j = 0; j < n; j++) if (dist[j] < dist[i]) rank++;
            int score = rank + CROWD_W * crowd[i];
    ...
    static boolean stepToward(MapLocation to) throws GameActionException {
        if (CROWD_W > 0) return stepTowardDeclumped(to);
