...
public class RobotPlayer {
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        String dtag = "";
        if (rc.getType().isRobotType() && towerN > 0) {
            MapLocation meD = rc.getLocation();
            int bestD = Integer.MAX_VALUE;
            for (int i = towerN; --i >= 0; ) {
                int dx = meD.x - (towerMem[i] >> 6), dy = meD.y - (towerMem[i] & 63);
                int d = dx * dx + dy * dy;
                if (d < bestD) bestD = d;
            }
            dtag = " D=" + bestD;
        }
        rc.setIndicatorString("[" + BUILD + "] " + dtag + " bc=" + used + "/" + limit + " max=" + bcMaxUsed
