...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i37_res";
    ...
    static String runTower() throws GameActionException {
        ...
        // ---- Iteration 37: the splasher floor is measured against the wrong number, and it
        // has become an OFF SWITCH FOR THE SOLDIER -- the fourth gate-as-off-switch in this
        // lineage, and the first one I built myself (iteration 30).
        //
        // The arithmetic, stated exactly. reserve is CHIP_RESERVE=1200, or 0 when `freed`.
        //   splasher passes at  chips >= reserve + 400                    = 1600  (400 when freed)
        //   soldier  passes at  chips >= max(reserve + 250, 2000 + 250)   = 2250  (ALWAYS)
        // The 2000 is an absolute constant that does not track `reserve`, so the `freed` escape
        // hatch -- which exists to UNBLOCK spending -- drops the splasher gate to 400 and leaves
        // the soldier's at 2250, widening the gap from 650 chips to 1850.
        //
        // Measured from replays at zero VM cost (carol-tools/mixcheck/trajectory.py, 9 games):
        // the treasury sawtooths between ~1200 and ~1750 for the whole game and NEVER reaches
        // 2250, because splashers at 400 chips drain it as fast as it fills. In 7 of 9 games
        // carol builds ZERO soldiers from round ~600 to round 2000 while building a metronomic
        // 13-16 splashers per 200 rounds, and the tower count freezes at 4-5.
        //
        // Why that closes a ratchet rather than just skewing a mix: workOnRuin is called only
        // from runSoldier, so SOLDIERS ARE THE ONLY UNIT THAT CLAIMS RUINS. No soldiers means no
        // new towers, which means no new income, which keeps the treasury under 2250, which
        // means no soldiers. Pooled over 22 games the realized mix is 44% soldier / 55% splasher
        // against an intended 75/10/15 -- iteration 30 overshot its target by 3.7x.
        //
        // External corroboration (tournament, the only opponents this lineage did not produce):
        // on maps >= 2500 tiles carol reaches the paint threshold in 2 of 64 games; on maps
        // <= 900 tiles it does so in 40 of 76. The coverage engine does not scale, and the
        // engine is towers.
        //
        // THE CHANGE, one mechanism: stop comparing against a magic 2000 and compare against
        // the price of the thing the floor exists to protect. Iteration 30's intent -- do not
        // let cheaper units drain the treasury below a splasher -- is preserved exactly; only
        // the number it is measured against changes, and it now tracks `reserve` for free.
        boolean afford = chips >= reserve + want.moneyCost;
        if (afford && want != UnitType.SPLASHER
                && chips - want.moneyCost < reserve + UnitType.SPLASHER.moneyCost) {
            afford = false;                      // KEEP A SPLASHER IN RESERVE: soldier at 1850
        }
