...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i45bq";
    ...
    /** Iteration 45 dose. */
    static final int ACCEPT_RANK = 1;
    static final boolean FALLBACK_ONLY = false;
    static int drainSaves = 0;   // decision counter: drain-aware step taken
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak + " ds=" + drainSaves
            + " | " + state);
        Clock.yield();
    }
    ...
    static String runSoldier() throws GameActionException {
        ...
        // MANIPULATION CHECK (iteration 45). Identical instrumentation to carol_i45q, so the
        // underfoot distribution of the CANDIDATE is comparable cell-for-cell with the
        // BASELINE's on the same matchup and maps. Reads only.
        {
            MapLocation p0 = rc.getLocation();
            PaintType u = rc.senseMapInfo(p0).getPaint();
            int aA = 0, aN = 0, aX = 0;
            for (Direction d : DIRS) {
                MapLocation l = p0.add(d);
                if (!rc.canSenseLocation(l)) continue;
                MapInfo t = rc.senseMapInfo(l);
                if (!t.isPassable()) continue;
                PaintType q = t.getPaint();
                if (q.isAlly()) aA++; else if (q.isEnemy()) aX++; else aN++;
            }
            state += " u" + (u.isAlly() ? "A" : u.isEnemy() ? "X" : "N")
                   + " nb" + aA + "/" + aN + "/" + aX;
        }
    ...
    static void moveExploring(MapLocation target) throws GameActionException {
        ...
        if (stepTowardDrainAware(explore)) return;
    ...
    /**
     * Iteration 45 — DRAIN-AWARE STEPPING.
     *
     * RULES.md [E], end-of-turn paint drain: **enemy tile -2, neutral -1, ally 0**, and the
     * clumping term doubles on enemy territory. Paint is this lineage's binding resource
     * (100.0% of chips-available no-builds are paint-limited), and nothing in this bot has ever
     * consulted the tile it is stepping ONTO for that reason.
     *
     * Measured on the accepted build (probe carol_i45r/s, three maps, identity-checked):
     * 13.5%-26.9% of stepToward moves land on enemy paint while a legal non-enemy destination
     * existed among the directions this method ALREADY considers. 98.3% of those moves are
     * toward the EXPLORE target, which newExploreTarget() draws as the farthest of four
     * UNIFORMLY RANDOM map coordinates -- so a one-step deviation there costs progress toward a
     * heading that carries no information. That is the whole price argument, and it is read off
     * the code rather than assumed.
     *
     * Play-symmetry: candidates are visited in the incumbent's exact order (straight, then
     * left/right ordered by robot ID, then the two wider rotations), and a tie in drain rank
     * keeps that order untouched. The rule "prefer my own paint" is symmetric between teams.
     */
    static boolean cheaperEnough(MapLocation dest, int rank) throws GameActionException {
        PaintType q = rc.senseMapInfo(dest).getPaint();
        int k = q.isAlly() ? 0 : q.isEnemy() ? 2 : 1;
        return k <= rank;
    }

    /** Incumbent candidate order: straight, ID-ordered left/right, then the wider rotations. */
    static Direction[] stepOrder(Direction d) {
        Direction l = d.rotateLeft(), r = d.rotateRight();
        boolean even = (rc.getID() & 1) == 0;
        return new Direction[] { d, even ? l : r, even ? r : l, l.rotateLeft(), r.rotateRight() };
    }

    /**
     * Drain-aware variant. ACCEPT_RANK selects the dose:
     *   1 -> take the first legal destination that is not ENEMY paint (ally or neutral)
     *   0 -> take the first legal ALLY tile, else the first legal non-enemy, else unchanged
     * FALLBACK_ONLY restricts the whole mechanism to turns where the straight direction is
     * already blocked, which has no deviation cost at all.
     */
    static boolean stepTowardDrainAware(MapLocation to) throws GameActionException {
        MapLocation me = rc.getLocation();
        Direction d = me.directionTo(to);
        if (d == Direction.CENTER) return false;
        if (FALLBACK_ONLY && rc.canMove(d)) { rc.move(d); return true; }
        Direction[] order = stepOrder(d);
        Direction incumbent = null;
        for (Direction c : order) {
            if (c == null || !rc.canMove(c)) continue;
            if (incumbent == null) incumbent = c;          // what the baseline would have taken
            if (cheaperEnough(me.add(c), ACCEPT_RANK)) {
                if (c != incumbent) drainSaves++;   // count only a genuine DEVIATION
                rc.move(c); return true;
            }
        }
        if (ACCEPT_RANK == 0) {                            // second pass: settle for non-enemy
            for (Direction c : order) {
                if (c == null || !rc.canMove(c)) continue;
                if (cheaperEnough(me.add(c), 1)) {
                    if (c != incumbent) drainSaves++;
                    rc.move(c); return true;
                }
            }
        }
        if (incumbent != null) { rc.move(incumbent); return true; }
        return false;
    }
