...
public class RobotPlayer {
    ...
    /* Iteration 54 instrumentation. pRolls = build rolls chips would have allowed; pFloor =
     * those the paint floor refused; pFloorBig = refusals attributable to the NEW scope
     * (soldiers and splashers). The DOSE is pFloorBig/pRolls -- a SHARE, because iteration 52
    ...
    static String runTower() throws GameActionException {
        ...
        // ---- Iteration 54 arms. FLOOR_ALL_TYPES is the whole manipulation.
        // false = iteration 36's original scope (cheaper-than-a-soldier only) = the EXACT
        // zero arm; true = the floor applies to every unit type.
        final boolean FLOOR_ALL_TYPES = true;
        if (afford) {
            pRolls++;                       // rolls that CHIPS would have allowed
            // ---- DOSE CALIBRATION (iteration 54, after i54_a realized 88.8%% and froze the
            // build). PAINT_FLOOR=200 against a soldier's 200 paint cost demands 400 in the
            // tower, which the trace says a tower almost never holds: ~310 each in the healthy
            // phase, ~68 each in the spiral. So the floor did not discriminate between the two
            // regimes, it simply refused nearly everything. BIG_FLOOR is a SEPARATE, smaller
            // floor for the expensive units, chosen to sit between those two observed levels.
            final int BIG_FLOOR = 100;
