...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i15c";
    ...
    static void newExploreTarget() {
        ...
        MapLocation best = null;
        int bestD = -1;
        // Iteration 15c: the ORIGINAL sampler -- 4 uniform draws over the map, keep the
        // FARTHEST -- with the single change that a draw landing on an already-visited cell
        // is re-rolled (up to 6 tries). Iteration 15b bundled two changes: "random -> memory"
        // AND "farthest -> nearest". It lost 11/40, so this isolates the first of them and
        // leaves the distance policy exactly as the accepted baseline had it. When most cells
        // are visited the re-roll budget runs out and this degenerates to the old behaviour,
        // which is the correct limit.
        for (int i = 0; i < 4; i++) {
            MapLocation c = null;
            for (int t = 0; t < 6; t++) {
                c = new MapLocation(rng.nextInt(w), rng.nextInt(h));
                if (memSeen[memIdx(c)] == 0) { expNew++; break; }
                if (t == 5) expOld++;
            }
