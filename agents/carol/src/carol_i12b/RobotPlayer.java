...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i12b";
    ...
    /**
     * Iteration 12b. Iteration 12 put an upgrade behind a chip gate and the gate was never
     * reachable: UPG fired ONCE in 11,016 tower-turns, because runTower drains the treasury to
     * ~1,450 on every spawn, so chips only ever accumulate when spawning is already blocked by
     * zero tower paint -- and on those maps carol builds no towers, so almost none of hers are
     * upgradable paint towers. Measured: gridworld 92.5% chip-rich but 2.1% upgradable-paint;
     * DefaultMedium 68.4% upgradable-paint but 0.0% chip-rich. Conjunction ~0 everywhere.
     *
     * The fix is priority, not a smaller number: an upgradable paint tower WITHHOLDS SPAWNING
     * until it has saved CHIP_RESERVE + the next level's cost. That trades unit production for
     * permanent paint income, which is the right trade on evidence already in hand --
     * iterations 5, 8 and 10 raised production by three different routes (iteration 10 fielded
    ...
    static String runTower() throws GameActionException {
        ...
        // ---- Iteration 12b: upgrade this paint tower, saving for it if need be. ----
        String upg = "";
        boolean saving = false;
        if (rc.getType().getBaseType() == UnitType.LEVEL_ONE_PAINT_TOWER
                && rc.getType().canUpgradeType()) {
            // Gate computed per level, never a constant: a fixed CHIP_RESERVE+2500 would let a
            // lv2->lv3 upgrade (5,000) strand the treasury below the ruin-completion reserve,
            // which is how iteration 6 lost DefaultSmall by annihilation at round 69.
        ...
        UnitType want = (roll < SPLASHER_IN_20) ? UnitType.SPLASHER
                      ...
                      : UnitType.SOLDIER;
        if (!saving && chips >= reserve + want.moneyCost) {
