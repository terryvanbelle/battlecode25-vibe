...
public class RobotPlayer {
    ...
    static String runTower() throws GameActionException {
        ...
        // ---- ITERATION 62: the depot circuit-breaker -------------------------------------
        // The floor above applies ONLY to units cheaper than a soldier -- i.e. moppers -- so a
        // tower holding 200 paint will spend all of it on a soldier and keep nothing for the
        // refill traffic that iteration 60's D3 created. Measured on Mirage (the corpus-median
        // map, r2000): carol_iter45 holds 380 tower paint across 9 towers (42 each) against
        // carol_iter44's 1,000, builds 219 soldiers per 1,000 rounds of which 207 STARVE,
        // fields ZERO splashers because no tower can reach a splasher's 300, and sits on
        // $80,850 of unspendable chips while its paint acts collapse 2,596 -> 38.
        //
        // The circuit is: spawn -> drain the tower -> units walk home to a dry tower -> starve
        // -> spawn again. ALL_FLOOR is the breaker, and it is SELF-REGULATING rather than
        // tuned: a too-large standing army drains the depot, and a drained depot stops
        // spawning. This is carol_r1's TOWER_PAINT_KEEP, which was the fix for exactly this
        // failure in the rewrite; iteration 60 ported D3's walk-home half and not this half.
        // 0 disables the branch entirely (zero arm, byte-identical play).
        final int ALL_FLOOR = 450;
        if (afford && ALL_FLOOR > 0 && rc.getPaint() - want.paintCost < ALL_FLOOR) {
            afford = false;
        }
        if (afford) {
