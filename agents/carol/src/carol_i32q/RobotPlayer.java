...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i32q";

    // ---- PROBE q (iteration 32): TILE-LOCAL lattice-SRP reachability. Pure reads ------------
    //
    // Probe p returned sOK=0 on all four maps and its pre-registered kill condition fired. But
    // the kill is not clean, and saying so is the point of this second probe: probe p tested a
    // condition WIDER than the mechanism needs, and my own iteration 26a wrote the rule it
    // broke -- "check that the counter's condition is the same width as the decision the bot
    // would actually get to make". Two guards in p are provably too wide:
    //
    //  1. FULL-CELL SENSING. p required all 25 tiles of the cell to be sensable. Vision is
    //     r2=20 and a cell corner sits at (4,4)=r2 32 from the opposite corner, so only 13 of
    //     the 25 standing positions inside a cell can see the whole cell. ~48% of turns failed
    //     on geometry, not on the map. And p folded that into the SAME counter (sD) as real
    //     dirt, so p cannot say how much of Parking_lot's 96% sD was ever real.
    //  2. THE RUIN GUARD. p excluded any cell overlapping ANY visible ruin. But RULES.md says
    //     "Tower survives even if its pattern is later painted over" -- so a ruin that ALREADY
    //     HAS A TOWER no longer needs its pattern preserved and is not a conflict at all. On
    //     gridworld that guard alone took 44 of 44 turns, and on DefaultLarge 62 of 67.
    //
    // So probe q measures the decision the SHIPPING mechanism would make, which is tile-local
    // and needs no cell validation at all: repaint one ally tile that is the wrong colour for
    // its own lattice cell. Correctness is not my problem to prove -- the ENGINE validates,
    // free, via canCompleteResourcePattern, which is why the whole-cell check can go.
    static final int SRP_MASK = 28873275;
    static int srpTurns = 0;   // idle soldier turns reaching the probe (the whole budget)
    static int srpAct   = 0;   // ... with >=1 actionable wrong-coloured tile -> REACHABILITY
    static int srpWork  = 0;   // total actionable tiles seen (how much work is available)
    static int srpRuinT = 0;   // tiles vetoed by an UNBUILT ruin's pattern (the real conflict)
    static int srpRuinB = 0;   // tiles p would ALSO have vetoed for a BUILT ruin (p's overreach)
    static int srpEdgeT = 0;   // tiles whose lattice cell runs off the map
    static int srpEmp   = 0;   // in-range tiles that are EMPTY (wall/ruin/unpainted)
    static int srpEnm   = 0;   // in-range tiles holding ENEMY paint
    static int srpCC    = 0;   // turns canCompleteResourcePattern was already true (free money)
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " ST=" + srpTurns + " sA=" + srpAct + " sW=" + srpWork
            + " sRT=" + srpRuinT + " sRB=" + srpRuinB + " sET=" + srpEdgeT
            + " sEm=" + srpEmp + " sEn=" + srpEnm + " sCC=" + srpCC + " | " + state);
        Clock.yield();
    }
        ...
        if (rc.isActionReady() && rc.getPaint() >= UnitType.SOLDIER.attackCost) {
            ...
            if (myTile.getPaint() == PaintType.EMPTY && rc.canAttack(me)) {
                ...
                if (best != null) { rc.attack(best); state += " pnt"; }
                    ...
                    // ---- PROBE q (iteration 32) --------------------------------------------
                    // Same firing condition as p: action ready, >=5 paint, nothing EMPTY to
                    // paint in action range. Pure reads; no attack, no move, no rng draw.
                    srpTurns++;
                    ...
                    for (MapInfo t : rc.senseNearbyMapInfos(9)) {
                        MapLocation l = t.getMapLocation();
                        PaintType p = t.getPaint();
                        if (p.isEnemy()) { srpEnm++; continue; }
                        if (!p.isAlly()) { srpEmp++; continue; }   // empty, wall or ruin tile
                        // The tile's OWN lattice cell -- a pure function of the tile, so every
                        // soldier that ever sees it agrees, with nothing communicated. Same
                        // device towerTypeFor() uses for ruins ("a pure function of the ruin so
                        // that every soldier agrees every turn").
                        int cx = (l.x / 5) * 5 + 2, cy = (l.y / 5) * 5 + 2;
                        if (cx + 2 >= mw || cy + 2 >= mh) { srpEdgeT++; continue; }
                        boolean vetoUnbuilt = false, vetoBuilt = false;
                        for (MapLocation r : ruins) {
                            if (Math.abs(r.x - l.x) > 2 || Math.abs(r.y - l.y) > 2) continue;
                            if (rc.canSenseRobotAtLocation(r)) vetoBuilt = true;   // tower up: pattern is spent
                            else { vetoUnbuilt = true; break; }
                        }
                        if (vetoUnbuilt) { srpRuinT++; continue; }
                        boolean sec = ((SRP_MASK >>> (5 * (l.x - cx + 2) + (l.y - cy + 2))) & 1) != 0;
                        if ((p == PaintType.ALLY_SECONDARY) == sec) continue;      // already correct
                        if (vetoBuilt) srpRuinB++;                                 // p would have lost this one
                        if (!rc.canAttack(l)) continue;
                        act++;
                    }
                    if (act > 0) { srpAct++; srpWork += act; }
                    // Free-money check: is any lattice centre in reach already pattern-exact?
                    int qcx = (me.x / 5) * 5 + 2, qcy = (me.y / 5) * 5 + 2;
                    if (qcx + 2 < mw && qcy + 2 < mh
                            && rc.canCompleteResourcePattern(new MapLocation(qcx, qcy))) srpCC++;
                    // ---- end probe q -------------------------------------------------------
