...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i29q";
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " id=" + rc.getID()
            + " drT=" + drT + " drO=" + drO + " drP=" + drP
            + " drA=" + drA + " drN=" + drN + " drE=" + drE
            + " drC=" + drC + " drCe=" + drCe
            + " drX=" + drX + " drD=" + drD + " drH=" + drH
            + " | " + state);
        Clock.yield();
    }
    ...
    // ---- i29 PROBE: exact end-of-turn paint-drain decomposition (NO-OP; reads only) --------
    // The offline replay accounting could not be closed: measured drain is 3.2-3.5 paint/turn
    // with a heavy tail at 5,6,7,9,11, while the rules cap terrain at 2/turn (-1 neutral,
    // -2 enemy, 0 ally) and clumping at -1 per adjacent ally (-2 per ally on enemy ground).
    // Odd values of 7/9/11 need 6/8/10 adjacent allies on ally-or-neutral ground, and 10 is
    // impossible (8 neighbours). Enemy TARGETED attacks were measured to explain only
    // 0.8-4.6% of total drain, so theft is refuted. Something in my model of the drain rule
    // is therefore wrong, and only the robot itself can read the tile and the neighbours.
    //
    // Method: read paint at the TOP of the turn (before refill or any action) and compare it
    // with the paint recorded at the END of the previous turn. That difference is EXACTLY the
    // engine's end-of-turn drain, with no confound from our own spending, which happens
    // during the turn. At the end of the turn record the tile underfoot and the adjacent-ally
    // count, which is what the drain about to be applied will be computed from.
    //   drT  turns contributing a measured drain
    //   drO  observed drain, summed        drP  drain PREDICTED by my model of the rules
    //   drA/drN/drE  end-of-turn tile was ally / neutral / enemy paint
    //   drC  adjacent allies summed (all turns)     drCe adjacent allies while on enemy paint
    //   drX  turns where observed != predicted      drD  signed residual (observed - predicted)
    //   drH  largest single observed drain
    static int drT=0, drO=0, drP=0, drA=0, drN=0, drE=0, drC=0, drCe=0, drX=0, drD=0, drH=0;
    static int drPrevPaint = -1;      // paint recorded at the end of our previous turn
    static int drPrevPred  = 0;       // drain my model predicted for that end-of-turn

    /** Top of turn: the only place the engine's applied drain is observable. */
    static void drainObserve() {
        if (drPrevPaint >= 0) {
            int obs = drPrevPaint - rc.getPaint();
            if (obs >= 0 && drPrevPaint > 0) {   // skip the clamp-at-zero turns, which floor the loss
                drT++; drO += obs; drP += drPrevPred;
                if (obs != drPrevPred) { drX++; drD += obs - drPrevPred; }
                if (obs > drH) drH = obs;
            }
        }
    }

    /** End of turn: record the state the engine is about to charge us for. */
    static void drainRecord() throws GameActionException {
        MapLocation me = rc.getLocation();
        PaintType p = rc.senseMapInfo(me).getPaint();
        int terrain;
        boolean onEnemy = p.isEnemy();
        if (p.isAlly())      { drA++; terrain = 0; }
        else if (onEnemy)    { drE++; terrain = 2; }
        else                 { drN++; terrain = 1; }
        int adj = rc.senseNearbyRobots(2, rc.getTeam()).length;
        drC += adj;
        if (onEnemy) drCe += adj;
        drPrevPred  = terrain + adj * (onEnemy ? 2 : 1);
        drPrevPaint = rc.getPaint();
    }
