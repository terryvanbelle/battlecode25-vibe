...
public class RobotPlayer {
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " vis=" + visPaintable + " rch=" + reachPaintable + " it=" + idleTurns + " rt=" + refillTrips + " ht=" + homeTurns + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak
            + " | " + state);
        Clock.yield();
    }
    ...
    static long visPaintable = 0, reachPaintable = 0, idleTurns = 0, idleVisPaintable = 0;

    static String runSplasher() throws GameActionException {
        {   // PROBE ONLY -- measurement, no behaviour change
            ...
            for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
                ...
                if (p == PaintType.EMPTY || p.isEnemy()) {
                    vis++;
                    if (me0.distanceSquaredTo(t.getMapLocation()) <= 8) reach++;
                }
            }
            visPaintable += vis; reachPaintable += reach;
        }
        refillIfPossible();
        ...
        if (ready && fueled) {
            ...
            idleTurns++; idleVisPaintable += visPaintable;
            }
        } else {
