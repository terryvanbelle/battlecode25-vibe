...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i38";

    /** Iteration 38 dose: return to a tower to refill below this % of paint
     *  capacity. 0 = incumbent behaviour (never return). */
    static final int RETURN_PCT = 50;
    static int homeAsk = 0, homeGo = 0;   // instrument the DECISION, not the outcome
    ...
    /**
     * Iteration 38: nearest remembered ally tower, or null.
     *
     * `seenLoc` has stored every distinct ally tower's (x<<6)|y since iteration 34 and its
     * COORDINATES have never been read by anything -- only the paint/money counts derived from
     * it. The memory this needs already exists and is already maintained every turn by
     * censusTowers(), so this costs no new sensing.
     */
    static MapLocation nearestKnownTower() {
        MapLocation me = rc.getLocation();
        MapLocation best = null;
        int bestD = Integer.MAX_VALUE;
        for (int i = seenN; --i >= 0; ) {
            int k = seenLoc[i];
            MapLocation t = new MapLocation(k >> 6, k & 63);
            int d = me.distanceSquaredTo(t);
            if (d < bestD) { bestD = d; best = t; }
        }
        // Already within transfer range and STILL low means refillIfPossible just failed this
        // turn: the tower is dry, or it has been destroyed since we saw it and seenLoc is
        // stale. Homing to it again would deadlock the unit on the spot, so fall through to
        // exploring. This is the only failure mode of a memory that is never invalidated.
        if (best != null && bestD <= 2) return null;
        return best;
    }
    ...
    static void moveExploring(MapLocation target) throws GameActionException {
        ...
        // ---- Iteration 38: RETURN TO REFILL -------------------------------------------
        // 88% of all carol unit deaths are paint starvation, not combat (3,905 of 4,450 over
        // 16 cached replays); the engine makes robot-vs-robot HP damage impossible, so paint
        // is the whole attrition mechanic. A unit that runs dry cannot move or act at all and
        // loses 20 HP/turn until it dies, and BEFORE that, below 50% paint every cooldown is
        // raised by (100 - 2*X)% -- so a low-paint unit is already crawling at up to half
        // speed while it paints nothing.
        //
        // 50 is not a tuned constant: it is the engine's own cooldown-penalty knee, and it is
        // also exactly where refillIfPossible is already willing to act. Triggering there
        // means the walk home happens at full speed instead of at the penalised rate.
        //
        // Refill range is r2<=2 for EVERY unit type -- assertCanTransferPaint calls
        // assertCanActLocation(loc, 2) with a hardcoded 2, NOT the unit's action radius
        // (disassembled this iteration; RULES.md updated). So a unit must physically walk to
        // touch a tower; it can never top up in passing. That is why memory, not radius, is
        // the lever.
        //
        // MOVEMENT ONLY. No action logic changes, so a unit still paints whatever it can
        // reach on the way home and the mechanism under test is exactly one thing.
        if (RETURN_PCT > 0 && rc.getPaint() * 100 < rc.getType().paintCapacity * RETURN_PCT) {
            homeAsk++;
            MapLocation home = nearestKnownTower();
            if (home != null) { homeGo++; target = home; }
        }
