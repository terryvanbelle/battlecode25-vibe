...
public class RobotPlayer {
    ...
    // ---- ITERATION 72: obstacle tracing (bug navigation) ------------------------------
    // reference/RESEARCH.md section 5 calls this "the most consistent engineering story across
    // every year": teams start with a greedy/textbook step, discover it fails, and converge on
    // greedy + a bounded escape for the shapes greedy movement actually gets stuck on.
    // carol has NEVER had one. stepToward tries the direct direction then +-1 and +-2 and gives
    // up -- it re-decides which way to skirt every turn, so it cannot circumnavigate anything.
    //
    // Simulated on real terrain from tournament replays, running carol's EXACT step policy from
    // her own towers:
    //     ruins carol CLAIMED on galaxy   : 8/8   = 100% greedy-reachable
    //     ruins carol NEVER REACHED       : 6 of 18 are NOT reachable -- the sim LOOPS forever
    //     share of passable tiles reachable: Mirage 89.3%, galaxy 79.3%, Gears 80.3%
    // So 10-20% of the map is unreachable by carol's navigation, and reachability is worse on
    // the larger maps.
    //
    // The sharpest consequence is in my own accepted D3 code: walkHomeIfDry() calls stepToward
    // DIRECTLY and has no stuck-escape of any kind (moveExploring at least re-targets after
    // stuckTurns >= 6). A splasher whose tower sits behind a concave obstacle therefore loops
    // until it starves -- which is consistent with the measured 15-48 turn refill trips and the
    // 35-59% HOME share.
    //
    // Mechanism: go direct when possible; when blocked, commit to ONE rotation direction and
    // keep it across turns until the unit gets strictly closer to the target than it has ever
    // been (the progress reset that escapes concave traps), with a time-box as the safety valve.
    // The side is chosen by robot ID, never by team, for play-symmetry.
    // TRACE_MAX = 0 disables it and restores the incumbent path exactly (zero arm).
    static final int TRACE_MAX = 100;
    static int traceRot = 0;
    static int traceBest = Integer.MAX_VALUE;
    static int traceTurns = 0;
    static MapLocation traceTarget = null;
    ...
    /** Greedy step with obstacle tracing and a progress reset (RESEARCH.md section 5). */
    static boolean stepTracing(MapLocation to) throws GameActionException {
        MapLocation me = rc.getLocation();
        if (!to.equals(traceTarget)) {          // new target: forget the old trace entirely
            traceTarget = to; traceRot = 0; traceBest = Integer.MAX_VALUE; traceTurns = 0;
        }
        int d2 = me.distanceSquaredTo(to);
        if (d2 < traceBest) {                   // strictly closer than ever -> obstacle cleared
            traceBest = d2; traceRot = 0; traceTurns = 0;
        }
        Direction d = me.directionTo(to);
        if (d == Direction.CENTER) return false;
        if (traceRot == 0) {
            if (rc.canMove(d)) { rc.move(d); return true; }
            traceRot = ((rc.getID() & 1) == 0) ? 1 : -1;   // pick a side by ID, not by team
        }
        if (++traceTurns > TRACE_MAX) {         // safety valve: abandon the trace
            traceRot = 0; traceBest = Integer.MAX_VALUE; traceTurns = 0;
            return false;
        }
        int di = dirIndex(d);
        if (di < 0) return false;
        for (int i = 0; i < 8; i++) {
            Direction t = DIRS[((di + traceRot * i) % 8 + 8) % 8];
            if (rc.canMove(t)) { rc.move(t); return true; }
        }
        return false;
    }

    static boolean stepToward(MapLocation to) throws GameActionException {
        if (TRACE_MAX > 0) return stepTracing(to);
