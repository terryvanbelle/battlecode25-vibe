package carol_denier2;

/* SYNTHETIC ARCHETYPE -- "the denier", v2. CANDIDATE, not yet promoted.
 *
 * Provenance: carol_iter44 (the accepted, tournament-shipping snapshot) with ONE
 * mechanism changed -- the two paint-removal units NAVIGATE toward enemy paint
 * instead of exploring. Nothing else differs; diff it against src/carol_iter44
 * and the whole change is nearestVisibleEnemyPaint() plus its two call sites and
 * the dn= counter.
 *
 * Why it exists (iteration 52). My mopper screen returned a dead-centre null at a
 * 13.4% mopper share, and the free replay measurement showed why the null may be
 * uninformative rather than true: moppers are the only unit that REMOVES enemy
 * paint, so they pay exactly insofar as the opponent takes paint off me -- and
 * every opponent in my pool descends from a lineage that builds 0% moppers and
 * whose splashers wander. NO OPPONENT I OWN CONTESTS PAINT. A defensive mechanism
 * cannot be priced on an instrument that never loads it.
 *
 * The mechanism, and why it is the whole change. carol_iter44 already builds an
 * overwhelmingly splasher army (48 splashers / 4 soldiers / 0 moppers on a
 * DefaultMedium census) and its splasher SCORING already prefers enemy paint --
 * 3 points per enemy tile inside the r2<=2 convertible core against 2 for empty.
 * What it never does is GO there: both runSplasher and runMopper end in
 * moveExploring(null), so the denial capacity is built and then walked away from.
 * The archetype supplies the missing half and nothing else.
 *
 * It makes the pool HARDER on one specific axis; it does not make it more
 * INDEPENDENT. It shares every blind spot this lineage has, and iteration 35
 * already established that a yardstick I construct cannot be independent of me.
 * Its job is to stop the gate being BLIND to paint denial, not to manufacture a
 * pass for a mechanism I like. The tournament remains the arbiter.
 *
 * ---- v2, after v1 FAILED its pre-registered criterion 1 ----
 *
 * v1 hunted the NEAREST enemy tile and made things worse, not better: carol_iter44's
 * peak coverage ROSE from 387 to 687 per-mille on DefaultMedium against it. The
 * instrumented tags say why, and there were two causes, only one of which I predicted.
 *
 *  - PREDICTED: hunting parks units on enemy paint, where drain is -2/turn. Median
 *    splasher paint fell 139 -> 53 against a 50-paint attack cost, and 49.4% of
 *    hunting splasher-turns could not afford to fire at all (roaming: 38.3%).
 *  - NOT PREDICTED, and the bigger error: "lowScore" tripled, 8.6% -> 25.5%.
 *    NEAREST IS NOT DENSEST. Walking to the closest enemy tile parks a splasher
 *    beside ONE tile on a thin frontier edge, and SPLASH_MIN_SCORE=8 needs roughly
 *    three convertible tiles in the r2<=2 core. v1 optimised the distance to enemy
 *    paint and thereby de-optimised the thing the shot is actually scored on.
 *
 * Net: hunting splashers fired on 4.9% of their turns against roaming ones' 10.2%.
 * The archetype did LESS denial than the bot it was built from -- 137 splashes to the
 * control's 229. It did not deny and starve; it mostly failed to deny.
 *
 * v2 repairs exactly those two, and nothing else:
 *  1. target the DENSEST cluster of enemy paint in vision, not the nearest tile,
 *     via an O(E) 2x2 bucket count (no per-tile engine calls, so it is cheap).
 *  2. a unit that cannot afford its attack walks to an ally tower to refill instead
 *     of hunting. A starving unit in enemy territory poses no threat, which is the
 *     one thing an archetype exists to do.
 */
...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i44a";
    ...
    /**
     * Enemy paint tiles this robot has actually converted or removed, summed over its life.
     * This is the REALIZED DOSE of the quantity the archetype is about -- iteration 52's
     * lesson: instrument the value your hypothesis names, not the number of times a code
     * path ran. A mop is exactly one tile; a splash converts exactly the enemy tiles inside
     * r2<=2 of its centre, which is counted at fire time from the same scan that chose it.
     * Ground truth for denial is still the VICTIM's coverage curve, not this counter.
     */
    static int denied = 0;
    ...
    static String runMopper() throws GameActionException {
        ...
        // THE ARCHETYPE. carol_iter44 explores here; the denier walks at the nearest enemy
        // paint it can see. Null falls through to the identical exploration, so behaviour is
        // unchanged wherever there is nothing to deny.
        // v2: a mopper mops for 0 paint, but it still pays -4/turn standing on enemy paint,
        // so it starves too. Refill takes priority over the hunt.
        MapLocation hunt = null; String nav = "";
        if (rc.getPaint() * 4 < rc.getType().paintCapacity) { hunt = refillTarget(); nav = " refill"; }
        if (hunt == null) { hunt = nearestVisibleEnemyPaint(); nav = hunt != null ? " hunt" : ""; }
        moveExploring(hunt);
        return state + nav + " dn=" + denied;
    }
    ...
    static String runSplasher() throws GameActionException {
        ...
        if (ready && fueled) {
            ...
            if (best != null && bestScore >= SPLASH_MIN_SCORE) {
                // Count the enemy tiles this shot actually converts, BEFORE firing: only
                // r2<=2 of the centre is overwritable [E: RULES.md splasher attack], which is
                // the same disc the scoring uses, so this is an exact count and not an
                // estimate. Cheap -- 13 tiles, once per fire, and a splasher fires at most
                // once every 5 turns.
                for (MapInfo t : rc.senseNearbyMapInfos(best, 2)) {
                    if (t.getPaint().isEnemy()) denied++;
                }
                rc.attack(best);
                tag = " SPLASH";
            } else {
        ...
        // THE ARCHETYPE (see runMopper). The splasher is the unit that matters here: it is
        // the only one that converts enemy paint in BULK, and iter44 already prefers such
        // centres when one happens to be in range. This makes it seek them out.
        // v2: cannot afford a shot -> go and fix that, do not walk deeper into enemy paint
        // at -2/turn. This is the direct repair of v1's 49.4% unable-to-fire rate.
        MapLocation hunt = null; String nav = "";
        if (rc.getPaint() < UnitType.SPLASHER.attackCost) { hunt = refillTarget(); nav = " refill"; }
        if (hunt == null) { hunt = nearestVisibleEnemyPaint(); nav = hunt != null ? " hunt" : ""; }
        moveExploring(hunt);
        ...
        return "P" + tag + nav + " s=" + bestScore
                + " p=" + rc.getPaint() + " dn=" + denied;
    }
    ...
    /**
     * Nearest ENEMY-painted passable tile anywhere in vision (r2=20), or null. The mirror of
     * nearestVisibleEmpty, and deliberately the whole vision disc rather than the action
     * radius: the action radius is the region a denial unit has already dealt with, and the
     * point of the archetype is to close the distance to paint it cannot yet reach.
     */
    static MapLocation nearestVisibleEnemyPaint() throws GameActionException {
        MapLocation me = rc.getLocation();

        // v2: DENSEST, not nearest. Vision is r2<=20, so dx,dy are in [-4,4]; bucket the
        // disc into 2x2 cells (a 5x5 grid) and count enemy tiles per cell in ONE pass over
        // one sense call. O(E) with no per-tile engine calls, which is what makes this
        // affordable -- scoring each candidate centre with its own senseNearbyMapInfos
        // would be ~69 engine calls a turn and is why the obvious version is unaffordable.
        int[] count = new int[25];
        MapLocation[] rep = new MapLocation[25];
        for (MapInfo t : rc.senseNearbyMapInfos(-1)) {
            if (!t.getPaint().isEnemy() || !t.isPassable()) continue;
            MapLocation l = t.getMapLocation();
            int bx = (l.x - me.x + 4) >> 1, by = (l.y - me.y + 4) >> 1;
            if (bx < 0) bx = 0; else if (bx > 4) bx = 4;
            if (by < 0) by = 0; else if (by > 4) by = 4;
            int i = by * 5 + bx;
            count[i]++;
            // Keep the member CLOSEST to us as the cell's representative, so that having
            // chosen a dense cell we still approach it by its near edge.
            if (rep[i] == null || me.distanceSquaredTo(l) < me.distanceSquaredTo(rep[i])) rep[i] = l;
        }
        int bestI = -1, bestC = 0;
        for (int i = 0; i < 25; i++) {
            if (count[i] > bestC) { bestC = count[i]; bestI = i; }
        }
        return bestI < 0 ? null : rep[bestI];
    }

    /**
     * v2: an ally tower within vision to refill at, or null. A unit that cannot afford its
     * attack is not a denier, so it goes and fixes that first. transferPaint needs r2<=2
     * [E: RULES.md] -- it must physically walk to touch the tower, which is exactly why
     * this has to be a NAVIGATION target and not an opportunistic top-up.
     */
    static MapLocation refillTarget() throws GameActionException {
        MapLocation me = rc.getLocation();
        MapLocation best = null;
        int bestD = Integer.MAX_VALUE;
        for (RobotInfo r : rc.senseNearbyRobots(-1, rc.getTeam())) {
            if (!r.type.isTowerType() || r.paintAmount <= 0) continue;
            int d = me.distanceSquaredTo(r.location);
            if (d < bestD) { bestD = d; best = r.location; }
        }
        return best;
    }
