...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i45q";
    ...
    static String runSoldier() throws GameActionException {
        ...
        // PROBE ONLY (iteration 45q). Records the tile UNDERFOOT and the adjacent choice set on
        // EVERY soldier turn. Motivation: RULES.md [E] end-of-turn paint drain is -2 on an enemy
        // tile, -1 on neutral, 0 on ally -- and nothing in this bot has ever consulted the tile it
        // is standing on for that reason. IDLE-ENEMY counts enemy tiles in r2=9, which is NOT the
        // same referent as the tile underfoot, so the drain claim cannot be read off it.
        // Reads only; no state is assigned, so play is unchanged and the (winner, rounds)
        // identity check must pass against the baseline.
        {
