...
public class RobotPlayer {
    ...
    // ---- Iteration 27a: instrument the TARGET CHOICE, at the full width of the options ------
    // Pre-check 4, and it is the one that has caught this lineage three times. LEARNINGS: a
    // decision counter narrower than its decision is a refutation generator -- i26a's srpOk
    // tested ONE of ~25 legal pattern centres. So this counts every candidate the bot could
    // have picked, not the one it did.
    //
    // What pre-check 3 changed about what to measure: the hypothesis began as "IDLE soldiers
    // wander" (frontNone, 72-88% of idle turns), but the plateau measurement says carol's
    // ACTIVE soldiers convert paint at 12.7% of face value after ~r300 while growth-phase
    // actions convert at 82.1%. So the interesting decision is not only where an idle soldier
    // walks -- it is which tile EVERY painting soldier picks, all game.
    //
    // Contestedness proxy: enemy-painted tiles among a candidate's 8 neighbours. A tile with
    // enemy paint beside it is on the front where paint does not stay; a tile with none is
    // ground nobody is taking back. Counters are cumulative per robot and emitted every turn:
    //   pcN    paint decisions made (the chosen-tile branch actually fired)
    //   pcCand total candidates that were available across those decisions (the WIDTH)
    //   pcHot  decisions where the CHOSEN tile had >=1 enemy neighbour
    //   pcBest decisions where a strictly LESS contested candidate existed and was not chosen
    static int pcN = 0, pcCand = 0, pcHot = 0, pcBest = 0;
    // 9x9 vision window, enemy-paint flags, rebuilt once per painting turn and shared by every
    // candidate evaluation so contestedness costs array lookups rather than sense calls.
    static final boolean[] foeGrid = new boolean[121];   // 11x11 covers r2=20 (dx,dy in -5..5)

    static int foeIdx(int dx, int dy) { return (dx + 5) * 11 + (dy + 5); }

    /** Enemy-painted neighbours (8-dir) of loc, from the prebuilt window. Off-window = 0. */
     ...
     */
    static final String BUILD = "i27a";
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " pcN=" + pcN + " pcC=" + pcCand + " pcH=" + pcHot + " pcB=" + pcBest
            + " | " + state);
        Clock.yield();
    }
    ...
    static String runSoldier() throws GameActionException {
        ...
        if (rc.isActionReady() && rc.getPaint() >= UnitType.SOLDIER.attackCost) {
            ...
            if (myTile.getPaint() == PaintType.EMPTY && rc.canAttack(me)) {
                ...
                // ---- 27a probe: read-only, runs AFTER `best` is fixed so it cannot alter it.
                if (best != null) {
                    ...
                    int chosen = contestedness(me, best), minC = chosen, cand = 0;
                    for (MapInfo t : rc.senseNearbyMapInfos(9)) {
                        if (t.getPaint() != PaintType.EMPTY || !t.isPassable()) continue;
                        MapLocation l = t.getMapLocation();
                        if (!rc.canAttack(l)) continue;
                        cand++;
                        int c = contestedness(me, l);
                        if (c < minC) minC = c;
                    }
                    pcN++; pcCand += cand;
                    if (chosen > 0) pcHot++;
                    if (minC < chosen) pcBest++;
                }
