...
public class RobotPlayer {
    ...
    // ---- ITERATION 71: give carol the capability it has NEVER had -- removing enemy paint --
    // Traced and replicated on two large maps and against two different opponents:
    //     Gears  vs alice: alice 2,024 unpaints/1,000 rounds, carol 0
    //     galaxy vs alice: alice 1,470,                       carol 0
    //     Gears  vs bobf : bobf    828 unpaints,              carol 0
    // carol's unpaint count is 0 in EVERY game ever measured in this lineage. The opponent
    // erases roughly 70% of carol's gross production and carol erases nothing.
    //
    // Denial is worth the same as painting for the outcome: the first tiebreak compares the two
    // teams' painted areas [E, RULES.md], so removing one of theirs moves the same quantity as
    // adding one of mine -- and it also defends against their outright >70% win.
    //
    // A MOPPER'S ATTACK COSTS 0 PAINT [E]. Only the unit is paid for, which is why alice's
    // measured mopping runs at 0.47 tiles per paint against ~0.20 for anything carol does.
    //
    // Why carol builds none: the inherited gates block moppers twice over -- SPLASH_FLOOR wants
    // chips >= 1200+300+2000 = 3,500 AND PAINT_FLOOR blocks any unit cheaper than a soldier
    // unless the tower keeps 200 paint. carol's towers hold ~60. So the roll asks for a mopper
    // 10% of the time and it is essentially never affordable.
    //
    // The scheduler (iteration 69's machinery, which demonstrably forces a unit into the mix
    // regardless of affordability thresholds) forces a mopper after MOPPER_EVERY other builds
    // and applies ONLY the base legal requirement -- because the inherited floors are precisely
    // what has kept the count at zero. 0 disables it (zero arm, byte-identical play).
    //
    // STANDING PRIOR TO BEAT: agents/bob/CLOSED.md #17 closes "raise mopper spawn share" at
    // -5/-6/-11 out of 50, dose-monotone harm, with the re-open condition "a mechanism exists
    // that raises mopper share WITHOUT reducing total paint actions". That clause is exactly
    // what my stage-0 check tests head-on.
    static final int MOPPER_EVERY = 16;
    static int buildsSinceMopper = 0;
    ...
    static String runTower() throws GameActionException {
        ...
        // Iteration 71: force a mopper into the mix, past the floors that have kept it at zero.
        if (MOPPER_EVERY > 0 && want != UnitType.MOPPER
                && buildsSinceMopper >= MOPPER_EVERY) {
            want = UnitType.MOPPER;
            afford = chips >= reserve + UnitType.MOPPER.moneyCost
        ...
        if (afford) {
            ...
            if (rc.canBuildRobot(want, loc)) {
                ...
                if (want == UnitType.MOPPER) buildsSinceMopper = 0;
                else buildsSinceMopper++;
            }
        }
