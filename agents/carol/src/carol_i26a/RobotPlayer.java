...
public class RobotPlayer {
    ...
    // ---- Iteration 26a: SRP reachability probe. INSTRUMENTATION ONLY, no behaviour change. ----
    // SRPs are the one scored mechanic carol has never implemented. Engine constants, read from
    // the jar rather than from my digest: COMPLETE_RESOURCE_PATTERN_COST = 200 chips,
    // MARK_PATTERN_PAINT_COST = 25 paint, EXTRA_RESOURCES_FROM_PATTERN = 3,
    // RESOURCE_PATTERN_ACTIVE_DELAY = 50, RESOURCE_PATTERN_RADIUS_SQUARED = 8, PATTERN_SIZE = 5.
    // The bonus is gated inside `if (type.paintPerTurn != 0) addPaint(paintPerTurn + 3*numSRPs)`
    // so it lands on PAINT towers only, +3 per SRP per paint tower per turn.
    //
    // Before building anything I count the DECISION, because that pre-check has now caught this
    // lineage three times in one session (i20 ferry, i24a, i24b). The question is not "is an SRP
    // worth it" -- the arithmetic says overwhelmingly yes -- but "can a carol soldier ever
    // actually lay one". Two candidate blockers, and the probe separates them:
    ...
    // carol's soldiers are chronically paint-starved (75-80% of her deaths are paint deaths),
    // so `srpPoor` being the bulk would mean the mechanism is unreachable for the same reason
    // the bot needs it -- which would be a genuine finding, not a disappointment.
    static int srpTurns = 0, srpPoor = 0, srpOk = 0;
     ...
     */
    static final String BUILD = "i26a";
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " st=" + srpTurns + " sPoor=" + srpPoor + " sOk=" + srpOk + " | " + state);
        Clock.yield();
    }
    ...
    static String runSoldier() throws GameActionException {
        ...
        // 26a probe: read-only, takes no action and changes no state the bot reads.
        srpTurns++;
        if (rc.getPaint() < GameConstants.MARK_PATTERN_PAINT_COST) srpPoor++;
        else if (rc.canMarkResourcePattern(rc.getLocation())) srpOk++;
