...
public class RobotPlayer {
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " sym=" + (symOK[0]?"R":"-") + (symOK[1]?"H":"-") + (symOK[2]?"V":"-") + " pin=" + symPinned + "/" + symTurns + " inf=" + inferUsed + " rn=" + ruinN + " rt=" + refillTrips + " ht=" + homeTurns + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak
            + " | " + state);
        Clock.yield();
    }
    ...
    // ---- ITERATION 75: symmetry inference, unblocked by a RUIN memory ------------------
    // Iteration 43 built symmetry inference and measured it BLOCKED, not wrong: refutation
    // was correct (no false kill of the truth) but needed a tile and its mirror visible
    // SIMULTANEOUSLY, so all three candidates stayed alive for 1,321 of 1,525 turns. Its
    // named prerequisite was "a terrain memory so refutation does not require simultaneous
    // visibility". This is that memory in its cheapest form: remember RUIN CENTRES only.
    //
    // Ruins are the ideal witness, measured on galaxy's 28 ruins:
    //     rot180 maps ruin->ruin 28/28 = 100%   |   both reflections 0/28 = 0%
    // so a handful of remembered ruins identifies the transform outright.
    //
    // Sized against the deficit (galaxy vs alice, 28 ruins): carol ever senses 13, marks 11,
    // claims 8. Of the 17 ruins carol never marks, **11 (65%) have a mirror carol ALREADY
    // marked** -- so inference hands carol 11 targets it never sensed. At carol's own measured
    // marked->claimed rate of 73% that is ~+8 sites, ~+6 paint towers, ~+30 paint/turn on a
    // base of 55 = +55% paint income.
    //
    // Why bob's closure does not transfer: agents/bob/CLOSED.md #32 closed this BY SIZING at
    // 31.5% mirror coverage and 0.53 ruins/game against a 2.72-ruin gap. Mine is 65% and 11
    // ruins against a 20-ruin gap -- roughly 20x the absolute prize, on a different
    // architecture. His verdict does not transfer; his sizing method does, and I used it.
    static final int INFER_IN_20 = 10;
    static final int RUIN_MEM = 32;
    static int[] ruinMem = new int[RUIN_MEM];
    static int ruinN = 0;
    static boolean[] symOK = { true, true, true };   // 0 ROTATIONAL, 1 HORIZONTAL, 2 VERTICAL
    static int symRefutes = 0, symPinned = 0, symTurns = 0, inferUsed = 0;

    static MapLocation mirrorOf(int sym, MapLocation p) {
        int W = rc.getMapWidth(), H = rc.getMapHeight();
        if (sym == 0) return new MapLocation(W - 1 - p.x, H - 1 - p.y);
        if (sym == 1) return new MapLocation(p.x, H - 1 - p.y);
        return new MapLocation(W - 1 - p.x, p.y);
    }

    static boolean ruinKnown(int key) {
        for (int i = ruinN; --i >= 0; ) if (ruinMem[i] == key) return true;
        return false;
    }

    /** Remember ruin centres, then refute symmetries against the memory (no simultaneity). */
    static void rememberRuins() throws GameActionException {
        if (!rc.getType().isRobotType()) return;
        MapLocation[] now = rc.senseNearbyRuins(-1);
        for (MapLocation r : now) {
            int key = (r.x << 6) | r.y;
            if (!ruinKnown(key) && ruinN < RUIN_MEM) ruinMem[ruinN++] = key;
        }
        MapLocation me = rc.getLocation();
        for (int sym = 0; sym < 3; sym++) {
            if (!symOK[sym]) continue;
            for (int i = ruinN; --i >= 0; ) {
                MapLocation a = new MapLocation(ruinMem[i] >> 6, ruinMem[i] & 63);
                MapLocation m = mirrorOf(sym, a);
                if (!rc.onTheMap(m)) { symOK[sym] = false; symRefutes++; break; }
                if (me.distanceSquaredTo(m) > 20) continue;   // must be in vision NOW
                boolean isRuin = false;
                for (MapLocation r : now) if (r.equals(m)) { isRuin = true; break; }
                if (!isRuin) { symOK[sym] = false; symRefutes++; break; }
            }
        }
        int alive = 0;
        for (int i = 0; i < 3; i++) if (symOK[i]) alive++;
        if (alive == 0) { symOK[0] = symOK[1] = symOK[2] = true; }   // cannot happen; fail safe
        symTurns++;
        if (alive == 1) symPinned++;
    }

    /** Nearest mirror of a remembered ruin that carol has never sensed. Pinned symmetry only. */
    static MapLocation inferredTarget() throws GameActionException {
        int sym = -1, alive = 0;
        for (int i = 0; i < 3; i++) if (symOK[i]) { sym = i; alive++; }
        ...
        for (int i = ruinN; --i >= 0; ) {
            ...
            if (ruinKnown((m.x << 6) | m.y)) continue;      // already sensed: nothing to infer
    ...
    static void newExploreTarget() throws GameActionException {
        if (INFER_IN_20 > 0 && rng.nextInt(20) < INFER_IN_20) {
            MapLocation t = inferredTarget();
            if (t != null) { explore = t; exploreAge = 0; inferUsed++; return; }
        }
        MapLocation me = rc.getLocation();
        int w = rc.getMapWidth(), h = rc.getMapHeight();
        MapLocation best = null;
