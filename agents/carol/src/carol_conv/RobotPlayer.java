...
/* SYNTHETIC ARCHETYPE -- "the converter". FROZEN: never edit this file.
 * Forked from src/carol at carol_iter44 (commit 5be82ca). Sole difference: SPLASH_FLOOR 2000 -> 0.
 * Purpose: an opponent that converts ruins into towers, so that ruin-dense maps can
 * discriminate. carol's self-play gate is measured blind to ruin count (rho=+0.009 over 5,768
 * games); this archetype exists to give that gate sight. Re-fork it if carol's build gate
 * changes shape, or it goes stale and silently inflates win rates. */
...
public class RobotPlayer {
    ...
    static String runTower() throws GameActionException {
        ...
        // SYNTHETIC ARCHETYPE "the converter": SPLASH_FLOOR is the off-switch that holds
        // carol's realized mix at ~95% splasher / ~5% soldier. Soldiers are the ONLY unit that
        // calls workOnRuin, so with the off-switch in place carol builds 2-3 soldiers per game
        // and converts ~8 of 52 ruins on a ruin-dense map. Setting it to 0 is the whole of the
        // archetype: it does the one thing this lineage never does, which is what an archetype
        // is for. Everything else is byte-identical to carol_iter44.
        final int SPLASH_FLOOR = 0;
