...
public class RobotPlayer {
    ...
    static boolean walkHomeIfDry(int cap) throws GameActionException {
        ...
        if (!refilling) { refilling = true; refillTrips++; homeStart = turnCount; }
        // Iteration 70: give up on a trip that has run too long and go back to work.
        if (MAX_HOME > 0 && turnCount - homeStart >= MAX_HOME) { refilling = false; return false; }
    ...
    // ---- ITERATION 70: time-box the refill trip -------------------------------------
    // D3 (iteration 60) latches a unit into REFILL below REFILL_LOW and unlatches only at
    // capacity/2 -- 150 paint for a splasher. Measured: carol's towers hold ~42 paint each on
    // Mirage, so that exit condition is often UNREACHABLE, and a splasher tours several towers
    // per trip instead of returning to work. Smoking gun from the replay:
    //     id13357  rt=3 ht=45  ... | P HOME p=43
    // three trips, 45 turns spent walking, and still at 43 paint. Measured trip length is
    // 15-48 turns against a work cycle of 25 turns per 5 splashes.
    //
    // Aggregate cost, and it scales with map area exactly as the deficit does:
    //     HOME share of splasher turns = 35.5% on Mirage (1,600) and 58.8% on Leaf (3,600).
    // The lineage's largest measured deficit is the area gradient (26.6% vs alice on maps
    // >1600), and this is the only large-map-specific quantity I have measured.
    //
    // A time-box bounds the trip regardless of WHY filling failed. 0 disables it entirely
    // (zero arm, byte-identical play).
    static final int MAX_HOME = 32;
    static int homeStart = 0;
