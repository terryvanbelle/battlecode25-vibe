...
public class RobotPlayer {
    ...
    static boolean walkHomeIfDry(int cap) throws GameActionException {
        // ---- carol_r2 CHANGE 2 of 3: NO RECALL -------------------------------------------
        // D3's dedicated refill trip is what leashes this bot: p90 unit-to-own-tower distance
        // is 6.3 tiles against the rival's 17 under an IDENTICAL r^2<=2 transferPaint constraint,
        // and HOME is 35-59% of splasher turns. Opportunistic top-up (refillIfPossible) is kept,
        // so a unit that passes a tower still fills. What is removed is the journey.
        // Standalone this loses 26 games (it is iteration 60's zero arm); it is only viable
        // paired with CHANGE 1, which replaces the units it strands.
        if (true) return false;
        if (REFILL_LOW <= 0) return false;
    ...
    static String runTower() throws GameActionException {
        ...
        // ---- carol_r2 CHANGE 1 of 3: PRODUCTION -- splasher-first ------------------------
        // Measured: carol builds 36 splashers/1,000r against an income ceiling of 125 (29%).
        // The cap is not income; it is the roll (15% splasher) plus SPLASH_FLOOR (which gates a
        // 250-chip soldier at 2,250 while exempting a 400-chip splasher at 1,600) and PAINT_FLOOR.
        // Here a tower buys the unit that paints whenever it can afford one, and spends on a
        // soldier only paint that could not have bought a splasher.
        int roll = rng.nextInt(20);
        UnitType want = null;
        int tp = rc.getPaint();
        if (tp >= UnitType.SPLASHER.paintCost && chips >= reserve + UnitType.SPLASHER.moneyCost) {
            want = UnitType.SPLASHER;
        } else if (tp >= UnitType.SOLDIER.paintCost && chips >= reserve + UnitType.SOLDIER.moneyCost) {
            want = UnitType.SOLDIER;
        } else if (rc.getType().paintPerTurn == 0
                   && tp >= UnitType.MOPPER.paintCost
    ...
    /** carol_r2 CHANGE 3 of 3: FORWARD POSTURE. Measured: carol sees 49% of map tiles to the
     *  rival's 90%, and spends 10-41% of unit-turns on enemy ground to the rival's 85-88%.
     *  Targets are drawn from the half of the map FARTHER from this robot's spawn anchor --
     *  units spawn at towers and towers sit in carol's half, so "away from spawn" is "forward"
     *  without needing symmetry inference (whose link 1 fails at 37-41%) or any comms. */
    static MapLocation anchor = null;

    static void newExploreTarget() {
        MapLocation me = rc.getLocation();
        if (anchor == null) anchor = me;
        int w = rc.getMapWidth(), h = rc.getMapHeight();
        MapLocation best = null;
        int bestD = -1;
        for (int i = 0; i < 4; i++) {
            MapLocation c = new MapLocation(rng.nextInt(w), rng.nextInt(h));
            if (c.distanceSquaredTo(anchor) < me.distanceSquaredTo(anchor)) continue;  // not forward
            int d = c.distanceSquaredTo(anchor);
            if (d > bestD) { bestD = d; best = c; }
        }
        if (best == null) best = new MapLocation(rng.nextInt(w), rng.nextInt(h));
        explore = best;
        exploreAge = 0;
    }
