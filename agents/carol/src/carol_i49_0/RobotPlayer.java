...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i490";
    ...
    // ---- Iteration 49: SRP (special resource pattern) ---------------------------------------
    // The API sweep at iteration 48 found the ENTIRE resource-pattern mechanic unused: zero calls
    // in every accepted snapshot from iter0 to iter44. It was selected as iteration 7, deferred
    // that same session for the CHIP_RESERVE dead-band fix, and never picked back up.
    //
    // Why it is the best purchase in the game, from my own disassembly of processBeginningOfRound:
    //     if (paintPerTurn != 0) addPaint(paintPerTurn + 3*numSRPs);      // per PAINT tower
    //     if (moneyPerTurn != 0) addMoney(moneyPerTurn + 3*numSRPs);      // per MONEY tower
    // One SRP is 200 chips for +3/turn to EVERY tower, forever. A money tower is 1000 chips for
    // +20/turn (50-round payback); the lv2 paint upgrade is 2500 for +5 (500-round payback).
    //
    // And iteration 48 established the thing that makes this the RIGHT conversion: paint IS the
    // score (instant win at >70% painted; the round-2000 tiebreak reads area painted FIRST), and
    // chips are not. Iteration 48 failed by converting chips into more chips. An SRP is the only
    // mechanism in the game that converts chips into PAINT INCOME.
    //
    // The pattern's own 25 tiles are painted in ALLY_PRIMARY/ALLY_SECONDARY, and PaintType.isAlly()
    // is true for both (verified by disassembly), so laying an SRP paints area we are scored on.
    // The paint is not a cost against coverage -- only the choice of WHICH tiles gets constrained.
    //
    // SRP_MIN_CHIPS is the dose. Integer.MAX_VALUE disables the mechanism entirely, which makes
    // the zero arm byte-identical to the incumbent by construction (doctrine 5 step 0).
    static final int SRP_MIN_CHIPS = 2147483647;
    static final int SRP_PATIENCE  = 30;   // turns a soldier will stay parked on one pattern
    // 9 candidate centres at spacing 2, all within the engine's r2=8 mark radius.
    static final int[] SRP_DX = { 0,  2, -2,  0,  0,  2,  2, -2, -2 };
    static final int[] SRP_DY = { 0,  0,  0,  2, -2,  2, -2,  2, -2 };

    static MapLocation srpCenter = null;   // pattern this soldier is laying, or null
    static int srpTurns = 0;
    static int srpAsk = 0, srpMark = 0, srpDone = 0, srpDrop = 0;
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " sA=" + srpAsk + " sM=" + srpMark + " sD=" + srpDone + " sX=" + srpDrop
            + " | " + state);
        Clock.yield();
    }
    ...
    static String runSoldier() throws GameActionException {
        ...
        // Iteration 49: SRP, only when there is NO unclaimed ruin in sight. Ruins keep absolute
        // priority -- a tower is worth more than a pattern, and gating on ruin==null also keeps
        // SRP marks away from a tower pattern's 5x5, so the two mark-driven loops cannot collide.
        // Ruins take priority for STARTING a pattern, but srpWork must run EVERY turn once one
        // is started. The first version guarded this call on `ruin == null` while guarding the
        // move on `srpCenter == null` -- two different conditions, so a soldier holding a pattern
        // that then saw a ruin would neither advance srpTurns (no timeout, ever) nor move. That
        // is a permanent freeze, and it is exactly the "read the guard you are nesting inside"
        // pre-check: the new clause sat under an outer condition that excluded its own exit path.
        state += srpWork(ruin != null);
        ...
        // While a pattern is being laid, stand still: every one of its 25 tiles is inside this
        // soldier's attack radius from the centre, so moving can only lose the pattern. Bounded
        // by SRP_PATIENCE, and the soldier is painting a scored tile every turn it stands there,
        // so this is the productive-parking case, not a livelock.
        if (srpCenter == null) moveExploring(ruin);
    ...
    /**
     * Lay and complete one special resource pattern. Returns an indicator fragment.
     *
     * Choice set, checked before building (doctrine: reachability means the CHOICE SET): a
     * soldier's ATTACK radius is r2=9 and the far corner of a 5x5 pattern is at r2=8 from its
     * centre, so a soldier standing ON the centre can paint every one of the 25 tiles without
     * moving. The centre candidate is therefore the tile underfoot -- a choice set of one, chosen
     * deliberately rather than by accident, because ranking over a set of one is not a ranking.
     *
     * canMarkResourcePattern does the whole legality check inside the engine (is a SOLDIER,
     * r2<=8, isValidPatternCenter(loc,false), robot paint >= 25 -- all read from the bytecode),
     * so this does not re-implement it and cannot drift from it.
     */
    static String srpWork(boolean ruinPresent) throws GameActionException {
        MapLocation me = rc.getLocation();
        if (srpCenter == null) {
            // Only the START of a pattern is gated -- on a ruin being present, and on a treasury
            // floor so an SRP can never starve the 1000-chip tower completion or unit production
            // (iteration 48 is the standing evidence that chips spent in the wrong place cost
            // games). An ALREADY-STARTED pattern is advanced or dropped below regardless of both,
            // because those are the only paths that can release the soldier.
            if (ruinPresent || rc.getChips() < SRP_MIN_CHIPS) return "";
            srpAsk++;
            // CHOICE SET, widened after the first stage 0 measured it. With the tile underfoot as
            // the only candidate, 34-45 asks produced 1-3 marks: ~3-9%, far below the 29% of Leaf
            // tiles that are legal centres, because the ONE tile a soldier happens to stand on is
            // usually not one of them. A mechanism that barely fires cannot be evaluated -- a null
            // would not distinguish "ran and failed" from "never ran" -- so this is finishing the
            // implementation, not tuning it.
            //
            // Candidates are spaced 2 apart so their 5x5s barely overlap: a neighbour one tile
            // away fails for the same wall or ruin the centre did, so it buys almost nothing.
            // All 9 offsets are within r2=8, which is the mark radius the engine asserts.
            // canMarkResourcePattern does the full legality test per candidate, so this cannot
            // drift from the engine's rule; the cap of 9 bounds the bytecode.
            for (int i = 0; i < 9; i++) {
                MapLocation c = me.translate(SRP_DX[i], SRP_DY[i]);
                if (!rc.canMarkResourcePattern(c)) continue;
                rc.markResourcePattern(c);
                srpCenter = c; srpTurns = 0; srpMark++;
                return " srpM";
            }
            return "";
        }

        // Bounded, so a soldier can never park forever on a pattern it cannot finish (an enemy
        // tile inside the 5x5 makes it permanently uncompletable, exactly as for a ruin).
        if (++srpTurns > SRP_PATIENCE || me.distanceSquaredTo(srpCenter) > 8) {
            srpCenter = null; srpDrop++; return " srpX";
        }
        if (rc.canCompleteResourcePattern(srpCenter)) {
            rc.completeResourcePattern(srpCenter);
            srpDone++; srpCenter = null;
            return " srpD";
        }
        // Same idiom as workOnRuin: paint one mismatched marked tile per turn, in the colour the
        // mark asks for, skipping enemy paint a soldier can never overwrite (iteration 29).
        for (MapInfo t : rc.senseNearbyMapInfos(srpCenter, 8)) {
            PaintType mk = t.getMark();
            if (mk == PaintType.EMPTY || mk == t.getPaint()) continue;
            if (t.getPaint().isEnemy()) continue;
            if (rc.canAttack(t.getMapLocation())) {
                rc.attack(t.getMapLocation(), mk == PaintType.ALLY_SECONDARY);
                return " srpP";
            }
        }
        return " srpW";
