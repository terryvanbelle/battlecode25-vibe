...
public class RobotPlayer {
    ...
    // ---- ITERATION 69: enforce the INTENDED mix, which affordability silently destroys ----
    // Measured on Gears (55x55) r1000-2000: carol builds 307 soldiers and 7 splashers -- a
    // realized ratio of 44:1 against an INTENDED ratio of 5:1 (the roll is 15% splasher,
    // 75% soldier). A 9x divergence, and my own LEARNINGS says "realized != intended means one
    // of them is wrong, and I assumed four times it was the realized one".
    //
    // The cause is a RACE, not a threshold. A tower accumulates ~5-15 paint/turn; at 200 a
    // soldier roll (75% of turns) takes the whole stash, so it almost never reaches a
    // splasher's 300. Iterations 62/63/64 all attacked this with paint thresholds or a phase
    // switch and all three failed -- because a threshold on a quantity the soldier consumes
    // cannot separate "save for a splasher" from "never build anything".
    //
    // A SCHEDULER can. After SPLASHER_EVERY soldiers, this tower refuses to build anything but
    // a splasher until it can afford one -- so it simply accumulates. That guarantees the
    // realized ratio regardless of the paint dynamics, and it costs tower growth only the
    // delay, not the soldiers. 0 disables it (zero arm, byte-identical play).
    static final int SPLASHER_EVERY = 4;
    ...
    static String runTower() throws GameActionException {
        ...
        // Iteration 69: hold for a splasher once the quota of soldiers has been spent.
        if (SPLASHER_EVERY > 0 && want != UnitType.SPLASHER
                && soldiersSinceSplasher >= SPLASHER_EVERY) {
            want = UnitType.SPLASHER;
            afford = chips >= reserve + want.moneyCost
                  && rc.getPaint() >= want.paintCost;   // else build NOTHING and accumulate
        }
        if (afford) {
