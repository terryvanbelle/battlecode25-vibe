...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i31-200";
    ...
    static String runTower() throws GameActionException {
        ...
        int tpIn = rc.getPaint();   // tower's OWN paint stash, at the decision point
        ...
        // Iteration 31: MOPPER PAINT FLOOR. Measured at the DECISION point on three maps
        // (Parking_lot, Snowman, sunrise -- the last two drawn at random): 100.0% of
        // builds blocked while chips were available were blocked by tpIn < paintCost, and
        // the REALIZED mix is 50-57% moppers against an intended 10%. The mopper wins that
        // share by costing 100 paint against a soldier's 200 and a splasher's 300, so its
        // roll clears the stash ~10x more often -- an allocation set by cost ordering, not
        // chosen. This is a MIX change at constant paint spend, not a production cut: the
        // paint is spent either way. That is what distinguishes it from iteration 19a,
        // which deleted the mopper roll outright and ran at 32%.
        // The > 0 guard makes MOP_PAINT_FLOOR = 0 a provable no-op: without it, touching
        // `afford` would skip the rng.nextInt(8) below and desynchronise the whole stream,
        // so the zero arm would not be byte-identical to iteration 30.
        final int MOP_PAINT_FLOOR = 200;
        if (MOP_PAINT_FLOOR > 0 && afford && want == UnitType.MOPPER
                && tpIn < want.paintCost + MOP_PAINT_FLOOR) {
            afford = false;
        }
        if (afford) {
