...
public class RobotPlayer {
    ...
    /** Iteration 58: round this TOWER last built a SPLASH_FLOOR-exempt soldier. */
    static int lastFloorSoldier = 0;
    /** Iteration 58 attribution arm: the round this tower first took a turn. */
    static int bornRound = 0;
    /** Iteration 58 dose. 0 = OFF, and the arm is then byte-identical to the baseline. */
    static final int SOLDIER_GAP = 250;
    ...
    static String runTower() throws GameActionException {
        // ATTRIBUTION ARM: a TRUE per-tower rate limit. In carol_i58_250 lastFloorSoldier is a
        // per-robot static initialised to 0, so a NEW tower's clock starts already expired and
        // every new tower grants an immediate exempt soldier -- a compounding loop, not a floor.
        // Here the clock starts at the tower's own first turn, so a tower must WAIT one full
        // SOLDIER_GAP before its first exemption. This is the one-line difference that separates
        // 'a new tower comes with a free soldier' from 'each tower may build one per GAP rounds'.
        if (bornRound == 0) { bornRound = rc.getRoundNum(); lastFloorSoldier = bornRound; }
        ...
        // ---- Iteration 58: SOLDIER FLOOR. A RATE LIMIT, not a threshold change.
        // Traced on Leaf: soldier id12362 completed 4 towers in 170 rounds, refilling from each
        // tower it built, then starved at r220 -- and carol built no further soldier for the
        // remaining 527 rounds, because the soldier gate (2250) sits above the treasury's
        // limit-cycle ceiling (~1600; 0.04% of 5,425 tower-turns clear it). This exempts ONE
        // soldier per tower per SOLDIER_GAP rounds from SPLASH_FLOOR, so the standing soldier
        // population cannot reach zero and stay there. It does NOT move any threshold, so it
        // is not iteration 47's dose: volume is capped by the gap, not by a gate.
        boolean floorSoldier = SOLDIER_GAP > 0 && want == UnitType.SOLDIER
                && rc.getRoundNum() - lastFloorSoldier >= SOLDIER_GAP;
        if (afford && want != UnitType.SPLASHER && !floorSoldier
                && chips - want.moneyCost < SPLASH_FLOOR) {
            afford = false;
        }
        ...
        if (afford) {
            ...
            if (rc.canBuildRobot(want, loc)) {
                ...
                if (floorSoldier) lastFloorSoldier = rc.getRoundNum();
            }
        }
