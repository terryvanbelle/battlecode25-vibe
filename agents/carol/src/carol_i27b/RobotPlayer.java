...
public class RobotPlayer {
    ...
    // ---- Iteration 27b: the OTHER branch -- the r2=20 navigation choice --------------------
    // i27a refuted re-ranking the PAINT target: that choice is made inside the action radius
    // (r2=9), is only 2.3-5.1 candidates wide, and on 85.5-100% of decisions no less-contested
    // option existed. The distinction I traced lives at map scale and simply is not present in
    // that decision. But nearestVisibleEmpty() is a DIFFERENT decision over the full vision
    // radius r2=20 -- a much larger candidate set, and the one that actually steers the robot.
    // i27a left it untouched, so it is untested rather than refuted. This tests it.
    //
    //   fvN     times the frontier-target decision fired (an empty tile was visible)
    //   fvCand  candidates summed across those decisions (the WIDTH)
    //   fvHot   the chosen (nearest) tile had >=1 enemy-painted neighbour
    //   fvBest  a strictly LESS contested candidate existed and was not chosen
    //   fvSkip  probe skipped itself for lack of bytecode (see below)
    static int fvN = 0, fvCand = 0, fvHot = 0, fvBest = 0, fvSkip = 0;

    // SELF-LIMITING, and this is a direct lesson from i27a: that probe took peak robot
    // bytecode from the baseline's 37.5% of 17500 to 88.5% with near-misses. An overrun does
    // not throw -- it silently truncates the turn -- so an over-budget probe does not produce
    // obviously wrong numbers, it produces unreadable ones. This one refuses to start unless
    // there is comfortable headroom, and counts how often it declined so the sampled fraction
    // is visible rather than assumed.
    static final int PROBE_BYTECODE_FLOOR = 9000;
    static final boolean[] foeGrid = new boolean[121];   // 11x11 window, dx/dy in -5..5

    static int foeIdx(int dx, int dy) { return (dx + 5) * 11 + (dy + 5); }

    static int contestedness(MapLocation me, MapLocation loc) {
        int n = 0;
        for (int ddx = -1; ddx <= 1; ddx++) for (int ddy = -1; ddy <= 1; ddy++) {
            if (ddx == 0 && ddy == 0) continue;
            int dx = loc.x + ddx - me.x, dy = loc.y + ddy - me.y;
            if (dx < -5 || dx > 5 || dy < -5 || dy > 5) continue;
            if (foeGrid[foeIdx(dx, dy)]) n++;
        }
        return n;
    }

    /** Read-only. Takes no action and writes nothing the bot reads. */
    static void probeFrontierChoice(MapLocation chosenTarget) throws GameActionException {
        if (Clock.getBytecodesLeft() < PROBE_BYTECODE_FLOOR) { fvSkip++; return; }
        MapLocation me = rc.getLocation();
        java.util.Arrays.fill(foeGrid, false);
        MapInfo[] vis = rc.senseNearbyMapInfos(-1);
        for (MapInfo t : vis) {
            if (!t.getPaint().isEnemy()) continue;
            MapLocation l = t.getMapLocation();
            int dx = l.x - me.x, dy = l.y - me.y;
            if (dx < -5 || dx > 5 || dy < -5 || dy > 5) continue;
            foeGrid[foeIdx(dx, dy)] = true;
        }
        int chosen = contestedness(me, chosenTarget), minC = chosen, cand = 0;
        for (MapInfo t : vis) {
            if (t.getPaint() != PaintType.EMPTY || !t.isPassable()) continue;
            cand++;
            int c = contestedness(me, t.getMapLocation());
            if (c < minC) minC = c;
        }
        fvN++; fvCand += cand;
        if (chosen > 0) fvHot++;
        if (minC < chosen) fvBest++;
    }
     ...
     */
    static final String BUILD = "i27b";
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " fvN=" + fvN + " fvC=" + fvCand + " fvH=" + fvHot
            + " fvB=" + fvBest + " fvS=" + fvSkip + " | " + state);
        Clock.yield();
    }
