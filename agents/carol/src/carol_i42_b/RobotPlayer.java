...
public class RobotPlayer {
    ...
    /**
     * Iteration 42: RADIAL EXPLORATION. The dose is how many soldiers in 8 use it; 0 is the
     * incumbent's uniform-random target.
     *
     * `newExploreTarget()` samples four UNIFORMLY RANDOM map coordinates and keeps the farthest
     * from the robot. It already biases outward, but its DIRECTION is random, so on arrival a
     * soldier picks another random far point -- frequently back across the board -- and spends
     * the crossing walking over ground that is already ours. Measured on carol_iter36's own
     * indicator strings: IDLE-ALLY (nothing paintable in r^2=9, surrounded by our paint) is
     * 49.2% of soldier turns on TheBest (60x60) against 1.4% on rain (30x30). Four times the
     * area, thirty-five times the idleness, because the wasted crossing scales with the board.
     *
     * That is the mechanism behind the deficit this lineage actually has, measured
     * cross-lineage over 300 tournament games: 54.8% on small maps, 29.5% on large.
     *
     * The fix is to give each soldier a FIXED heading derived from its ID and send it to the map
     * edge along it, so the fleet fans out into sectors and no soldier re-crosses the territory
     * it just left. A heading is re-drawn only on arrival or when stuck, not every 120 rounds.
     *
     * Iterations 37-41 all attacked the resource economy for one accept. This one attacks the
     * geometry, which is where the measurement has been pointing for some time.
     */
    static final int EXPLORE_RADIAL_IN_8 = 4;
    static int myHeading = -1;
    ...
    static void newExploreTarget() {
        ...
        if (rc.getID() % 8 < EXPLORE_RADIAL_IN_8) {
            // Radial: commit to one heading and run it to the map edge. Advancing the heading
            // (rather than re-drawing it at random) means a soldier that exhausts one sector
            // sweeps into the NEXT one instead of teleporting its interest across the board.
            if (myHeading < 0) myHeading = rc.getID() % 8;
            else myHeading = (myHeading + 1) % 8;
            Direction d = DIRS[myHeading];
            int far = w + h;
            int tx = me.x + d.dx * far, ty = me.y + d.dy * far;
            if (tx < 0) tx = 0; if (tx > w - 1) tx = w - 1;
            if (ty < 0) ty = 0; if (ty > h - 1) ty = h - 1;
            explore = new MapLocation(tx, ty);
            exploreAge = 0;
            return;
        }
