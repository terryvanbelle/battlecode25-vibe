...
public class RobotPlayer {
    ...
    static int tgt1 = 0, tgt2 = 0;   // probe: soldier turns with exactly 1 / >=2 attackable enemy towers
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            + " ov=" + bcOverruns + " nm=" + bcNearMisses + " t1=" + tgt1 + " t2=" + tgt2 + " | " + state);
        Clock.yield();
    }
    ...
    static String runSoldier() throws GameActionException {
        ...
        RobotInfo[] enemies = rc.senseNearbyRobots(-1, rc.getTeam().opponent());
        int nTowers = 0;
        for (RobotInfo e : enemies) if (e.type.isTowerType() && rc.canAttack(e.location)) nTowers++;
        if (nTowers == 1) tgt1++; else if (nTowers >= 2) tgt2++;
        for (RobotInfo e : enemies) {
