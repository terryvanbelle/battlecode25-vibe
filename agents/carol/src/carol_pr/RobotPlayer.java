...
public class RobotPlayer {
     ...
     */
    /**
     * Below this many team towers, every ruin is completed as a PAINT tower.
     *
     * Iteration 8. Losing the last paint tower is an UNRECOVERABLE loss: a money tower has
     * paintPerTurn == 0 and the engine gates paint income on that field, so it never gains
     * paint by mining or from SRPs; robot paint comes from the building tower's stash; so with
     * every tower dry, canBuildRobot fails forever, no robot can paint a ruin, and no paint
     * tower can ever be rebuilt. Measured on Dominoes: the starting paint tower died around
     * r200 and carol sat on one dry money tower for 1,800 rounds while chips reached 60,000.
     * The control arm shows this costs the accepted baseline 2 of 40 games vs carol_rush.
     *
     * carol starts with exactly ONE paint tower (NUMBER_INITIAL_PAINT_TOWERS = 1) and the key
     * below never asks what the team already holds, so a single death ends the game. Forcing
     * the first completions to be PAINT turns one point of failure into two or three. Team-wide
     * and exact via getNumberTowers(); deliberately does nothing once carol has depth, which is
     * correct -- the traces show she loses no towers at all once past three.
     */
    /**
     * Paint a tower keeps back so it can always still SPAWN a unit, rather than having its
     * stash drained by soldiers topping themselves up. Exactly the CHIP_RESERVE idea on the
     * other resource, and the parallel is the argument for it: iteration 2 added a chip reserve
     * because unreserved spawning drained the treasury and cost towers, and the identical
     * failure on paint has had no reserve at all since iteration 0.
     *
     * Measured motivation: across 27 traced games carol spends a median of 28 of the first 100
     * rounds with NO tower holding the 200 paint a soldier costs, and 9 of 27 spend more than
     * half the opening that way -- while chips sit at 1,200-1,500. A lv1 paint tower mines
     * 5/turn, so two soldiers cycling home to refill drain it faster than it regenerates.
     *
     * Dose: 0 (zero arm = iteration 0's greedy refill) / 100 / 200.
     */
    static final int TOWER_PAINT_RESERVE = UnitType.SOLDIER.paintCost;   // 200

    static final int PAINT_FLOOR_TOWERS = 3;
    ...
    static UnitType towerTypeFor(MapLocation ruin) {
        // Paint-tower floor (iteration 8) -- see PAINT_FLOOR_TOWERS. Checked BEFORE the key, so
        // it overrides it rather than being overridden by it.
    ...
    static void refillIfPossible() throws GameActionException {
        ...
        for (RobotInfo ally : rc.senseNearbyRobots(2, rc.getTeam())) {
            if (ally.type.isTowerType()) {
                // Take only the surplus above the tower's spawn reserve, so refuelling an
                // existing soldier can never consume the ability to create a new one.
