...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i32p";

    // ---- PROBE (iteration 32): lattice-SRP reachability. Pure reads, NO action, NO state ----
    //
    // Iteration 26 proved the SRP MECHANIC works (88 patterns completed) and that what fails is
    // PER-SOLDIER COMMITMENT WITH NAVIGATION CAPTURE: once `srpCenter` was set, `SRPfar` fired
    // 573-4,583 times per game steering the soldier, and it arrived back broke. The ledger's own
    // re-opening condition names the replacement: "an opportunistic version with no `srpCenter`
    // at all ... no in-transit spend to lose".
    //
    // The missing piece was COORDINATION: with no per-robot target, soldiers must agree on WHERE
    // a pattern goes without communicating. A fixed 5-stride LATTICE does it -- centre(x,y) =
    // (5*(x/5)+2, 5*(y/5)+2) is a pure function of the tile, so every soldier that ever stands in
    // a cell computes the same centre, and 5x5 cells on a 5-stride lattice tile the plane exactly
    // with no overlap. This is the same device `towerTypeFor` already uses one function below
    // ("must be a pure function of the ruin so that every soldier agrees every turn").
    //
    // Also load-bearing, from RULES.md: MARKS ARE OPTIONAL -- "pattern match is on paint only".
    // So this design pays NO 25-paint deposit, which was iteration 26's non-refundable entry cost.
    //
    // What this probe must establish BEFORE a run is bought: does the firing condition ever hold?
    // Per iteration 26a's recorded probe flaw ("check that the counter's condition is the SAME
    // WIDTH as the decision the bot would actually get to make"), every condition below is
    // exactly the one the shipping mechanism would test -- own cell only, same ruin guard, same
    // action radius. It cannot produce a false negative by asking a narrower question.
    static final int SRP_MASK = 28873275;        // RULES.md, re-verified: decodes to XX.XX/X...X/..X../X...X/XX.XX
    static int srpTurns = 0;   // idle soldier turns reaching the probe (the whole budget)
    static int srpEdge  = 0;   // ... cell runs off the map, can never complete
    static int srpRuin  = 0;   // ... cell overlaps a visible ruin's tower pattern (guard)
    static int srpDirty = 0;   // ... some cell tile not ally-painted or not sensable
    static int srpOk    = 0;   // ... cell CLEAN: all 25 ally-painted -> THE REACHABILITY NUMBER
    static int srpFull  = 0;   // ... of clean, already pattern-exact (would complete now)
    static int srpAct   = 0;   // ... of clean, >=1 wrong-coloured tile INSIDE action radius r2=9
    static int srpFar   = 0;   // ... of clean, wrong tiles exist but none in reach this turn
    static int srpMinW  = 99;  // fewest wrong tiles seen on a clean cell (how close we ever get)
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " ST=" + srpTurns + " sE=" + srpEdge + " sR=" + srpRuin + " sD=" + srpDirty
            + " sOK=" + srpOk + " sF=" + srpFull + " sA=" + srpAct + " sX=" + srpFar
            + " sMW=" + srpMinW + " | " + state);
        Clock.yield();
    }
        ...
        if (rc.isActionReady() && rc.getPaint() >= UnitType.SOLDIER.attackCost) {
            ...
            if (myTile.getPaint() == PaintType.EMPTY && rc.canAttack(me)) {
                ...
                if (best != null) { rc.attack(best); state += " pnt"; }
                    ...
                    // ---- PROBE (iteration 32) ----------------------------------------------
                    // Reaching here is the mechanism's exact firing condition: action ready,
                    // >=5 paint, and NOTHING empty to paint in action range -- a turn this
                    // lineage has measured as doing literally nothing, 72-88% of idle-ally
                    // turns. Everything below is a read; no attack, no move, no rng draw, no
                    // field the bot reads is written. Its games must reproduce iteration 30's.
                    srpTurns++;
                    int scx = (me.x / 5) * 5 + 2, scy = (me.y / 5) * 5 + 2;
                    if (scx + 2 >= rc.getMapWidth() || scy + 2 >= rc.getMapHeight()) {
                        srpEdge++;
                    } else {
                        boolean ruinNear = false;
                        // A ruin's 5x5 tower pattern overlaps this cell iff both offsets <= 4.
                        // Recolouring a tower-pattern tile would make SRP work and workOnRuin
                        // repaint the same tiles against each other -- a paint-burning loop.
                        for (MapLocation r : rc.senseNearbyRuins(-1)) {
                            if (Math.abs(r.x - scx) <= 4 && Math.abs(r.y - scy) <= 4) { ruinNear = true; break; }
                        }
                        if (ruinNear) {
                            srpRuin++;
                        } else {
                            boolean clean = true;
                            int wrong = 0, wrongReach = 0;
                            outer:
                            for (int dx = -2; dx <= 2; dx++) {
                                for (int dy = -2; dy <= 2; dy++) {
                                    MapLocation l = new MapLocation(scx + dx, scy + dy);
                                    if (!rc.canSenseLocation(l)) { clean = false; break outer; }
                                    PaintType p = rc.senseMapInfo(l).getPaint();
                                    // Walls and ruins read EMPTY and are unpaintable, so this one
                                    // test subsumes the wall check: a cell that is wholly
                                    // ally-painted provably contains no wall.
                                    if (!p.isAlly()) { clean = false; break outer; }
                                    boolean sec = ((SRP_MASK >>> (5 * (dx + 2) + (dy + 2))) & 1) != 0;
                                    if ((p == PaintType.ALLY_SECONDARY) != sec) {
                                        wrong++;
                                        if (me.distanceSquaredTo(l) <= 9) wrongReach++;
                                    }
                                }
                            }
                            if (!clean) srpDirty++;
                                ...
                                srpOk++;
                                if (wrong == 0) srpFull++;
                                else if (wrongReach > 0) srpAct++;
                                else srpFar++;
                                if (wrong > 0 && wrong < srpMinW) srpMinW = wrong;
                            }
                        }
                    }
                    // ---- end probe ---------------------------------------------------------
