...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i46q";
    ...
    static String runMopper() throws GameActionException {
        ...
        // PROBE ONLY (iteration 46q). Answers the ONE question that decides whether a targeted-mop
        // iteration can pay at all, BEFORE any is built -- the check iteration 45 skipped.
        //
        // Iteration 45 established that soldiers standing on enemy paint bleed 2 paint/turn
        // [E: RULES.md, end-of-turn drain] and that stopping them from stepping there confines
        // them and loses the map. The alternative is to remove the paint instead of avoiding the
        // tile: a mopper attack clears enemy paint within r2=2 and does not move anybody.
        //
        // But this bot's runMopper takes the FIRST enemy tile senseNearbyMapInfos returns, in
        // arbitrary order, with no reference to whether an ally is standing on it. Retargeting is
        // free -- same action, same cost -- but only if the situation actually arises. So count:
        //   mt   mopper turns
        //   e    enemy-painted tiles this mopper could attack (the current candidate set)
        //   a    of those, how many have an ALLY ROBOT standing on them (the drain-relieving ones)
        //   h    1 if the tile this mopper is ABOUT to mop already has an ally on it (already
        //        optimal by luck), 0 if a better target existed and was passed over
        // Reads only; nothing is assigned, so play is unchanged and the (winner, rounds) identity
        // check against the baseline must pass. If `a` is ~0 per turn the mechanism cannot fire
        // and iteration 46 is abandoned here, at the cost of one match instead of a gauntlet.
        {
            MapLocation mloc = rc.getLocation();
            RobotInfo[] pals = rc.senseNearbyRobots(2, rc.getTeam());
            int pe = 0, pa = 0, ph = 0;
            MapLocation firstPick = null;
            for (MapInfo t : rc.senseNearbyMapInfos(2)) {
                if (!t.getPaint().isEnemy()) continue;
                MapLocation tl = t.getMapLocation();
                if (!rc.canAttack(tl)) continue;
                pe++;
                if (firstPick == null) firstPick = tl;
                for (RobotInfo r : pals) {
                    if (r.location.equals(tl)) { pa++; break; }
                }
            }
            if (firstPick != null) {
                for (RobotInfo r : pals) {
                    if (r.location.equals(firstPick)) { ph = 1; break; }
                }
            }
            state += " mt e" + pe + " a" + pa + " h" + ph;
        }
