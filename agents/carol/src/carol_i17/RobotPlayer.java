...
public class RobotPlayer {
    ...
    // Iteration 17: nearest ally tower this robot has seen. A robot is BUILT adjacent to a
    // tower, so this is set on turn 1 and is never null thereafter -- no map memory needed.
    static MapLocation homeTower = null;
    static int refillWalk = 0, refillHome = 0;   // dry turns walking home / turns arrived+refuelled
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " rw=" + refillWalk + " rh=" + refillHome + " | " + state);
        Clock.yield();
    }
    ...
    static String runSplasher() throws GameActionException {
        int paintBefore = rc.getPaint();
        refillIfPossible();
        if (rc.getPaint() > paintBefore) refillHome++;
        // Iteration 17: remember the nearest ally tower. Cheap and refreshed whenever one is
        // in vision; seeded on turn 1 because buildRobot places the robot within r2=4 of the
        // tower that made it [E: RULES.md].
        MapLocation myLoc = rc.getLocation();
        int homeD = Integer.MAX_VALUE;
        for (RobotInfo a : rc.senseNearbyRobots(-1, rc.getTeam())) {
            if (!a.type.isTowerType()) continue;
            int d = myLoc.distanceSquaredTo(a.location);
            if (d < homeD) { homeD = d; homeTower = a.location; }
        }
        String tag = "";
        int bestScore = 0;
        ...
        // Iteration 17: a splasher below its attack cost cannot do ANYTHING, and
        // refillIfPossible only reaches a tower within r2=2 -- which a wandering splasher is
        // essentially never next to. Measured over 8 complete iteration-14 games, `noPaint` is
        // the dominant splasher state by an order of magnitude: 1,987 turns on Parking_lot
        // against 10 actual splashes, 381 on gridworld against 20. A splasher costs 300 paint
        // + 400 chips to build and then does nothing for most of the game. So walk it back to
        // the tower that built it instead of exploring ground it cannot paint.
        if (!fueled && homeTower != null) {
            refillWalk++;
            moveExploring(homeTower);
        } else {
            moveExploring(null);
        }
