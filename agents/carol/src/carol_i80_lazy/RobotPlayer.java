...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "m80L";
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " rt=" + refillTrips + " ht=" + homeTurns + " dn=" + denyBans + " pb=" + patienceBans + " bs=" + banSkips + " bp=" + banPeak + " mn=" + memNavs
            + " | " + state);
        Clock.yield();
    }
    ...
    // ---------------- ITERATION 80: the RUIN MEMORY.
    // nearestEmptyRuin() is sense-range only, so an unclaimed ruin that leaves vision is
    // forgotten completely -- while carol's soldiers sight 67% of the map's ruins over a
    // game and finish with a mean of 2.75 structures. Stage 0 measured that on the maps
    // carrying the margin, 71.2% of blind soldier-turns have a remembered unclaimed ruin
    // available, and that carol demonstrably COMPLETES towers at median d=8.89 (max 25.24)
    // from her nearest known tower with REFILL_LOW active -- so steering to one does not
    // require breaking the refill tether, which is refuted at -26.
    //
    // MEM_REACH is the dose: max d2 from the soldier to a remembered ruin for it to be
    // adopted as the explore target. 0 disables the branch entirely (zero arm, identical
    // play); -1 is unlimited.
    static final int MEM_REACH = -1;
    static final int RUIN_MEM = 24;
    static int[] ruinMem = new int[RUIN_MEM];
    static int ruinN = 0;
    static int memNavs = 0;

    static void memRemember(MapLocation r) {
        int key = (r.x << 6) | r.y;
        for (int i = ruinN; --i >= 0; ) if (ruinMem[i] == key) return;
        if (ruinN >= RUIN_MEM) return;
        ruinMem[ruinN++] = key;
    }

    /** Staleness handling. Stage 0 measured >=3.7% of remembered ruins already claimed. */
    static void memForget(MapLocation r) {
        int key = (r.x << 6) | r.y;
        for (int i = ruinN; --i >= 0; ) if (ruinMem[i] == key) { ruinMem[i] = ruinMem[--ruinN]; return; }
    }

    static MapLocation nearestRememberedRuin() {
        if (MEM_REACH == 0 || ruinN == 0) return null;
        MapLocation me = rc.getLocation();
        int mx = me.x, my = me.y;
        int best = -1, bestD = Integer.MAX_VALUE;
        for (int i = ruinN; --i >= 0; ) {
            int dx = mx - (ruinMem[i] >> 6), dy = my - (ruinMem[i] & 63);
            int d = dx * dx + dy * dy;
            if (d >= bestD) continue;
            // LAZY FILTER: a ban is a TEMPORARY condition, so test it when choosing a target
            // rather than when recording one. An expired ban restores the ruin for free.
            if (ruinBanned(new MapLocation(ruinMem[i] >> 6, ruinMem[i] & 63))) continue;
            bestD = d; best = i;
        }
        if (best < 0 || (MEM_REACH > 0 && bestD > MEM_REACH)) return null;
        return new MapLocation(ruinMem[best] >> 6, ruinMem[best] & 63);
    }
    ...
    static String runSoldier() throws GameActionException {
        ...
        if (ruin != null) {
            ...
            MapLocation mem = nearestRememberedRuin();
            if (mem != null) {
                explore = mem; exploreAge = 0; memNav = true; memNavs++;
                state += " memNav=" + mem;
            }
        }
    ...
    static MapLocation nearestEmptyRuin() throws GameActionException {
        ...
        for (MapLocation r : rc.senseNearbyRuins(-1)) {
            if (rc.canSenseRobotAtLocation(r)) { memForget(r); continue; }  // tower already there
            memRemember(r);                                   // LAZY: remember it even if banned --
            if (ruinBanned(r)) { banSkips++; continue; }      // the ban EXPIRES; do not encode it here
