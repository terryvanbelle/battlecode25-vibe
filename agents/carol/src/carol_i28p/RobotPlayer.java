...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i28p";
    ...
    // ---- i28 PROBE: exhausted-cell map memory (NO-OP; measures the decision only) ----------
    // Mechanism under test: mark the coarse map cell a soldier stands in as EXHAUSTED when the
    // frontNone branch fires (nothing EMPTY anywhere in vision r2=20 -- the signal, already
    // computed, that there is nothing left to paint around here), then aim exploration at the
    // nearest non-exhausted cell instead of a uniform-random coordinate.
    //
    // This probe computes every quantity the mechanism would use and then THROWS IT AWAY: the
    // explore target is still the original random draw, so play is unchanged and the counters
    // below describe iteration 25's own behaviour. Registered counters:
    //   exF  fires of newExploreTarget()
    //   exB  fires where the RANDOM target lands in a cell this robot already exhausted
    //        (the waste the mechanism removes -- if this is ~0 the design is dead)
    //   exW  cumulative non-exhausted cell count at fire time (mean = the CHOICE SET WIDTH)
    //   exD  fires where the nearest non-exhausted cell differs from the random target's cell
    //   exX  cells this robot has ever marked exhausted;  exN  total cells on this map
    static final int CELL = 6;
    static boolean[] exhausted = null;
    static int cellW = 0, cellH = 0, cellN = 0;
    static int exF = 0, exB = 0, exW = 0, exD = 0, exX = 0;

    static void cellInit() {
        if (exhausted != null) return;
        cellW = (rc.getMapWidth() + CELL - 1) / CELL;
        cellH = (rc.getMapHeight() + CELL - 1) / CELL;
        cellN = cellW * cellH;
        exhausted = new boolean[cellN];
    }

    static int cellOf(MapLocation l) { return (l.y / CELL) * cellW + (l.x / CELL); }

    static MapLocation cellCenter(int c) {
        int cx = (c % cellW) * CELL + CELL / 2, cy = (c / cellW) * CELL + CELL / 2;
        if (cx >= rc.getMapWidth()) cx = rc.getMapWidth() - 1;
        if (cy >= rc.getMapHeight()) cy = rc.getMapHeight() - 1;
        return new MapLocation(cx, cy);
    }

    /** Mark the cell underfoot exhausted. Called only from the frontNone branch. */
    static void markExhausted() {
        cellInit();
        int c = cellOf(rc.getLocation());
        if (!exhausted[c]) { exhausted[c] = true; exX++; }
    }
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " id=" + rc.getID()
            + " exF=" + exF + " exB=" + exB + " exW=" + exW + " exD=" + exD
            + " exX=" + exX + " exN=" + cellN
            + " | " + state);
        Clock.yield();
    }
            ...
            if (myTile.getPaint() == PaintType.EMPTY && rc.canAttack(me)) {
                ...
                if (best != null) { rc.attack(best); state += " pnt"; }
                    ...
                    if (foe == 0) {
                        ...
                        else { state += " frontNone"; markExhausted(); }
                    }
    ...
    static void newExploreTarget() {
        ...
        // ---- i28 PROBE: what WOULD the memory have chosen? Computed, then discarded. -------
        // Deliberately placed AFTER the rng draws so the random stream is untouched: moving it
        // earlier would consume no randomness either, but keeping the original four nextInt
        // calls contiguous makes the no-op argument checkable by eye.
        // Soldiers only: moppers and splashers never mark, so their memory is empty by
        // construction and would report a full-width choice set and exB=0 on every fire --
        // diluting exactly the ratio this probe exists to measure.
        if (rc.getType() != UnitType.SOLDIER) { explore = best; exploreAge = 0; return; }
        cellInit();
        exF++;
        int free = 0, nearest = -1, nearestD = Integer.MAX_VALUE;
        for (int c = 0; c < cellN; c++) {
            if (exhausted[c]) continue;
            free++;
            int d = me.distanceSquaredTo(cellCenter(c));
            if (d < nearestD) { nearestD = d; nearest = c; }
        }
        exW += free;
        if (exhausted[cellOf(best)]) exB++;
        if (nearest >= 0 && nearest != cellOf(best)) exD++;
        // ---- end probe; `explore` is still the random draw, so behaviour is unchanged -------
        explore = best;
        exploreAge = 0;
    }
