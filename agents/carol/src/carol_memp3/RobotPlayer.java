...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "memr";
    ...
    // ---------------- ITERATION 80 PROBE. Recorded and reported; NEVER read by a decision.
    // No branch below consults ruinMem. Play is byte-identical to src/carol; the only
    // differences are the indicator string and bytecode consumed.
    static final int RUIN_MEM = 24;
    static int[] ruinMem = new int[RUIN_MEM];
    static int ruinN = 0;

    static void probeRemember(MapLocation r) {
        int key = (r.x << 6) | r.y;
    ...
    static void probeForget(MapLocation r) {
        int key = (r.x << 6) | r.y;
        for (int i = ruinN; --i >= 0; ) if (ruinMem[i] == key) { ruinMem[i] = ruinMem[--ruinN]; return; }
    }

    /**
     * Emitted ONLY on a blind soldier turn (nearestEmptyRuin() returned null). One token per
     * blind turn, so the replay can be aggregated by counting tokens -- no robot attribution
     * needed. s = soldier's d2 to the nearest remembered unclaimed ruin. t = the GENEROUS
     * falsifier quantity: min over ALL remembered ruins of d2 to the nearest known ally tower,
     * i.e. does a tether-legal target EXIST, not merely the nearest one. Pure int arithmetic,
     * no MapLocation allocation, so the worst case is 24x12 cheap iterations.
     */
    static String probeBlind() {
        if (ruinN == 0) return " P e";
        MapLocation me = rc.getLocation();
        int mx = me.x, my = me.y;
        int bestS = 99999, bestT = -1;
        for (int i = ruinN; --i >= 0; ) {
            int rx = ruinMem[i] >> 6, ry = ruinMem[i] & 63;
            int dx = mx - rx, dy = my - ry;
            int ds = dx * dx + dy * dy;
            if (ds < bestS) bestS = ds;
            for (int j = towerN; --j >= 0; ) {
                int tx = towerMem[j] >> 6, ty = towerMem[j] & 63;
                int ex = rx - tx, ey = ry - ty;
                int dt = ex * ex + ey * ey;
        ...
        int usable = 0;
        for (int i = ruinN; --i >= 0; ) {
            MapLocation rr = new MapLocation(ruinMem[i] >> 6, ruinMem[i] & 63);
            if (!ruinBanned(rr)) usable++;
        }
        return " P s=" + bestS + " t=" + bestT + " n=" + ruinN + " u=" + usable;
    }

    /** PROBE: d2 from a ruin to the nearest ally tower this robot knows about. */
    static int probeTowerD2(MapLocation r) {
        int best = -1;
        for (int j = towerN; --j >= 0; ) {
            int ex = r.x - (towerMem[j] >> 6), ey = r.y - (towerMem[j] & 63);
            int d = ex * ex + ey * ey;
    ...
    /** PROBE: set at the completeTowerPattern site, drained by runSoldier. */
    static int probeClaimD = -1;
    ...
    static String runSoldier() throws GameActionException {
        ...
        if (ruin != null) {
            state += " ruin=" + ruin;
            workOnRuin(ruin);
            state += " W t=" + probeTowerD2(ruin);             // PROBE: engaged envelope
            if (probeClaimD >= 0) { state += " C t=" + probeClaimD; probeClaimD = -1; }
        } else {
            state += probeBlind();                            // PROBE: measurement only
        }
    ...
    static MapLocation nearestEmptyRuin() throws GameActionException {
        ...
        for (MapLocation r : rc.senseNearbyRuins(-1)) {
            if (rc.canSenseRobotAtLocation(r)) { probeForget(r); continue; }  // tower already there
            probeRemember(r);                                 // PROBE: unclaimed, so remember it
    ...
    static void workOnRuin(MapLocation ruin) throws GameActionException {
        ...
        if (rc.canCompleteTowerPattern(kind, ruin)) {
            probeClaimD = probeTowerD2(ruin);                  // PROBE: CLAIMED envelope
