...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i52_0";
    ...
    /**
     * ITERATION 52 -- MOPPER DROUGHT WAIVER.
     *
     * Measured, from two tournament replays (the sanctioned cross-lineage channel): carol builds
     * essentially ZERO moppers, against the {MOPPER_IN_20} = 2/20 = 10% this file intends.
     * Not a thinned share -- zero, for whole games.
     *
     * Three gates must all pass for a mopper, and each is often false:
     ...
     * The measured treasury oscillates roughly [440, 4240] and tower paint sits at 160-400 for
     * long stretches, so the conjunction of three gates times a 10% roll rounds to never.
     *
     * This is the exact fault iteration 30 named and iteration 35 quoted -- "a gate above where
     * the treasury actually sits is not a policy, it is an off switch" -- committed this time by
     * my OWN fixes. Iteration 36 was right that moppers at a measured 60-75% share were crowding
     * soldiers out; it corrected that by absolute veto, which moved the variable from one extreme
     * to the other. THE MIDDLE HAS NEVER BEEN PLAYED.
     *
     * Why it should matter, mechanically: a soldier CANNOT overwrite enemy paint (RULES.md,
     * iteration 29) and a mopper is the only unit that REMOVES it. With zero moppers, every tile
     * the enemy paints is permanently lost to us. That is visible in the replay: on Circuit our
     * coverage peaks at round ~700 (520 tiles) and then falls monotonically to 348 while alice
     * climbs to 636 -- we do not merely stop gaining ground, we bleed it.
     *
     * The waiver, not a dose on the floors: the floors keep their veto, but a tower counts the
     * mopper rolls they have refused, and every MOP_DROUGHT-th refusal is waived. That restores a
     * bounded trickle without reverting iteration 36's protection of the soldier. Base
     * affordability and the engine's own canBuildRobot still apply, so a waiver can never build
     * something the tower cannot pay for.
     *
     * MOP_DROUGHT = Integer.MAX_VALUE disables the waiver, making the zero arm byte-identical.
     */
    static final int MOP_DROUGHT = Integer.MAX_VALUE;
    static int mopVeto = 0;    // consecutive mopper rolls refused by a floor
    static int mopWaive = 0;   // moppers let through by the waiver (manipulation check)
    ...
    static String runTower() throws GameActionException {
        ...
        boolean floored = false;   // iteration 52: a FLOOR refused this, not affordability
        if (afford && want != UnitType.SPLASHER && chips - want.moneyCost < SPLASH_FLOOR) {
            afford = false; floored = true;
        }
        ...
        // ---- Iteration 52: the drought waiver. Only ever relaxes a FLOOR, never base
        // affordability -- `floored` is set solely by the two floors above, so a mopper the
        // tower genuinely cannot pay for is unaffected and still fails here (and again inside
        // canBuildRobot). Counted so the manipulation check is a measurement, not an assumption.
        if (want == UnitType.MOPPER && floored) {
            mopVeto++;
            if (mopVeto >= MOP_DROUGHT) { afford = true; mopVeto = 0; mopWaive++; }
        }
        if (afford) {
