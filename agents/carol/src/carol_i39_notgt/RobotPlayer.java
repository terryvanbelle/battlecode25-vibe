...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i39";

    /** Iteration 39 dose: steer a non-firing splasher at the nearest visible
     *  empty tile. true = whenever it did not fire (lowScore or noTgt);
     *  false = only when it had no target at all (bestScore == 0). */
    static final boolean STEER_ON_LOWSCORE = false;
    static int sFront = 0, sNone = 0;   // conditional reachability counter
    ...
    static String runSplasher() throws GameActionException {
        ...
        String tag = "";
        int bestScore = 0;
        MapLocation steer = null;   // iteration 39: frontier target for this turn's move
        ...
        if (ready && fueled) {
            ...
            if (best != null && bestScore >= SPLASH_MIN_SCORE) {
                ...
                // ---- Iteration 39: SPLASHER FRONTIER STEERING ------------------------------
                // The splasher is the only unit that never steers. runSoldier has redirected
                // idle soldiers at nearestVisibleEmpty() since iteration 29 and it was accepted
                // there; splashers and moppers still call moveExploring(null), a random far
                // target. Measured on the incumbent (replay windows, no new VM time):
                //
                //   splasher bestScore distribution   median 0, p75 2, p90 4-6, max 18-27
                //   turn states, rounds 200-240       fired 4.4%, noPaint 45.4%, targetless 28.8%
                //   turn states, rounds 900-940       fired 2.0%, noPaint 14.8%, targetless 69.1%
                //
                // The median splasher turn scores ZERO -- it is standing where there is nothing
                // to paint. Meanwhile the soldier's own frontier counter reports frontFound on
                // 41 of 41 idle turns in the same window, so empty ground IS visible at r2=20
                // while the splasher's r2~8 scoring window sees none of it. That gap is exactly
                // what steering closes.
                //
                // Zero marginal cost: this redirects a move that was already going to happen and
                // spends no paint or chips -- the same shape as the accepted soldier version.
                // sFront/sNone is the CONDITIONAL reachability counter, counted only among turns
                // that reach this line, so one replay says whether the frontier was findable.
                // Passed to moveExploring as its TARGET, not written into `explore`. Writing
                // `explore` would be discarded exactly when it matters: moveExploring replaces
                // the target when distanceSquaredTo(explore) <= 8, and the nearest empty tile
                // is frequently that close for a splasher (its scoring window is only r2~8, so
                // "nothing worth hitting" and "nothing within 8" are different conditions).
                // The soldier version writes `explore` and is safe from this only because an
                // idle soldier, by definition, has no empty tile inside its r2=9 action radius.
                if (STEER_ON_LOWSCORE || best == null) {
                    steer = nearestVisibleEmpty();
                    if (steer != null) sFront++; else sNone++;
                }
            }
        } else {
