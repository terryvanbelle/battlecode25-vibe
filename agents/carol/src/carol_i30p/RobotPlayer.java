...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i30p";
    ...
    // ---- i30 PROBE: instrument the tower BUILD DECISION (NO-OP; reads only) ----------------
    // The splasher shortfall survives iteration 29: 1.2-2.7% of units built against an intended
    // 15%, and affordability is refuted as the cause (on Bunny the splasher-affordable share of
    // tower turns rose 40x, 0.3% -> 12.1%, while the realized share moved 0.7% -> 1.5%). So
    // something between the ROLL and the BUILD discards almost every splasher, and I have been
    // counting the outcome instead of the decision.
    //
    // Every counter below is read AT THE DECISION POINT, before buildRobot resolves, which is
    // what doctrine 15 requires -- a post-spend replay reading of "could it afford one" is
    // conditioned on the very build it is meant to predict.
    //   rS/rM/rO   rolls landing on splasher / mopper / soldier
    //   cS/cM/cO   of those, rejected by the CHIPS gate
    //   pS/pM/pO   chips ok, but tower paint < the type's paint cost   (the real affordability)
    //   dS/dM/dO   chips ok and paint ok, but the ONE drawn cell failed canBuildRobot
    //   kS/kM/kO   actually built
    //   wS/wM/wO   cumulative count of how many of the 8 neighbours WOULD have worked, summed
    //              over the rolls that reached the cell draw. Mean = how much room the single
    //              no-retry draw had. If this is ~8 the draw is harmless; if ~0-1 it is the bug.
    static int rS,rM,rO, cS,cM,cO, pS,pM,pO, dS,dM,dO, kS,kM,kO, wS,wM,wO;

    /** Classify one build decision. Pure reads: no rng, no action, no state the bot uses. */
    static void probeBuild(UnitType want, int chips, int reserve, MapLocation drawn) {
        boolean sp = want == UnitType.SPLASHER, mo = want == UnitType.MOPPER;
        if (sp) rS++; else if (mo) rM++; else rO++;
        if (chips < reserve + want.moneyCost) { if (sp) cS++; else if (mo) cM++; else cO++; return; }
        if (rc.getPaint() < want.paintCost)   { if (sp) pS++; else if (mo) pM++; else pO++; return; }
        int room = 0;
        for (int i = 0; i < 8; i++) {
            MapLocation l = rc.getLocation().add(DIRS[i]);
            if (rc.canBuildRobot(want, l)) room++;
        }
        if (sp) wS += room; else if (mo) wM += room; else wO += room;
        if (rc.canBuildRobot(want, drawn)) { if (sp) kS++; else if (mo) kM++; else kO++; }
        else                               { if (sp) dS++; else if (mo) dM++; else dO++; }
    }
    ...
    static String runTower() throws GameActionException {
        ...
        if (chips >= reserve + want.moneyCost) {
            ...
            probeBuild(want, chips, reserve, loc);          // i30 probe: classify, change nothing
            if (rc.canBuildRobot(want, loc)) rc.buildRobot(want, loc);
        } else {
            probeBuild(want, chips, reserve, rc.getLocation());
        }
