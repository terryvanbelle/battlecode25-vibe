...
public class RobotPlayer {
    ...
    // ---- ITERATION 76: the explore-target sampling RANK -------------------------------
    // The cheapest member of the coverage class -- one constant, no memory of any kind.
    //
    // Measured (galaxy, carol vs alice, 11 sampled rounds over 1,870 passable tiles):
    //     tiles ever within vision of a CAROL unit : 49%
    //     tiles ever within vision of an ALICE unit: 90%
    //     carol by quadrant: own half 76-77%, opponent half 18-23%
    // and of the 15 genuinely never-marked ruins, **6 are in carol's OWN half** -- 40% of the
    // gap needs no incursion into contested ground and no inference of any kind.
    //
    // The incumbent picks 4 uniform points and keeps the FARTHEST, which dispatches every unit
    // clean across the map. Combined with D3's refill tether (HOME is 35-59% of splasher turns)
    // those trips are not completed, so paths become radial spokes from towers and the gaps
    // between spokes are never covered. EXPLORE_RANK selects the k-th farthest of the same 4
    // samples: 1 = farthest = the incumbent exactly (zero arm), 4 = nearest.
    static final int EXPLORE_RANK = 4;

    static void newExploreTarget() {
        MapLocation me = rc.getLocation();
        int w = rc.getMapWidth(), h = rc.getMapHeight();
        MapLocation[] c = new MapLocation[4];
        int[] dd = new int[4];
        for (int i = 0; i < 4; i++) {
            c[i] = new MapLocation(rng.nextInt(w), rng.nextInt(h));
            dd[i] = me.distanceSquaredTo(c[i]);
        }
        // selection sort by descending distance; index EXPLORE_RANK-1 is the k-th farthest
        for (int i = 0; i < 4; i++) {
            int bi = i;
            for (int j = i + 1; j < 4; j++) if (dd[j] > dd[bi]) bi = j;
            int td = dd[i]; dd[i] = dd[bi]; dd[bi] = td;
            MapLocation tc = c[i]; c[i] = c[bi]; c[bi] = tc;
        }
        explore = c[EXPLORE_RANK - 1];
        exploreAge = 0;
    }
