...
public class RobotPlayer {
    ...
    // ---- Iteration 43: MAP SYMMETRY INFERENCE (reference/RESEARCH.md section 6).
    //
    // Every BC25 map is guaranteed one of three symmetries. `battlecode.world.MapSymmetry`
    // {ROTATIONAL, HORIZONTAL, VERTICAL} is engine-side only -- it is NOT in battlecode.common,
    // verified by javap through tools/engine-jar.sh against the resolved 3.1.0 jar -- so a bot
    // cannot read it and must infer it. RESEARCH.md calls this standard practice everywhere
    // else; carol has never had it. Every `symmetr` hit in this file before now was about
    // play-symmetry, a fairness constraint on my own build mix, not about the map.
    //
    // Why this rather than another resource gate. Iterations 37-42 were all gates and floors
    // regulating a resource, and all were rejected. Iteration 42 in particular proved soldiers
    // MOVE fine -- radial exploration cut idleness 71% -> 6.8% and cut deaths sevenfold -- and
    // still lost, which says the deficit is not how soldiers travel but that they do not know
    // WHERE to go. Carol reaches 21 of 48 ruins on TheBest. Symmetry does not search better; it
    // DERIVES. Every ruin seen implies its mirror exactly, with no communication.
    //
    // The three candidate transforms, held live and eliminated by contradiction. Naming is
    // deliberately avoided: I test all three shapes rather than rely on which enum name means
    // which axis, so a misreading of the engine's vocabulary cannot silently invert the bot.
    static final boolean[] symAlive = { true, true, true };
    static int symChecks = 0, symKills = 0;
    static final int RUIN_MEM = 24;
    static final MapLocation[] ruinMem = new MapLocation[RUIN_MEM];
    static int ruinMemN = 0;

    /** Mirror of `l` under candidate `c`: 0 = 180-rotation, 1 = left-right, 2 = up-down. */
    static MapLocation symMirror(MapLocation l, int c) {
        int w = rc.getMapWidth() - 1, h = rc.getMapHeight() - 1;
        if (c == 0) return new MapLocation(w - l.x, h - l.y);
        if (c == 1) return new MapLocation(w - l.x, l.y);
        return new MapLocation(l.x, h - l.y);
    }

    /** How many candidates are still alive. */
    static int symLive() {
        return (symAlive[0] ? 1 : 0) + (symAlive[1] ? 1 : 0) + (symAlive[2] ? 1 : 0);
    }

    /**
     * Eliminate candidates that the terrain contradicts, and remember ruins.
     *
     * A candidate can only be refuted where a tile AND its mirror are both sensible right now,
     * which happens near a symmetry axis. That makes refutation gradual rather than immediate --
     * and it does not need to be immediate, because an UNPINNED candidate set is already useful:
     * k live candidates turn one seen ruin into k exact guesses, every one of which beats the
     * uniform-random coordinate this replaces.
     */
    static void symObserve() throws GameActionException {
        for (MapLocation r : rc.senseNearbyRuins(-1)) {
            boolean known = false;
            for (int i = 0; i < ruinMemN; i++) if (ruinMem[i].equals(r)) { known = true; break; }
            if (!known && ruinMemN < RUIN_MEM) ruinMem[ruinMemN++] = r;
        }
        if (symLive() <= 1) return;

        // REFUTE ON RUINS FIRST. Walls are a weak signal: only 440 of 3,600 tiles distinguish
        // the wrong candidates on TheBest, and they must both be sensible at once, so the first
        // build refuted almost nothing (all three candidates still alive after 10,157 soldier
        // turns on DefaultMedium). Ruins are sparse -- 48 on a 3,600-tile board -- so the
        // ABSENCE of a ruin where a candidate predicts one is decisive rather than marginal,
        // and it does not need the original tile to still be in view because the ruin is
        // remembered.
        for (int i = 0; i < ruinMemN; i++) {
            for (int c = 0; c < 3; c++) {
                if (!symAlive[c]) continue;
                MapLocation m = symMirror(ruinMem[i], c);
                if (m.equals(ruinMem[i]) || !rc.canSenseLocation(m)) continue;
                symChecks++;
                if (!rc.senseMapInfo(m).hasRuin()) { symAlive[c] = false; symKills++; }
            }
        }
        if (symLive() <= 1) return;

        MapInfo[] near = rc.senseNearbyMapInfos(8);
        int budget = 6;
        for (MapInfo t : near) {
            if (budget-- <= 0) break;
            ...
            for (int c = 0; c < 3; c++) {
                if (!symAlive[c]) continue;
                MapLocation m = symMirror(l, c);
                if (m.equals(l) || !rc.canSenseLocation(m)) continue;
                MapInfo o = rc.senseMapInfo(m);
                symChecks++;
                if (o.isPassable() != t.isPassable()) { symAlive[c] = false; symKills++; }
            }
        }
    }

    /** Nearest symmetry-derived ruin mirror, or null. Dose 0 disables the whole mechanism. */
    static MapLocation symTarget() {
        if (SYM_ON == 0 || ruinMemN == 0) return null;
        ...
        for (int i = 0; i < ruinMemN; i++) {
            for (int c = 0; c < 3; c++) {
                ...
                int d = me.distanceSquaredTo(m);
                // Ignore mirrors we are already standing on top of; they teach nothing.
                if (d <= 8) continue;
    ...
    /** Dose: 1 enables symmetry-derived exploration targets, 0 is the incumbent. */
    static final int SYM_ON = 0;
    ...
    static String runSoldier() throws GameActionException {
        ...
        // Iteration 43: accumulate terrain evidence and remember ruins before deciding anything.
        symObserve();
        state += " sym" + (symAlive[0] ? "R" : "-") + (symAlive[1] ? "H" : "-")
               + (symAlive[2] ? "V" : "-") + "/" + ruinMemN;
    ...
    static void newExploreTarget() {
        ...
        // Iteration 43: a symmetry-derived ruin mirror is an EXACT location, not a guess, once
        // the symmetry is pinned -- and even unpinned it is one of at most three exact
        // candidates. Either way it strictly dominates the uniform-random coordinate below,
        // which is what iteration 42 showed carol cannot fix by travelling better.
        MapLocation sym = symTarget();
        if (sym != null) { explore = sym; exploreAge = 0; return; }
