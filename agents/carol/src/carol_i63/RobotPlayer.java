...
public class RobotPlayer {
    ...
    static String runTower() throws GameActionException {
        ...
        // ---- ITERATION 63: splasher headroom ---------------------------------------------
        // Iteration 62 killed the OPPOSITE asymmetry for three games. A flat floor over every
        // unit is compared against unit cost, so it throttles the EXPENSIVE unit hardest --
        // and the expensive unit is the splasher, the only one that paints in bulk. At
        // ALL_FLOOR=450 the bot built 3 splashers in a whole game and covered 82 tiles.
        //
        // The measured problem on Mirage is the reverse. D3 keeps soldiers alive, so the bot
        // reaches 9 towers, so chips explode to $80,850 idle -- and the incumbent's spawn gates
        // (SPLASH_FLOOR 2000, CHIP_RESERVE 1200) are CHIP gates, tuned when the treasury
        // hovered near 1,400. A permanently rich team satisfies them on every tower-turn, so
        // every tower spawns the instant it holds 200 paint and the roll is 75% soldier. Tower
        // paint never accumulates past ~42, and a splasher needing 300 is differentially locked
        // out: 26 of 357 builds, against the 15% the roll asks for.
        //
        // So: throttle the CHEAP unit to protect the expensive one's headroom. A unit cheaper
        // than a splasher may only be built if the tower still holds a splasher's cost
        // afterwards. Splashers themselves are never gated by this. No new tuning constant --
        // the threshold IS UnitType.SPLASHER.paintCost, the way iteration 61 established that
        // REFILL_LOW is UnitType.SPLASHER.attackCost.
        if (afford && want.paintCost < UnitType.SPLASHER.paintCost
                && rc.getPaint() - want.paintCost < UnitType.SPLASHER.paintCost) {
            afford = false;
        }
        if (afford) {
