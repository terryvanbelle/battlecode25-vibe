...
public class RobotPlayer {
    ...
    /**
     * Iteration 40 dose: a ruin becomes a MONEY tower when k % MONEY_MOD == 0, so this sets the
     * money share (~1 in MONEY_MOD). Incumbent is 4, from iteration 34.
     *
     * Why iteration 34's answer may have flipped, stated as a mechanism rather than a hunch:
     * iteration 34 reduced money towers because paint binds and chips did not. That was measured
     * on a build where the upgrade gate was CHIP_RESERVE + 2500 = 3700 -- a threshold iteration
     * 35 later showed the treasury never reaches, firing 4 times against 9,147 `upgPoor`. On that
     * build, chips beyond production really were worthless, so trading chips for paint was right.
     *
     * ITERATION 35 CHANGED WHAT A CHIP BUYS. The upgrade gate is now 2500 flat, and a level-2
     * paint tower makes 10 paint/turn against a level-1's 5. So chips now purchase PAINT INCOME
     * directly, and iteration 34's premise -- that chips have no use paint would rather have --
     * no longer holds. This is my own LEARNINGS entry "a measured constant is only valid for the
     * build it was measured on", applied to a constant I set myself six iterations ago.
     *
     * Measured against opponents this lineage did not produce (tournament replays, the sanctioned
     * cross-agent channel), tower upgrades per game:
     *     DefaultHuge  carol 9 (8 paint)   alice 90 (25 paint)
     *     SMILE        carol 2 (1 paint)   alice 17 (13 paint)
     *     galaxy       carol 3 (2 paint)   bob  30 (10 paint)
     *     SMILE        carol 2 (1 paint)   bob  22 (13 paint)
     * A 7-10x gap. The tournament build is iteration 29 and the current build upgrades more
     * (8-11 per game in self-play), so the gap is smaller than the table but nowhere near closed.
     */
