...
public class RobotPlayer {
    ...
    // ---- Iteration 41: SPECIAL RESOURCE PATTERNS. Carol has never built one; the per-team
    // `srp` counter reads 0 in every game this lineage has played. Each ACTIVE SRP pays
    // +3 paint/turn to every PAINT tower (GameConstants.EXTRA_RESOURCES_FROM_PATTERN, and
    // RULES.md [E]: the bonus sits inside the paintPerTurn != 0 guard, so money towers get
    // chips instead), for a one-off 200 chips. At 5 paint towers that is +15 paint/turn for
    // 200 chips, ~37x the paint-per-chip of a 2500-chip tower upgrade, and it is the only
    // chip purchase carol has that does NOT rebound onto the paint account the way
    // iteration 40's soldiers did.
    static MapLocation srpFocus = null;    // SRP centre currently being laid
    static int srpTurns = 0;               // turns spent on it
    static int srpDone = 0;                // completed by this robot (indicator only)
    static final int SRP_PATIENCE = 70;    // turns before abandoning a site
    // SRP work is attempted only when no unclaimed ruin is visible (0 = strictly subordinate).
    static final int SRP_RUIN_D2 = 0;
    /**
     * Dose axis, and the mechanism-critical constant: how wide a ring of OUR OWN paint must
     * surround the 5x5 before a site is adopted. 0 reproduces the naive gate.
     *
     * Set from two pre-flights, not from theory.
     *
     * Pre-flight 1 gated only on "passable, no enemy paint". It completed 5 patterns on
     * DefaultMedium and exactly ONE ever activated (round 638), surviving 28 rounds.
     * RESOURCE_PATTERN_ACTIVE_DELAY is 50 undisturbed rounds, so the rest never paid anything.
     *
     * Pre-flight 2 added "all 25 tiles already ours" and changed NOTHING -- every tag count was
     * identical to the byte, because all 6 sites were already fully ours. Rendering the arena
     * found the real killer. At round 650 the pattern at (11,26) read exactly
     * AAaAA/AaaaA/aaAaa/AaaaA/AAaAA; at round 680 two tiles of the top row had gone from A
     * (ally SECONDARY) to a (ally PRIMARY). Still our paint -- the enemy never touched it.
     *
     * OUR OWN SPLASHERS BREAK OUR OWN SRPs. A splasher paints ally primary over its whole
     * footprint, and it scores centres by enemy paint (r2<=2) and EMPTY tiles. There was enemy
     * paint one tile off the SRP's left edge, so a perfectly correct splash at that target
     * repainted our secondary tiles primary and reset the 50-round clock.
     *
     * Hence the ring: if everything around the pattern is already ours, no splash centre near
     * it can score, so no splasher has a reason to fire there. This keeps the whole mechanism
     * inside the soldier rather than adding a second rule to the splasher.
     *
     * Why 1 is the ceiling: the ring must be SENSED to be checked, and vision is r2=20. A 7x7
     * corner is (3,3), d2=18 -- inside. A 9x9 corner is (4,4), d2=32 -- outside, so margin 2
     * would fail its own sense check on every call and silently adopt nothing.
     */
    static final int SRP_MARGIN = 0;

    /**
     * Dose axis. Whether a splasher refuses a centre that would break one of our own patterns.
     *
     * This is the causal fix for what the arena render proved: a splasher paints ally PRIMARY
     * over its whole footprint, so a correct splash at enemy paint beside an SRP silently
     * converts our secondary tiles to primary and resets the 50-round activation clock. The
     * ring gate (SRP_MARGIN) was an attempt to fix it from the soldier's side by placing
     * patterns out of reach; pre-flight 3 showed it merely stops patterns being placed at all
     * (1 site adopted against 608 rejections, and that one still died).
     *
     * A tile "we are actively keeping secondary" is one whose mark is ally-secondary and whose
     * paint currently matches it. That set is completed SRPs plus completed tower patterns.
     * Protecting tower patterns is unnecessary -- a tower survives its pattern being painted
     * over -- but it is harmless here: those tiles sit on ruins inside our own territory, where
     * a splash scores near nothing anyway, so the over-restriction costs little. Stated so the
     * cost is on the record rather than discovered later.
     */
    static final boolean SRP_GUARD = false;
    ...
    static String runSoldier() throws GameActionException {
        ...
        // Iteration 41: SRP work, strictly subordinate to ruin work. Iteration 36 established
        // that displacing soldiers off ruins is expensive -- only soldiers call workOnRuin, so
        // every displaced soldier is a ruin not claimed -- so this runs only on turns the
        // soldier had no ruin to work. SRP_RUIN_D2 is the dose: 0 keeps it to "no unclaimed
        // ruin visible at all".
        boolean srpAllowed = (ruin == null)
                || (SRP_RUIN_D2 > 0 && rc.getLocation().distanceSquaredTo(ruin) > SRP_RUIN_D2);
        if (srpAllowed) {
            state += srpWork();
        } else {
            srpFocus = null; srpTurns = 0;
        }
        state += " sd=" + srpDone;
        ...
        // While a pattern is being laid the soldier must STAY: standing on the centre is what
        // puts all 25 tiles inside the action radius (corner distance^2 = 8 <= 9), and
        // moveExploring would walk away because stepToward(here) is a CENTER no-op that falls
        // through to the random explore target.
        if (srpFocus != null && rc.getLocation().distanceSquaredTo(srpFocus)
                <= GameConstants.RESOURCE_PATTERN_RADIUS_SQUARED) {
            state += " srpHold";
        } else {
            moveExploring(srpFocus != null ? srpFocus : ruin);
        }
    ...
    /**
     * True if the 5x5 centred at `c` is a legal, workable SRP site: fully sensed, fully
     * passable (a wall or a ruin tile fails `isPassable`), and free of enemy paint, which a
     * soldier cannot overwrite (iteration 29). Checked once on adoption, not every turn.
     */
    static boolean srpSiteOk(MapLocation c) throws GameActionException {
        int r = 2 + SRP_MARGIN;
        if (c.x < r || c.y < r || c.x > rc.getMapWidth() - 1 - r
                || c.y > rc.getMapHeight() - 1 - r) {
            return false;
        }
        for (int dx = -r; dx <= r; dx++) {
            for (int dy = -r; dy <= r; dy++) {
                ...
                MapInfo t = rc.senseMapInfo(l);
                boolean core = (dx >= -2 && dx <= 2 && dy >= -2 && dy <= 2);
                if (core) {
                    // The pattern itself: must be paintable ground we already hold.
                    if (!t.isPassable()) return false;
                    if (!t.getPaint().isAlly()) return false;
                } else {
                    // The ring: a wall or ruin is harmless (a splasher cannot score it), but
                    // empty or enemy ground here is exactly what draws a splash into range.
                    if (t.isPassable() && !t.getPaint().isAlly()) return false;
                }
            }
        }
        return true;
    ...
    /**
     * Lay and complete a resource pattern. Subordinate to ruin work by construction: the
     * caller only invokes this when there is no ruin worth working.
     *
     * The pattern is laid with `markResourcePattern` rather than by decoding
     * GameConstants.RESOURCE_PATTERN (28873275) into a 5x5 by hand. That is deliberate: a
     * transposed or flipped hand-decode produces a pattern that never completes, i.e. a
     * mechanism that silently does nothing, which is this lineage's most common failure
     * shape. The engine cannot be off by an orientation. Painting then reuses iteration 29's
     * paint-to-match-marks loop, enemy-tile skip included.
     */
    static String srpWork() throws GameActionException {
        MapLocation me = rc.getLocation();

        if (srpFocus != null && ++srpTurns > SRP_PATIENCE) { srpFocus = null; srpTurns = 0; }

        if (srpFocus == null) {
            if (!srpSiteOk(me)) return " srpNo";
            srpFocus = me; srpTurns = 0;
        }
        if (me.distanceSquaredTo(srpFocus) > GameConstants.RESOURCE_PATTERN_RADIUS_SQUARED) {
            return " srpFar";
        }

        // Completing is the cheapest possible win, so try it before spending any more paint.
        // The reserve guard keeps a 1000-chip ruin completion safe, exactly as tower spawning
        // does -- 200 chips is small, but it is drawn from the same shared treasury.
        if (rc.canCompleteResourcePattern(srpFocus)
                && rc.getChips() >= CHIP_RESERVE + GameConstants.COMPLETE_RESOURCE_PATTERN_COST) {
            rc.completeResourcePattern(srpFocus);
            srpDone++; srpFocus = null; srpTurns = 0;
            return " SRPDONE";
        }

        // Mark once. The SRP's centre bit is secondary, so an unmarked centre is a reliable
        // "not laid yet" test -- the same cheap orientation check workOnRuin uses.
        if (rc.senseMapInfo(srpFocus).getMark() == PaintType.EMPTY
                && rc.canMarkResourcePattern(srpFocus)) {
            rc.markResourcePattern(srpFocus);
            return " srpMark";
        }

        for (MapInfo tile : rc.senseNearbyMapInfos(srpFocus,
                GameConstants.RESOURCE_PATTERN_RADIUS_SQUARED)) {
            PaintType mark = tile.getMark();
            if (mark != PaintType.EMPTY && mark != tile.getPaint()) {
                if (tile.getPaint().isEnemy()) continue;
    ...
    /**
     * True if splashing at `c` would repaint a tile we are actively holding as ally SECONDARY
     * to satisfy a mark -- i.e. a completed resource or tower pattern. See SRP_GUARD.
     */
    static boolean breaksOwnPattern(MapLocation c) throws GameActionException {
        for (MapInfo t : rc.senseNearbyMapInfos(c, 4)) {
            if (t.getMark() == PaintType.ALLY_SECONDARY
                    && t.getPaint() == PaintType.ALLY_SECONDARY) {
                return true;
            }
        }
        return false;
    ...
    static String runSplasher() throws GameActionException {
        ...
        if (ready && fueled) {
            ...
            for (MapLocation c : rc.getAllLocationsWithinRadiusSquared(me, 4)) {
                ...
                if (SRP_GUARD && breaksOwnPattern(c)) continue;
