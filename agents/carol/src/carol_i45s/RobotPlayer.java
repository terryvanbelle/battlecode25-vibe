...
public class RobotPlayer {
     ...
     */
    static final String BUILD = "i45s";
    ...
    static String runSoldier() throws GameActionException {
        ...
        return state + stepDbg + " p=" + rc.getPaint();
    }
    ...
    /** PROBE ONLY (iteration 45s). drain rank: 0 = ally (0/turn), 1 = neutral (-1), 2 = enemy (-2). */
    static int drainRank(MapLocation l) throws GameActionException {
        if (!rc.canSenseLocation(l)) return 1;
        PaintType q = rc.senseMapInfo(l).getPaint();
        return q.isAlly() ? 0 : q.isEnemy() ? 2 : 1;
    }

    /** PROBE ONLY. Per stepToward move: call SITE (R = toward a ruin, E = toward the random
     *  explore target), whether the STRAIGHT direction was the one taken, the drain rank taken,
     *  and the cheapest rank legally available. Records the DECISION; changes nothing. */
    static String stepDbg = "";
    static char stepSite = '?';
    static void probeStep(MapLocation from, Direction chosen, Direction straight, Direction[] cands)
            throws GameActionException {
        int ck = drainRank(from.add(chosen));
        int best = ck;
        for (Direction c : cands)
            if (c != null && rc.canMove(c)) { int k = drainRank(from.add(c)); if (k < best) best = k; }
        stepDbg += " s" + stepSite + (chosen == straight ? "D" : "f") + ck + best;
    }

    static boolean stepToward(MapLocation to) throws GameActionException {
        MapLocation from0 = rc.getLocation();
        Direction d = rc.getLocation().directionTo(to);
        if (d == Direction.CENTER) return false;
        Direction[] cands0 = { d, d.rotateLeft(), d.rotateRight(),
                               d.rotateLeft().rotateLeft(), d.rotateRight().rotateRight() };
        if (rc.canMove(d)) { probeStep(from0, d, d, cands0); rc.move(d); return true; }
        ...
        if ((rc.getID() & 1) == 0) {
            if (rc.canMove(l)) { probeStep(from0, l, d, cands0); rc.move(l); return true; }
            if (rc.canMove(r)) { probeStep(from0, r, d, cands0); rc.move(r); return true; }
        } else {
            if (rc.canMove(r)) { probeStep(from0, r, d, cands0); rc.move(r); return true; }
            if (rc.canMove(l)) { probeStep(from0, l, d, cands0); rc.move(l); return true; }
        }
        Direction l2 = l.rotateLeft(), r2 = r.rotateRight();
        if (rc.canMove(l2)) { probeStep(from0, l2, d, cands0); rc.move(l2); return true; }
        if (rc.canMove(r2)) { probeStep(from0, r2, d, cands0); rc.move(r2); return true; }
        return false;
    }
}
