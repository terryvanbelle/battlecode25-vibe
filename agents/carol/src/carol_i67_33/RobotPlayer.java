...
public class RobotPlayer {
    ...
    // ---- ITERATION 67: splash scoring weighted by RETENTION, not by instantaneous value ----
    // Measured free from replay paint-grids, replicated on two disjoint maps: a tile converted
    // from ENEMY paint is lost back far more often than one converted from EMPTY ground.
    //     Mirage (r400->r1900):  empty lost 28.3%,  enemy lost 75.0%  -> 2.7x
    //     Leaf   (r300->r950):   empty lost  2.1%,  enemy lost 13.5%  -> 6.4x
    // The win condition is MY painted area (>70% instant win; tiebreak on area painted), so a
    // converted tile is worth +1 to me whatever its origin -- and its EXPECTED value is that
    // times its retention. The incumbent weights enemy 3 against empty 2, i.e. it prefers the
    // tiles that do not stick by 1.5x, when retention says empty is worth 1.1-2.9x enemy.
    // That is why iteration 66 failed: raising the floor steered fire onto exactly those tiles.
    static final int W_EMPTY = 3;
    static final int W_ENEMY = 3;
            ...
            for (MapLocation c : rc.getAllLocationsWithinRadiusSquared(me, 4)) {
                ...
                for (MapInfo t : rc.senseNearbyMapInfos(c, 4)) {
                    ...
                    if (p.isEnemy()) {
                        ...
                        if (c.distanceSquaredTo(t.getMapLocation()) <= 2) score += W_ENEMY;
