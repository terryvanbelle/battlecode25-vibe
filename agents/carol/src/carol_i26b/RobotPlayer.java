...
public class RobotPlayer {
    ...
    // ---- Iteration 26: SRPs, built out of turns measured to be worth nothing ---------------
    // Engine constants (javap -constants on the jar, not my digest): 200 chips to complete,
    // 25 paint to mark, +3/turn per SRP, 50-round activation delay, mark radius r2=8, 5x5.
    // The bonus is inside `if (type.paintPerTurn != 0) addPaint(paintPerTurn + 3*numSRPs)`,
    // so it lands on PAINT towers only. At carol's observed 3-6 paint towers one SRP is
    // +9..+18 paint/turn for a one-off ~90 paint (RESOURCE_PATTERN has 13 of 25 bits set, and
    // deep in our own territory the other 12 are already ally primary). Payback ~13 rounds.
    //
    // WHERE the turns come from, and why this is not competing with iteration 14. The idle
    // soldier branch splits into frontFound (a visible empty tile exists -- iteration 14
    // redirects the soldier to it) and frontNone (nothing paintable anywhere in vision).
    // Measured on the i26a probe replays, frontNone is the DOMINANT case:
    //     DefaultLarge 740/1032   DefaultMedium 2634/3443
    //     Fossil       2607/2947  gridworld     5026/6647
    // 71-88% of idle-ally turns, thousands per game, spent doing literally nothing. SRP work
    // fires ONLY in that branch, so the two mechanisms partition the idle budget by an explicit
    // decision rather than by which one happens to be written first (algorithm 5b).
    //
    // It is also the right PLACE, not just the right time: frontNone means the soldier is deep
    // inside saturated ally territory, which is where a pattern can sit undisturbed for its 50
    // rounds and where most of its 25 tiles are already the correct colour.
    // Iteration 26b/26c: the commit gate is the COMPLETION cost, not the mark cost.
    // i26 gated on MARK_PATTERN_PAINT_COST (25) -- exactly enough to mark and nothing left to
    // finish with -- and 59-88% of marks were then abandoned at <5 paint, burning 25 each:
    // 1200 paint on DefaultMedium, 2300 on Bunny. That is the "cost the price, not just the
    // benefit" error in my own LEARNINGS, committed again: I priced the action I was about to
    // take instead of the transaction I was entering. RESOURCE_PATTERN has 13 of 25 bits set,
    // so finishing costs 25 to mark plus 13x5 = 90 in already-ally territory and up to
    // 25 + 25x5 = 150 on ground that still needs painting. Two doses, one run, shared maps.
    static final int SRP_COMMIT_PAINT = 150;
    static MapLocation srpCenter = null;         // the SRP this robot has committed to
    static int srpMark = 0, srpDone = 0, srpDrop = 0;   // decisions, not outcomes
     ...
     */
    static final String BUILD = "i26b";
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            ...
            + " sM=" + srpMark + " sD=" + srpDone + " sX=" + srpDrop + " | " + state);
        Clock.yield();
    }
    ...
    static String runSoldier() throws GameActionException {
        ...
        // Iteration 26: a soldier that has committed to an SRP finishes it before anything
        // else. Committing happens only from the frontNone idle branch below, so reaching here
        // with srpCenter set means this robot had nothing better to do when it started.
        if (srpCenter != null) state += workOnSrp();
                ...
                if (best != null) { rc.attack(best); state += " pnt"; }
                    ...
                    if (foe == 0) {
                        ...
                        if (f != null) { explore = f; exploreAge = 0; state += " frontFound"; }
                            ...
                            state += " frontNone";
                            // Nothing paintable in action range AND nothing empty anywhere in
                            // vision: this turn is measured waste. Spend it starting an SRP.
                            if (srpCenter == null
                                    && rc.getPaint() >= SRP_COMMIT_PAINT
    ...
    /**
     * Iteration 26. Finish the SRP this robot committed to: paint the marked tiles that do not
     * yet match, then complete the pattern when the engine says it is whole. Mirrors the tower
     * pattern loop in workOnRuin, which is the same three-step mark/paint/complete contract.
     *
     * Commitment matters: marking already spent 25 paint, so a soldier that wanders off has
     * wasted it. While committed the robot steers back toward its centre rather than exploring,
     * and gives up only if it cannot afford to continue -- srpDrop counts that, so an
     * abandonment rate that eats the benefit will be visible rather than inferred.
     */
    static String workOnSrp() throws GameActionException {
        MapLocation c = srpCenter;
        if (rc.canCompleteResourcePattern(c)) {
            rc.completeResourcePattern(c);
            srpCenter = null; srpDone++;
            return " SRPdone";
        }
        // Out of paint to continue: drop it rather than stall this robot forever.
        if (rc.getPaint() < UnitType.SOLDIER.attackCost) {
            srpCenter = null; srpDrop++;
            return " SRPdrop";
        }
        // Steer back if we have drifted off the pattern.
        if (rc.getLocation().distanceSquaredTo(c) > GameConstants.RESOURCE_PATTERN_RADIUS_SQUARED) {
            explore = c; exploreAge = 0;
            return " SRPfar";
        }
        if (rc.isActionReady()) {
            for (MapInfo t : rc.senseNearbyMapInfos(c, GameConstants.RESOURCE_PATTERN_RADIUS_SQUARED)) {
