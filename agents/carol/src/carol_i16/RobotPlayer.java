...
public class RobotPlayer {
    ...
    static int aoeFreed = 0;   // iteration 16: AoE attacks the old isActionReady() guard skipped
    ...
    static void monitorAndYield(int startRound, String state) {
        ...
        rc.setIndicatorString("[" + BUILD + "] bc=" + used + "/" + limit + " max=" + bcMaxUsed
            + " ov=" + bcOverruns + " nm=" + bcNearMisses + " aoe=" + aoeFreed + " | " + state);
        Clock.yield();
    }
    ...
    static String runTower() throws GameActionException {
        ...
        // Iteration 16: the AoE attack was gated on rc.isActionReady(). RULES.md's engine
        // probe of assertCanAttackTower says in terms: tower attacks check only the per-turn
        // hasTowerSingleAttacked/hasTowerAreaAttacked flags, never assertIsActionReady, and
        // towerAttack adds no action cooldown -- "Never gate tower attacks on isActionReady()".
        // Spawning a robot DOES consume action cooldown, so the guard silently suppressed the
        // free area attack on exactly the turns a tower was doing its main job. This is damage
        // the engine was always willing to give us, declined by our own code.
        if (enemies.length > 0) {
            if (!rc.isActionReady()) aoeFreed++;
            rc.attack(null);
        }
