...
public class RobotPlayer {
        ...
        if (rc.isActionReady() && rc.getPaint() >= UnitType.SOLDIER.attackCost) {
            ...
            if (myTile.getPaint() == PaintType.EMPTY && rc.canAttack(me)) {
                ...
                if (best != null) { rc.attack(best); state += " pnt"; }
                    ...
                    if (foe == 0) {
                        // Iteration 78: do not let the idle branch overwrite a forward target
                        // with a backward one -- this writer silently defeated the mechanism.
    ...
    // ---- ITERATION 78: FORWARD TARGETING (harvested from the aborted carol_r2) ------------
    // Measured asymmetry: carol sees 49% of map tiles to the rival's 90%, and spends 10-41% of
    // unit-turns on enemy ground to the rival's 85-88% -- under an IDENTICAL r^2<=2 constraint
    // (p90 unit-to-own-tower distance 6.3 vs 17), so the leash is design, not engine.
    //
    // Targets are drawn from the half of the map FARTHER from this robot's spawn anchor. Units
    // spawn at towers and towers sit in carol's half, so "away from spawn" is "forward" with no
    // symmetry inference (link 1 fails at 37-41%) and no comms.
    //
    // This is a SINGLE-variable change and it is the piece the rewrite's registered salvage
    // clause named in advance -- and the coupling enumeration then showed it had never actually
    // run, because runSoldier overwrote `explore` with the NEAREST visible empty tile, which on
    // a forward unit lies BEHIND it. Both halves are here: the target, and the guard.
    ...
    static void newExploreTarget() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (c.distanceSquaredTo(anchor) < me.distanceSquaredTo(anchor)) continue;
