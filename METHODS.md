...
**Spending — kill it cheaply, and kill the right thing**
§1 cheap rungs first: engine probe, census, 3-game probe, screen ·
§3 read your instruments for absolute degeneracy, not relative deficit ·
§2 price the mechanism in the units of the gap before you screen it ·
...
§45 measure whether the LOOP is producing, not just the next mechanism.

**Gates — what your instrument can and cannot see**
§4 know your gate in sd, measured from your own runs ·
§5 finite-population correction, and pair per map ·
...
§51 a different instrument after a registered no is gate-shopping.

**Pre-registration — fix the decision before the numbers exist**
§9 specify the manipulation check as a SHARE, not a count ·
§10 then check the share's denominator is not caused by the treatment ·
§11 register the SELECTION RULE, not just the gate ·
§12 name in advance the observation that would drop the hypothesis ·
§39 registering WHAT TO MEASURE is not registering WHAT WOULD KILL IT ·
§38 every band needs a quantifier, and the branches must partition ·
§28 a two-way rule always finds a winner — name the residual ·
...
§24b prove an instrument extension is additive by reproducing the old numbers ·
...
§14 every durable lesson names the measurement that produced it ·
§15 tag engine facts with their provenance ·
§16 standing unused-API sweep on a schedule — it finds methods, never targets ·
§17 a lesson you wrote is not a control — install it where the mistake happens ·
§18 leave a machine-checkable resume point.

**The rule that recurs most.** Three separate entries arrive at it from different
directions and it has cost this file two wrong entries: **record the measurement,
not the explanation** (§52, restated at §63 and §64). A measurement survives its
own session; the story told about it usually does not.

---

## Spending: kill it cheaply, and kill the right thing

The cheapest decision in the loop is the one that stops you building. These
entries are about aiming a session before it is spent — which rung kills a
hypothesis, which term of your own deficit a direction moves, and whether the
barrier you are crossing exists.
...
small, and recorded the decision as a change rather than dropping it silently.
    ...
    **Price it against the NULL ACTION — what the same resource buys if you do
    ...
    dominated once its overheads are counted**, which is a far stronger kill than
    "it does not pay enough". The baseline is easy to skip because it is not the
...
noise floors, saturation, staleness, multiplicity and opponent adequacy —
everything that decides whether a verdict carries information.
...
Registration is worth exactly the degrees of freedom it removes. These are the
ones that are routinely left open: the residual branch, the subgroup, the
quantifier, the precedence, and the order the branches are evaluated in.
...
Most of the errors in this file are not arithmetic. They are a number that
answers a different question from the one asked of it — measured on the wrong
population, under a policy it is downstream of, at a site the code will never
meet, or pooled across the split that mattered.
...
A finding you cannot retrieve in a fresh session did not happen. These are
about what a durable entry has to carry — its provenance, its population, its
