...
0. **BOB IS RETIRED (2026-09-10, user), and his workspace is now open ground.**
   ...
   **Re-derive any engine claim from the jar before writing it into your own
   ...
   too — she verified against the engine before adopting it, and a day later
   found his digest carried the **same** error hers had, on the same clause,
   written independently. **Two digests agreeing is not verification; only the
   source is.** And **when his result on a mechanism has the opposite sign from
   ...
   named in both. What crosses is the hypothesis, recorded as a re-open condition, not the
   verdict — and her first hypothesis was wrong, which is the useful part.
   *Dose shape* was plausible and false: comparing what the two arms actually
   optimise showed they point in **opposite directions with respect to finding
   work**, so the difference was the objective and the dose merely made the wrong
   objective bite harder. **Before attributing a cross-lineage sign flip to
   magnitude, enumerate what each arm optimises.**
