#!/usr/bin/env bash
# ============================================================================
# BENCHMARK WITH REPLAYS KEPT -- an explicit, owner-granted exception
# ============================================================================
# The owner granted permission on 2026-09-14 to LOOK AT GAME REPLAYS AGAINST v3.
# tools/benchmark.sh cannot do that: it deliberately omits -Dbc.server.save-file
# so that "there is no game record to examine and none to discard afterwards",
# which made the old rule hold by construction rather than by discipline. That
# guarantee is worth keeping for the default path, so this is a SEPARATE script
# rather than a flag on that one -- benchmark.sh still writes nothing, and anyone
# reading it still sees a tool that cannot produce a v3 replay.
#
# WHAT IS AND IS NOT GRANTED. Replays of games darla played against v3: yes.
# The benchmark bot's SOURCE: still no, and nothing here reads it. The bots live
# only on battlecode-dev under ~/bc25-benchmarks and never enter this repository.
#
# WHY IT IS WORTH THE EXCEPTION. Three accepted iterations moved the roster by
# +19, +15 and +29 games and moved v3 by +2, +2 and -1. The roster and the
# benchmark measure different things and no amount of roster work has closed the
# gap, because nothing in this project has ever been able to see HOW v3 wins.
# Scores say we lose 32 maps two-nil; they cannot say why.
#
# Usage: MAPS="shell Crab" tools/benchmark-replay.sh
# ============================================================================
# Measure an agent against a benchmark bot AND KEEP THE REPLAYS.
#
...
#   * REPLAYS ARE WRITTEN. This is the one deliberate exception, see header below.
...
game () {  # <agent> <bench> <map> <side>
  ...
  acquire_slot
  # Replay IS saved here -- the granted exception. Named by map and side.
  mkdir -p replays-v3
  local SAVE="-Dbc.server.save-file=replays-v3/\${A}__\${K}__\${MAP}__bot\${SIDE}.bc25"
  LOG=\$(java -Xmx2g \$SAVE \\
