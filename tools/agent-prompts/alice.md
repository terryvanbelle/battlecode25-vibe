...
5. `agents/alice/CLOSURE_MAP.md` — **your orientation document.** Every axis and
...
6. `agents/alice/LEARNINGS.md` and `RULES.md`.
...
need one — `grep -n 'iteration 43' agents/alice/TRAINING_LOG.md` — and read the last
...
**Hard isolation from carol, and only from carol.** Never read anything
under `agents/carol/` — not code, not snapshots, not logs, not on
...
unaffected, so `<scratchpad>/alice/...` works normally and writes still
