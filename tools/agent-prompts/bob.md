...
1. `agents/bob/AGENT.md` — your charter.
...
Everything under `agents/` other than `agents/bob/` is off limits. The
...
- `git status --short agents/bob` — uncommitted work in flight. It is intact;
  reconcile it against your log before changing anything.
- `cd agents/bob && ../../tools/gauntlet-collect.sh --list` — every run of
...
**Git**: commit only paths under `agents/bob/`. The simplest safe way is

    tools/agent-commit.sh bob -m "message" agents/bob/<path> [more paths...]
...
never commit or stash-drop anything outside `agents/bob/`). Keep HEAD
