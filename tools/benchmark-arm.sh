#!/usr/bin/env bash
# Benchmark an ARM (not just a committed agent) against a benchmark bot.
#
#   AGENT=darla BOTS=darla102 BENCH=v3 tools/benchmark-arm.sh
#
# WHY: as of 2026-09-14 the owner allows v3 results to count toward accepting an
# iteration, so every candidate now needs a v3 number alongside its roster runs.
# tools/benchmark.sh cannot provide one: it stages agents/$B/src/$B, so $B must be
# an agent whose shipped build is being measured. This stages
# agents/$AGENT/src/$B instead, which lets an arm be measured before it is
# promoted -- the whole point of a candidate.
#
# Everything else is benchmark.sh unchanged, including the rule that NO REPLAY IS
# WRITTEN. Replays against v3 come only from tools/benchmark-replay.sh, which is
# separately opt-in.
#
# The arm must be COMMITTED: staging is `git archive HEAD`.
...
AGENT="${AGENT:-darla}"
BOTS="${BOTS:?set BOTS to one or more arm package names under agents/$AGENT/src}"
